...
export function createBffQueryClient(options: {
  ...
  function withRoute(route: string, body: unknown): unknown {
    if (body !== null && typeof body === "object" && !Array.isArray(body))
      return { ...(body as Record<string, unknown>), route };
    return { route, body };
  }
  return {
    queryProject: (route, body) => post(options.projectPath, withRoute(route, body)),
    queryConcrete: options.concretePath
      ? (route, body) => post(options.concretePath!, withRoute(route, body))
      : undefined,
    query: options.queryPath
      ? (route, body) => post(options.queryPath!, withRoute(route, body))
      : undefined,
