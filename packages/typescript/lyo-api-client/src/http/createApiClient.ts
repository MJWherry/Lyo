...
import {buildUrl, trimRoute, withIncludes} from "./request.js";
...
import type {PatchRequest} from "../types/results.js";
...
export interface ApiClient {
    ...
    getById<TResult = unknown>(
        baseRoute: string,
        id: string,
        include?: readonly string[]
    ): ApiResponse<TResult>;

    /** `PATCH {baseRoute}` with a {@link PatchRequest} body. */
    patch<TResult = unknown>(baseRoute: string, body: PatchRequest): ApiResponse<TResult>;
}
    ...
    const client: ApiClient = {
        ...
        getById<TResult = unknown>(
            ...
            return client.request<TResult>({
                method: "GET",
                path: withIncludes(
                    `${trimRoute(baseRoute)}/${encodeURIComponent(id)}`,
                    include
                ),
            });
        },
        ...
        patch<TResult = unknown>(baseRoute: string, body: PatchRequest): ApiResponse<TResult> {
            return client.request<TResult, PatchRequest>({
                method: "PATCH",
