...
Domain-specific Person endpoints live in ``lyo_person_api_client``.
Job and Reporting endpoints live in ``lyo_job_api_client`` and ``lyo_reporting_api_client``.
Generic Lyo.Api metadata helpers (``get_metadata`` / ``get_crud_metadata`` /
``get_entity_metadata``) plus ``get_by_id`` / ``patch`` are on this client.
"""
...
from .models import ApiRequest, ApiResponse, PatchRequest, TransportRequest
...
def with_includes(path: str, include: list[str] | None = None) -> str:
    """Append repeated ``include`` query params (EF-style navigation paths)."""
    items = [item.strip() for item in include or [] if item and item.strip()]
    if not items:
        return path
    qs = urlencode([("include", item) for item in items])
    sep = "&" if "?" in path else "?"
    return f"{path}{sep}{qs}"


def file_name_from_disposition(header: str | None) -> str | None:
    """Parse ``Content-Disposition`` filename, matching the TypeScript helper."""
    if not header:
        return None
    import re

    star = re.search(r"filename\*=UTF-8''([^;]+)", header, re.IGNORECASE)
    if star and star.group(1):
...
class ApiClient:
    ...
    def get_by_id(self, base_route: str, entity_id: str, include: list[str] | None = None) -> ApiResponse:
        """``GET {baseRoute}/{id}`` with optional ``include`` navigation paths."""
        path = f"{normalize_route_prefix(base_route)}/{quote(entity_id, safe='')}"
        return self.request(ApiRequest(method="GET", path=with_includes(path, include)))

    def patch(self, base_route: str, body: PatchRequest | dict[str, Any]) -> ApiResponse:
        """``PATCH {baseRoute}`` with a PatchRequest body."""
        return self.request(ApiRequest(method="PATCH", path=normalize_route_prefix(base_route), body=body))
