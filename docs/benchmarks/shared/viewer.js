...
(function () {
    ...
    function notesSection(report) {
        ...
        return el("section", {}, [
            el("h2", {text: "Notes"}),
            el("div", {className: "callout"}, report.notes.map(function (note) {
    ...
    function slaLegend(items) {
        ...
        (items || []).forEach(function (m) {
            if (m.slaStandard && !seen[m.slaStandard]) {
                seen[m.slaStandard] = 1;
                standards.push(m.slaStandard);
            }
        });
        var children = [
            el("p", {
        ...
        standards.forEach(function (s) {
            children.push(el("p", {className: "muted-sub", text: "Standard: " + s}));
        });
    ...
    function minMean(items) {
        return Math.min.apply(null, items.map(function (m) {
            return m.meanNs || 0;
        }));
    }
    ...
    function measurementTable(measurements) {
        ...
        var hasSla = measurements.some(function (m) {
            return m.slaResult;
        });
        ...
        order.sort(function (a, b) {
            return minMean(byMethod[a]) - minMean(byMethod[b]);
        });

        var headers = [{label: ""}, {label: "Method"}, {label: "Parameters"}, {label: "Mean", className: "num"}, {label: "Allocated", className: "num"}, {
            label: "Ratio",
            className: "num"
        }];
        if (showDelta) {
        ...
        order.forEach(function (method) {
            var items = byMethod[method].slice().sort(function (a, b) {
                return (a.meanNs || 0) - (b.meanNs || 0);
            });
            var first = items[0];
            ...
            anyMatrix = true;
            var means = items.map(function (m) {
                return m.meanNs || 0;
            });
            var allocs = items.map(function (m) {
        ...
        if (anyMatrix) children.push(el("p", {
            className: "table-note",
            text: "Click a row (\u25B8) to expand its per-parameter matrix breakdown (mean/allocated and Δ shown as a range across parameters; SLA shows the worst verdict)."
        }));
        if (measurements.some(function (m) {
            return m.isBaseline;
        })) {
    ...
    function comparisonSection(comparison) {
        ...
        (comparison.groups || []).forEach(function (group) {
            ...
            order.forEach(function (label) {
                ...
                var hasSla = rowsForLabel.some(function (r) {
                    return r.slaResult;
                });
    ...
    function scenariosSection(scenarios, casesById) {
        ...
        var thead = el("thead", {}, [
            el("tr", {}, headers.map(function (h) {
                return el("th", {text: h, className: numeric[h] ? "num" : ""});
            })),
        ]);
        var tbody = el("tbody");
