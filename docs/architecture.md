...
| `Data`          | Persistence and data handling: file storage (local/S3/Blob), compression, CSV/XLSX/PDF, images, QR codes, Postgres migration helpers, query shapes, seed graphs.                                                                            |
...
| `Integration`   | Vendor clients and product verticals (Google, Endato, Discord, Typecast, ESPN).                                                                                                                                                            |
| `Plugins`      | Drop-in modules: contracts/models, Postgres, `.Web.Components` (comments, notes, favorites, ratings, tags, typed config, comic, home inventory, jobs store, reporting store/UI). No route mapping.                                                                 |
| `Apps`          | HTTP and processes: `.Api` / `.Api.Host` / `.Client`, workers, schedulers, agents, plus the platform toolkit (`Apps/Api`, `Apps/Web`, `Apps/Http`).                                                                                     |
| `Examples`      | Runnable sample hosts that show how to wire Lyo libraries (job worker and scheduler).                                                                                                                                                      |
| `Tools`         | Utilities and workbenches for trying components end to end (CLI, migrations, TestApi, TestGateway, TestConsole).                                                                                                                           |
...
- **E. Platform.** Cross-cutting app infrastructure (`Plugins/*`, `Apps/*`, `Data/*`, `Tools/*`, `Examples/*`). Drift's store lives under `Security/Drift` (security control), with HTTP/agent in `Apps/Drift`.
...
```mermaid
flowchart TD
  Apps["Apps (HTTP, clients, hosts, Api/Web/Http toolkit)"]
  Plugins["Plugins (models, Postgres, Web.Components)"]
  Integration["Integration (vendor clients)"]
  Communication["Communication (capabilities)"]
  Security["Security (capabilities + Drift store)"]
...
- `Core` -> `Apps` platform packages, `Integration` vendors, or any vendor SDK
...
4. Add a row to the relevant inventory section in `package-layout.md` if the package is Apps, Plugins, Integration, Communication, or Security.
