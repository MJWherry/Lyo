...
- The same `VERSION` is passed to both `dotnet build` and `dotnet pack`, so the NuGet package version and the embedded assembly metadata (`AssemblyInformationalVersion`,
  `FileVersion`, etc.) stay aligned.
