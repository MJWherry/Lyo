...
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **API & query**         | Minimal APIs and CRUD built on EF Core, plus a **Query** engine (filters, includes, **projection** with optional **computed columns**, **`entityTypes`** metadata on projected responses, bulk operations, exports).                                                                                     | [Lyo.Api](Lyo.Net/Integration/Api/Lyo.Api/README.md): RESTful minimal APIs, CRUD, query and projection. Shapes/DTOs: [Lyo.Query.Models](Lyo.Net/Data/Query/Lyo.Query.Models/README.md).                                                           |
...
| [`Lyo.Net/Data/`](Lyo.Net/Data/)                   | Data handling and persistence helpers: file storage (local/S3/Azure), compression, CSV/XLSX/PDF, images, Postgres migration helpers, **`Lyo.Query.Models`** shapes, QR codes, file-system watching, temporary IO, and related parsers/processors.                                                                                                      |
...
- [Lyo.Cache](Lyo.Net/Core/Cache/Lyo.Cache/README.md): local and Fusion **`ICacheService`**, typed payload codecs, optional query-tag granularity for **`Lyo.Api`**.
...
- [Lyo.PackageMetadata](Lyo.Net/Core/PackageMetadata/Lyo.PackageMetadata/README.md): **`IPackageMetadataStore`**, **`PackageMetadata`**, digest helpers for diagnostics frame enrichment.
...
- [Lyo.FileSystemWatcher](Lyo.Net/Data/FileSystemWatcher/Lyo.FileSystemWatcher/README.md): snapshot-based file watching,
...
- [Lyo.Favorite.Postgres](Lyo.Net/Features/Favorite/Lyo.Favorite.Postgres/README.md): PostgreSQL favorites by **`EntityRef`** (for/from) with migrations.
...
- [Lyo.Config.Api](Lyo.Net/Apps/Config/Lyo.Config.Api/README.md): HTTP host for centralized typed app config (PostgreSQL + **`Lyo.Config`**), ETags and polling.
- [Lyo.Config.Api.Client](Lyo.Net/Apps/Config/Lyo.Config.Api.Client/README.md): typed HTTP client and **`AddConfigApiClientFromConfiguration`**.
- [Lyo.Config.Api.Hosting](Lyo.Net/Apps/Config/Lyo.Config.Api.Hosting/README.md): polling + **`IOptionsMonitor<T>`** integration for consuming apps.
- [Lyo.Config.Api.Models](Lyo.Net/Apps/Config/Lyo.Config.Api.Models/README.md): resolve contracts and DTOs for config API traffic.
