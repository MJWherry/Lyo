...
def _should_skip_project(path: Path) -> bool:
    ...
    if ".Tests" in name or ".Benchmarks" in name or "TestConsole" in name or name.endswith(".Host") or name.endswith(".Example"):
        return True
    if name.startswith("Lyo.TestConsole"):
        return True
    if "Tests" in parts or "Benchmarks" in parts or "Tools" in parts or "Examples" in parts:
        return True
    return False
