...
internal static class CompressCommands
{
    ...
    {
        yield return CreateOne("compress", compress: true);
        yield return CreateOne("decompress", compress: false);
    }
    ...
    private static Command CreateOne(string name, bool compress)
    {
        ...
        var algoOpt = new Option<string?>("--algorithm", "-a") {
            Description = compress
                ? "Algorithm (default brotli). Also gzip, deflate, zlib, lz4, zstd, snappier, lzma, bzip2, xz"
                : "Algorithm; omit to auto-detect when possible"
        };
        ...
        cmd.SetAction(async (pr, ct) => {
            ...
            try {
                ...
                var (output, leaveOut, _) = CliIO.OpenOutput(
                    ...
                    p => compress ? CliIO.AppendExtension(p, ext) : CliIO.StripExtension(p, ext));
                try {
                    if (compress)
