...
/// <summary>Turns CLI flags into Lyo query request DTOs.</summary>
internal static class CliQueryBuilder
{
    ...
    /// <summary>
    /// Coerces a CLI filter value to the most specific type it will parse, so <c>--where age:gt:30</c> is a number compare instead of a string compare. If nothing matches, the original
    /// string is kept. Booleans accept <c>yes</c>, <c>on</c>, and <c>1</c> because those show up on the command line.
    /// </summary>
    private static object? Coerce(string value)
    {
        ...
        if (TypeConversion.TryToBoolean(value, out var flag))
            return flag;

        if (TypeConversion.TryConvertTo<long>(value, out var whole))
            return whole;

        return TypeConversion.TryConvertTo<decimal>(value, out var number) ? number : value;
    }
}