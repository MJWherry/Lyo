...
Minimal-API host that backs `Lyo.TestGateway` and `Lyo.TestConsole`. It wires the Lyo Postgres stores, RabbitMQ-driven job system, S3 file storage with two-key encryption, and exposes the file storage workbench endpoints that the TestGateway `/filestorage-workbench` page talks to.
