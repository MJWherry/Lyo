...
public sealed class TestGatewayFileTransformer
{
    ...
    /// <summary>Reads Lyo encryption envelope fields from the binary layout (and raw magic at offset 0 when the input is not encrypted) without decrypting.</summary>
    ...
    /// <summary>Outermost Lyo encryption suffix on <paramref name="fileName" /> (longest match), or null if none.</summary>
    ...
    /// <summary>Reads length fields from the two-key stream layout after <see cref="EncryptionHeader" /> (wrapped DEK and first sealed chunk length).</summary>
    ...
    /// <summary>Reads symmetric V1 envelopes that pack nonce, a 16-byte auth tag, and ciphertext (AES-GCM, ChaCha, AES-CCM, XChaCha).</summary>
    ...
    /// <summary>Reads AES-SIV V1 envelopes (16-byte synthetic IV plus CTR payload; the stream has no separate tag).</summary>
    ...
    /// <summary>
    /// Best-effort label for common stream magics at offset 0, using <see cref="CompressionAlgorithm.Detectable" /> so this diagnostic stays aligned with the
    /// signatures the compressors advertise.
    /// </summary>
    private static string DescribeCompressionMagicExtended(byte[] d)
    {
        if (d.Length == 0)
            return "No data.";

        var match = CompressionAlgorithm.Detectable.FirstOrDefault(a => a.MatchesMagic(d));
        if (match is not null)
            return $"Looks like {match.Name} ({DescribeMatchedPrefix(match, d)}).";

        // Deflate and Brotli advertise no signature because they have none; the old 0x81-0x83 "Brotli window" guess hit random binary.
        return $"No recognized signature at offset 0 among {string.Join(" / ", CompressionAlgorithm.Detectable.Select(static a => a.Name))} (raw Deflate and Brotli have none).";
    }

    private static string DescribeMatchedPrefix(CompressionAlgorithm algorithm, byte[] data)
    {
        var prefix = algorithm.MagicPrefixes.First(p => data.Length >= p.Length && data.Take(p.Length).SequenceEqual(p));
        return string.Join(" ", prefix.Select(static b => b.ToString("X2")));
    }
    ...
    {
        // TwoKeyEncryptionService<TKeyEncryptionService, TDataEncryptionService>(dek, kek, keyStore) construction note
    ...
    /// <summary>Builds key bytes from the secret with SHA-256/512, then trims to the length the algorithm needs.</summary>
