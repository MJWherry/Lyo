...
public sealed record PersonRes
{
    ...
    public Guid? MostRecentAddressId => MostRecentAddress?.Id;

    public string FullName => $"{FirstName}{(string.IsNullOrEmpty(MiddleName) ? " " : $" {MiddleName} ")}{LastName}";

    public override string ToString() => $"PersonRes: id={Id}, name={FullName}, source={SourceEntityType}, addresses={Addresses?.Count ?? 0}";
}