...
/// <summary>Whether one migration is applied or still pending.</summary>
public sealed record MigrationStatus(string Name, bool IsApplied);

/// <summary>
/// Applies EF Core migrations on demand for any postgres context. Pulls the connection string from ConnectionStringProvider at call time so menu edits
...
public sealed class MigrationRunner
{
    ...
    /// <summary>Brings the given context up to date by applying every pending migration.</summary>
        ...
        where TContext : DbContext
    {
        ...
        try {
            ...
            await PostgresSchema.EnsureAsync(context, schema, ct);
    ...
    /// <summary>Moves to a named target migration (older than current rolls back). Use "0" or Migration.InitialDatabase to undo every migration.</summary>
    ...
    /// <summary>Every migration defined in the assembly, each marked applied or pending.</summary>
    ...
    /// <summary>Name of the most recently applied migration, or null when none are applied.</summary>
    ...
    private TContext CreateContext<TContext>(string schema)
        where TContext : DbContext
        => PostgresSchema.CreateContext<TContext>(_connStr.GetOrThrow(), schema);
}