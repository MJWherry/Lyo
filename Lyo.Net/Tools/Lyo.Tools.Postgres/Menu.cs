...
internal static class Menu
{
        ...
        while (!ct.IsCancellationRequested) {
            ...
            var choice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    ...
                    .AddChoices("Seeds", "Migrations", "Change Connection String", "Exit"));

            if (ct.IsCancellationRequested || choice == "Exit") break;

            try {
                switch (choice) {
                    case "Seeds":                   await SeedsMenuAsync(sp, ct); break;
                    case "Migrations":              await MigrationsMenuAsync(sp.GetRequiredService<MigrationRunner>(), ct); break;
                    case "Change Connection String": ChangeConnectionString(connStr); break;
                }
            }
            catch (OperationCanceledException) { break; }
            catch (Exception ex) { WriteError(ex); Pause(); }
        }
    }
    ...
    private static void ChangeConnectionString(ConnectionStringProvider connStr)
    {
        ...
        var input = AnsiConsole.Prompt(
            new TextPrompt<string>("New connection string [grey](blank to cancel)[/]:")
                .AllowEmpty());
    ...
    private static async Task SeedsMenuAsync(IServiceProvider sp, CancellationToken ct)
    {
        while (!ct.IsCancellationRequested) {
            ...
            var choice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    .Title("Select a seeder:")
                    .AddChoices("Comic Database", "People Database", BackLabel));

            if (choice == BackLabel) break;

            try {
                switch (choice) {
                    case "Comic Database":  await SeedComicAsync(sp, ct); break;
                    case "People Database": await SeedPeopleAsync(sp, ct); break;
                }
            }
            catch (OperationCanceledException) { break; }
            catch (Exception ex) { WriteError(ex); Pause(); }
    ...
    private static async Task SeedComicAsync(IServiceProvider sp, CancellationToken ct)
    {
        ...
        var count = AnsiConsole.Prompt(
            new TextPrompt<int>("Number of series to generate:")
                .DefaultValue(20));

        var seedInput = AnsiConsole.Prompt(
            new TextPrompt<string>("[grey](Optional)[/] Random seed [grey](blank = random)[/]:")
                .AllowEmpty());
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("Seeding comic database...", async _ =>
                await sp.GetRequiredService<ComicDbSeeder>().SeedAsync(count, seed, ct));
    ...
    private static async Task SeedPeopleAsync(IServiceProvider sp, CancellationToken ct)
    {
        ...
        var count = AnsiConsole.Prompt(
            new TextPrompt<int>("Number of persons to generate:")
                .DefaultValue(50));
        ...
        await AnsiConsole.Status()
            .Spinner(Spinner.Known.Star)
            .SpinnerStyle(Style.Parse("green"))
            .StartAsync("Seeding people database...", async _ =>
                await sp.GetRequiredService<PeopleDbSeeder>().SeedAsync(count, seed, ct));
    ...
    private static async Task ContextMenuAsync<TContext>(
        MigrationRunner runner, string schema, string label, CancellationToken ct)
                ...
                switch (choice) {
                    case "Migrate to Latest":
                        await AnsiConsole.Status()
                            .Spinner(Spinner.Known.Star)
                            .SpinnerStyle(Style.Parse("green"))
                            .StartAsync($"Migrating {label} to latest...", async _ =>
                                await runner.MigrateLatestAsync<TContext>(schema, ct));
                        AnsiConsole.MarkupLine("[green]Done![/]");
                        Pause();
                        break;
    ...
    private static async Task MigrateToTargetAsync<TContext>(
        MigrationRunner runner, string schema, string label, CancellationToken ct)
        where TContext : DbContext
    {
        AnsiConsole.Clear();
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("Fetching migrations...", async _ =>
                statuses = await runner.GetStatusAsync<TContext>(schema, ct));
        ...
        var selected = AnsiConsole.Prompt(
            new SelectionPrompt<MigrationStatus>()
                .Title($"Select target for [blue]{label}[/]:")
                .PageSize(20)
                .UseConverter(s => s.Name switch {
                    "__rollback__" => "[grey][ Roll back all ][/]",
                    "__cancel__"   => "[ Cancel ]",
                    _              => $"{(s.IsApplied ? "[green]✓ APPLIED[/]" : "[yellow]· PENDING[/]")}  {s.Name}"
                })
                .AddChoices(choices));

        if (selected.Name == "__cancel__") return;

        string target;
        if (selected.Name == "__rollback__") {
            if (!AnsiConsole.Confirm("[red]Roll back ALL migrations?[/]")) return;
            target = "0";
        }
        else {
        ...
        await AnsiConsole.Status()
            .Spinner(Spinner.Known.Star)
            .SpinnerStyle(Style.Parse("green"))
            .StartAsync($"Migrating {label} → {target}...", async _ =>
                await runner.MigrateToAsync<TContext>(schema, target, ct));

        AnsiConsole.MarkupLine("[green]Done![/]");
        Pause();
    }

    private static async Task PrintStatusAsync<TContext>(
        MigrationRunner runner, string schema, string label, CancellationToken ct)
        where TContext : DbContext
    {
        AnsiConsole.Clear();
        ...
        if (statuses.Count == 0) {
            ...
            foreach (var s in statuses)
                table.AddRow(
                    s.IsApplied ? "[green]✓ APPLIED[/]" : "[yellow]· PENDING[/]",
    ...
    private static async Task PrintCurrentVersionAsync<TContext>(
        MigrationRunner runner, string schema, string label, CancellationToken ct)
        where TContext : DbContext
    {
        AnsiConsole.Clear();
        ...
        await AnsiConsole.Status()
            .Spinner(Spinner.Known.Dots)
            .StartAsync("Fetching version...", async _ =>
                version = await runner.GetCurrentVersionAsync<TContext>(schema, ct));
