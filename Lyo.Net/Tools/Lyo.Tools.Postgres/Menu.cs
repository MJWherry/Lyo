...
internal static class Menu
{
    ...
    {
        var contributors = sp.GetRequiredService<IEnumerable<SeedContributor>>().OrderBy(c => c.Name).ToList();
        if (contributors.Count == 0) {
            AnsiConsole.MarkupLine("[yellow]No seed contributors registered.[/]");
            Pause();
            return;
        }
        ...
        while (!ct.IsCancellationRequested) {
            ...
            var labels = contributors.Select(c => c.Name).Concat([BackLabel]).ToList();
            var choice = AnsiConsole.Prompt(new SelectionPrompt<string>().Title("Select a seeder:").AddChoices(labels));
            if (choice == BackLabel)
                break;

            try {
                var contributor = contributors.First(c => c.Name == choice);
                await RunContributorAsync(sp, contributor, ct);
            }
            catch (OperationCanceledException) {
                break;
            }
    ...
    private static async Task RunContributorAsync(IServiceProvider sp, SeedContributor contributor, CancellationToken ct)
    {
        AnsiConsole.Clear();
        AnsiConsole.Write(new Rule($"[blue]{Markup.Escape(contributor.Name)}[/]").RuleStyle("grey"));
        AnsiConsole.WriteLine();
        var via = contributor.SupportedTransports;
        SeedTransportKind transportKind;
        if (via.HasFlag(SeedTransportKind.Ef) && via.HasFlag(SeedTransportKind.Api)) {
            var pick = AnsiConsole.Prompt(new SelectionPrompt<string>().Title("Transport:").AddChoices("Direct (EF)", "API (Lyo.Api bulk)"));
            transportKind = pick.StartsWith("API", StringComparison.Ordinal) ? SeedTransportKind.Api : SeedTransportKind.Ef;
        }
        else
            transportKind = via.HasFlag(SeedTransportKind.Api) ? SeedTransportKind.Api : SeedTransportKind.Ef;

        var defaultCount = contributor.Name.Equals("Comic", StringComparison.OrdinalIgnoreCase) ? 20 : 50;
        var count = AnsiConsole.Prompt(new TextPrompt<int>("Number of root rows to generate:").DefaultValue(defaultCount));
        ...
        var conflict = AnsiConsole.Confirm("Skip if destination already has rows?", false)
            ? SeedConflictMode.SkipIfNotEmpty
            : SeedConflictMode.Append;
        if (transportKind == SeedTransportKind.Ef && contributor.Name.Equals("Comic", StringComparison.OrdinalIgnoreCase)
            && AnsiConsole.Confirm("Replace existing comic rows if the database already has series?", false))
            conflict = SeedConflictMode.Replace;

        string? apiBaseUrl = null;
        if (transportKind == SeedTransportKind.Api)
            apiBaseUrl = AnsiConsole.Prompt(new TextPrompt<string>("API base URL:").DefaultValue(SeedExecution.DefaultApiBaseUrl));
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync(
                $"Seeding {contributor.Name}...",
                async _ => result = await SeedExecution.RunAsync(
                    contributor,
                    sp.GetRequiredService<ISeedRunner>(),
                    sp.GetRequiredService<ConnectionStringProvider>(),
                    transportKind,
                    new() { Count = count, RandomSeed = seed, Conflict = conflict },
                    apiBaseUrl,
                    ct));

        result ??= SeedResult.Fail(contributor.Name, ["No result"]);
        if (result.Skipped)
            AnsiConsole.MarkupLine("[yellow]Skipped — destination already has rows.[/]");
        else if (!result.Success)
            AnsiConsole.MarkupLine($"[red]Failed:[/] {Markup.Escape(string.Join("; ", result.Errors))}");
        else
            AnsiConsole.MarkupLine($"[green]Done![/] {Markup.Escape(string.Join(", ", result.Counts.Select(kv => $"{kv.Key}={kv.Value}")))}");
