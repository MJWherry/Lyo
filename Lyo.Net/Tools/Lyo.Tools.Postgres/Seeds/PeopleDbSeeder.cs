...
/// <summary>
/// Seeds the people database with randomised fake data using Bogus.
/// Creates a PeopleDbContext directly from the active ConnectionStringProvider.
/// </summary>
public sealed class PeopleDbSeeder
{
    ...
    /// <summary>
    /// Seeds the database with fake person records. Skips if any persons already exist.
    /// </summary>
    public async Task SeedAsync(int count = 50, int? seed = null, CancellationToken ct = default)
    {
        ...
        if (socials.Count > 0) db.SocialMediaProfiles.AddRange(socials);
        if (jobs.Count > 0)    db.Employments.AddRange(jobs);
        await db.SaveChangesAsync(ct);

        // Link contacts (phones, emails, addresses)
        var isPrimaryPhone = new HashSet<Guid>();
        foreach (var (personId, phone) in phones) {
            db.ContactPhoneNumbers.Add(new ContactPhoneNumberEntity {
                Id            = Guid.NewGuid(),
                PersonId      = personId,
        ...
        foreach (var (personId, email) in emails) {
            db.ContactEmailAddresses.Add(new ContactEmailAddressEntity {
                Id             = Guid.NewGuid(),
                PersonId       = personId,
        ...
        foreach (var (personId, addr) in addrs) {
            db.ContactAddresses.Add(new ContactAddressEntity {
                Id        = Guid.NewGuid(),
                PersonId  = personId,
                AddressId = addr.Id,
    ...
    private PeopleDbContext CreateContext()
    {
        ...
        var opts = new DbContextOptionsBuilder<PeopleDbContext>()
            .UseNpgsql(connStr, n => n.MigrationsHistoryTable("__EFMigrationsHistory", "people"))
            .Options;
        return new PeopleDbContext(opts);
    }

    private static List<PersonEntity> BuildPersons(Faker faker, int count)
    {
        var personFaker = new Faker<PersonEntity>()
            ...
            .RuleFor(p => p.DateOfBirth, f => f.Random.Bool(0.8f)
                ? DateOnly.FromDateTime(f.Date.Between(new DateTime(1950, 1, 1), new DateTime(2005, 1, 1)))
                : null)
    ...
    private static List<PersonRelationshipEntity> BuildRelationships(Faker faker, List<PersonEntity> persons)
    {
        ...
        for (var i = 0; i < pairCount && i * 2 + 1 < shuffled.Count; i++) {
            ...
            relationships.Add(new PersonRelationshipEntity {
                Id              = Guid.NewGuid(),
                PersonId        = a.Id,
