...
/// <summary>
/// Seeds the comic database with randomised fake data using Bogus.
/// Creates DbContext instances directly from the active ConnectionStringProvider.
/// </summary>
public sealed class ComicDbSeeder
{
    ...
    [
        ...
        "historical", "mecha", "psychological", "school", "martial-arts", "isekai",
        "military", "music", "cooking", "magic", "time-travel", "harem"
    ];
    ...
    /// <summary>
    /// Seeds the database with fake comic data. Skips seeding if series already exist.
    /// </summary>
    public async Task SeedAsync(int seriesCount = 20, int? seed = null, CancellationToken ct = default)
    {
        ...
        var faker = seed.HasValue ? new Faker { Random = new Randomizer(seed.Value) } : new Faker();
        ...
        foreach (var character in allCharacters) {
            ...
            if (seriesVolumes.Count == 0) continue;
    ...
    private ComicDbContext CreateComicContext()
    {
        ...
        var opts = new DbContextOptionsBuilder<ComicDbContext>()
            .UseNpgsql(connStr, n => n.MigrationsHistoryTable("__EFMigrationsHistory", "comic"))
            .Options;
        return new ComicDbContext(opts);
    }
    ...
    private TagDbContext CreateTagContext()
    {
        ...
        var opts = new DbContextOptionsBuilder<TagDbContext>()
            .UseNpgsql(connStr, n => n.MigrationsHistoryTable("__EFMigrationsHistory", "tag"))
            .Options;
        ...
        foreach (var s in series) {
            ...
            foreach (var tag in picked) {
                var exists = await tagDb.Tags.AnyAsync(
                    t => t.ForEntityType == SeriesEntityType && t.ForEntityId == s.Id.ToString() && t.Tag == tag, ct);
                if (exists) continue;

                tagDb.Tags.Add(new TagEntity {
                    Id = Guid.NewGuid(),
    ...
    private static List<SeriesEntity> BuildSeries(Faker faker, int count)
    {
        ...
        var seriesFaker = new Faker<SeriesEntity>()
            ...
            .RuleFor(s => s.ComicType, f => f.PickRandom<ComicType>(
                ComicType.Manga, ComicType.Manhwa, ComicType.Manhua, ComicType.Webtoon, ComicType.Western))
            .RuleFor(s => s.Status, f => f.PickRandom<ComicStatus>(
                ComicStatus.Ongoing, ComicStatus.Completed, ComicStatus.Hiatus, ComicStatus.Cancelled))
    ...
    private static List<ChapterEntity> BuildChapters(Faker f, SeriesEntity series)
    {
        ...
        for (var i = 0; i < chapterCount; i++) {
            ...
            chapters.Add(new ChapterEntity {
                Id            = Guid.NewGuid(),
                SeriesId      = series.Id,
                ChapterNumber = i + 1,
                ...
                Language      = f.PickRandom(Languages),
                PageCount     = pageCount,
                PublishedDate = f.Random.Bool(0.8f)
                    ? DateOnly.FromDateTime(f.Date.Between(new DateTime(2000, 1, 1), DateTime.UtcNow))
                    : null,
