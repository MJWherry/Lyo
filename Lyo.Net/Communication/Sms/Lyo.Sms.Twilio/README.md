...
Twilio SMS and MMS via `Lyo.Sms`. `TwilioSmsService` implements `ISmsService`.

## Features

- **Twilio.** SMS and MMS via the Twilio API.
- **Bulk.** Batch SMS with rate limiting.
- **MMS.** As many as 10 media attachments per message.
- **Query.** `GetMessagesAsync` filtered by `SmsMessageQueryFilter`.
- **Errors.** Twilio-specific codes on `TwilioSmsResult`.
- **Logging.** Writes through Microsoft.Extensions.Logging.
- **Metrics.** Optional instrumentation for SMS operations.
- **DI.** `AddTwilioSmsService` plus `AddTwilioSmsServiceFromConfiguration`.
- **Async.** Every method accepts a `CancellationToken`.
- **Concurrency.** Safe to use from multiple threads.
- **Validation.** `TwilioOptionsValidator` verifies required options.
...
Both wire the same singletons: `TwilioOptions`, `TwilioOptionsValidator`, `TwilioSmsService`, plus
...
configuration apply on their own.
...
Events still fire when operations fail, so you can track every attempt. Useful for monitoring, logging, and user notifications.

## Twilio failure codes

Twilio-specific error codes land on the result via `TwilioSmsResult.TwilioErrorCode`:
...
Built-in retry or timeout logic is not included. Apply resilience at the application layer (e.g. using [Lyo.Resilience](https://www.nuget.org/packages/Lyo.Resilience) with `AddLyoResilienceHandler` on the HttpClient, or by wrapping `ISmsService` calls) as needed.
...
- Every instance field is readonly
- Bulk runs use thread-safe collections (`ConcurrentBag`)
- Rate limiting uses `SemaphoreSlim` so concurrency stays thread-safe
- The Twilio SDK client underneath is thread-safe

## `TwilioOptions` fields
...
| `BulkSmsConcurrencyLimit` | `int` | No | 10 | Maximum concurrent bulk SMS requests |
| `MaxMessageBodyLength` | `int` | No | 1600 | Maximum message body length in characters |
| `MaxBulkSmsLimit` | `int` | No | 1000 | Maximum messages in one bulk operation |
| `EnableMetrics` | `bool` | No | false | Turn on metrics collection |
...
Logs mask phone numbers (last 4 digits only).

## Metrics

Turn on `EnableMetrics` to collect:
...
- Validation is wired via `services.AddOptions<TwilioOptions>().ValidateOnStart()` when using `AddTwilioSmsServiceFromConfiguration()` / `AddTwilioSmsService(IConfiguration, ...)`.
