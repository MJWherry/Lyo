...
- **`AddSmsDbContextFactory(PostgresSmsOptions)`** / **`AddSmsDbContextFactory`** (`Action<PostgresSmsOptions>`) — singleton options + **`AddDbContextFactory`** with **
  `UseNpgsql` ** and migration history schema.
