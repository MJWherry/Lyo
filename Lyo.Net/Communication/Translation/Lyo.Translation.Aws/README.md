...
`AwsTranslationOptions` if no `IAmazonTranslate` is already registered. The other two overloads do **not** register an `IAmazonTranslate`; bring your own if you want explicit
credentials or sharing. The service constructor accepts an optional `IAmazonTranslate` and resolves one from DI when present.
