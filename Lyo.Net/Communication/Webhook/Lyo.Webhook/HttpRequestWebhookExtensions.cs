...
/// <summary>Reads raw bytes for signature verification; the payload must match what the sender signed.</summary>
public static class HttpRequestWebhookExtensions
{
    /// <summary>Buffers and reads the full request body, then rewinds the stream so later middleware or model binding can read it again.</summary>
    ...
    /// <summary>Builds a case-insensitive header dictionary for <see cref="WebhookVerificationContext.Headers" />.</summary>
    ...
    /// <summary>Resolves the public request URL (scheme + host + path + query). Prefer forwarded headers when sitting behind a reverse proxy.</summary>
