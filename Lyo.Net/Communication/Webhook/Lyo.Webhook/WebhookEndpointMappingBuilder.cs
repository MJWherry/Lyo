...
public sealed class WebhookEndpointMappingBuilder
{
    ...
    /// <summary>Registers the signature verifier that runs before your handler.</summary>
    public VerifiedWebhookEndpointBuilder Verify(IWebhookSignatureVerifier verifier)
    {
        ArgumentHelpers.ThrowIfNull(verifier);
        return new(_endpoints, _pattern, _httpMethods, verifier);
    }
}

/// <summary>Configures the handler after <see cref="WebhookEndpointMappingBuilder.Verify" />.</summary>
public sealed class VerifiedWebhookEndpointBuilder
{
    ...
    /// <summary>Maps a handler that is given the verified raw body.</summary>
    public RouteHandlerBuilder Handle(Func<WebhookHandlerContext, Task> handler)
    {
        ArgumentHelpers.ThrowIfNull(handler);
    ...
    /// <summary>Maps a handler that receives the verified raw body plus a cancellation token.</summary>
    ...
    /// <summary>Maps a JSON handler after signature verification, deserializing the body to <typeparamref name="TRequest" />.</summary>
    ...
    /// <summary>Maps a JSON handler that also receives a cancellation token, after signature verification and deserialize to <typeparamref name="TRequest" />.</summary>
    ...
    /// <summary>Builds <see cref="WebhookVerificationContext" /> and, for providers like Twilio, optional form-urlencoded parameters.</summary>
