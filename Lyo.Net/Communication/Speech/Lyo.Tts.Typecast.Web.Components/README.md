...
- **Synthesize** / **Test Connection** actions backed by the resolved `TypecastTtsService` (and its bound `TypecastOptions` for defaults). The component requires `TypecastTtsService`, `TypecastOptions`, `IJsInterop`, and `ISnackbar` in DI, registered via [`Lyo.Tts.Typecast`](../Lyo.Tts.Typecast/README.md) and [`Lyo.Typecast.Client`](../../../Integration/Typecast/Lyo.Typecast.Client/README.md).
