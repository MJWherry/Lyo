...
- `TypecastTtsAppService` — a thin adapter that converts `TtsResult<TypecastTtsRequest>` into
  `TtsSynthesisResult`.
- `ITtsService` (non-generic) backed by `TypecastTtsAppService`, so consumers that only depend on the simple contract can resolve it directly.

`DefaultVoiceId`, `DefaultModel`, `MaxTextLength`, and bulk limits come from `TypecastOptions` (which inherits shared fields from [
`TtsServiceOptions`](../Lyo.Tts.Models/README.md)). `TypecastTtsService`
