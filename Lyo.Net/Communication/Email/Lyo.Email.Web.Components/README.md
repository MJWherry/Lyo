...
MudBlazor workbench that sends mail through an injected `IEmailService`.

## Components

- `EmailWorkbench` builds and sends mail through the email service.
- To / Cc / Bcc chip fields that validate email format.
- From address and From name overrides.
- Subject, HTML body via `LyoRichTextEditor`, plus a plain-text or raw-HTML fallback editor.
- Attachment uploads via `LyoFileUpload` (as many as five files, with progress and callbacks for `Started`, `Progress`, `Completed`, `Cancelled`, and `Failed`).
