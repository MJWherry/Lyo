...
/// <summary>RabbitMQ-backed <c>IMqService</c> with error handling, metrics, logging, and connection lifecycle.</summary>
public sealed class RabbitMqService : IRabbitMqService, IAsyncDisposable
{
    ...
    /// <summary>Wait queues already declared for delayed publish (see <see cref="SendToQueueDelayed" />), so later publishes skip a second declare.</summary>
    ...
    /// <summary>Closes the broker connection and frees owned resources. The instance cannot be used after disposal.</summary>
    ...
    public async Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        try {
            ...
            // Retry a bounded number of times while the broker is still coming up.
            ...
            // Declare DefinedQueues when the options list them
        ...
        CancellationToken ct = default)
    {
        ...
        try {
            ...
            // Create this queue's processing semaphore when a per-queue or global limit is set
    ...
    public async Task<bool> DeleteQueue(string queueName, bool ifUnused = false, bool ifEmpty = false, CancellationToken ct = default)
    {
        ...
        var sw = Stopwatch.StartNew();
        try {
            // Cancel the consumer when one is registered
            ...
            // Drop the processing semaphore when one was created
    ...
    public async Task<bool> SendToQueue(string queueName, byte[] data)
    {
        ...
        try {
            ...
            // Confirms are on and the broker nacked the publish (or never confirmed it).
            sw.Stop();
    ...
    /// <summary>
    /// Publishes a message that appears on <paramref name="queueName" /> only after <paramref name="delay" />. Uses a companion wait queue
    /// (<c>{queueName}.wait</c>) whose dead-letter target is the destination: the payload is published there with a per-message TTL, then the broker
    /// dead-letters it when the TTL expires. Needs no broker plugin.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        ...
        if (!created) {
            // PRECONDITION_FAILED on redeclare is typical: RabbitMQ will not change arguments on a queue that already exists.
            _logger.LogError(
    ...
    public async Task<bool> SubscribeToQueue(string queueName, Func<byte[], Task<bool>> onMessage, CancellationToken ct = default)
    {
        ...
        try {
            // Open one channel per subscriber. Dispatch concurrency is how many handler calls run in parallel on that channel
            // (7.x defaults to 1, so handlers run one after another).
            var limit = _options.GetProcessingLimit(queueName);
            ...
            // Prefetch backpressure: at most `limit` unacked messages reach this consumer; the rest remain on the broker.
            if (limit > 0)
                await subscriptionChannel.BasicQosAsync(0, (ushort)Math.Min(limit, ushort.MaxValue), false, ct).ConfigureAwait(false);

            // Extra in-process cap on top of prefetch and dispatch limits.
            if (limit > 0 && !_processingSemaphores.ContainsKey(queueName))
                _processingSemaphores[queueName] = new(limit, limit);
    ...
    /// <summary>Publish properties for every outbound message: persistence from <see cref="RabbitMqOptions.PersistentMessages" />, a message id, and a UTC timestamp.</summary>
    ...
    /// <summary>Converts a management API queue object into <see cref="MessageQueueInfo" />. AMQP flags, <c>x-*</c> arguments, and rates go into AdditionalProperties.</summary>
    ...
    /// <summary>Converts a management API exchange object into <see cref="MessageExchangeInfo" />.</summary>
    ...
    /// <summary>
    /// Confirms the queue already exists (passive declare). Subscribers do not create queues — provision them at host startup via <see cref="CreateQueue" /> or
    /// <c>DefinedQueues</c>.
    /// </summary>
    ...
    private async ValueTask ConnectCoreAsync(CancellationToken ct)
    {
        try {
            ...
            // Shared channel for publish and queue-management work. With confirms on,
            // BasicPublishAsync returns only after the broker confirms (and throws on nack).
                ...
                Task.Run(
                    async () => {
                        try {
                            ...
                            // Close then dispose the subscription channel
                            try {
                            ...
                            // Close the channel even when cancel failed
                            try {
                                await channel.CloseAsync(ct).ConfigureAwait(false);
                                await channel.DisposeAsync().ConfigureAwait(false);
                            }
    ...
    private async Task HandleMessageAsync(string queueName, BasicDeliverEventArgs args, Func<byte[], Task<bool>> onMessage, IChannel channel, CancellationToken ct)
    {
        ...
        try {
            // Take the semaphore when a per-queue or global processing limit is on
    ...
    private async Task HandleMessageExceptionAsync(string queueName, string messageId, ulong deliveryTag, Exception ex, IChannel channel, CancellationToken ct = default)
    {
        ...
        switch (_options.ExceptionHandling) {
            ...
            case MessageProcessingExceptionHandling.ThrowAndRemoveFromQueue:
                ...
                // Follow the contract: ack (drop) the message and rethrow the original exception.
                // The client sends consumer-callback exceptions to CallbackExceptionAsync; the consumer stays up.
    ...
    private Task OnConnectionShutdownAsync(object sender, ShutdownEventArgs shutdownEventArgs)
    {
        ...
        // Automatic recovery reconnects and restores channels and consumers, so keep consumer bookkeeping.
        // Without recovery the channels are dead for good — drop them.
