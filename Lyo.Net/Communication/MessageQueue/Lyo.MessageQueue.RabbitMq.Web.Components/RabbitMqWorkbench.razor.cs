...
public partial class RabbitMqWorkbench : IAsyncDisposable
{
    ...
    /// <summary>True when AMQP is connected, or the management API just responded — listing queues does not need AMQP.</summary>
    ...
    {
        if (!await DialogService.ConfirmAsync("Clear queue", $"Purge all messages from '{queue.Name}'?", "Clear"))
            return;
    ...
    {
        if (!await DialogService.ConfirmDeleteAsync($"queue '{queue.Name}'", title: "Delete queue"))
            return;
    ...
    private async Task DeleteExchangeAsync(MessageExchangeInfo exchange)
    {
        ...
        if (!await DialogService.ConfirmDeleteAsync($"exchange '{exchange.Name}'", title: "Delete exchange"))
            return;
    ...
    /// <summary>Publishes to the selected queue with the current envelope, priority, and optional delay.</summary>
    ...
    private async Task RunStatsAutoRefreshAsync(CancellationToken ct)
    {
        ...
        try {
            ...
            // Normal when auto-refresh is off or the component has been disposed.
        }
    }
    ...
    private async Task TryConnectQuietAsync()
    {
        ...
        try {
            ...
            // Listing still goes through the HTTP management API; the chip uses _managementOk as fallback.
        }
    }
