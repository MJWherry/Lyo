...
public sealed class QueueWorkerOptions
{
    ...
    /// <summary>
    /// Base delay applied between retry attempts (<see cref="QueueWorkerBase{TRequest, TResult}.RequeueDelay" />), scaled linearly by the attempt number (attempt 2 waits 2x,
    /// attempt 3 waits 3x, ...). Requires a delay-capable transport (<see cref="IDelayedMqService" />, e.g. RabbitMQ); other transports retry immediately. Null or zero disables the
    /// delay. Default: 2 seconds, so a failing message cannot burn through its whole retry budget in milliseconds.
    /// </summary>
