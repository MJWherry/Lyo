...
- `MessageQueueInfo(Name, State?, Type?, Messages, MessagesReady, MessagesUnacknowledged, Consumers, AdditionalProperties)`. Per-queue snapshot. AMQP declare flags (`durable`, `exclusive`, `auto_delete`) and `x-*` arguments are RabbitMQ-only and live in `AdditionalProperties`.
