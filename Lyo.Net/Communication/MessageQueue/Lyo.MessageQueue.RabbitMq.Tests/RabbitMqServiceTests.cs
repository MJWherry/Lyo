...
/// <summary>Integration tests for <see cref="RabbitMqService" /> against a live RabbitMQ broker (Testcontainers). Docker is required.</summary>
public sealed class RabbitMqServiceTests
{
    ...
    /// <summary>Polls until <paramref name="condition" /> is true or the timeout expires. Management API stats trail broker state by a few seconds.</summary>
    ...
    public async Task Connect_Disconnect_Reconnect_Works_On_Same_Instance()
    {
        ...
        // Guards a regression: DisconnectAsync used to mark the instance disposed, which blocked reconnects.
        await service.ConnectAsync(Ct);
        Assert.True(service.IsConnected());
    ...
    public async Task Create_Clear_Delete_Queue_And_Peek_Via_Management_Api()
    {
        ...
        // Peek does not ack, so the message remains on the queue.
    ...
    public async Task Handler_True_Redelivers_False_Acks()
    {
        ...
        // After the ack the queue is empty.
        Assert.True(await WaitUntilAsync(async () => (await service.PeekQueueMessages(queue, ct: Ct)).Count == 0));
    }
    ...
    public async Task Delayed_Message_Arrives_After_Not_Before_Its_Delay()
    {
        ...
        // Not sooner: nothing is delivered while the message sits in the wait queue.
    ...
    public async Task Dlq_Auto_Wiring_Routes_Broker_Rejected_Messages_To_Dlq()
    {
        ...
        // Per-queue TTL dead-letters an unconsumed message on the broker — the app never sees it.
    ...
    public async Task Queue_Stats_Reflect_Published_Messages()
    {
        ...
        // Management API stats emit on an interval — poll until they catch up.
        Assert.True(
    ...
    /// <summary>Records the peak number of handler invocations running at once across a fixed set of messages.</summary>
