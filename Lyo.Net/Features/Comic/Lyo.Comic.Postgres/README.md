...
Migrations reside under **`Migrations/`**. Entities map series/volumes/chapters/pages/characters/alternate titles with foreign keys aligning to store operations (consult `*_Entity.cs` + configurations for cascades—you want deletes on series to prune volumes/chapters unless you purposely soft-delete in future revisions).
...
Internal steps mirror other Postgres features: **`AddSingleton<IOptions<PostgresComicOptions>>`**, **`IDbContextFactory<ComicDbContext>`**, `UseNpgsql` with migrations history schema from options.
