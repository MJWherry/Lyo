...
public partial class ComicViewer : IAsyncDisposable
{
    ...
    /// <summary>
    /// Full ordered list of chapters available for navigation (same language / filter as the reading session).
    /// Used to populate the chapter selector and chapter-skip buttons.
    /// </summary>
    [Parameter]
    ...
    /// <summary>
    /// Raised when the viewer wants to change to a different chapter (e.g. user picks from the selector
    /// or presses the skip-chapter buttons). Your host should update <see cref="CurrentChapter"/> and
    /// reset <see cref="CurrentPage"/> to 1, then call <see cref="ReloadPageAsync"/>.
    /// </summary>
    [Parameter]
    public EventCallback<ComicChapter> OnChapterChanged { get; set; }

    /// <summary>
    /// Raised specifically when the user goes backwards across a chapter boundary (i.e. pressing
    /// previous on page 1). Your host should update <see cref="CurrentChapter"/> and set
    /// <see cref="CurrentPage"/> to the chapter's last page, then call <see cref="ReloadPageAsync"/>.
    /// Falls back to <see cref="OnChapterChanged"/> if not set.
    /// </summary>
    [Parameter]
    ...
    /// <summary>
    /// Raised when the viewer wants to navigate to a different page. Your host should update
    /// <see cref="CurrentPage"/> then call <see cref="ReloadPageAsync"/>.
    /// </summary>
    [Parameter]
    public EventCallback<int> OnPageChanged { get; set; }

    /// <summary>
    /// Total number of pages in the current chapter. Used to drive the progress slider and
    /// page counter. Falls back to <see cref="ComicChapter.PageCount"/> when set to 0.
    /// </summary>
    [Parameter]
    public int TotalPages { get; set; }

    /// <summary>
    /// Called by the viewer when it needs the image for a given page. Receives the series,
    /// chapter, page number, and a <see cref="CancellationToken"/>. Must return a display-ready
    /// URL or Base-64 data-URI, or <c>null</c> if the page is unavailable.
    /// </summary>
    [Parameter, EditorRequired]
    ...
    private int EffectiveTotalPages =>
        TotalPages > 0 ? TotalPages : (CurrentChapter?.PageCount ?? 1);

    /// <summary>
    /// The page number shown in the counter and slider. During rapid JS navigation
    /// this reflects the target page immediately; once the image loads it falls
    /// back to <see cref="CurrentPage"/> (the authoritative Blazor value).
    /// </summary>
    ...
    private int CurrentChapterIndex
    {
        get
        {
            if (CurrentChapter is null) return -1;
            for (var i = 0; i < Chapters.Count; i++)
                if (Chapters[i].Id == CurrentChapter.Id) return i;
            return -1;
        }
    }
    ...
    private static string ChapterToString(ComicChapter? c) =>
        c is null ? string.Empty : $"Ch. {c.ChapterNumber:G}{(string.IsNullOrWhiteSpace(c.Title) ? "" : $" – {c.Title}")}";
        ...
        if (firstRender)
        {
            try
            {
                _jsModule = await Js.InvokeAsync<IJSObjectReference>(
                    "import", "./_content/Lyo.Comic.Web.Components/scripts/comicViewer.js");
            }
            catch (JSException)
            {
                ...
                _jsModule = await Js.InvokeAsync<IJSObjectReference>(
                    "import", $"./_content/Lyo.Comic.Web.Components/scripts/comicViewer.js?v={bust}");
            }
    ...
    /// <summary>
    /// Re-fetches and displays a page image. Pass <paramref name="forPage"/> when calling
    /// immediately after updating your local state — at that point the <see cref="CurrentPage"/>
    /// parameter may not yet reflect the new value (Blazor re-renders asynchronously).
    /// Omit it to use the current parameter value (e.g. after a chapter change where the
    /// page resets to 1 and you haven't changed it locally beforehand).
    /// </summary>
    public async Task ReloadPageAsync(int? forPage = null) =>
        await FetchAndDisplayPageAsync(forPage ?? CurrentPage);
    
    /// <summary>
    /// Called by JS on every keypress to keep the page counter and slider in sync
    /// during rapid navigation, without triggering an image load.
    /// </summary>
    [JSInvokable]
    ...
    /// <summary>
    /// Called by JS after it navigates to a page. When <paramref name="imageAlreadyShown"/> is
    /// true, JS already swapped the &lt;img&gt; src so Blazor skips the server round-trip and
    /// only syncs state (page counter, slider).
    /// </summary>
    [JSInvokable]
    public async Task OnJsPageChanged(int page, bool imageAlreadyShown)
    {
        if (imageAlreadyShown) _jsShownPage = page;
    ...
    /// <summary>
    /// Tells JS the current page number and total pages so it can track chapter boundaries.
    /// Does NOT register the current page's URL — only upcoming pages go into JS's urlMap
    /// (via PrefetchPagesAsync) to avoid triggering a browser cache revalidation on the
    /// already-visible image.
    /// </summary>
    private async Task SyncJsStateAsync(int page)
    {
        if (_jsModule is null) return;
        try { await _jsModule.InvokeVoidAsync("setPageState", _viewerId, page, EffectiveTotalPages); }
        catch { }
    }

    private async Task PrefetchPagesAsync(int currentPage)
    {
        if (LoadPageImageAsync is null || CurrentChapter is null || _jsModule is null) return;

        foreach (var offset in new[] { 1, 2, 3, -1 })
        {
            var p = currentPage + offset;
            if (p < 1 || p > EffectiveTotalPages) continue;
            if (_prefetchCache.ContainsKey(p)) continue;
    ...
    private async Task OnChapterSelectedAsync(ComicChapter chapter) =>
