...
{
    /// <summary>
    /// Executes a search against your data source. Receives a <see cref="ComicSeriesQuery"/> built
    /// from the current filter state and a <see cref="CancellationToken"/>; must return a list of
    /// matching <see cref="ComicSeries"/> results.
    /// </summary>
    [Parameter, EditorRequired]
    public Func<ComicSeriesQuery, CancellationToken, Task<IReadOnlyList<ComicSeries>>>? SearchFunc { get; set; }

    /// <summary>
    /// All tag strings available for filtering. The host provides these (e.g. by loading distinct
    /// tags for the "ComicSeries" entity type from <c>ITagStore</c>).
    /// </summary>
    [Parameter]
    ...
    /// <summary>
    /// Resolves a display-ready cover image URL from the series' <see cref="ComicSeries.CoverImageRef"/> storage key.
    /// Return <c>null</c> to show the placeholder. Implement once you have your file-storage infrastructure in place.
    /// </summary>
    [Parameter]
    ...
    private IReadOnlyList<LyoSelectOption<string>> TagItems =>
        AvailableTags.Select(t => new LyoSelectOption<string>(t, t)).ToList();

    private string? ResolveCoverUrl(ComicSeries series) =>
        ResolveCoverUrlFunc?.Invoke(series);

    private ComicSeriesQuery BuildQuery(int skip, int limit) => new()
    {
