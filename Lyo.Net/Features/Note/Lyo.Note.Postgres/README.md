...
PostgreSQL implementation of `Lyo.Note` using Entity Framework Core. Persists notes to the `note.note` table (`PostgresNoteOptions.Schema = "note"`) and ships migrations. Notes have **subject** / **actor** entity references (`for_entity_*` / `from_entity_*`).
