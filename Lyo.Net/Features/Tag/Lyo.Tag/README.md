...
Abstractions for tagging entities. Tags key off an `EntityRef` (what is tagged) and an optional second `EntityRef` (who applied the tag), so a feature can attach tags without a foreign-key coupling.
