...
`address_source`](../Lyo.Geolocation.Postgres/README.md):

- Owner identity on parent **`address`** (`address_id` FK)
- **`source_entity_*`** + **`imported_at`** — external source (e.g. `GoogleMapsPlace` + place id)

The **`source_entity_type`** string is chosen by the **importing application** (e.g. `Lyo.Google.Geolocation.Client`), not this package. Use *
