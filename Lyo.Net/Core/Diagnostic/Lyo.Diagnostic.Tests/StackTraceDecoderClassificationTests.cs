...
public sealed class StackTraceDecoderClassificationTests
{
    ...
    {
        var decoder = new StackTraceDecoder(new() { UserCodePrefixes = ["MyApp"], RestrictUserCodeToListedPrefixes = true });
    ...
    {
        var decoder = new StackTraceDecoder(new() { ExtraSystemPrefixes = ["Moq."], RestrictUserCodeToListedPrefixes = false });
        var trace = decoder.Decode(MoqAndTestStack);
        var moqFrame = trace.AllFrames.Single(f => f.FullMethod.Contains("Moq.Mock.Verify", StringComparison.Ordinal));
        Assert.Equal(FrameCategory.SystemOrThirdParty, moqFrame.Category);
    }
    ...
    public void Decode_StrictUserPrefixes_TreatsUnknownLibs_As_ThirdParty()
    {
        ...
        var strict = new StackTraceDecoder(new() { UserCodePrefixes = ["MyApp."], RestrictUserCodeToListedPrefixes = true });
    ...
    public void Decode_Legacy_UnclassifiedNamespace_Is_UserCode_When_Not_Strict()
    {
        ...
        var legacy = new StackTraceDecoder(new() { RestrictUserCodeToListedPrefixes = false });
    ...
    public void Decode_ExtraSystemPrefixes_Overrides_UnknownNamespace_UnderStrict()
    {
        ...
        var decoder = new StackTraceDecoder(new() { UserCodePrefixes = ["MyApp."], ExtraSystemPrefixes = ["WeirdCo."], RestrictUserCodeToListedPrefixes = true });
