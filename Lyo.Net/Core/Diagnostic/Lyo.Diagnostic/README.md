...
Stack trace decoding, exception classification, breadcrumb trails, an in-memory error inbox, sanitisation, and structured logging.
...
- **Breadcrumbs** (`IBreadcrumbTrail`, `RingBufferBreadcrumbTrail`). Bounded FIFO trail of short events for triage. Cap per scope, for example an HTTP request.
...
Call `IBreadcrumbTrail.Add` at meaningful steps (cache miss, downstream call started, etc.). Keep `Message` and `Data` values small. Avoid secrets, tokens, full query strings, emails, and raw URLs. Prefer opaque IDs and coarse categories. Optional `IBreadcrumbRedactor` can strip known keys on each add.

## In-memory inbox limits

`InMemoryErrorInbox` drops oldest occurrences when over `MaxOccurrences`. Data is lost on restart and is not visible across multiple server processes. For durable aggregation, implement `IErrorOccurrenceSink` / `IErrorInboxReader` with Postgres or an external product such as Sentry.

## Fingerprint

The fingerprint hashes user stack frame method names only, not line numbers. The same defect shape often keeps the same key across minor edits. See `StackTraceDecoder`.
