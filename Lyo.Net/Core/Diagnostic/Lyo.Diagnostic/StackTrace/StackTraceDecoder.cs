...
public sealed class StackTraceDecoder : IStackTraceDecoder
{
    ...
    /// <summary>Splits a stripped method path into namespace / type / method segments.</summary>
    private static readonly Regex MethodNameRegex = new(@"^(?<ns>.+?)\.(?<type>[^.]+)\.(?<method>[^.(]+)", RegexOptions.Compiled | RegexOptions.CultureInvariant);

    /// <summary>Detects the "   ---> ExceptionType: message" inner-exception separator.</summary>
    ...
    {
        // Caller-supplied prefixes take priority so internal libs are never
        // misclassified as third-party frames
    ...
    /// <summary>
    /// Innermost user frame across this exception and nested inners (first user frame in the innermost inner that has one, else the outer trace’s innermost user frame). Mirrors
    ...
    private IReadOnlyList<RecursionInfo> DetectRecursion(IReadOnlyList<StackFrame> frames)
    {
        ...
        while (i < frames.Count) {
            ...
            for (var period = 1; period <= limit; period++) {
                ...
                // One block of length `period` is not repetition; need at least two full cycles (for example ABAB, or AAA).
    ...
    /// <summary>
    /// Repeating known BCL / third-party-only tails (for example AutoMapper <c>Map → Map</c>) are ignored. Under
    ...
    {
        // Hash method signatures only (no line numbers) so the fingerprint stays
        ...
        // First 8 bytes → 16 uppercase hex characters (64-bit prefix of SHA-256).
