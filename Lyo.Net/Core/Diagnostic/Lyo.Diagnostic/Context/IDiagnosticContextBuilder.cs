...
{
    /// <summary>Builds a full diagnostic context from a live exception, without request metadata.</summary>
    DiagnosticContext Build(Exception exception);

    /// <summary>Builds a full diagnostic context from a live exception plus request metadata.</summary>
    DiagnosticContext Build(Exception exception, RequestMetadata request);

    /// <summary>Builds from an already-decoded trace (avoids decoding twice in middleware).</summary>
    DiagnosticContext Build(DecodedStackTrace trace, Exception exception, RequestMetadata request);

    /// <summary>
    /// Like <see cref="Build(Exception)" /> but uses <see cref="IStackTraceDecoder.DecodeAsync(Exception, CancellationToken)" /> when a package store is
    /// configured.
    /// </summary>
