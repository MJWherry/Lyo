...
/// <summary>Periodic-table element, optionally carrying a standard atomic weight.</summary>
/// <remarks>See <see cref="PeriodicTable.All" /> for the curated list through oganesson (Z = 118).</remarks>
[DebuggerDisplay("{ToString(),nq}")]
public sealed record ChemicalElement
{
    /// <summary>Atomic number Z (proton count); must be positive.</summary>
    public int AtomicNumber { get; init; }

    /// <summary>IUPAC element symbol, for example <c>Fe</c> or <c>U</c>.</summary>
    public string Symbol { get; init; }

    /// <summary>English element name, for example <c>Iron</c>.</summary>
    public string Name { get; init; }

    /// <summary>Optional relative atomic mass (dimensionless, carbon-12 scale) when this catalog row has one.</summary>
    /// <remarks>When present, must be strictly positive. <see langword="null" /> means no numeric mass was supplied (typical for synthetic or heavy stub rows).</remarks>
    public double? AtomicMass { get; init; }

    /// <summary>Mints an element row after checking identifiers and the optional mass.</summary>
    /// <param name="atomicNumber">Strictly positive atomic number Z.</param>
    /// <param name="symbol">Non-empty IUPAC symbol.</param>
    /// <param name="name">Non-empty English name.</param>
    /// <param name="atomicMass">Optional mass; when supplied, must be strictly greater than zero.</param>
    /// <exception cref="ArgumentOutsideRangeException">
    /// <paramref name="atomicNumber" /> is not positive, or <paramref name="atomicMass" /> is present and not strictly positive.
    /// </exception>
...
/// <summary>Catalog of <see cref="ChemicalElement" /> rows covering Z = 1 through 118.</summary>
/// <remarks>Many rows omit atomic masses; use <see cref="ElementAtomicMasses" /> for numeric weights in stoichiometry.</remarks>
public static class PeriodicTable
{
    /// <summary>Elements in ascending <see cref="ChemicalElement.AtomicNumber" /> order.</summary>
