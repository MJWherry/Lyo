...
/// <summary>One term in a parsed empirical or molecular formula: element plus stoichiometric count.</summary>
...
public sealed record ChemicalFormulaPart
{
    ...
    /// <summary>Periodic-table element this term refers to.</summary>
    public ChemicalElement Element { get; init; }

    /// <summary>Mints a formula part after checking that <paramref name="count" /> is positive.</summary>
...
/// <summary>Parsed chemical formula split into <see cref="ChemicalFormulaPart" /> entries.</summary>
[DebuggerDisplay("{ToString(),nq}")]
public sealed record ChemicalCompound
{
    /// <summary>Original formula text, for example <c>H2O</c> or <c>Ca(OH)2</c>.</summary>
    public string Formula { get; init; }

    /// <summary>Expanded element/count pairs emitted by the parser.</summary>
    public IReadOnlyList<ChemicalFormulaPart> Parts { get; init; }

    /// <summary>Mints a compound after checking a non-empty <paramref name="formula" /> and non-null <paramref name="parts" />.</summary>
