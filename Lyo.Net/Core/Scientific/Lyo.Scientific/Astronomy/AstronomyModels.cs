...
/// <summary>Solar-system planet and dwarf-planet rows with bulk physical parameters.</summary>
/// <remarks>Masses, radii, and orbits are reference values for demos and calculators; use mission data for precision work.</remarks>
/// <summary>Coarse classification for rows in <see cref="PlanetaryBodies" />.</summary>
public enum PlanetaryBodyKind
{
    /// <summary>Classical planet (the cleared-neighborhood rule is not enforced here).</summary>
    Planet,

    /// <summary>Dwarf-planet catalog row.</summary>
    DwarfPlanet
}

/// <summary>Reference bulk properties for a solar-system planet or dwarf planet used by helpers.</summary>
/// <remarks>Orbital and rotation periods are illustrative, not ephemeris-grade.</remarks>
[DebuggerDisplay("{ToString(),nq}")]
public sealed record PlanetaryBody
{
    /// <summary>English name, for example <c>Earth</c> or <c>Pluto</c>.</summary>
    public string Name { get; init; }

    /// <summary>Known natural-satellite count from this catalog snapshot (not live).</summary>
    public int NaturalSatelliteCount { get; init; }

    /// <summary>Planet versus dwarf-planet classification for display.</summary>
    public PlanetaryBodyKind Kind { get; init; }

    /// <summary>Estimated bulk mass.</summary>
    public Mass Mass { get; init; }

    /// <summary>Mean spherical radius of the body.</summary>
    public Length MeanRadius { get; init; }

    /// <summary>Heliocentric semi-major axis under a circular-orbit abstraction.</summary>
    ...
    /// <summary>Representative mean surface temperature for the body.</summary>
    public Temperature MeanSurfaceTemperature { get; init; }

    /// <summary>True when the body is commonly illustrated with a ring system.</summary>
    public bool HasRingSystem { get; init; }

    /// <summary>Mints a catalog row after checking <paramref name="name" /> and a non-negative moon count.</summary>
...
/// <summary>Curated solar-system catalog consumed by astronomy helper functions.</summary>
