...
- Input: distinct stripped prefixes (typically one per decoded stack frame subtree).
- Output: a dictionary with one entry per input key; the value is `null` when nothing matches contract rules.
- Requested keys are never dropped silently. That invariant keeps `Lyo.Diagnostic` bulk decode paths deterministic.

## Caching the prefix catalog (`PostgresPrefixCatalogCachingMode`)

Repeated DB chatter is gated by `PostgresPackageMetadataOptions.PrefixCatalogCaching`:

| Mode | What it means |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `InvalidateOnRegisterManyOrClear` | Keep an immutable in-process snapshot. Reload when mutations bump the generation counter (`RegisterManyAsync` or `ClearPrefixCatalogCache` on the same instance). |
| `Disabled` | Always hit Postgres. Wrap with an outer cache yourself if importing huge catalogs asynchronously. |

When caching is disabled, `ClearPrefixCatalogCache()` is a no-op on the cache. It only bumps the generation.
...
Migrations live under `Migrations/`. Table names and schema follow `PostgresPackageMetadataOptions.Schema`. Inspect the snapshot for authoritative DDL.

## Related reading

- Stack decode and breadcrumb tooling that optionally consumes `IPackageMetadataStore`: [`Lyo.Diagnostic`](../../Diagnostic/Lyo.Diagnostic/README.md).
