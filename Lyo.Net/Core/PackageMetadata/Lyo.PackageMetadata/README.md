...
- **`PackageMetadata`** — `sealed record` carrying ecosystem, name/version, optional `ArtifactDigestAlgorithm`/`ArtifactDigestHex`, project/repository/license URLs, SPDX `LicenseExpression` (+ parsed `LicenseExpressionSyntax`), `CreatedAt`/`UpdatedAt` timestamps.
