...
/// <summary>Parses SPDX license expressions into <see cref="SpdxLicenseExpressionSyntax" /> (keeps <c>AND</c>, <c>OR</c>, <c>WITH</c>).</summary>
public static class PackageLicenseExpression
{
    /// <summary>Parses <paramref name="licenseExpression" /> when it matches a supported SPDX subset; otherwise returns <see langword="null" />.</summary>
    public static SpdxLicenseExpressionSyntax? TryParseSyntax(string? licenseExpression) => SpdxLicenseExpressionParser.TryParse(licenseExpression);

    /// <summary>Distinct license and exception identifiers in <paramref name="licenseExpression" /> (sorted); <see langword="null" /> when parse fails.</summary>
