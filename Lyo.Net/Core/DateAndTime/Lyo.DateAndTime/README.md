...
Dates, times, US timezone conversion, day-of-week scheduling, and US holiday metadata. The API is static and thread-safe. No mutable shared state.

**Target frameworks.** `netstandard2.0`, `net10.0`

## Time zones and `DateAndTime`

`GeographicInfo.FromState(usState)` in `Lyo.Common.Json` is how `DateAndTime` resolves `TimeZoneInfo`. Each US state maps to an IANA identifier (for example `America/New_York`, `America/Chicago`, `America/Denver`, `America/Los_Angeles`, `America/Phoenix`, `America/Anchorage`, `Pacific/Honolulu`). If the OS does not recognize the ID, `GetTimeZoneByState` returns `null` and conversion helpers return `null` instead of throwing.
...
Local `DateTime` in the given state is what scheduling uses (`GetCurrentLocalDateTime` / conversions). `DayFlags` is a `[Flags]` enum from `Lyo.Common.Json` (weekday bits). Internally the
day flag is `1 << (int)date.DayOfWeek`.

**Constants (not configurable).** The next-slot search looks up to 7 days ahead. Discrete-time past-due scans up to 7 days of history when `lastRunDateTime` is null. The default
...
**`TimeOnly` vs `TimeOnlyModel`.** On `net6.0` or greater, scheduling APIs use `System.TimeOnly`. Use `Lyo.DateAndTime.TimeOnlyModel` on `netstandard2.0`, which is
...
`HolidayDateRule` values: `FixedDate`, `Unknown`, `LastWeekdayOfMonth`, `NthWeekdayOfMonth`.

An immutable `record`, `HolidayInfo` has `Name`, `Slug`, `Description`, `IsFederal`, `IsObservedWhenWeekend`, `DateRule`, `Month`, `DayOfMonth`, `DayOfWeek`, `Occurrence`,
