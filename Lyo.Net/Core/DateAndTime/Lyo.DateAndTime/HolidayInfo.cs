...
/// <summary>Immutable metadata for a U.S. civic or federal holiday, including rules that compute calendar dates and optional weekend observance shifts.</summary>
/// <remarks>
/// <para>
/// Static instances register themselves into lookup dictionaries (by slug, name, and aliases) in the static constructor. Use <see cref="FromSlug" />, <see cref="FromName" />, or
...
/// <param name="Description">Plain-language explanation suitable for tooltips.</param>
/// <param name="IsFederal"><see langword="true" /> when this catalog treats the holiday as a U.S. federal holiday.</param>
/// <param name="IsObservedWhenWeekend">When <see langword="true" />, <see cref="GetObservedDate" /> moves Saturday/Sunday occurrences to adjacent weekdays.</param>
/// <param name="DateRule">Algorithm that locates the holiday in a calendar year.</param>
...
/// <param name="Occurrence">1-based weekday occurrence in the month when <paramref name="DateRule" /> is <see cref="HolidayDateRule.NthWeekdayOfMonth" />.</param>
    ...
    string[] Aliases)
{
    ...
    /// <summary>Stable id, the same value as <see cref="Slug" />.</summary>
    public string CanonicalName => Slug;

    /// <summary>Every compiled-in holiday instance (sentinel duplicates omitted).</summary>
    public static IReadOnlyList<HolidayInfo> All => AllHolidays;

    /// <summary>Holidays whose <see cref="IsFederal" /> flag is <see langword="true" />.</summary>
    ...
    /// <summary>Looks up a holiday by canonical <see cref="Name" />, then falls back to <see cref="FromAlias" />.</summary>
    ...
    /// <summary>Looks up a holiday by its URL <see cref="Slug" />.</summary>
    ...
    /// <summary>Looks up a holiday by any registered alias (case-insensitive).</summary>
    ...
    /// <summary>Calculates the holiday's calendar date in <paramref name="year" />.</summary>
    /// <exception cref="InvalidOperationException">The rule is unsupported for this instance (except <see cref="EasterSunday" />).</exception>
    ...
    /// <summary>Calculates the publicly observed date, moving weekend occurrences when <see cref="IsObservedWhenWeekend" /> is on.</summary>
    ...
    /// <summary>True when <paramref name="date" /> matches the holiday's actual calendar day, or the observed day when that option is on.</summary>
    ...
    /// <summary>Returns the first non-<see cref="Unknown" /> holiday that matches <paramref name="date" />, if one exists.</summary>
    ...
    // Meeus/Jones/Butcher algorithm for Western (Gregorian) Easter.
