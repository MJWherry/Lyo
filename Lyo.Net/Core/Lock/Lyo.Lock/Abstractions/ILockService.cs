...
/// <summary>Takes exclusive locks by string key. Implementations may be in-process only or distributed (for example Redis-backed).</summary>
/// <remarks>
/// For manual acquire/release, prefer <c>await using</c> on the returned handle (when non-null) so the lock is released on every path. Distributed implementations honor the
/// lock-duration argument on <see cref="AcquireAsync" /> so a crashed holder cannot block the key forever; local locks ignore it.
/// </remarks>
public interface ILockService
{
    /// <summary>Tries to take an exclusive lock for <paramref name="key" />.</summary>
    /// <param name="key">Logical lock identity (for example <c>order:123</c>). May be normalized per implementation or options.</param>
    /// <param name="timeout">How long to wait for the lock; implementation default when null.</param>
    /// <param name="lockDuration">Distributed locks: Redis key TTL / safety expiry; ignored for purely local locks.</param>
    /// <param name="ct">Cancellation token; waiting operations honor cancellation.</param>
    ...
    /// <summary>Takes the lock, runs <paramref name="action" />, then releases. Throws <see cref="TimeoutException" /> if the lock is not acquired.</summary>
    ...
    /// <summary>Takes the lock, runs <paramref name="action" />, returns its result, then releases.</summary>
