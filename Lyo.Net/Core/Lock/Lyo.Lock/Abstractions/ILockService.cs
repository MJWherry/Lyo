...
/// <summary>
/// Acquires exclusive locks by string key. Implementations may be in-process only or distributed (for example Redis-backed).
/// </summary>
/// <remarks>
/// For manual acquire/release, prefer <c>await using</c> on the returned handle (when non-null) so the lock is released on all paths.
/// The lock-duration argument on <see cref="AcquireAsync"/> is honored by distributed implementations to bound how long a crashed holder can block the key; local locks ignore it.
/// </remarks>
