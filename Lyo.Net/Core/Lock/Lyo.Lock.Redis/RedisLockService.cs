...
/// <summary>
/// Distributed exclusive lock on Redis: acquire is <c>SET key token NX PX ttl</c>; release is a Lua script so only the token owner deletes the key. Waiters can use
/// pub/sub on a companion notify channel to cut polling latency.
/// </summary>
/// <remarks>
/// Register through <see cref="RedisLockServiceExtensions" /> with an existing <see cref="IConnectionMultiplexer" /> or a connection string. Pick
/// <see cref="RedisLockOptions.DefaultLockDuration" /> large enough for typical critical sections; if work outlives the TTL, another instance may acquire — design for that.
/// </remarks>
public sealed class RedisLockService : ILockService
{
    ...
    /// <param name="redis">Shared multiplexer (the same one as the cache stack, when that applies).</param>
    /// <param name="logger">Optional diagnostics.</param>
    /// <param name="options">Timeouts, key prefix, pub/sub versus poll, TTL defaults.</param>
    /// <param name="metrics">Written when <see cref="LockOptions.EnableMetrics" /> is true.</param>
