...
/// <summary>
/// Guards for file name and path-segment validation (for example multipart uploads). For directory/file existence, use <see cref="ExceptionThrower" /> or
...
public static class FileHelpers
{
    ...
    /// <summary>Throws when the file name is invalid for multipart uploads or safe path operations. Rejects path traversal, absolute paths, and invalid characters.</summary>
    /// <param name="fileName">File name to validate (for example "document.pdf", not a full path).</param>
    /// <param name="paramName">Name written to ParamName.</param>
    /// <exception cref="ArgumentException">Thrown if fileName is null, empty, whitespace, contains path traversal (..), is an absolute path, or contains invalid characters.</exception>
#if NET6_0_OR_GREATER
    [StackTraceHidden]
#endif
    ...
    /// <summary>Validates and returns a safe file name for multipart uploads. Trims the path and returns only the final segment.</summary>
    /// <param name="pathOrFileName">A file path or a file name.</param>
    /// <param name="paramName">Name written to ParamName.</param>
    /// <returns>The validated file name (for example "document.pdf").</returns>
    /// <exception cref="ArgumentException">Thrown if pathOrFileName is null, empty, whitespace, or contains invalid characters.</exception>
#if NET6_0_OR_GREATER
    [StackTraceHidden]
#endif
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>Attempts to validate a file name. Returns true when valid.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// whitespace-only. Meant for callers that build storage keys / listing prefixes from user-supplied input.
    /// </summary>
    /// <param name="value">Optional path-prefix string (for example <c>"/tenant/alpha/"</c>).</param>
    /// <returns>Trimmed prefix, or <c>""</c> when the input is null / empty / whitespace.</returns>
#if NET6_0_OR_GREATER
    [StackTraceHidden]
#endif
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <exception cref="ArgumentException">Thrown if <paramref name="value" /> contains a traversal pattern.</exception>
#if NET6_0_OR_GREATER
    [StackTraceHidden]
#endif
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Combines <see cref="NormalizePathPrefix" /> and <see cref="ThrowIfPathPrefixTraversal" />: returns the trimmed prefix on success and throws <see cref="ArgumentException" />
    ...
    /// <exception cref="ArgumentException">Thrown if the prefix contains a traversal pattern.</exception>
#if NET6_0_OR_GREATER
    [StackTraceHidden]
#endif
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
