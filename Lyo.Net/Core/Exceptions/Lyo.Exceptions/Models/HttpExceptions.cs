...
public sealed class GenericHttpException : HttpException
{
    ...
    /// <summary>Builds a <see cref="GenericHttpException" /> for <paramref name="statusCode" />.</summary>
...
/// <summary>Helpers that mint <see cref="HttpException" /> instances from raw HTTP status codes (for example when rethrowing server errors in API clients).</summary>
public static class HttpExceptions
{
    /// <summary>
    /// Returns the dedicated <see cref="HttpException" /> subclass for a known status code (400, 401, 403, 404, 409, 410, 422, 429, 503, 504), or a
    /// <see cref="GenericHttpException" /> for any other code.
    /// </summary>
    /// <param name="statusCode">HTTP status code.</param>
    /// <param name="message">Error text.</param>
    /// <param name="innerException">Cause of this exception.</param>
    /// <returns>Best-matching <see cref="HttpException" /> for <paramref name="statusCode" />.</returns>
