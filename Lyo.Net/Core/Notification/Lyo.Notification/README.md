...
So notifications are **best-effort** side effects: logging + continue. If you need **transactional** semantics or **fail closed** behavior, call handlers explicitly or wrap `PublishAsync` yourself.

Cancellation: **`CancellationToken`** is passed through to **`HandleAsync`**. If cancelled mid-loop, handlers that observe the token stop; handlers already running complete unless they cancel internally.
