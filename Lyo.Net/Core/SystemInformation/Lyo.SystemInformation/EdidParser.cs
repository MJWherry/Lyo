...
/// <summary>Minimal parser for EDID (Extended Display Identification Data) blobs exposed by the kernel (for example <c>/sys/class/drm/*/edid</c>).</summary>
internal static class EdidParser
{
    ...
    /// <summary>True when <paramref name="edid" /> starts with the fixed 8-byte EDID header (<c>00 FF FF FF FF FF FF 00</c>) and is at least one 128-byte block long.</summary>
    ...
    /// <returns>The manufacturer code (for example <c>AUO</c>, <c>DEL</c>), or <see langword="null" /> when the blob is invalid or the letters are out of range.</returns>
    ...
    /// <summary>Reads the monitor model from the display product name descriptor (tag <c>0xFC</c>) in one of the four 18-byte descriptor blocks.</summary>
    /// <returns>The model name trimmed at the <c>0x0A</c> terminator, or <see langword="null" /> when the blob is invalid or has no product-name descriptor.</returns>
    internal static string? GetModelName(byte[]? edid)
    {
        ...
        foreach (var offset in DescriptorOffsets) {
            // Display descriptors (unlike detailed timing descriptors) start with two zero bytes, then a reserved zero byte, then the tag.
