...
/// <see cref="Encrypted" />, <see cref="Compressed" />, <see cref="SizeBytes" />, and <see cref="Expires" /> apply to keys; tags leave them null.
    ...
    DateTime? Expires = null)
{
    ...
    public static CacheItem Key(
        ...
        long? sizeBytes = null,
        DateTime? expires = null)
        => new(name, CacheItemTypeEnum.Key, created ?? DateTime.UtcNow, encrypted, compressed, sizeBytes, expires);

    /// <summary>Creates an item representing a tag used in the tag index. Storage flags and <see cref="Expires" /> stay null.</summary>
    ...
    /// </summary>
    public static CacheItem FromStoredBytes(string name, byte[] stored, DateTime? created = null, DateTime? expires = null)
    {
        ...
        if (!CachePayloadFrame.TryInspect(stored, out var flags, out _))
            return Key(name, created, encrypted, compressed, stored.Length, expires);
