...
/// <summary>An <see cref="ICacheService" /> backed by FusionCache, with an optional Redis backplane.</summary>
public sealed class FusionCacheService : ICacheService
{
        ...
        ICachePayloadSerializer? payloadSerializer = null)
    {
        ...
        _fusionCache.DefaultEntryOptions.FailSafeMaxDuration = _options.FailSafeMaxDuration;
        _fusionCache.DefaultEntryOptions.FailSafeThrottleDuration = _options.FailSafeThrottleDuration;
        ...
        _fusionCache.Events.Set += (_, args) => {
            try {
                if (IsFusionControlKey(args.Key))
                    return;
        ...
        where TDb : class
    {
        ...
        await InvalidateCacheItemByTag(Constants.Tags.EntityType(typeof(TDb))).ConfigureAwait(false);
    }
    ...
    public async Task ClearAsync()
    {
        ...
        try {
            ...
            // FusionCache fires Set/Expire for tombstones (__fc:t:*, __fc:t:!) around the Clear event, so a
            // concurrent _items.Clear() in that handler can lose the race. Wipe L1 tracking after ClearAsync
            // finishes so Items matches LocalCacheService (empty after a successful clear).
            ForgetAllTracked();
            _metrics.RecordGauge(Constants.Metrics.CacheSize, 0);
            stopwatch.Stop();
        }
    ...
    /// <summary>FusionCache tag keys used as clear/expire tombstones; they do not appear as user-visible <see cref="Items" /> rows.</summary>
    private static bool IsFusionControlKey(string key)
        => key.StartsWith(TagPrefix + TagPrefix, StringComparison.Ordinal)
           || key.Equals(TagPrefix + "*", StringComparison.Ordinal)
           || key.Equals(TagPrefix + "!", StringComparison.Ordinal);

    private void ForgetAllTracked()
    {
        _items.Clear();
