...
3. For **tag invalidation** from **`Lyo.Api`**: prefer **`Broad`** vs **`Granular`** tag strategies as documented under **`Lyo.Cache`**—Granular adds per-PK tags and costs more CPU
   on writes.
