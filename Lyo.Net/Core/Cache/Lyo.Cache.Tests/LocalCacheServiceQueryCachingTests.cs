...
/// <summary>
/// <see cref="LocalCacheService" /> keeps a bidirectional tag index on top of <see cref="IMemoryCache" />. Production often uses <see cref="Fusion.FusionCacheService" />
/// with Redis; these tests lock in tagging for local/in-memory scenarios (manual tag work, dev, and tests).
/// </summary>
public sealed class LocalCacheServiceQueryCachingTests : IDisposable
{
    ...
    public LocalCacheServiceQueryCachingTests()
    {
        ...
        _cache = new(_memoryCache, NullLogger<LocalCacheService>.Instance, options);
    }
    ...
    {
        var typeTag = Constants.Tags.EntityType(typeof(PersonTagTestStub));
        var key1 = "q1";
        var key2 = "q2";
