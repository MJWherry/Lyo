...
/// <summary>
/// Measures FusionCache operations with a StackExchange Redis backplane attached, to surface backplane overhead vs the
/// in-process FusionCache. Requires Docker; spins up a throwaway Redis container in <see cref="GlobalSetup" />.
/// </summary>
[BenchmarkDescription("FusionCache get/set/invalidate-by-tag with a StackExchange Redis backplane (throwaway Docker Redis), surfacing backplane overhead vs the in-process FusionCache.")]
...
public class FusionRedisBackplaneBenchmarks
{
    ...
    public void Setup()
    {
        ...
        _value = new CachePayload { Id = 1, Name = "redis", Data = "backplane-value" };
