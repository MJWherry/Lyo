...
The legacy **`person.source`** varchar column is removed; use **`person_source`** instead. Domain: **`EntitySourceRecord.From(source, importedAt)`** on **`IEntitySourceDerived`**
aggregates; lookup by external key uses **`source_entity_*`** (e.g. `GetPersonBySourceAsync`).
