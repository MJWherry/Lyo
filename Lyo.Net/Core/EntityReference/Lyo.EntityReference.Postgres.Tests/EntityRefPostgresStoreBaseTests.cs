...
public class EntityRefPostgresStoreBaseTests
{
    ...
    [Fact]
    public void Ctor_NullTenancyOptions_Throws() => Assert.Throws<ArgumentNullException>(() => new RequiredTenantStore(Options.Create(new EntityRefOptions()), null!));
    ...
    {
        var store = new RequiredTenantStore(Options.Create(new EntityRefOptions { Mode = TenancyMode.SingleTenantDefault, DefaultTenantId = GlobalDefault }), new());
    ...
    {
        var store = new RequiredTenantStore(Options.Create(new EntityRefOptions()), new() { Mode = TenancyMode.MultiTenantStrict });
    ...
    {
        var store = new NullableTenantStore(Options.Create(new EntityRefOptions()), new() { Mode = TenancyMode.SystemOnly });
    ...
    {
        var store = new NullableTenantStore(Options.Create(new EntityRefOptions()), new() { Mode = TenancyMode.MultiTenantStrict });
    ...
    {
        var store = new NullableTenantStore(Options.Create(new EntityRefOptions { DefaultTenantId = GlobalDefault }), new());
