...
`EntityRelationEntityBase` and `EntityRelationOptionalActorBase` implement `[DebuggerDisplay(...)]` and `ToString()` for inspection in the debugger and logs.

## Tenancy

Per-feature tenancy policy is configured via `TenancyOptions` on each `Postgres*Options`, for example `PostgresFavoriteOptions.Tenancy`. The store resolves caller-supplied
...
Bind `EntityRefOptions` once at the host with `AddEntityRefOptionsFromConfiguration`. Each module's `Extensions.cs` binds its own `Postgres*Options`
section.
