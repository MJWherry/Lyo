...
/// <summary>Shared options, interceptor pipeline, and tenant resolution used by PostgreSQL association stores.</summary>
public abstract class EntityRefPostgresStoreBase
{
    /// <summary>Host entity-ref options (default tenant, mode, and related settings).</summary>
    protected EntityRefOptions EntityRefOptions { get; }

    /// <summary>Per-feature tenancy block. Each unset property inherits from <see cref="EntityRefOptions" />.</summary>
    protected TenancyOptions FeatureTenancy { get; }

    /// <summary>Interceptors registered for this store (invoked in registration order).</summary>
    protected IReadOnlyList<IEntityRefActionInterceptor> Interceptors { get; }

    /// <summary>
    /// When <see langword="true" /> the underlying entity column is non-nullable, so <see cref="TenancyMode.SystemOnly" /> is invalid and the ctor will reject it.
    /// </summary>
    /// <remarks>
    /// Stores derived from <see cref="Database.EntityRelationEntityBase" /> leave the default; subclasses on a nullable-tenant entity should pass <see langword="false" /> to the
    /// base ctor.
    /// </remarks>
    protected bool RequiresNonNullTenant { get; }

    /// <summary>Mints the base with resolved options, per-feature tenancy, and optional interceptors.</summary>
        ...
        bool requiresNonNullTenant = true)
    {
        ...
        ArgumentHelpers.ThrowIfNull(featureTenancy);
        EntityRefOptions = entityRefOptions;
    ...
    /// <summary>Resolves a nullable caller tenant under the feature/global policy and requires a non-null result.</summary>
    ...
    /// <summary>
    /// Resolves a nullable caller tenant under the feature/global policy, returning <see langword="null" /> when the mode is <see cref="TenancyMode.SystemOnly" />.
    /// </summary>
    /// <param name="tenantId">Caller-supplied tenant, if any.</param>
    ...
    /// <summary>Invokes registered interceptors for the given persistence phase.</summary>
