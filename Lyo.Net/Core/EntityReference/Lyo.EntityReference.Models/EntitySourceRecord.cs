...
/// <param name="Source">External entity this row was imported from (for example Endato PS person).</param>
...
public readonly record struct EntitySourceRecord(EntityRef Source, DateTime ImportedAt)
{
    ...
    /// <summary>Builds a provenance record for import; owner is determined from the parent aggregate on persist.</summary>
    public static EntitySourceRecord From(EntityRef source, DateTime importedAt) => new(source, importedAt);

    /// <summary>Builds a provenance record using the logical type of <typeparamref name="T" /> and a <see cref="Guid" /> source key.</summary>
    ...
    /// <summary>Builds a provenance record using the logical type of <typeparamref name="T" /> and a string source key.</summary>
    /// <typeparam name="T">CLR type used to resolve the stored source entity type discriminator.</typeparam>
    ...
    /// <summary>Builds a provenance record using the logical type of <typeparamref name="T" /> and one or more source key segments.</summary>
    /// <typeparam name="T">CLR type used to resolve the stored source entity type discriminator.</typeparam>
    /// <param name="importedAt">When this source link was recorded (UTC).</param>
    ...
    /// <summary>Builds a provenance record from an entity instance and a selector that extracts the source key or keys.</summary>
    /// <typeparam name="T">CLR type used to resolve the stored source entity type discriminator.</typeparam>
    ...
    /// <summary>Builds a provenance record from an entity instance and a selector that returns one or more source keys.</summary>
    /// <typeparam name="T">CLR type used to resolve the stored source entity type discriminator.</typeparam>
    /// <param name="entity">Non-null instance to read keys from.</param>
    /// <param name="selector">Returns key segments, including via collection expressions (for example <c>e => [e.Id]</c>).</param>
    /// <param name="importedAt">When this source link was recorded (UTC).</param>
    /// <returns>A new <see cref="EntitySourceRecord" />.</returns>
    public static EntitySourceRecord From<T>(T entity, Func<T, object?[]> selector, DateTime importedAt)
        where T : class
