...
/// <remarks>
/// <para>
/// <see cref="EntityType" /> identifies the kind of entity (for example <c>Person</c>, <c>Product</c>). <see cref="EntityId" /> is typically a Guid string but may be a composite
/// (multi-part keys from <see cref="For{T}(object[])" />).
/// </para>
...
{
    /// <summary>Separator used by <see cref="ToOpaqueToken" /> and <see cref="TryParseOpaque" />.</summary>
    public const char OpaqueSeparator = '\x1f';

    private static readonly ConcurrentDictionary<Type, string> LogicalTypeCache = new();
    ...
    /// <summary>Builds a reference from an entity instance using a selector to extract the key or keys.</summary>
    ...
    /// <returns>
    /// An <see cref="EntityRef" /> whose <see cref="EntityType" /> comes from <typeparamref name="T" /> and whose <see cref="EntityId" /> is built from the keys.
    /// </returns>
    ...
    /// <summary>Builds a reference from an entity instance using a selector that returns one or more keys.</summary>
    /// <typeparam name="T">CLR type used to resolve the logical entity type name.</typeparam>
    /// <param name="entity">Non-null instance to read keys from.</param>
    /// <param name="selector">Returns key segments, including via collection expressions (for example <c>e => [e.Id, e.SecondId]</c>).</param>
    /// <returns>
    /// An <see cref="EntityRef" /> whose <see cref="EntityType" /> comes from <typeparamref name="T" /> and whose <see cref="EntityId" /> is built from the keys.
    /// </returns>
    public static EntityRef For<T>(T entity, Func<T, object?[]> selector)
    ...
    /// <summary>Builds a reference using the logical type of <typeparamref name="T" /> and a single <see cref="Guid" /> key.</summary>
    ...
    /// <summary>
    /// Builds a reference using the logical or CLR type name of <typeparamref name="T" /> (see <see cref="EntityRefLogicalTypeAttribute" />) and the given key(s). Multiple keys
    /// are ordered lexically and encoded with <see cref="EntityRefCompositeEncoding" /> so literal <c>:</c> inside a segment remains unambiguous.
    /// </summary>
    /// <typeparam name="T">CLR type used to resolve the stored entity type discriminator.</typeparam>
    ...
    /// <summary>Mints a reference for an entity identified by a <see cref="Guid" /> under the given type name.</summary>
    ...
    /// <summary>Mints a reference with an explicit type name and id string.</summary>
    /// <param name="entityType">Logical entity kind (non-empty).</param>
    ...
    public static EntityRef ParseOpaque(string value)
    {
        ...
        return TryParseOpaque(value.AsSpan(), out var r) ? r : throw new FormatException("Value must be non-empty entityType, U+001F separator, then non-empty entityId.");
    }
