...
/// <summary>Helpers for mapping an <see cref="EntityRef" /> onto a single PostgreSQL <c>uuid</c> column (“Option A” persistence).</summary>
/// <remarks>
/// Composite ids produced by <see cref="EntityRef.For{T}(object[])" /> do not round-trip through these methods; use stores that persist <see cref="EntityRef.EntityId" /> as
/// text or a dedicated encoding instead.
/// </remarks>
public static class EntityRefPersistedGuid
{
    /// <summary>Attempts to interpret <paramref name="entityRef" />.<see cref="EntityRef.EntityId" /> as exactly one <see cref="Guid" />.</summary>
    /// <param name="entityRef">Reference whose <see cref="EntityRef.EntityId" /> should be a GUID string.</param>
    /// <param name="guid">Parsed identifier when this method returns <see langword="true" />.</param>
    /// <returns><see langword="true" /> when trimming <see cref="EntityRef.EntityId" /> yields a valid GUID string.</returns>
