...
/// <summary>Optional check that <see cref="EntityRef.EntityType" /> is one of a closed set (feature gates, API bindings).</summary>
    ...
    public static void EnsureKnown(EntityRef entityRef, IReadOnlyCollection<string> allowedTypes, string? paramName = null)
    {
        ...
        if (!allowedTypes.Contains(entityRef.EntityType)) {
            throw new ArgumentOutOfRangeException(
                paramName ?? nameof(entityRef), entityRef.EntityType, $"Entity type '{entityRef.EntityType}' is not in the allowed set for this operation.");
        }
    }
}