...
/// <summary>Myers O(ND) diff with trace-based backtracking (J. Coglan, "The Myers diff algorithm: part 3").</summary>
internal static class MyersDiffCalculator
{
    ...
    private static List<TextDiffChunk> Backtrack(List<int[]> trace, int dEnd, int n, int m, int offset, TextToken[] oldTokens, TextToken[] newTokens)
    {
        ...
        for (var d = dEnd; d >= 0; d--) {
            ...
            // Horizontal step in the edit graph (delete old) versus vertical (insert new); see Myers / Coglan backtrack.
            if (x == prevX)
