...
/// <summary>One page of a paged query. Use as the data payload of <see cref="Result{T}" />: <c>Result&lt;PagedResult&lt;T&gt;&gt;</c>.</summary>
...
public record PagedResult<T>
{
    ...
    /// <summary>True when this page contains no items.</summary>
    public bool IsEmpty => Items.Count == 0;

    /// <summary>True when this is the first page.</summary>
    public bool IsFirstPage => Page == 1;

    /// <summary>True when this is the last page.</summary>
    public bool IsLastPage => Page >= TotalPages;

    /// <summary>0-based index of the first item on this page relative to the full result set.</summary>
    ...
    /// <summary>Builds an empty paged result with no items and a total count of zero.</summary>
    public static PagedResult<T> Empty(int page = 1, int pageSize = 20) => new([], 0, page, pageSize);

    /// <summary>Builds a paged result treating the provided list as a complete single-page result set.</summary>
    public static PagedResult<T> SinglePage(IReadOnlyList<T> items, int pageSize = 20) => new(items, items.Count, 1, pageSize);

    /// <summary>Projects each item to a new type, keeping paging metadata.</summary>
