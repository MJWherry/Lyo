...
/// <summary>Guards for validation scenarios.</summary>
public static class ValidationHelpers
{
    /// <summary>Validates a value using several validation functions.</summary>
    /// <typeparam name="T">Type of value to validate.</typeparam>
    /// <param name="value">Value under validation.</param>
    /// <param name="validators">Validation functions that return (isValid, errorCode, errorMessage) tuples.</param>
    /// <returns>A Result containing the value when all validations pass, or errors when any fail.</returns>
    ...
    /// <summary>Validates a value using several validation functions with custom severity.</summary>
    /// <typeparam name="T">Type of value to validate.</typeparam>
    /// <param name="value">Value under validation.</param>
    ...
    /// <typeparam name="T">Type of value to validate (must be a reference type).</typeparam>
    /// <param name="value">Value under validation.</param>
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.NullValue" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    /// <returns>A Result containing the value when not null, or an error when null.</returns>
    ...
    /// <typeparam name="T">Type of value to validate (must be a value type).</typeparam>
    /// <param name="value">Nullable value under validation.</param>
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.NullValue" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    /// <returns>A Result containing the value when not null, or an error when null.</returns>
    public static Result<T> ValidateNotNull<T>(T? value, string errorCode = ValidationErrorCodes.NullValue, string? errorMessage = null)
    ...
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.EmptyString" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    /// <returns>A Result containing the string when not null or empty, or an error otherwise.</returns>
    public static Result<string> ValidateNotEmpty(string? value, string errorCode = ValidationErrorCodes.EmptyString, string? errorMessage = null)
        => value is { Length: > 0 } ? Result<string>.Success(value) : Result<string>.Failure(errorMessage ?? "String cannot be null or empty", errorCode);

    /// <summary>Validates that a string is not null, empty, or whitespace.</summary>
    /// <param name="value">String under validation.</param>
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.WhitespaceString" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    /// <returns>A Result containing the string when not null, empty, or whitespace, or an error otherwise.</returns>
    public static Result<string> ValidateNotWhitespace(string? value, string errorCode = ValidationErrorCodes.WhitespaceString, string? errorMessage = null)
        => value is null || string.IsNullOrWhiteSpace(value)
            ? Result<string>.Failure(errorMessage ?? "String cannot be null, empty, or whitespace", errorCode)
            : Result<string>.Success(value);

    /// <summary>Validates that a collection is not null or empty.</summary>
    /// <typeparam name="TCollection">Type of collection to validate (must implement ICollection).</typeparam>
    /// <param name="collection">Collection under validation.</param>
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.EmptyCollection" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    /// <returns>A Result containing the collection when not null or empty, or an error otherwise.</returns>
    ...
    /// <param name="errorCode">Error code used when validation fails. Defaults to <see cref="ValidationErrorCodes.OutOfRange" />.</param>
    /// <param name="errorMessage">Error message used when validation fails. When null, uses a default message.</param>
    ...
    /// <param name="errorCode">Error code used when validation fails.</param>
    ...
    public static Result<string> ValidateEmail(string? value, string errorCode = ValidationErrorCodes.InvalidEmail, string? errorMessage = null)
        => value is null || string.IsNullOrWhiteSpace(value) ? Result<string>.Failure(errorMessage ?? "Email cannot be null or empty", ValidationErrorCodes.EmptyString) :
    ...
    public static Result<string> ValidatePhone(string? value, string errorCode = ValidationErrorCodes.InvalidPhone, string? errorMessage = null)
        => value is null || string.IsNullOrWhiteSpace(value) ? Result<string>.Failure(errorMessage ?? "Phone cannot be null or empty", ValidationErrorCodes.EmptyString) :
    ...
    public static Result<string> ValidateUri(string? value, UriKind kind = UriKind.Absolute, string errorCode = ValidationErrorCodes.InvalidUri, string? errorMessage = null)
        => value is null || string.IsNullOrWhiteSpace(value) ? Result<string>.Failure(errorMessage ?? "URI cannot be null or empty", ValidationErrorCodes.EmptyString) :
