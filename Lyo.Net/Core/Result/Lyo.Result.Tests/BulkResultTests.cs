...
public class BulkResultTests
{
    ...
    {
        var bulk = BulkResult<int>.FromResults([Result<int>.Success(1), Result<int>.Success(2), Result<int>.Failure("fail", "CODE")]);
    ...
    {
        var bulk = BulkResult<int>.FromErrors([new("a", "A"), new("b", "B")]);
    ...
    {
        var bulk = BulkResult<int>.FromResults([Result<int>.Success(1), Result<int>.Failure("fail", "CODE")]);
    ...
    {
        var bulk = BulkResult<string>.FromResults([Result<string>.Success("a"), Result<string>.Failure("err", "E"), Result<string>.Success("b")]);
    ...
    {
        var bulk = BulkResult<string>.FromResults([Result<string>.Success("ok"), Result<string>.Failure("fail", "CODE")]);
    ...
    {
        var bulk = BulkResult<int>.FromResults([Result<int>.Failure("msg1", "A"), Result<int>.Failure("msg2", "A"), Result<int>.Failure("msg3", "B")]);
    ...
    {
        var bulk = BulkResult<int>.FromData([1, 2]);
    ...
    {
        var bulk = BulkResult<int>.FromErrors([new("e", "CODE")]);
        IResult iResult = bulk;
    ...
    {
        var bulk = BulkResultBuilder<int>.Create().AddSuccess(1).AddSuccess(2).AddFailure(new("fail", "F")).Build();
        Assert.Equal(3, bulk.TotalCount);
        Assert.Equal(2, bulk.SuccessCount);
        Assert.Equal(1, bulk.FailureCount);
    }
    ...
    {
        var bulk = BulkResultBuilder<string>.Create().AddEach(["a", "b", "c"], s => Result<string>.Success(s.ToUpper())).Build();
    ...
    {
        var bulk = new BulkResult<string, int>([Result<string, int>.Success("req1", 1), Result<string, int>.Failure("req2", new Error("fail", "F"))]);
    ...
    {
        var bulk = BulkResultBuilder<string, int>.Create().AddSuccess("r1", 100).AddFailure("r2", new("fail", "ERR")).Build();
        Assert.Equal(1, bulk.SuccessCount);
        Assert.Equal(1, bulk.FailureCount);
