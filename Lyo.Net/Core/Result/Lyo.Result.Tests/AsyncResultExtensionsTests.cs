...
public class AsyncResultExtensionsTests
{
    ...
    {
        var result = await Result<int>.Success(5).ThenAsync(x => Task.FromResult(Result<string>.Success(x.ToString())), TestContext.Current.CancellationToken);
    ...
    public async Task ThenAsync_CancelledToken_ThrowsBeforeExecution()
    {
        ...
        await Assert.ThrowsAsync<OperationCanceledException>(() => Result<int>.Success(1).ThenAsync((val, _) => Task.FromResult(Result<int>.Success(val)), cts.Token));
    }

    [Fact]
    public async Task ThenAsync_TaskOverload_ChainsCorrectly()
    {
        var result = await Task.FromResult(Result<int>.Success(10)).ThenAsync(x => Task.FromResult(Result<string>.Success(x.ToString())), TestContext.Current.CancellationToken);
    ...
    public async Task WhenAll_AllSuccess_ReturnsCombinedList()
    {
        var combined = await AsyncResultExtensions.WhenAll(
            Task.FromResult(Result<int>.Success(1)), Task.FromResult(Result<int>.Success(2)), Task.FromResult(Result<int>.Success(3)));
    ...
    {
        var combined = await AsyncResultExtensions.WhenAll(
            Task.FromResult(Result<int>.Success(1)), Task.FromResult(Result<int>.Failure("fail1", "A")), Task.FromResult(Result<int>.Failure("fail2", "B")));
    ...
    public async Task FirstSuccess_ReturnsFirstSuccessfulResult()
    {
        var result = await AsyncResultExtensions.FirstSuccess(
            Task.FromResult(Result<int>.Failure("fail", "A")), Task.FromResult(Result<int>.Success(42)), Task.FromResult(Result<int>.Success(99)));
    ...
    {
        var result = await AsyncResultExtensions.FirstSuccess(Task.FromResult(Result<int>.Failure("f1", "A")), Task.FromResult(Result<int>.Failure("f2", "B")));
        Assert.False(result.IsSuccess);
    ...
    {
        var result = await Task.FromResult(Result<int>.Success(5)).MapAsync(x => Task.FromResult(x * 3), TestContext.Current.CancellationToken);
        Assert.True(result.IsSuccess);
    ...
    {
        var result = await Task.FromResult(Result<int>.Success(1)).CombineAsync(Task.FromResult(Result<string>.Success("hello")));
        Assert.True(result.IsSuccess);
    ...
    {
        var result = await Task.FromResult(Result<int>.Success(1)).CombineAsync(Task.FromResult(Result<string>.Failure("fail", "CODE")));
        Assert.False(result.IsSuccess);
