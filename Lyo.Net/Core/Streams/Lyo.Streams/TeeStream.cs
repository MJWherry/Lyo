...
/// <summary>Write-only stream that fans writes to several outputs at once (Unix <c>tee</c>). Typical use: hash while the bytes continue down the processing pipeline.</summary>
public class TeeStream : Stream
{
    ...
    /// <summary>Mints a TeeStream. The caller keeps ownership of every stream; disposing this instance does not dispose them.</summary>
    /// <param name="primary">Primary stream that receives each write.</param>
    /// <param name="secondary">One or more additional streams that receive the same write.</param>
    public TeeStream(Stream primary, params Stream[] secondary)
        : this(primary, false, secondary) { }

    /// <summary>Mints a TeeStream.</summary>
    /// <param name="primary">Primary stream that receives each write.</param>
    /// <param name="ownsStreams">True to dispose the primary and secondary streams when this stream is disposed.</param>
    /// <param name="secondary">One or more additional streams that receive the same write.</param>
