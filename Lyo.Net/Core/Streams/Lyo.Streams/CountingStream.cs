using Lyo.Exceptions;

namespace Lyo.Streams;

/// <summary>Stream wrapper that counts bytes read and written.</summary>
public class CountingStream : Stream
{
    private readonly Stream _baseStream;
    private bool _disposed;

    public override bool CanRead => !_disposed && _baseStream.CanRead;

    public override bool CanSeek => !_disposed && _baseStream.CanSeek;

    public override bool CanWrite => !_disposed && _baseStream.CanWrite;

    public override long Length {
        get {
            ThrowIfDisposed();
            return _baseStream.Length;
        }
    }

    public override long Position {
        get {
            ThrowIfDisposed();
            return _baseStream.Position;
        }
        set {
            ThrowIfDisposed();
            _baseStream.Position = value;
        }
    }

    /// <summary>Total bytes read through this wrapper.</summary>
    public long BytesRead { get; private set; }

    /// <summary>Total bytes written through this wrapper.</summary>
    public long BytesWritten { get; private set; }

    /// <summary>Total bytes transferred (read plus written).</summary>
    public long BytesTransferred => BytesRead + BytesWritten;

    public CountingStream(Stream baseStream)
    {
        ArgumentHelpers.ThrowIfNull(baseStream);
        _baseStream = baseStream;
    }

    public override void Flush()
    {
        ThrowIfDisposed();
        _baseStream.Flush();
    }

    public override async Task FlushAsync(CancellationToken ct)
    {
        ThrowIfDisposed();
        await _baseStream.FlushAsync(ct).ConfigureAwait(false);
    }

    public override int Read(byte[] buffer, int offset, int count)
    {
        ThrowIfDisposed();
        StreamValidation.ValidateReadBuffer(buffer, offset, count);
        var bytesRead = _baseStream.Read(buffer, offset, count);
        BytesRead += bytesRead;
        return bytesRead;
    }

#if NETSTANDARD2_1_OR_GREATER || NET5_0_OR_GREATER
    public override int Read(Span<byte> buffer)
    {
        ThrowIfDisposed();
        var bytesRead = _baseStream.Read(buffer);
        BytesRead += bytesRead;
        return bytesRead;
    }
#endif

    public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken ct)
    {
        ThrowIfDisposed();
        StreamValidation.ValidateReadBuffer(buffer, offset, count);
        var bytesRead = await _baseStream.ReadAsync(buffer, offset, count, ct).ConfigureAwait(false);
        BytesRead += bytesRead;
        return bytesRead;
    }

#if NETSTANDARD2_1_OR_GREATER || NET5_0_OR_GREATER
    public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken ct = default)
    {
        ThrowIfDisposed();
        var bytesRead = await _baseStream.ReadAsync(buffer, ct).ConfigureAwait(false);
        BytesRead += bytesRead;
        return bytesRead;
    }
#endif

    public override long Seek(long offset, SeekOrigin origin)
    {
        ThrowIfDisposed();
        return _baseStream.Seek(offset, origin);
    }

    public override void SetLength(long value)
    {
        ThrowIfDisposed();
        _baseStream.SetLength(value);
    }

    public override void Write(byte[] buffer, int offset, int count)
    {
        ThrowIfDisposed();
        StreamValidation.ValidateWriteBuffer(buffer, offset, count);
        _baseStream.Write(buffer, offset, count);
        BytesWritten += count;
    }

#if NETSTANDARD2_1_OR_GREATER || NET5_0_OR_GREATER
    public override void Write(ReadOnlySpan<byte> buffer)
    {
        ThrowIfDisposed();
        _baseStream.Write(buffer);
        BytesWritten += buffer.Length;
    }
#endif

    public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken ct)
    {
        ThrowIfDisposed();
        StreamValidation.ValidateWriteBuffer(buffer, offset, count);
        await _baseStream.WriteAsync(buffer, offset, count, ct).ConfigureAwait(false);
        BytesWritten += count;
    }

#if NETSTANDARD2_1_OR_GREATER || NET5_0_OR_GREATER
    public override async ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken ct = default)
    {
        ThrowIfDisposed();
        await _baseStream.WriteAsync(buffer, ct).ConfigureAwait(false);
        BytesWritten += buffer.Length;
    }
#endif

    /// <summary>Sets the byte counters back to zero. Safe to call at any point in the stream lifetime.</summary>
    public void ResetCounters()
    {
        ThrowIfDisposed();
        BytesRead = 0;
        BytesWritten = 0;
    }

    private void ThrowIfDisposed() => OperationHelpers.ThrowIfDisposed(_disposed, GetType().Name);

    protected override void Dispose(bool disposing)
    {
        if (!_disposed && disposing)
            _disposed = true;

        base.Dispose(disposing);
    }
}