...
/// <summary>
/// Read-only stream that yields a deterministic byte sequence of a fixed <see cref="Length" /> from a seed. Bytes are produced on demand so the full payload is never
/// allocated. Intended for large streaming benchmarks and tests.
/// </summary>
/// <remarks>
/// Seeking to an arbitrary position resets the PRNG and skips ahead; sequential reads and <c>Seek(0)</c> are faster. The same length and seed always produce
/// the same sequence.
/// </remarks>
public sealed class DeterministicPayloadStream : Stream
{
    /// <summary>
    /// Shared default seed (<c>0x4C594F42</c> / "LYOB") for reproducible payloads. Tests read <c>Lyo.Testing.TestData.Seed</c>; benchmarks read
    ...
    /// <summary>Mints a stream that yields <paramref name="length" /> bytes from <see cref="DefaultSeed" />.</summary>
    public DeterministicPayloadStream(long length)
        : this(length, DefaultSeed) { }

    /// <summary>Mints a stream that yields <paramref name="length" /> bytes from <paramref name="seed" />.</summary>
    /// <param name="length">Exact number of readable bytes; must be non-negative.</param>
    /// <param name="seed">PRNG seed; the same seed and length always produce the same sequence. Prefer <see cref="DefaultSeed" />.</param>
    ...
    /// <remarks>Tests should use <c>Lyo.Testing.TestData</c> and benches <c>BenchmarkData</c>; this method is the shared implementation both wrap.</remarks>
    ...
    /// <summary>Allocates <paramref name="sizeBytes" /> bytes and fills them via <see cref="Fill" />.</summary>
