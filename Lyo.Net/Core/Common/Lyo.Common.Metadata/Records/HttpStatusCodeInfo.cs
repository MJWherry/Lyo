...
/// <summary>Catalog row for an HTTP status: numeric code, name, description, category, and the wrapped <see cref="HttpStatusCode" />.</summary>
public record HttpStatusCodeInfo(int Code, string Name, string Description, HttpStatusCodeCategory Category, HttpStatusCode HttpStatusCode)
{
    ...
    // Lookup tables keyed by numeric code and enum
    ...
    /// <summary>Looks up a status by numeric code.</summary>
    /// <param name="code">HTTP code, for example 200, 404, or 500.</param>
    /// <returns>The catalog row, or <see cref="Unknown" /> when the code is not registered.</returns>
    public static HttpStatusCodeInfo FromCode(int code) => ByCode.TryGetValue(code, out var statusCode) ? statusCode : Unknown;

    /// <summary>Looks up a status from a <see cref="HttpStatusCode" /> member.</summary>
    /// <param name="httpStatusCode">The <see cref="HttpStatusCode" /> value.</param>
    ...
    /// <summary>Lists catalog rows in <paramref name="category" />.</summary>
    /// <param name="category">The HTTP status bucket.</param>
    ...
    /// <summary>Implicit conversion to <see cref="HttpStatusCode" /> so APIs can take the catalog row.</summary>
    public static implicit operator HttpStatusCode(HttpStatusCodeInfo info) => info.HttpStatusCode;

    /// <summary>Implicit conversion from <see cref="HttpStatusCode" /> so enum members resolve to catalog rows.</summary>
