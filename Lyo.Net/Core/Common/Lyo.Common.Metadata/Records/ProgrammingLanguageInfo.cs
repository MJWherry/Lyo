...
/// <summary>Catalog row for a programming language; <see cref="ShortName" /> is the canonical key.</summary>
    ...
    string[] Aliases)
{
    ...
    /// <summary>Every registered programming-language catalog row.</summary>
    public static IReadOnlyList<ProgrammingLanguageInfo> All => AllLanguages;

    static ProgrammingLanguageInfo()
    {
        var fields = typeof(ProgrammingLanguageInfo).PublicStaticFields<ProgrammingLanguageInfo>();
    ...
    public static ProgrammingLanguageInfo FromName(string? name)
    {
        ...
        // Leave short-name matching to FromShortName, even when a lower-cased
        // display name collides with the short name (for example "zig" vs "Zig").
