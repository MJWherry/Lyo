...
/// <summary>Static binary-to-text codecs (Base64, Base64Url, Hex) with span-first hot paths.</summary>
public static class BinaryEncoding
{
    private const int StreamBufferSize = 8192;
    private const int MimeLineLength = 76;

    /// <summary>Largest encoded character count for <paramref name="byteCount" /> input bytes (without line wraps).</summary>
    ...
    /// <summary>Encodes bytes to text. Empty input yields <see cref="string.Empty" />.</summary>
    ...
    /// <summary>Encodes a stream to the end (materializes). Does not close <paramref name="stream" />.</summary>
    ...
    /// <summary>
    /// Tries to encode into <paramref name="destination" />. Line wrapping is not applied (use <see cref="Encode(BinaryEncodingKind, ReadOnlySpan{byte}, TextLetterCase, int)" />
    /// for wrapped output).
    /// </summary>
    ...
    /// <summary>Decodes encoded text to bytes. Whitespace is ignored for Base64/Base64Url.</summary>
    ...
    /// <summary>PEM-armors Base64 (76-char lines) with BEGIN/END labels.</summary>
    ...
    /// <summary>Decodes PEM-armored Base64; returns false when armor or payload is invalid.</summary>
    ...
    /// <summary>Decodes PEM-armored Base64; throws on invalid armor or payload.</summary>
    ...
    /// <summary>Chunked encode: binary in to text out. Does not close streams.</summary>
    ...
    /// <summary>Streaming decode: text in to binary out. Does not close streams. Whitespace is ignored for Base64.</summary>
    ...
    /// <summary>Encodes file contents to a string.</summary>
