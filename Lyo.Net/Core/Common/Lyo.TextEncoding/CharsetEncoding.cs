...
/// <summary>Static charset resolve, encode, decode, convert, and detect helpers.</summary>
public static class CharsetEncoding
{
    ...
    /// <summary>Registers the code-pages provider (idempotent).</summary>
    public static void EnsureCodePagesRegistered() => Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);

    /// <summary>Resolves by web/.NET name or numeric code-page string. Throws if unknown.</summary>
    ...
    /// <summary>Resolves from <see cref="CharsetInfo" /> with optional fallbacks from options.</summary>
    ...
    /// <summary>Maps a BCL encoding to a catalog entry (well-known or custom).</summary>
    ...
    /// <summary>Replays <see cref="CharsetDetectionResult.ConsumedPrefix" /> then the rest of <paramref name="stream" />.</summary>
    ...
    /// <summary>Writes text to a stream. Does not close <paramref name="stream" /> when <paramref name="leaveOpen" /> is true.</summary>
    ...
    /// <summary>Converts a stream with a stateful encoder/decoder (sync). Does not close streams.</summary>
    ...
    /// <summary>Converts a stream with a stateful encoder/decoder. Does not close streams.</summary>
    ...
    /// <summary>Clones <paramref name="encoding" /> when options specify fallbacks (does not mutate BCL singletons).</summary>
