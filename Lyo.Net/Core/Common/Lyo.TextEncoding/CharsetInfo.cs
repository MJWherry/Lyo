...
/// <summary>
/// Curated charset metadata (well-known statics plus custom instances). Does not replace <see cref="Encoding" /> — use <see cref="ToEncoding" /> or service resolve for the BCL
/// type.
/// </summary>
public sealed record CharsetInfo(string Name, string WebName, int? CodePage, string Description, IReadOnlyList<string>? Aliases = null)
{
    ...
    /// <summary>Every well-known static entry for UI/config pickers. Custom instances are not included.</summary>
    // Must be initialized before the lookups below, which enumerate it.
    ...
    /// <summary>Mints a custom charset entry (not limited to the well-known set). Resolve at runtime via <see cref="ToEncoding" />.</summary>
    ...
    /// <summary>Resolves the BCL encoding for this entry (no fallback mutation).</summary>
    ...
    /// <summary>Finds a well-known entry by web name or alias (case-insensitive). Returns null if not in the catalog.</summary>
    ...
    /// <summary>Finds a well-known entry by code page. Returns null if not in the catalog.</summary>
    public static CharsetInfo? FromCodePage(int codePage) => ByCodePage.TryGetValue(codePage, out var info) ? info : null;

    /// <summary>Maps a BCL encoding to a well-known entry when possible; otherwise a <see cref="Custom" /> instance.</summary>
