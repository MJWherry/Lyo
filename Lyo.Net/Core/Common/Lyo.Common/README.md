...
- **Record metadata catalogs** (`Records/`) — `FileTypeInfo` (`.GetFileTypeFromExtension`, MIME mapping, two-key envelope suffix, common storage-resolution suffix list),
  `FileSizeUnitInfo`, `HttpStatusCodeInfo`, `PortInfo` (well-known ports + `PortCategory`, `FromPort`/`FromName`/`ByCategory`, implicit `int`), `LanguageCodeInfo`,
  `ProgrammingLanguageInfo`, `BoundingBox2D`.
...
- **Pathing** (`Pathing/`) — `PathStyle` (`Host` / `Posix`) and `PathHelpers` for combine, full-path normalize (`.`/`..`), file/dir name, and under-root jail checks with Uri-style
  throw helpers (`ThrowIfEscapesRoot`, `ThrowIfInvalidPath`).
