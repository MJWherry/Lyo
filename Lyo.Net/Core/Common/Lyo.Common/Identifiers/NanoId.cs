...
/// <summary>
/// NanoID generator: a compact, URL-safe, cryptographically random string identifier.
/// Uses rejection sampling against a power-of-2 bitmask for a perfectly uniform distribution
/// over any alphabet without bias.
/// </summary>
public static class NanoId
{
    ...
    /// <summary>
    /// Generates <paramref name="count"/> NanoIDs with the default alphabet and default length in a single batch.
    /// Uses one pre-sized RNG fill for the whole batch; a refill only happens in the (extremely rare)
    /// event that rejection sampling exhausts the pre-allocated pool.
    /// </summary>
    ...
    private static int PoolSize(int size, int mask, int alphabetLength) =>
        Math.Max(size, (int)Math.Ceiling(size * 1.6 * mask / alphabetLength) + 1);
}
