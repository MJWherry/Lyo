...
/// A ULID is a 26-character Crockford base32 string: 10 chars of 48-bit millisecond timestamp
...
public static class Ulid
{
    ...
    [
        ...
        10, 11, 12, 13, 14, 15, 16, 17, 255, 18, 19, 255, 20, 21, 255,
    ...
    /// <summary>
    /// Generates <paramref name="count"/> ULID strings in a single batch using the current UTC time.
    /// All ULIDs share the same millisecond timestamp captured at batch start.
    ...
    public static DateTimeOffset GetTimestamp(string ulid)
    {
        ...
        long ms = 0;
        for (var i = 0; i < 10; i++)
            ms = (ms << 5) | (long)DecodeChar(ulid[i], i);
