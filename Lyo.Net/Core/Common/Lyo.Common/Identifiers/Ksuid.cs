...
/// A KSUID is a 27-character base62 string encoding 20 bytes: a 32-bit second-precision timestamp
/// (relative to 2014-05-13T16:53:20Z) followed by 128 random bits.
/// KSUIDs sort lexicographically by creation time and are safe to use in URLs.
/// </summary>
public static class Ksuid
{
    ...
    /// <summary>
    /// Generates <paramref name="count"/> KSUID strings in a single batch using the current UTC time.
    /// All KSUIDs share the same second-precision timestamp captured at batch start.
    /// Uses one RNG fill for all <paramref name="count"/> × 16 random bytes.
    /// </summary>
    ...
    private static void DecodeBase62(string ksuid, Span<byte> output)
    {
        ...
        {
            if (c >= 128) throw new ArgumentException($"Invalid KSUID character '{c}'.", nameof(ksuid));
            var value = DecodeMap[c];
            if (value == 255) throw new ArgumentException($"Invalid KSUID character '{c}'.", nameof(ksuid));
