...
/// <summary>
/// NanoID factory: a compact, URL-safe, cryptographically random string identifier. Uses rejection sampling against a power-of-2 bitmask for a perfectly uniform
/// distribution over any alphabet without bias.
/// </summary>
public static class NanoId
{
    /// <summary>Default URL-safe alphabet (64 characters: digits, lowercase, uppercase, underscore, hyphen).</summary>
    ...
    /// <summary>Mints a NanoID with the default alphabet and default length.</summary>
    public static string Create() => Generate(DefaultAlphabet, DefaultSize);

    /// <summary>Mints a NanoID with the default alphabet and a custom <paramref name="size" />.</summary>
    public static string Create(int size) => Generate(DefaultAlphabet, size);

    /// <summary>Mints a NanoID with a custom <paramref name="alphabet" /> and <paramref name="size" />.</summary>
    public static string Create(string alphabet, int size) => Generate(alphabet, size);

    /// <summary>
    /// Mints <paramref name="count" /> NanoIDs with the default alphabet and default length in one batch. Uses one pre-sized RNG fill for the whole batch; a refill only
    ...
    /// <summary>Mints <paramref name="count" /> NanoIDs with the default alphabet and a custom <paramref name="size" />.</summary>
    public static string[] CreateBulk(int count, int size) => GenerateBulk(DefaultAlphabet, size, count);

    /// <summary>Mints <paramref name="count" /> NanoIDs with a custom <paramref name="alphabet" /> and <paramref name="size" />.</summary>
    ...
    private static string[] GenerateBulk(string alphabet, int size, int count)
    {
        ...
        for (var n = 0; n < count; n++) {
            ...
            var filled = 0;
            while (filled < size) {
                // Refill only in the extremely rare case that rejection sampling empties the pool.
                if (bufPos == totalBuf) {
                    CryptographicRandom.Fill(buf);
                    bufPos = 0;
                }
    ...
    // ~1.6× overallocation covers the worst-case rejection rate at a given mask/alphabet ratio.
