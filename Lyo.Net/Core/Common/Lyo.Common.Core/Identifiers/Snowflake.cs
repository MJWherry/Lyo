...
/// <summary>
/// Parameters for reading a millisecond timestamp from the high bits of a snowflake value. Different systems use different epoch origins and bit layouts (for example timestamp
/// field width).
/// </summary>
...
/// <summary>
/// A 64-bit snowflake identifier: a millisecond-resolution timestamp in the high bits and instance-specific data in the low bits. The exact layout comes from
...
public readonly record struct Snowflake(ulong Value) : IComparable<Snowflake>
{
    ...
    /// <summary>Reads a signed snowflake as stored by many APIs and databases (same bit pattern as <see cref="ulong" />).</summary>
    public static Snowflake FromInt64(long signed) => new(unchecked((ulong)signed));

    public long ToInt64() => unchecked((long)Value);

    /// <summary>Milliseconds since the Unix epoch taken from the timestamp field using <paramref name="layout" />.</summary>
...
/// <summary>
/// Thread-safe generator of monotonically increasing 64-bit <see cref="Snowflake" /> identifiers. The bit layout is: 1 unused sign bit, 41-bit millisecond
/// timestamp (relative to <see cref="DefaultEpochMs" />), 10-bit machine ID, 12-bit sequence counter — up to 4 096 unique IDs per millisecond per machine.
/// </summary>
public sealed class SnowflakeGenerator
{
    ...
    // TimestampShiftBits used when building IDs and when constructing Layout for parsing.
    ...
    /// <summary>Milliseconds since the Unix epoch for the default snowflake epoch of 2020-01-01T00:00:00Z.</summary>
    ...
    /// <summary>Shared default generator (machine ID 0, <see cref="DefaultEpochMs" />).</summary>
    public static SnowflakeGenerator Shared { get; } = new();

    /// <summary>Machine ID this generator was built with (0–1023).</summary>
    ...
    /// <see cref="Snowflake.GetUnixMillisecondsSinceUnixEpoch" /> to recover the embedded timestamp.
    /// </summary>
    public SnowflakeLayout Layout => new(TimestampShift, _epochMs);

    /// <summary>Builds a generator with machine ID 0 and the <see cref="DefaultEpochMs">default epoch</see>.</summary>
    public SnowflakeGenerator()
        : this(0) { }

    /// <summary>Builds a generator for <paramref name="machineId" /> (0–1023) using the <see cref="DefaultEpochMs">default epoch</see>.</summary>
    public SnowflakeGenerator(int machineId)
        : this(machineId, DefaultEpochMs) { }

    /// <summary>Builds a generator for <paramref name="machineId" /> (0–1023) and a custom <paramref name="epochMs" /> (milliseconds since the Unix epoch).</summary>
    ...
    /// <summary>Next snowflake. Thread-safe; spins for up to 1 ms if the sequence counter is exhausted within the current millisecond.</summary>
    ...
    /// <summary>Mints <paramref name="count" /> snowflakes in ascending order. Thread-safe.</summary>
