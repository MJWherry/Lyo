...
/// <summary>
/// ULID (Universally Unique Lexicographically Sortable Identifier) factory. A ULID is a 26-character Crockford base32 string: 10 chars of 48-bit millisecond timestamp
/// followed by 16 chars of 80-bit randomness. ULIDs sort lexicographically by creation time.
/// </summary>
public static class Ulid
{
    // Crockford base32 — I, L, O, U omitted to avoid ambiguous characters.
    ...
    /// <summary>Mints a ULID string using the current UTC time.</summary>
    public static string Create() => Create(DateTimeOffset.UtcNow);

    /// <summary>Mints a ULID string with an explicit <paramref name="timestamp" /> (useful for testing).</summary>
    ...
    /// <summary>
    /// Mints <paramref name="count" /> ULID strings in one batch using the current UTC time. All ULIDs share the same millisecond timestamp captured at batch start.
    ...
    /// <summary>UTC timestamp embedded in a ULID string.</summary>
    ...
    {
        // 26 Crockford base32 chars: 10 × 5-bit timestamp (MSB first) plus 16 × 5-bit random.
        ...
        // Fold 10 bytes (80 bits) into 16 × 5-bit base32 characters.
