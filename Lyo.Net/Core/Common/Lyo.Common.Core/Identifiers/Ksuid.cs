...
/// <summary>
/// KSUID (K-Sortable Unique Identifier) factory. A KSUID is a 27-character base62 string encoding 20 bytes: a 32-bit second-precision timestamp (relative to
...
{
    // Seconds from the Unix epoch (1970-01-01) to the KSUID epoch (2014-05-13T16:53:20Z).
    private const long EpochOffsetSeconds = 1_400_000_000L;

    // base62: digits, uppercase, lowercase — chosen so values sort lexicographically.
    private const string Base62Alphabet = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

    private const int StringLength = 27;
    private const int PayloadLength = 20; // 4-byte timestamp + 16-byte random

    // O(1) decode table indexed by ASCII value (0-127). 255 means invalid.
    ...
    /// <summary>Mints a KSUID string using the current UTC time.</summary>
    public static string Create() => Create(DateTimeOffset.UtcNow);

    /// <summary>Mints a KSUID string with an explicit <paramref name="timestamp" /> (useful for testing).</summary>
    ...
    /// <summary>
    /// Mints <paramref name="count" /> KSUID strings in one batch using the current UTC time. All KSUIDs share the same second-precision timestamp captured at batch
    ...
    public static string[] CreateBulk(int count)
    {
        ...
        // Reusable 20-byte payload: 4-byte timestamp (constant) plus a 16-byte random slice.
    ...
    /// <summary>UTC timestamp (second precision) from a KSUID string.</summary>
    ...
    private static string EncodeBase62(ReadOnlySpan<byte> bytes)
    {
        ...
        // then divide repeatedly by 62, filling the result right-to-left.
