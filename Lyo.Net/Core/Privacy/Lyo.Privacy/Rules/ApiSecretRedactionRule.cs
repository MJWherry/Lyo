...
/// <summary>Heuristic detector for API keys, PATs, and <c>KEY=value</c> material, with an optional entropy gate.</summary>
public sealed class ApiSecretRedactionRule : IRedactionRule
{
    ...
    /// characters); secret is Base64Url (≥ 22 chars).
    /// </summary>
    ...
    public IEnumerable<RedactionSpan> EnumerateSpans(string input)
    {
        ...
        if ((Patterns & ApiSecretPatterns.LyoToken) != 0) {
            foreach (Match m in LyoToken.Matches(input)) {
                if (m.Success)
                    yield return new(m.Index, m.Length, Kind);
            }
        }
