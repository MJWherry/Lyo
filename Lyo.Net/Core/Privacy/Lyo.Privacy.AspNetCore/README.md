...
| `services.AddLyoPrivacyPolicy(object serviceKey, Action<RedactionPolicyBuilder> configure)` | Registers a keyed `ITextRedactor` built from a custom `RedactionPolicyBuilder`. Use this when one host needs different redaction rules per workload (e.g. `"Support"`, `"Marketing"`). The policy's `Name` defaults to `serviceKey.ToString()` when not set. |
