...
/// <summary>
/// Parses cron expressions and computes the next occurrence. Accepts standard 5-field and extended 6-field (with seconds) forms. Field syntax: values, ranges
...
{
    // Slot index equals the numeric month (index 0 unused).
    ...
    /// <summary>Parses a cron expression string.</summary>
    ...
    public static CronExpression Parse(string expression, CronFormat format = CronFormat.Standard)
    {
        ...
        // Map the alternate Sunday value (7) onto 0.
    ...
    /// <summary>
    /// Returns the next occurrence strictly after <paramref name="from" />, evaluated in <paramref name="zone" />. Returns <c>null</c> when no occurrence is found within four
    /// years.
    /// </summary>
    public DateTimeOffset? GetNextOccurrence(DateTimeOffset from, TimeZoneInfo zone)
    {
        ...
        // Step forward one second and drop sub-second precision.
        ...
        while (dt <= limit) {
            ...
            // Fields match. Convert to UTC and skip DST gaps.
            try {
                var utc = TimeZoneInfo.ConvertTimeToUtc(dt, zone);
                return new DateTimeOffset(utc, TimeSpan.Zero);
            }
            catch (ArgumentException) {
                // Local time sits in a DST spring-forward gap; step past it.
                dt = dt.AddMinutes(1);
            }
        }
    ...
    {
        // Vixie cron: OR when both fields are restricted; otherwise the restricted field decides.
