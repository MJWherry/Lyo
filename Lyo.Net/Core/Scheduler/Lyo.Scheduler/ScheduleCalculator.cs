...
/// <summary>Computes upcoming fire times for <see cref="ScheduleDefinition" /> instances.</summary>
public static class ScheduleCalculator
{
    /// <summary>Look-ahead window used when the caller omits one. 366 days covers monthly schedules.</summary>
    public const int DefaultMaxDaysLookAhead = 366;

    /// <summary>Yields the next scheduled execution times for a schedule.</summary>
    ...
    /// <summary>Returns the next fire time for a recurring schedule, or null when none remains.</summary>
    ...
    /// <summary>Returns every scheduled time on a given date (SetTimes or Interval).</summary>
    ...
    private static DateTime? GetNextRunCron(ScheduleDefinition schedule, DateTime utcNow)
    {
        ...
        // Prefer 6-field (seconds included); fall back to the usual 5-field form.
        CronExpression? expr;
        try {
