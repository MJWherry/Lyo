...
/// <summary>Thread-safe <see cref="IMetrics" /> that keeps samples in memory.</summary>
...
public class MetricsService : IMetrics, IDisposable
{
    ...
    /// <summary>Builds a service with default options.</summary>
    public MetricsService()
        : this(new()) { }

    /// <summary>Builds a service with the given options.</summary>
            ...
            _counters.AddOrUpdate(
                key, _ => new() { Name = name, Value = longValue, Tags = dictTags != null ? new Dictionary<string, string>(dictTags) : null }, (_, existing) => {
                    try {
                        ...
                        // Hold at MaxValue or MinValue when the add overflows
    ...
    /// <summary>Recorded events, capped at the last N samples.</summary>
    ...
    /// <summary>Drops every stored metric and event.</summary>
    public void Clear()
    {
        _counters.Clear();
        _gauges.Clear();
    ...
    /// <summary>Builds a serializable snapshot of every stored metric.</summary>
    ...
    /// <summary>Counter value for a name and optional tags. Thread-safe read.</summary>
    ...
    /// <summary>Gauge value for a name and optional tags. Thread-safe read.</summary>
    ...
    /// <summary>Histogram for a name and optional tags. Returns a copy so the caller can iterate safely.</summary>
    public HistogramData? GetHistogram(string name, IEnumerable<(string, string)>? tags = null)
    {
        ...
        lock (lockInfo.Lock) {
            lockInfo.LastAccessed = DateTime.UtcNow;
            // Copy values so the caller can iterate without holding the lock
