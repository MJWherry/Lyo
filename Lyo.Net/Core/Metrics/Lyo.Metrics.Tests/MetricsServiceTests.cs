...
public class MetricsServiceTests : IDisposable
{
    ...
    public void RecordError_IncrementsErrorCounter()
    {
        ...
        // RecordError stamps error_type and error_message tags on its own
    ...
    {
        // Fixed message so platform-specific exception text does not affect the tags
    ...
    public void ConcurrentAccess_IsThreadSafe()
    {
        ...
        // Each thread counter should be exactly 100
        for (var i = 0; i < 10; i++) {
    ...
    public void Histogram_RespectsMaxValuesLimit()
    {
        ...
        // Ring buffer keeps the last 5 samples (5, 6, 7, 8, 9)
    ...
    public void TagValidation_SanitizesInvalidCharacters()
    {
        ...
        // Query with the sanitized forms: pipes stripped from keys, equals become underscores in values
        var sanitizedTags = new[] { ("keywithpipes", "value_with_equals"), ("normal_key", "normal_value") }; // key pipes gone; value equals → underscores
    ...
    {
        var options = new MetricsOptions { SamplingRate = 0.1 }; // sample 10%
        var service = new MetricsService(options);
        for (var i = 0; i < 100; i++)
            service.IncrementCounter("test.sampled");

        // 10% sampling should land near 10, with some variance
        var value = service.GetCounterValue("test.sampled");
        Assert.True(value < 100); // sampling keeps this below 100
        Assert.True(value > 0); // at least some samples land
    }
    ...
    public void OverflowProtection_HandlesLargeValues()
    {
        ...
        _metrics.IncrementCounter("test.overflow", 20); // overflows the counter
        var value = _metrics.GetCounterValue("test.overflow");
        // Clamped at long.MaxValue
