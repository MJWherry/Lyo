...
/// <summary>
/// Assembly-level metadata for a benchmark project: the stable machine <see cref="Name" /> and display
/// <see cref="Title" /> used in the emitted report. When absent, the exporter derives the name from the
/// assembly name (e.g. <c>Lyo.Hashing.Benchmarks</c> -&gt; <c>hashing</c>).
/// </summary>
...
public sealed class BenchmarkReportAttribute : Attribute
{
    ...
    /// <summary>
    /// Suite-level methodology copied into <c>MicroBenchmarkReport.Description</c>: what is measured, the data set,
    /// and what each benchmark exercises so individual rows are self-explanatory.
    /// </summary>
    public string? Description { get; set; }
}

/// <summary>
/// Human-readable description of what a benchmark class or method exercises. Picked up by the exporter and written
/// into the report's group / measurement <c>description</c> so readers do not have to infer intent from method names.
/// </summary>
...
public sealed class BenchmarkDescriptionAttribute : Attribute
{
    ...
    public BenchmarkDescriptionAttribute(string text)
    {
        Text = text;
    }
...
/// <summary>
/// Explains a <see cref="BenchmarkDotNet.Attributes.ParamsAttribute" /> parameter so reported values are meaningful
/// (e.g. <c>DataSize = 1048576</c> is "1 MB of bytes"). Apply once per parameter on the benchmark class.
/// </summary>
...
public sealed class BenchmarkParameterAttribute : Attribute
{
    ...
    public BenchmarkParameterAttribute(string name)
    {
        Name = name;
    }
...
/// <summary>
/// Declares the row/record/model type a benchmark operates on. The exporter reflects over it to emit a structured
/// <c>DatasetDescriptor</c> (columns, types, nesting depth) so the report captures the data structure - including
/// nested complexity - rather than just a row count.
/// </summary>
...
public sealed class BenchmarkDataShapeAttribute : Attribute
{
    ...
    public BenchmarkDataShapeAttribute(Type rowType)
    {
        RowType = rowType;
    }
...
/// <summary>
/// Declares a service-level objective / business-standard budget for a benchmark. Apply on a method (wins) or on the
/// class (default for every method). The exporter compares the measured mean / allocation / throughput against the
/// declared budgets and emits a <c>Meets</c> / <c>Exceeds</c> / <c>Miss</c> verdict plus a target display string, so
/// reported numbers can be judged against an expectation rather than read in isolation.
/// </summary>
...
/// <summary>
/// Marks the benchmark class that drives the report's comparison table (replaces the old magic
/// <c>AlgorithmComparisonBenchmarks</c> class-name convention). Methods opt into axes with
/// <see cref="ComparisonAxisAttribute" />.
/// </summary>
[AttributeUsage(AttributeTargets.Class)]
...
/// <summary>
/// Declares that a benchmark method participates in the comparison table under the given <see cref="Axis" />
/// (e.g. <c>Encrypt</c>, <c>Hash</c>), with the algorithm name taken from the remainder of the method name
/// (replaces the old <c>_Encrypt</c>/<c>_Hash</c> suffix string-matching).
/// </summary>
...
public sealed class ComparisonAxisAttribute : Attribute
{
    ...
    public ComparisonAxisAttribute(string axis)
    {
        Axis = axis;
    }
    ...
    /// <summary>
    /// Optional explicit algorithm name. When null, the algorithm is derived by stripping the axis suffix
    /// from the method name (e.g. <c>Sha256_Hash</c> with axis <c>Hash</c> -&gt; <c>Sha256</c>).
    /// </summary>
