...
/// <summary>
/// BenchmarkDotNet exporter that maps a run onto the shared <see cref="MicroBenchmarkReport" /> shape and writes <c>&lt;name&gt;.lyobench.json</c> under the artifacts folder.
/// It reads structured stats as-is (nanosecond means, GC bytes, parameters) so later steps do not have to parse strings.
/// </summary>
public sealed class LyoBenchmarkExporter : IExporter
{
    /// <summary>Shared instance consumed by the common config.</summary>
    ...
    {
        // This exporter writes a file only; it does not print to the console.
    }
    ...
    /// <summary>Maps a BenchmarkDotNet summary onto the shared report (public so tests can call it).</summary>
    public static MicroBenchmarkReport Build(Summary summary)
    {
        ...
        // Export executes in the host process after the run. Use "now" as the end, then subtract the
        // wall-clock duration BenchmarkDotNet reports to recover the start.
        ...
        // Roll up the worst SLA verdict per (class, method) for the suite-level SLO table.
    ...
    private static BenchmarkEnvironment ReadEnvironment(Summary summary, Assembly? assembly)
    {
        ...
        try {
            ...
            // GC memory info is best-effort; stay null when the platform does not expose it.
        }
    ...
    /// <summary>
    /// Pulls CPU name / core counts and the .NET SDK version from BenchmarkDotNet host info via reflection. Those types live in Perfolizer and have shifted across
    /// BenchmarkDotNet releases; reflection stays resilient, and a failure just leaves the optional fields null.
    /// </summary>
    private static void ReadHostCpuAndSdk(object host, BenchmarkEnvironment env)
    {
        try {
            ...
            // CPU/SDK probing is best-effort; the BCL-backed fields above stay filled.
        }
    }
    ...
    private static bool IsScalar(Type type) => type.IsScalar();
    ...
    {
        // A logical group is the same class plus the same parameter signature; the baseline method in that group is the reference.
    ...
    private static ComparisonTable? BuildComparison(Summary summary, List<BenchmarkReportRow> rows, Dictionary<BenchmarkReportRow, BenchmarkMeasurement> measurementByCase)
    {
        ...
        foreach (var axisGroup in GroupByAxis(suiteRows)) {
            ...
            // If no baseline name was set, take it from the [Benchmark(Baseline=true)] case.
            ...
            // Baseline-algorithm mean keyed by parameter set.
    ...
    /// <summary>
    /// Grades a measured row against its SLA: builds the target display string, computes throughput when a size parameter is present, and assigns Miss / Exceeds / Meets.
    /// Returns null when no budget applies.
    /// </summary>
    private static SlaEvaluation? EvaluateSla(BenchmarkReportRow row, BenchmarkSlaAttribute? sla)
    {
        ...
        // Throughput is only a fair target on bulk payloads. Below MinThroughputSizeBytes the number is
        // dominated by fixed per-call overhead, so small sizes skip it (not counted as a Miss).
    ...
    /// <summary>Running aggregate keyed by (class, method), later folded into a single SLO row.</summary>
    ...
    /// <summary>Per-case scratch row with enough state to compute ratios and comparison groups.</summary>
    public sealed class BenchmarkReportRow
    {
        /// <summary>Name of the benchmark class.</summary>
        public string ClassName { get; set; } = string.Empty;

        /// <summary>Name of the benchmark method.</summary>
        public string Method { get; set; } = string.Empty;

        /// <summary>Parameter values for this case.</summary>
        public Dictionary<string, string?> Parameters { get; set; } = [];

        /// <summary>Mean latency in nanoseconds.</summary>
        ...
        /// <summary>Mean divided by the group baseline mean.</summary>
        public double? RatioToBaseline { get; set; }

        /// <summary>Reflected workload method (used to look up axis / SLA attributes).</summary>
        public MethodInfo? WorkloadMethod { get; set; }

        /// <summary>Reflected benchmark class (used as the class-level SLA fallback).</summary>
        public Type? BenchmarkType { get; set; }
    }

    /// <summary>Result of grading one benchmark row against a declared SLA budget.</summary>
