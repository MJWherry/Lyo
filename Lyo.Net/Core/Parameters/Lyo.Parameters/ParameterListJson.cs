...
/// <summary>
/// Serialize and parse definition-list payloads stored in string columns (<c>AllowedValues</c>, multi <c>Value</c>). The payload is a JSON array of typed primitives; parse
/// always yields canonical invariant strings for UI and validation.
/// </summary>
public static class ParameterListJson
{
    /// <summary>
    /// Reads a JSON array into canonical string forms. Null, whitespace, or invalid/non-array JSON yields an empty list. Accepts string, number, and boolean elements; null
    /// elements are skipped. There is no pipe-separated fallback.
    /// </summary>
    ...
    /// <summary>
    /// Writes chip/string values as a JSON array. Returns null when kind filtering leaves no non-empty values. <paramref name="kind" /> selects JSON string, number, or bool
    /// wire shape.
    /// </summary>
