...
/// <c>MudMenu</c> popover.
/// </summary>
/// <typeparam name="TValue">Value type for each option (for example an enum).</typeparam>
    ...
    where TValue : notnull
{
    ...
    /// <summary>Full list of options shown in the dropdown.</summary>
    [Parameter]
    [EditorRequired]
    public IReadOnlyList<LyoSelectOption<TValue>> Items { get; set; } = [];

    /// <summary>Values currently selected.</summary>
    [Parameter]
    ...
    /// <summary>When true, shows a search field inside the dropdown. Defaults to false.</summary>
    [Parameter]
