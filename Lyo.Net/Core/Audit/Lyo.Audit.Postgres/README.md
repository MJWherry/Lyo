...
Need just the factory, for example to share with downstream services or run migrations explicitly?

## Health

`PostgresAuditRecorder` implements `Lyo.Health.IHealth` with `HealthCheckName = "audit-postgres"`. The probe opens an `AuditDbContext` and runs `Database.CanConnectAsync`. Connection failures return `HealthResult.Unhealthy`. Hosts that resolve `IEnumerable<IHealth>` include this recorder.
