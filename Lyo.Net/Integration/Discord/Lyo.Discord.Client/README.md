...
Configuration section: `LyoDiscordClient` (shadows the base `ApiClient` section). Inherits all [`ApiClientOptions`](../../Api/Lyo.Api.Client/README.md#options-apiclientoptions)
flags (`BaseUrl`, `EnsureStatusCode`, `AcceptEncodings`, `RequestCompression`, …). Default `BaseUrl` is `http://localhost:5251/` so it works against a locally-hosted Lyo API host
out of the box; override for any other deployment.
...
All overloads inject any registered `ILoggerFactory` and reuse a shared `HttpClient` from DI when present; otherwise the underlying `ApiClient` constructor builds its own handler
with auto-decompression enabled.
