...
/// <summary>Interaction endpoints on the Lyo API.</summary>
public sealed class InteractionManager(LyoDiscordClient client)
{
    public Task<QueryRes<DiscordInteractionRes>> QueryAsync(QueryConcreteReq request, CancellationToken ct = default)
        => client.PostAsAsync<QueryConcreteReq, QueryRes<DiscordInteractionRes>>($"{Constants.Rest.Discord.Interactions}/QueryConcrete", request, ct: ct);
