...
/// <summary>Role endpoints on the Lyo API.</summary>
public sealed class RoleManager(LyoDiscordClient client)
{
    public Task<QueryRes<DiscordRoleRes>> QueryAsync(QueryConcreteReq request, CancellationToken ct = default)
        => client.PostAsAsync<QueryConcreteReq, QueryRes<DiscordRoleRes>>($"{Constants.Rest.Discord.Roles}/QueryConcrete", request, ct: ct);
