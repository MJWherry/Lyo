...
/// <summary>Guild / guild-config endpoints on the Lyo Discord API.</summary>
public sealed class GuildManager(LyoDiscordClient client)
{
    ...
    /// <summary>Fetch per-guild bot settings from the config store (creates default null binding if missing).</summary>
    ...
    /// <summary>Write guild settings (unauthenticated until the host adds authorization).</summary>
