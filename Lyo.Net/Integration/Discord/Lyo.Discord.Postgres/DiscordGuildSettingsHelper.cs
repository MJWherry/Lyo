...
/// <summary>Guild-scoped key and helpers using <see cref="EntityRef.For{T}(object[])" /> for <see cref="DiscordGuild" />.</summary>
public static class DiscordGuildSettingsHelper
{
    /// <summary>Definition and binding key for <see cref="DiscordGuildSettings" />.</summary>
    ...
    /// <summary>Looks up the <see cref="EntityRef" /> for a guild snowflake id.</summary>
    public static EntityRef GuildRef(long guildId) => EntityRef.For<DiscordGuild>(guildId);

    /// <summary>Creates a config binding if missing with all-null settings (idempotent).</summary>
    public static async Task EnsureDefaultBindingAsync(IConfigStore store, long guildId, CancellationToken ct = default)
    {
        ...
        if (await store.GetBindingAsync(r, Key, null, ct).ConfigureAwait(false) != null)
            return;
    ...
    /// <summary>Reads resolved guild settings, or default empty object when no binding.</summary>
    public static async Task<DiscordGuildSettings> GetSettingsAsync(IConfigStore store, long guildId, CancellationToken ct = default)
    {
        ...
        var resolved = await store.LoadConfigAsync(GuildRef(guildId), null, ct).ConfigureAwait(false);
        ...
        var binding = await store.GetBindingAsync(GuildRef(guildId), Key, null, ct).ConfigureAwait(false);
        if (binding != null) {
            var revisions = await store.GetBindingRevisionsAsync(binding.Id, null, ct).ConfigureAwait(false);
