...
/// <summary>Minimal-API endpoints for per-guild bot settings (config store). Add authorization at the host when exposing this API.</summary>
public static class DiscordGuildSettingsEndpoints
{
    /// <summary>Wires GET/PUT <c>Discord/Guild/{guildId}/GuildSettings</c>. No-op when <see cref="IConfigStore" /> is not registered.</summary>
        ...
        app.MapPut(
                $"{Constants.Rest.Discord.Guilds}/{{guildId:long}}/GuildSettings", async Task<IResult> (
                    ...
                    if (await db.DiscordGuilds.FindAsync([guildId], ct).ConfigureAwait(false) == null)
                        throw ApiErrorException.From(LyoProblemDetailsBuilder.CreateWithActivity().NotFound("Discord guild", guildId.ToString()).Build());
