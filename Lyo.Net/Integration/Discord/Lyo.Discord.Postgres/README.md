...
- **`PostgresDiscordOptions`** ([`PostgresDiscordOptions.cs`](PostgresDiscordOptions.cs)) — `IPostgresMigrationConfig`. Section `"PostgresDiscord"`. Properties: `ConnectionString`,
  `EnableAutoMigrations` (default `false`); `Schema = "discord"` is constant.
...
Call once on your `TypeAdapterConfig` to wire the `*Req` ↔ EF ↔ `*Res` mappings used by `Lyo.Api` CRUD. Two name-sanitizers run for nullable input: `DiscordUsernameOrPlaceholder`
(≤ 35 chars, falls back to `"(unknown)"`) and `DiscordGuildNameOrPlaceholder` (≤ 50 chars). Audit timestamps (`CreatedTimestamp`, `UpdatedTimestamp`) are explicitly ignored from
request DTOs because the CRUD hooks set them.
