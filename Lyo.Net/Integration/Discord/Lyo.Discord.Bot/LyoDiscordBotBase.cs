...
/// <summary>
/// Discord bot base: opens DSharpPlus, optionally syncs the database through <see cref="IGuildDatabaseSyncService" />, and offers hooks for derived bots (slash/commands,
/// interactions, and extra events).
/// </summary>
public abstract class LyoDiscordBotBase : ILyoDiscordBotGateway
{
    ...
    /// <summary>Typed HTTP client for Lyo <c>Discord/*</c> API upserts.</summary>
    ...
    /// <summary>Service for posting to guild channels (log channel helpers, arbitrary embeds, etc.).</summary>
    ...
    /// <summary>Loops until <paramref name="ct" /> is cancelled: connect, block, disconnect.</summary>
    ...
    /// <summary>Extend by adjust gateway configuration before the native client is constructed.</summary>
    protected virtual void ConfigureDiscordConfiguration(DiscordConfiguration configuration) { }

    /// <summary>
    /// Extend by register extensions on the DSharpPlus client (e.g. CommandsNext, SlashCommands, Interactivity) after construction and before
    /// <see cref="RegisterDefaultSyncHandlers" />.
    /// </summary>
    protected virtual void ConfigureDiscordClient(NativeDiscordClient client) { }

    /// <summary>Listen for sync-related gateway events. Override to add handlers; call <c>base.RegisterDefaultSyncHandlers(client)</c> to keep DB sync.</summary>
    ...
    /// <summary>Extend by subscribe to non-sync events or additional handlers.</summary>
