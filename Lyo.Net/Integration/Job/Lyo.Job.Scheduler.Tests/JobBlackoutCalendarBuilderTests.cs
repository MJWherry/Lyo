...
public class JobBlackoutCalendarBuilderTests
{
    ...
    {
        var calendar = JobBlackoutCalendarBuilder.New("Federal").AddBlackoutHoliday(HolidayInfo.ChristmasDay, 2026, 2028).Build();
        Assert.Equal(3, calendar.CreateBlackoutWindows.Count);
        Assert.All(
            calendar.CreateBlackoutWindows, w => {
    ...
    {
        var calendar = JobBlackoutCalendarBuilder.New("Selected").AddBlackoutHolidays([HolidayInfo.ChristmasDay, HolidayInfo.ThanksgivingDay], 2026, 2026).Build();
