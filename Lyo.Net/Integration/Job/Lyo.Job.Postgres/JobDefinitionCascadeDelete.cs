...
/// <summary>
/// Removes all rows that reference a <see cref="JobDefinition" /> so the definition itself can be deleted
/// without violating PostgreSQL foreign keys (<c>ClientSetNull</c> / restrict semantics).
/// </summary>
public static class JobDefinitionCascadeDelete
{
    public static void RemoveDependents(JobContext db, Guid definitionId)
    {
        ...
        var workflowStepIds = db.JobWorkflowSteps
            .Where(s => s.JobDefinitionId == definitionId)
            .Select(s => s.Id)
            .ToList();

        var runs = db.JobRuns
            .Where(r => r.JobDefinitionId == definitionId)
        ...
        if (runIds.Count > 0 || workflowStepIds.Count > 0) {
            var workflowRunSteps = db.JobWorkflowRunSteps
                .Where(s => (s.JobRunId != null && runIds.Contains(s.JobRunId.Value))
                    || workflowStepIds.Contains(s.JobWorkflowStepId))
                .ToList();
        ...
        var schedules = db.JobSchedules
            .Where(s => s.JobDefinitionId == definitionId)
            .Include(s => s.JobScheduleParameters)
            .ToList();
        var calendarIds = schedules
            .Where(s => s.JobBlackoutCalendarId.HasValue)
        ...
        var triggers = db.JobTriggers
            .Where(t => t.JobDefinitionId == definitionId || t.TriggersJobDefinitionId == definitionId)
            .Include(t => t.JobTriggerParameters)
            .ToList();
        ...
        db.JobParallelRestrictions.RemoveRange(
            db.JobParallelRestrictions
                .Where(r => r.BaseJobDefinitionId == definitionId || r.OtherJobDefinitionId == definitionId)
                .ToList());
