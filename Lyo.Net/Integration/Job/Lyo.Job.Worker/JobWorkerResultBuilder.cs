...
/// <summary>
/// Fluent builder for collecting <see cref="JobRunResultReq" /> entries while a job executes. Call <see cref="Build" /> at the end to get the list to pass to the finish
/// endpoint.
/// </summary>
public sealed class JobWorkerResultBuilder
{
    ...
    /// <summary>
    /// Sets the overall outcome reported to the server. Defaults to <see cref="JobRunResult.Success" />. Call <see cref="Fail" /> or <see cref="Cancel" /> instead of setting
    /// this manually in most cases.
    /// </summary>
    ...
    {
        _results.Add(new(key, type, value));
        return this;
    }
    ...
    public JobWorkerResultBuilder AddError(string reason, int index = -1)
    {
        ...
        _results.Add(new(key, JobParameterType.String, reason));
        return Fail();
    }
    ...
    {
        _results.Add(new(Constants.Data.JobRunResultKey.FailedItem(index), JobParameterType.String, item));
        if (reason != null)
            _results.Add(new(Constants.Data.JobRunResultKey.FailureReason(index), JobParameterType.String, reason));
    ...
    {
        _results.Add(new(Constants.Data.JobRunResultKey.ApiCallTime(apiName), JobParameterType.Long, milliseconds));
        return this;
    }
    ...
    /// <summary>Finalises the result list. Injects the <c>Result</c> key with the current <see cref="_outcome" /> value so the server can parse it.</summary>
    public IReadOnlyList<JobRunResultReq> Build()
    {
        var all = new List<JobRunResultReq>(_results) { new(Constants.Data.JobRunResultKey.Result, JobParameterType.String, _outcome.ToString()) };
