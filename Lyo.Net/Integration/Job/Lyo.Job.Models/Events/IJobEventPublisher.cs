...
/// <summary>
/// Transport-agnostic abstraction for job lifecycle event publishing and subscription.
/// Implement this interface to use any message broker (RabbitMQ, Azure Service Bus, AWS SQS, etc.)
/// and register the implementation in the DI container. The default <c>MqJobEventPublisher</c>
/// (in <c>Lyo.Job.Postgres</c>) wraps <c>IMqService</c>.
/// </summary>
public interface IJobEventPublisher
{
    ...
    /// <summary>
    /// Connects to the underlying transport, creates any required queues/topics, and establishes
    /// exchange bindings. Called once on startup before subscriptions are established.
    /// </summary>
    /// <param name="ct">Cancellation token.</param>
    Task SetupAsync(CancellationToken ct = default);

    /// <summary>
    /// Send the newly-created run ID to the per-worker-type delivery queue and broadcast the
    /// creation event to any interested subscribers.
    /// </summary>
    ...
    /// <summary>
    /// Send the finished run ID to the scheduler completion queue and broadcast the finished
    /// event so the scheduler can process triggers and retries.
    /// </summary>
    ...
    /// <summary>
    /// Broadcast that a job definition has changed (e.g. enabled/disabled, schedule updated).
    /// Scheduler instances subscribe to this to refresh their in-memory definition cache.
    /// </summary>
    ...
    /// <summary>
    /// Subscribe to definition-change notifications. The scheduler calls this once on startup to
    /// invalidate its in-memory cache whenever a definition is updated.
    /// </summary>
    /// <param name="subscriberQueueName">The queue name to bind/subscribe on (transport-specific).</param>
    /// <param name="handler">
    /// Message handler — return <c>true</c> to requeue (e.g. on transient error), <c>false</c> to acknowledge.
    /// </param>
    /// <param name="ct">Cancellation token.</param>
    ...
    /// <summary>
    /// Subscribe to run-completion notifications. The scheduler calls this once on startup to
    /// process triggers and retry logic after each run finishes.
    /// </summary>
    /// <param name="handler">
    /// Message handler — return <c>true</c> to requeue (e.g. on transient error), <c>false</c> to acknowledge.
    /// </param>
    ...
    /// <summary>
    /// Subscribe to run-cancellation notifications for a specific worker type. Workers call this
    /// on startup to receive cancellation signals for runs they are processing.
    /// </summary>
