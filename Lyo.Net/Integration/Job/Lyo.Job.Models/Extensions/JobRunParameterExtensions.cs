...
/// <summary>
/// Typed convenience accessors for job run parameter and result collections.
/// These complement the generic <c>GetParameterValueAs&lt;T&gt;</c> / <c>GetResultValueAs&lt;T&gt;</c> methods on
/// <see cref="JobRunRes"/> by exposing well-known scalar types and working directly on the list type, making them
/// usable outside a full <see cref="JobRunRes"/> context.
/// </summary>
public static class JobRunParameterExtensions
{
    extension(IReadOnlyList<JobRunParameterRes>? parameters)
    {
        ...
        public string? GetString(string key)
            => parameters?.FirstOrDefault(p => p.Key.Equals(key, StringComparison.OrdinalIgnoreCase))?.Value;
        ...
        {
            var value = parameters.GetString(key);
            return DateTime.TryParse(value, null, System.Globalization.DateTimeStyles.RoundtripKind, out var result) ? result : null;
        }
        ...
        public T? GetAs<T>(string key, string? format = null)
            => parameters.GetString(key).ToScalar<T>(format);
    }
    ...
    extension(IReadOnlyList<JobRunResultRes>? results)
    {
        ...
        public string? GetString(string key)
            => results?.FirstOrDefault(r => r.Key.Equals(key, StringComparison.OrdinalIgnoreCase))?.Value;
        ...
        public T? GetAs<T>(string key, string? format = null)
            => results.GetString(key).ToScalar<T>(format);
    }
}
