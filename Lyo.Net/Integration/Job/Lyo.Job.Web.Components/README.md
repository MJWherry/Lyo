...
Blazor / MudBlazor dashboard for the Lyo job-management stack. Drop `JobManagement` into a host page and it renders the full Statistics / Definitions / Runs experience using only an injected `IApiClient` and the base route prefix.
...
Every grid / view component accepts at minimum the `IApiClient` and the relevant route (`BaseRoute`, `DefinitionRoute`, `ScheduleRoute`, `TriggerRoute`, `RunRoute`, …), so they can be hosted independently if you don't want the full `JobManagement` shell.
