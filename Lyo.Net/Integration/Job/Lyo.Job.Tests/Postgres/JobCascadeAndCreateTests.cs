...
public class JobCascadeAndCreateTests(JobPostgresFixture fixture) : IClassFixture<JobPostgresFixture>
{
    ...
    public async Task CreateDefinition_StampsCreatedTimestamps_OnNestedEntities()
    {
        ...
        var created = await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            BuildDefinitionReq(definitionId, withBlackout: true, withTrigger: false),
            before: ApplyDefinitionIds(definitionId),
    ...
    public async Task CreateDefinition_WithJsonStyleSharedBlackout_PersistsOneCalendarRow()
    {
        ...
        var calendarA = new JobBlackoutCalendarReq {
            ...
            CreateBlackoutWindows = [
                new() { Name = "W", DayFlags = DayFlags.Weekdays, StartTime = TimeOnly.Parse("01:00"), EndTime = TimeOnly.Parse("02:00") }
            ]
        };
        ...
        var created = await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new JobDefinitionReq {
                ...
                CreateSchedules = [
                    new() {
                        Type = ScheduleType.Cron, MonthFlags = MonthFlags.EveryMonth, DayFlags = DayFlags.EveryDay, CronExpression = "0 * * * *",
                        Enabled = true, CreateBlackoutCalendar = calendarA
                    },
                    new() {
                        Type = ScheduleType.Cron, MonthFlags = MonthFlags.EveryMonth, DayFlags = DayFlags.Weekdays, CronExpression = "0 9 * * *",
                        Enabled = true, CreateBlackoutCalendar = calendarB
                    }
                ]
            },
            before: ctx => {
    ...
    public async Task DeleteDefinition_GcsUnusedBlackoutCalendar_LeavesShared()
    {
        ...
        await using (var db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken)) {
            db.JobBlackoutCalendars.Add(new JobBlackoutCalendar {
                Id = sharedCalendarId,
                Name = "SharedCal",
                Enabled = true,
        ...
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            BuildDefinitionReq(exclusiveDefId, withBlackout: true, withTrigger: false),
            before: ApplyDefinitionIds(exclusiveDefId),
        ...
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new JobDefinitionReq {
                ...
                CreateSchedules = [
                    new() {
                        Type = ScheduleType.Cron, MonthFlags = MonthFlags.EveryMonth, DayFlags = DayFlags.EveryDay, CronExpression = "0 * * * *",
                        Enabled = true, JobBlackoutCalendarId = sharedCalendarId
                    }
                ]
            },
            before: ApplyDefinitionIds(sharedDefId),
            ct: TestContext.Current.CancellationToken)).IsSuccess);
        ...
        Assert.True((await delete.DeleteAsync<JobDefinition, JobDefinitionRes>(
            [exclusiveDefId],
    ...
    public async Task DeleteSchedule_RemovesParams_AndExclusiveCalendar_KeepsShared()
    {
        ...
        var sharedCalendar = new JobBlackoutCalendarReq {
            Name = "Shared",
            CreateBlackoutWindows = [new() { Name = "W", DayFlags = DayFlags.EveryDay, StartTime = TimeOnly.Parse("00:00"), EndTime = TimeOnly.Parse("01:00") }]
        };
        ...
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new JobDefinitionReq {
                ...
                CreateSchedules = [
                    new() {
                        Type = ScheduleType.Cron, MonthFlags = MonthFlags.EveryMonth, DayFlags = DayFlags.EveryDay, CronExpression = "0 * * * *",
                        Enabled = true, Description = "keep", CreateBlackoutCalendar = sharedCalendar,
                        CreateScheduleParameters = [
                            new JobScheduleParameterReq { Key = "Keep", Type = JobParameterType.String, Value = "1", Enabled = true }
                        ]
                    },
                    new() {
                        Type = ScheduleType.Cron, MonthFlags = MonthFlags.EveryMonth, DayFlags = DayFlags.Weekdays, CronExpression = "0 9 * * *",
                        Enabled = true, Description = "drop",
                        CreateBlackoutCalendar = new() {
                            Name = "Exclusive",
                            CreateBlackoutWindows = [
                                new() { Name = "X", DayFlags = DayFlags.Weekdays, StartTime = TimeOnly.Parse("03:00"), EndTime = TimeOnly.Parse("04:00") }
                            ]
                        },
                        CreateScheduleParameters = [
                            new JobScheduleParameterReq { Key = "Drop", Type = JobParameterType.String, Value = "2", Enabled = true }
                        ]
                    }
                ]
            },
        ...
        Assert.True((await delete.DeleteAsync<JobSchedule, JobScheduleRes>(
            [dropScheduleId],
            before: ctx => {
                ...
                if (ctx.Entity.JobBlackoutCalendarId is { } calendarId
                    && !db.JobSchedules.Any(s => s.Id != ctx.Entity.Id && s.JobBlackoutCalendarId == calendarId)) {
    ...
    public async Task DeleteDefinition_WithWorkflowRunSteps_SucceedsWithoutFkViolation()
    {
        ...
        var definitionId = LyoGuid.CreateCombPostgres();
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            BuildDefinitionReq(definitionId, withBlackout: false, withTrigger: false),
            before: ApplyDefinitionIds(definitionId),
            ct: TestContext.Current.CancellationToken)).IsSuccess);
        ...
        await using (var db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken)) {
            db.JobWorkflows.Add(new JobWorkflow {
                Id = workflowId, Name = "wf", Enabled = true, CreatedTimestamp = DateTime.UtcNow
            });
            ...
            db.JobRuns.Add(new JobRun {
                Id = runId, JobDefinitionId = definitionId, State = JobState.Finished, CreatedBy = "test",
                CreatedTimestamp = DateTime.UtcNow, AllowTriggers = false
            });
            ...
            db.JobWorkflowRunSteps.Add(new JobWorkflowRunStep {
                Id = LyoGuid.CreateCombPostgres(), JobWorkflowRunId = workflowRunId, JobWorkflowStepId = stepId, JobRunId = runId,
    ...
    public async Task DeleteTrigger_WithParamsAndRunRefs_SucceedsWithoutFkViolation()
    {
        ...
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new JobDefinitionReq { Name = ($"Src-{sourceDefId:N}".Length <= 100 ? $"Src-{sourceDefId:N}" : $"Src-{sourceDefId:N}"[..100]), Type = "Test", WorkerType = "cs", Enabled = true },
            before: ApplyDefinitionIds(sourceDefId),
            ct: TestContext.Current.CancellationToken)).IsSuccess);
        Assert.True((await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new JobDefinitionReq {
                ...
                CreateTriggers = [
                    new JobTriggerReq {
                        ...
                        CreateTriggerParameters = [
                            new JobTriggerParameterReq { Key = "P", Type = JobParameterType.String, Value = "v", Enabled = true }
                        ]
                    }
                ]
            },
            before: ApplyDefinitionIds(targetDefId),
            ct: TestContext.Current.CancellationToken)).IsSuccess);

        Guid triggerId;
        await using (var db = await factory.CreateDbContextAsync(TestContext.Current.CancellationToken)) {
            triggerId = await db.JobTriggers.Where(t => t.JobDefinitionId == targetDefId)
                .Select(t => t.Id)
                .SingleAsync(TestContext.Current.CancellationToken);
            db.JobRuns.Add(new JobRun {
                Id = LyoGuid.CreateCombPostgres(),
    ...
    private static JobDefinitionReq BuildDefinitionReq(Guid definitionId, bool withBlackout, bool withTrigger)
        => new() {
            ...
            Enabled = true,
            CreateParameters = [
                new JobParameterReq { Key = "Counties", Type = JobParameterType.String, Value = "Allegheny", Enabled = true }
            ],
            CreateBlackoutCalendar = withBlackout
                ? new JobBlackoutCalendarReq {
                    Name = "Maint",
                    CreateBlackoutWindows = [
                        new() { Name = "Night", DayFlags = DayFlags.Weekdays, StartTime = TimeOnly.Parse("02:00"), EndTime = TimeOnly.Parse("04:00") }
                    ]
                }
                : null,
            CreateSchedules = [
