...
public sealed class AutomationPlanRunnerExtendedStepsTests
{
    ...
    {
        var handler = new StubHttpHandler(_ => new(HttpStatusCode.Accepted) { Content = new StringContent("{\"ok\":true}") });
        using var httpClient = new HttpClient(handler);
        using var session = new FakeSession();
        var runner = new AutomationPlanRunner(httpClient, new NullSink(), new NullStorage(), new SimpleServiceProvider(new()));
        var plan = AutomationPlanBuilder.New("http request")
            .StoreLiteral("productId", "42")
            .HttpRequest("POST", "https://api.local/products", new() { ["Content-Type"] = "application/json" }, "{\"id\":\"{strings.productId}\"}", "responseJson", "statusCode")
            .Build();

        var result = await runner.RunWithResultAsync(session, plan, null, null, CancellationToken.None);
    ...
    {
        using var session = new FakeSession();
        var sp = new SimpleServiceProvider(new() { [typeof(ContextWriterScraper)] = new ContextWriterScraper() });
        using var httpClient = new HttpClient(new StubHttpHandler(_ => new(HttpStatusCode.OK)));
        var runner = new AutomationPlanRunner(httpClient, new NullSink(), new NullStorage(), sp);
        var plan = AutomationPlanBuilder.New("context persist")
            .InvokeDiMethod(typeof(ContextWriterScraper).AssemblyQualifiedName!, nameof(ContextWriterScraper.Scrape))
    ...
    public async Task RunWithResultAsync_InvokeDiMethod_MissingMethodCanBeNonThrowing()
    {
        ...
        using var httpClient = new HttpClient(new StubHttpHandler(_ => new(HttpStatusCode.OK)));
        var runner = new AutomationPlanRunner(httpClient, new NullSink(), new NullStorage(), sp);
        var plan = AutomationPlanBuilder.New("missing method")
            .InvokeDiMethod(typeof(ContextWriterScraper).AssemblyQualifiedName!, "DoesNotExist", throwOnMissingMethod: false)
    ...
    private sealed class FakeTabs : IWebAutomationTabs
    {
        public Task<IReadOnlyList<AutomationTabInfo>> ListTabsAsync(CancellationToken ct = default)
            => Task.FromResult<IReadOnlyList<AutomationTabInfo>>(Array.Empty<AutomationTabInfo>());
