...
public interface IWebAutomationSession : IAsyncDisposable, IDisposable
{
    ...
    /// <summary>
    /// Per-session root directory (<c>{ServiceRootDirectory}/session-{SessionId:N}</c>).
    /// Contains <c>browser-profile/</c>, <c>artifacts/</c>, <c>downloads/</c> and is the root for
    /// plan-run logs, snapshots and variables when <see cref="Plan.AutomationPlanRuntimeOptions.PlanRunDirectory" />
    /// is not set explicitly. <see langword="null" /> when the session was not created via a browser service.
    /// </summary>
