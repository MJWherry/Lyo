...
/// <summary>
/// Cookie access for browser engines that support it.
/// Consumers should check <c>browser is IBrowserCookies cookies</c> before using these members and gracefully
/// degrade when the browser does not implement this interface.
/// </summary>
public interface IBrowserCookies
{
    /// <summary>
    /// Returns all cookies visible to the current session, optionally filtered to those matching
    /// <paramref name="url" />. Pass <see langword="null" /> to return every cookie.
    /// </summary>
