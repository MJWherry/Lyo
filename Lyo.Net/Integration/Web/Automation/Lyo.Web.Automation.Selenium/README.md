...
- **`SeleniumBrowser`** — concrete `IWebAutomationBrowser` (`PageBase`); composes `SeleniumBrowserTabs`, `BrowserAlerts`, `BrowserKeyboard`, `FrameNavigator`,
  `SeleniumPolling`.
...
### WebDriver / Automation ([`WebDriver/`](WebDriver), [`Automation/ElementLocatorMapping.cs`](Automation/ElementLocatorMapping.cs))
...
- [`Lyo.Web.Automation`](../Lyo.Web.Automation/README.md) — engine-agnostic abstractions (`IWebAutomationBrowser`, `IWebAutomationSession`, automation plans).
- [`Lyo.Web.Automation.Playwright`](../Lyo.Web.Automation.Playwright/README.md) — Playwright counterpart with a parallel surface.
