...
Three DI extensions are available on `IServiceCollection`: All variants register `TypecastClient` as a singleton. `ILoggerFactory` and `HttpClient` are pulled from the container if
available; otherwise sensible defaults are used. Example `appsettings.json`:
