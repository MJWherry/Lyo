...
public class EndatoJsonSerializationTests
{
    ...
    {
        var query = PersonQueryBuilder.Create("John", "Smith").WithDateOfBirth("01/15/1980").WithResultsPerPage(10).Build();
    ...
    {
        var query = EnrichmentQueryBuilder.Create("John", "Smith").WithPhone("5125550100").WithDateOfBirth("01/15/1980").WithAddress("123 Main St", "Austin, TX").Build();
        var json = JsonSerializer.Serialize(query, Options);
