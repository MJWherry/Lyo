...
public class EnrichmentQueryBuilderTests
{
    ...
    {
        var ex = Assert.Throws<InvalidOperationException>(() => EnrichmentQueryBuilder.New().WithName("John", "Smith").Build());
    ...
    {
        var query = EnrichmentQueryBuilder.Create("John", "Smith").WithPhone("5125550100").Build();
    ...
    {
        var query = EnrichmentQueryBuilder.New().WithPhone("5125550100").WithEmail("john@example.com").Build();
        Assert.Equal("5125550100", query.Phone);
    ...
    {
        var query = EnrichmentQueryBuilder.Create("John", "Smith").WithAddress("123 Main St", "Austin, TX 78701").Build();
