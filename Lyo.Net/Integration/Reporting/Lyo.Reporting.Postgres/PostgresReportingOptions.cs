...
public sealed class PostgresReportingOptions : IPostgresMigrationConfig
{
    ...
    /// <summary>
    /// When false, generate requires a saved <c>ReportDefinitionId</c> and rejects ad-hoc <c>ReportDataJson</c> / <c>OverrideReportDataJson</c> payloads. Default true (preserves
    /// prior behavior).
    /// </summary>
    public bool AllowAdHocGeneration { get; set; } = true;

    /// <summary>Max report generations running concurrently per host. 0 (default) means unlimited. When the limit is reached, generate waits briefly and then fails with a "busy" error.</summary>
    public int MaxConcurrentGenerations { get; set; }

    /// <summary>Age after which terminal (Succeeded/Failed) generations are eligible for <see cref="ReportRetentionService" /> cleanup. Null (default) disables retention cleanup.</summary>
    public TimeSpan? GenerationRetention { get; set; }

    /// <summary>
    /// Wall-clock limit for a single generation (data provider, render, and hooks). When exceeded, the run is cancelled and persisted as Failed. Null disables the limit. Default
    /// 10 minutes.
    /// </summary>
    public TimeSpan? GenerationTimeout { get; set; } = TimeSpan.FromMinutes(10);

    /// <summary>
    /// Age after which non-terminal (Pending/Running) generations are considered stuck (e.g. the host crashed mid-generation) and are marked Failed by
    ...
    /// <summary>How often the opt-in maintenance worker (<c>AddReportingMaintenanceWorker</c>) runs retention cleanup and stuck-generation recovery. Default 15 minutes.</summary>
