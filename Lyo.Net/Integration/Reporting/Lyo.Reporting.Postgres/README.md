...
Definitions are also validated at **write time** (`ReportDefinitionWriteValidator`, wired into the API create/update/patch hooks): `ReportDataJson` must parse and respect `MaxReportDataJsonBytes`, `DefaultFormat` and parameter `Type` must be valid enum values, `ValidationRegex` must compile within the length cap, and `MinLength`/`MaxLength` must be coherent.
