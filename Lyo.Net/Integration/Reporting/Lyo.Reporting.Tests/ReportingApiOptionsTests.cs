...
public sealed class ReportingApiOptionsTests
{
    ...
    [Fact]
    public void Download_factory_defaults_to_null_so_endpoint_is_not_mapped()
        => Assert.Null(new ReportingApiOptions().DownloadStreamFactory);
    ...
    [Fact]
    public void WithAuth_download_factory_defaults_to_null()
        => Assert.Null(ReportingApiOptions.WithAuth(EndpointAuth.RequireAuthorization()).DownloadStreamFactory);
    ...
    [Fact]
    public void AddReportingApi_validates_options()
        => Assert.Throws<ArgumentException>(() => new ServiceCollection().AddReportingApi(new PostgresReportingOptions()));
    ...
    public void Denied_select_fields_block_nested_paths(string field, bool denied)
    ...
    public void Denied_projection_rejects_select_and_computed_templates()
    {
        var errors = DeniedSelectFieldPolicy.ValidateProjection(
            ["Key", "Parameters.EncryptedValue"],
            [new ComputedField { Name = "Sneaky", Template = "{EncryptedValue}" }],
            ["EncryptedValue"]);
    ...
    public void Denied_export_rejects_columns_and_query_selects()
    {
        var request = new ExportRequest {
            Query = new ProjectionQueryReq { Select = ["Parameters.EncryptedValue"] },
            Columns = new Dictionary<string, string> { ["Secret"] = "EncryptedValue" },
            ColumnList = [new ExportColumnMapping { Header = "Sneaky", Value = "{EncryptedValue}" }]
        };
    ...
    {
        var request = new ExportRequest {
            Query = new ProjectionQueryReq { Select = ["Key", "Value"] },
            Columns = new Dictionary<string, string> { ["Key"] = "Key" }
        };
