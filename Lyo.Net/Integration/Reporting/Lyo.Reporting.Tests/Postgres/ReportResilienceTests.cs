...
/// <summary>
/// Integration coverage for reporting hardening: stuck-run recovery, retention resilience to failing cleanup hooks,
/// generation wall-clock timeout, provider failure audit trail, OutputFileId persistence on failure, and the
/// unique definition-parameter key index.
/// </summary>
...
public sealed class ReportResilienceTests(ReportingPostgresFixture fixture) : IClassFixture<ReportingPostgresFixture>
{
    ...
    private IDbContextFactory<ReportingContext> DbFactory
        => fixture.ServiceProvider.GetRequiredService<IDbContextFactory<ReportingContext>>();
    ...
    private ReportService CreateReportService(
        ...
        IReadOnlyList<IReportDataProvider>? providers = null)
        => new(
    ...
    public async Task Stuck_recovery_marks_stale_pending_and_running_failed()
    {
        ...
        var stalePending = NewGeneration(nameof(ReportGenerationStatus.Pending), created: DateTime.UtcNow.AddHours(-3));
        var staleRunning = NewGeneration(nameof(ReportGenerationStatus.Running), created: DateTime.UtcNow.AddHours(-3), started: DateTime.UtcNow.AddHours(-3));
        var freshRunning = NewGeneration(nameof(ReportGenerationStatus.Running), created: DateTime.UtcNow, started: DateTime.UtcNow);
        db.ReportGenerations.AddRange(stalePending, staleRunning, freshRunning);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);

        var service = CreateRetentionService(new PostgresReportingOptions {
            ConnectionString = "unused",
            StuckGenerationTimeout = TimeSpan.FromHours(1)
        });
    ...
    {
        await using var db = await DbFactory.CreateDbContextAsync(TestContext.Current.CancellationToken);
        var stale = NewGeneration(nameof(ReportGenerationStatus.Running), created: DateTime.UtcNow.AddDays(-2), started: DateTime.UtcNow.AddDays(-2));
        db.ReportGenerations.Add(stale);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);

        var service = CreateRetentionService(new PostgresReportingOptions {
            ConnectionString = "unused",
            StuckGenerationTimeout = null
        });
    ...
    public async Task Retention_cleanup_continues_past_failing_cleanup_hook()
    {
        ...
        var poison = NewGeneration(nameof(ReportGenerationStatus.Succeeded), created: DateTime.UtcNow.AddDays(-40));
        var newer1 = NewGeneration(nameof(ReportGenerationStatus.Succeeded), created: DateTime.UtcNow.AddDays(-30));
        var newer2 = NewGeneration(nameof(ReportGenerationStatus.Failed), created: DateTime.UtcNow.AddDays(-20));
        db.ReportGenerations.AddRange(poison, newer1, newer2);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);

        var hooks = new ReportGenerationHooks {
            OnCleanupAsync = (ctx, _) => ctx.GenerationId == poison.Id
                ? throw new IOException("blob is gone")
                : ValueTask.CompletedTask
        };

        var service = CreateRetentionService(new PostgresReportingOptions {
            ConnectionString = "unused",
            GenerationRetention = TimeSpan.FromDays(7),
            StuckGenerationTimeout = null
        });
    ...
    public async Task Generate_times_out_and_persists_failed_with_timeout_message()
    {
        ...
        var reportService = CreateReportService(
            ...
            new PostgresReportingOptions { ConnectionString = "unused", GenerationTimeout = TimeSpan.FromMilliseconds(200) });

        var hooks = new ReportGenerationHooks {
            BeforeGenerateAsync = async (_, ct) => await Task.Delay(TimeSpan.FromSeconds(30), ct)
        };
        ...
        var failed = await db.ReportGenerations.AsNoTracking()
            .FirstOrDefaultAsync(g => g.CreatedBy == "timeout-user", TestContext.Current.CancellationToken);
    ...
    public async Task Provider_failure_persists_failed_generation_row()
    {
        ...
        var reportService = CreateReportService(
            scope.ServiceProvider,
            new PostgresReportingOptions { ConnectionString = "unused" },
        ...
        await using var verify = await DbFactory.CreateDbContextAsync(TestContext.Current.CancellationToken);
        var failed = await verify.ReportGenerations.AsNoTracking()
            .FirstOrDefaultAsync(g => g.CreatedBy == "provider-user", TestContext.Current.CancellationToken);
    ...
    public async Task Failed_generation_keeps_uploaded_output_file_id()
    {
        ...
        await using var db = await DbFactory.CreateDbContextAsync(TestContext.Current.CancellationToken);
        var failed = await db.ReportGenerations.AsNoTracking()
            .FirstOrDefaultAsync(g => g.CreatedBy == "orphan-user", TestContext.Current.CancellationToken);
    ...
    public async Task Duplicate_definition_parameter_keys_rejected_by_database()
    {
        ...
        db.ReportDefinitionParameters.AddRange(
            NewParameter(definitionId, "ClientId"),
            NewParameter(definitionId, "ClientId"));
