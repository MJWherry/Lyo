...
public sealed class ReportGenerationHardenTests(ReportingPostgresFixture fixture) : IClassFixture<ReportingPostgresFixture>
{
    ...
    public async Task Generate_uses_provider_and_profile_defaults()
    {
        ...
        var definition = new ReportDefinition {
            Id = Guid.NewGuid(),
            Name = "With Provider",
            ReportDataJson = JsonSerializer.Serialize(
                ReportBuilder<object>.New().SetTitle("Seed").AddSection(s => s.AddGrid("G", g => g.AddColumn("A").AddRow("1"))).Build()),
            IsActive = true,
        ...
        var reportService = new ReportService(
            ...
            [new StubProvider()],
            [
                new ReportingGenerationProfile {
                    Key = "stub-profile",
                    DefaultFormat = ReportFormat.Csv,
                    DefaultFileName = "from-profile.csv"
                }
            ],
            sp,
            sp.GetRequiredService<IOptions<PostgresReportingOptions>>(),
    ...
    public async Task Generate_merges_validates_and_persists_parameters()
    {
        ...
        var definition = new ReportDefinition {
            ...
            Name = "With Params",
            ReportDataJson = JsonSerializer.Serialize(
                ReportBuilder<object>.New().SetTitle("P").AddSection(s => s.AddGrid("G", g => g.AddColumn("A").AddRow("1"))).Build()),
            IsActive = true,
        ...
        await Assert.ThrowsAsync<ReportValidationException>(() => reportService.GenerateAsync(
            new GenerateReportReq {
                ReportDefinitionId = definitionId,
                Parameters = [new("ClientId", ReportParameterType.String, "")]
            },
            ct: TestContext.Current.CancellationToken));
        ...
        var result = await reportService.GenerateAsync(
            new GenerateReportReq {
                ReportDefinitionId = definitionId,
                Parameters = [new("ClientId", ReportParameterType.String, "override-client")],
                CreatedBy = "worker-p"
            },
            ct: TestContext.Current.CancellationToken);
        ...
        await using var verify = await fixture.ServiceProvider.GetRequiredService<IDbContextFactory<ReportingContext>>()
            .CreateDbContextAsync(TestContext.Current.CancellationToken);
        var stored = await verify.ReportGenerationParameters.AsNoTracking()
            .Where(p => p.ReportGenerationId == result.Id)
    ...
    private sealed class StubProvider : IReportDataProvider
    {
        ...
        public Task<ReportDataProviderResult> BuildAsync(ReportDataProviderRequest request, CancellationToken ct = default)
        {
            var report = ReportBuilder<object>.New()
                .SetTitle("FromProvider")
                .AddSection(s => s.AddGrid("G", g => g.AddColumn("A").AddRow("provider-row")))
                .Build();
            return Task.FromResult(new ReportDataProviderResult {
