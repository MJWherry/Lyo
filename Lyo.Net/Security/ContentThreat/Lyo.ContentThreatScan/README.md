...
The **[Lyo.FileStorage](../../../Data/FileStorage/Lyo.FileStorage/)** package includes `ContentThreatMalwareScanner` implementing `IFileMalwareScanner` by composing heuristics, optional reputation, thresholds, and `FileScanThreatLevel`.
...
For HTTP-backed reputation, prefer a **typed / named `HttpClient`** registered against `DefaultContentThreatReputationPipeline` plus `ReputationPipelineOptions` bound from configuration (timeouts, failure dispositions, API keys).
