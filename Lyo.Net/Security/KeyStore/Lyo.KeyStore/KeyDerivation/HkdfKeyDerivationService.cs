...
/// <summary>
/// HKDF (HMAC-based Extract-and-Expand Key Derivation Function) service. Uses HKDF as defined in RFC 5869. HKDF
/// is a modern, efficient KDF that does not need iterations the way PBKDF2 does. Note: the 'iterations' parameter is ignored — HKDF is single-pass.
...
public class HkdfKeyDerivationService : IKeyDerivationService
{
    ...
    /// <summary>Default salt size for HKDF (32 bytes / 256 bits).</summary>
    ...
    /// <summary>Derives a key from a password string using HKDF. The 'iterations' parameter is ignored — HKDF is single-pass.</summary>
    ...
    /// <summary>Derives a key from a password byte array using HKDF. The 'iterations' parameter is ignored — HKDF is single-pass.</summary>
    public byte[] DeriveKey(byte[] password, byte[]? salt = null, int iterations = 600000, int? keySizeBytes = null)
    {
        ...
        // HKDF does not use iterations — it is a single-pass algorithm
