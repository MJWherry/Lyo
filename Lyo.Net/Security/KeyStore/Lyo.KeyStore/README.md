...
**`Microsoft.Extensions.DependencyInjection.Extensions`** (this package): - **`AddLocalKeyStore()`** — registers a shared **`LocalKeyStore`** as **`IKeyStore`**. - **
`AddLocalKeyStore(Action<LocalKeyStore>)`** — configure keys before the container finishes building. Keyed encryption registration patterns live in **[
`EncryptionServiceExtensions`](../Lyo.Encryption/Extensions/EncryptionServiceExtensions.cs)** (for example * *`AddEncryptionServiceKeyed`**); see also **[
`Lyo.Encryption/README.md`](../Lyo.Encryption/README.md)**.
