# Lyo.KeyStore.Web.Components

In-process Blazor workbench for `IKeyStore`. Hosts inject `IKeyStore` (`LocalKeyStore` or AWS). The UI never calls an API and never displays key material.
