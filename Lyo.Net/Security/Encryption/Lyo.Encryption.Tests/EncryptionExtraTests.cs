...
public class EncryptionExtraTests : IDisposable, IAsyncDisposable
{
    ...
    {
        // each encrypt mints a fresh AES key and wraps it with RSA
        ...
        // encrypt with no key: random AES key, RSA-wrapped
        var enc = svc.Encrypt(plaintext);
        // decrypt with no key: unwrap the embedded key via the RSA private key
        var dec = svc.Decrypt(enc);
        Assert.Equal("using random key", Encoding.UTF8.GetString(dec));

        // successive encrypts must use distinct keys (ciphertext differs)
    ...
    public void AesGcmRsaEncryptionService_Dispose_DisposesRSA()
    {
        ...
        svc.Dispose();
        // after Dispose, further RSA use should throw; try encrypt
        var threw = false;
        try {
            svc.Encrypt([1, 2, 3]);
        }
