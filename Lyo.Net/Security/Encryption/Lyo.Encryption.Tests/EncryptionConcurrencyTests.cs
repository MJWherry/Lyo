...
/// <summary>
/// Confirms encrypt stays thread-safe with stateless random_base nonces: many concurrent single-shot and streaming encrypts on one shared KeyStore key
/// must never reuse a nonce and must always round-trip. Covers a shared singleton service and transient instances that share the KeyStore (the case the old, racy
...
public class EncryptionConcurrencyTests
{
    ...
    public async Task SingleShot_ParallelEncrypts_TransientServices_ProduceUniqueNoncesAndRoundtrip(string algorithm)
    {
        ...
        // every task constructs its own service over the shared KeyStore.
        await Parallel.ForEachAsync(
            Enumerable.Range(0, Concurrency), ct, (i, _) => {
    ...
    public async Task Streaming_ParallelEncrypts_SingletonService_ProduceGloballyUniqueNoncesAndRoundtrip(string algorithm)
    {
        ...
        // payloads span many chunks (chunkSize 16) so each stream emits a counter series under its own random prefix.
        ...
        // nonces unique inside a stream (counter) and across concurrent streams (random prefix).
    ...
    /// <summary>Reads the single-shot header (<c>[version][keyIdLen:4][keyId][keyVersion:string][nonceLen:4][nonce]...</c>) and returns the nonce.</summary>
    ...
    /// <summary>
    /// Reads the streaming header (per-stream nonce prefix) then walks compact frames (<c>[lengthAndFinalFlag:4][ciphertext][tag]</c>),
    /// deriving each chunk nonce as <c>prefix || counter</c> the same way the codec does — the wire stores no nonces.
    /// </summary>
