...
/// <summary>
/// Verifies that encrypt is thread-safe after switching to stateless random_base nonces: many concurrent single-shot and streaming encrypts on a single shared KeyStore key must
/// never reuse a nonce and must always round-trip. Covers both a shared singleton service instance and transient instances sharing the KeyStore (the scenario the old, racy
...
public class EncryptionConcurrencyTests
{
    ...
    public async Task SingleShot_ParallelEncrypts_SingletonService_ProduceUniqueNoncesAndRoundtrip(string algorithm)
    {
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, Concurrency), ct, (i, _) => {
    ...
    public async Task Streaming_ParallelEncrypts_SingletonService_ProduceGloballyUniqueNoncesAndRoundtrip(string algorithm)
    {
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, Concurrency), ct, async (i, token) => {
            using var input = new MemoryStream(plaintexts[i]);
            using var output = new MemoryStream();
            await svc.EncryptToStreamAsync(input, output, keyId, key: null, chunkSize: 16, ct: token);
