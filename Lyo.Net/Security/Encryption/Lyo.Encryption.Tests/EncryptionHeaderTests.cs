...
public class EncryptionHeaderTests
{
    ...
    public void GetHeaderSize_CalculatesCorrectly()
    {
        ...
        // size: 1 + 1 + 1 + 1 (DekKeyMaterialBytes) + 1 (DekEncoding) + 4 + 4 + 4 + 1 + 4 + 3
    ...
    public void GetHeaderSize_EmptyKeyId_CalculatesCorrectly()
    {
        ...
        // size: 1 + 1 + 1 + 1 (DekKeyMaterialBytes) + 1 (DekEncoding) + 4 + 0 + 4 + 0 + 4 + 2
    ...
    public void With_NewEncryptedDek_RecomputesDekEncoding()
    {
        ...
        // AES-KW to envelope (blob longer than dekLength + 8): replace the leftover AES-KW encoding byte.
        ...
        // envelope to AES-KW (blob exactly dekLength + 8 bytes).
        ...
        // no replacement DEK: encoding stays as-is.
        Assert.Equal(EncryptionHeader.DekEncodingEnvelope, toEnvelope.With(keyVersion: "3").DekEncoding);
    }

    /// <summary>
    /// Checks <see cref="EncryptionHeader.InferDekEncoding" /> against real two-key stream bytes: an AES Key Wrapped DEK is exactly
    /// <c>DekKeyMaterialBytes + 8</c> bytes, a KEK-service envelope is always larger, and inference matches the DekEncoding byte written by the encryptor.
    /// </summary>
    [Fact]
    public async Task InferDekEncoding_MatchesActualStreamHeaderEncoding()
    {
        ...
        // 32-byte KEK plus a symmetric AEAD KEK service selects AES Key Wrap.
        ...
        // 64-byte KEK (AES-SIV) is not a legal AES Key Wrap key, so the KEK-service envelope is used (always longer than dekLength + 8).
