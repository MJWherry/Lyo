...
internal static class AesCcmHelper
{
    ...
    /// <summary>
    /// Longest single-packet plaintext for a 12-byte nonce (RFC 3610: <c>q = 15 - nonceLen = 3</c> → <c>2^(8q) - 1</c>). Bigger payloads must use streaming APIs
    /// (one CCM packet per chunk).
    /// </summary>
    ...
    /// <summary>Encrypts into the caller's <paramref name="ciphertext" /> and <paramref name="tag" /> (BC emits ct‖tag; the method splits into those destinations).</summary>
    ...
    /// <summary>Decrypts into the caller's <paramref name="plaintext" /> (must match ciphertext length).</summary>
