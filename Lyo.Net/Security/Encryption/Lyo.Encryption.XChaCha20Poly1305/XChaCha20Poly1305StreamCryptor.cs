...
/// <summary>
/// Per-stream XChaCha20-Poly1305 cipher reused across every chunk of a streaming operation (see <see cref="IAeadStreamCryptor" />). XChaCha20 derives a subkey from the
/// first 16 nonce bytes via HChaCha20; within a stream those bytes come from the constant per-stream nonce prefix, so the subkey is computed once and cached (recomputed only if the
/// prefix changes). The BouncyCastle cipher and grow-once scratch buffers are reused for every chunk, so there is no per-chunk heap allocation.
/// </summary>
internal sealed class XChaCha20Poly1305StreamCryptor : IAeadStreamCryptor
{
    ...
    private void PrepareSubkeyAndNonce(ReadOnlySpan<byte> nonce24)
    {
        ...
        if (_subkeyParameter == null || !first16.SequenceEqual(_subkeyNonce16)) {
            ...
            _subkeyParameter = new KeyParameter(_subkey);
        }
    ...
    public void Encrypt(ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, Span<byte> ciphertextAndTag, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        _cipher.Init(
            true, new AeadParameters(_subkeyParameter, XChaCha20Poly1305Helper.TagSize * 8, _innerNonce, associatedData.IsEmpty ? null : associatedData.ToArray()));
    ...
    public void Decrypt(ReadOnlySpan<byte> ciphertextAndTag, ReadOnlySpan<byte> nonce, Span<byte> plaintext, ReadOnlySpan<byte> associatedData = default)
    {
        PrepareSubkeyAndNonce(nonce);
        _cipher.Init(
            false, new AeadParameters(_subkeyParameter, XChaCha20Poly1305Helper.TagSize * 8, _innerNonce, associatedData.IsEmpty ? null : associatedData.ToArray()));
