...
/// <summary>
/// Container helpers for the XChaCha20-Poly1305 addon. Same keyed-registration shape as <see cref="EncryptionServiceExtensions" />, specialized for
/// <see cref="XChaCha20Poly1305EncryptionService" />.
/// </summary>
public static class XChaCha20Poly1305EncryptionServiceExtensions
{
    /// <param name="services">Collection that receives the XChaCha20-Poly1305 registrations.</param>
    extension(IServiceCollection services)
    {
        /// <summary>
        /// Registers <see cref="XChaCha20Poly1305EncryptionService" /> as a singleton resolved from the given <see cref="IKeyStore" />. Use
        ...
        public IServiceCollection AddXChaCha20Poly1305Encryption()
        {
            ...
            services.AddSingleton<XChaCha20Poly1305EncryptionService>(provider => new(provider.GetRequiredService<IKeyStore>()));
            return services;
        }

        /// <summary>
        /// Registers <see cref="XChaCha20Poly1305EncryptionService" /> as a keyed singleton (DEK + KEK = XChaCha20-Poly1305) plus the matching
        /// <see cref="ITwoKeyEncryptionService" /> envelope.
        /// </summary>
        ...
        public IServiceCollection AddXChaCha20Poly1305EncryptionServiceKeyed(string keyName, string keyStoreName)
        {
            ...
            services.AddKeyedSingleton<IEncryptionService>(
                keyName,
                (provider, _) => provider.GetKeyedService<XChaCha20Poly1305EncryptionService>(keyName) ?? throw new InvalidOperationException(
                    $"Keyed encryption service '{keyName}' of type '{nameof(XChaCha20Poly1305EncryptionService)}' was not found."));

            return services.AddKeyedSingleton<ITwoKeyEncryptionService>(
                keyName, (provider, _) => {
                    var keyStore = provider.GetKeyedService<IKeyStore>(keyStoreName);
                    OperationHelpers.ThrowIfNull(keyStore, $"Keyed key store service '{keyStoreName}' was not found.");
                    var dek = provider.GetKeyedService<XChaCha20Poly1305EncryptionService>(keyName) ?? throw new InvalidOperationException(
                        $"Keyed encryption service '{keyName}' of type '{nameof(XChaCha20Poly1305EncryptionService)}' was not found.");
