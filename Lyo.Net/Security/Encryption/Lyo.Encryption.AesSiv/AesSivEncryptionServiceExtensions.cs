...
/// <summary>Dependency-injection extensions for the AES-SIV addon. Mirrors the keyed-registration shape from <see cref="EncryptionServiceExtensions" /> but specialized to <see cref="AesSivEncryptionService" />.</summary>
public static class AesSivEncryptionServiceExtensions
{
    /// <param name="services">The service collection.</param>
    extension(IServiceCollection services)
    {
        /// <summary>Registers <see cref="AesSivEncryptionService" /> as a singleton resolved from the provided <see cref="IKeyStore" />. Use <see cref="EncryptionServiceExtensions.AddDefaultEncryptionService{TConcrete}" /> for unkeyed <see cref="IEncryptionService" />.</summary>
        ...
        public IServiceCollection AddAesSivEncryption(AesSivKeySizeBits keySize = AesSivKeySizeBits.Bits256)
        {
            ...
            services.AddSingleton<AesSivEncryptionService>(provider => new(
                provider.GetRequiredService<IKeyStore>(), keySize));
            return services;
        }
        ...
        public IServiceCollection AddAesSivEncryptionServiceKeyed(string keyName, string keyStoreName, AesSivKeySizeBits keySize = AesSivKeySizeBits.Bits256)
        {
            ...
            services.AddKeyedSingleton<AesSivEncryptionService>(
                keyName, (provider, _) => {
                    ...
                    return new AesSivEncryptionService(keyStore, keySize);
                });
