...
/// <summary>
/// Recreates the legacy per-chunk stream frame (<c>[formatVersion:1][algorithmId:1][reserved:2]</c> then length-prefixed ciphertext chunks) used by the RSA
/// encrypt/decrypt services so on-disk <c>.rsa</c> streams/files stay byte-compatible after the split from <see cref="EncryptionServiceBase" />. The caller supplies the
/// per-chunk transform (RSA encrypt/decrypt of a buffer slice).
/// </summary>
internal static class RsaStreamCodec
{
    // cap encrypted chunk size at 200 MB to limit DoS while still allowing encryption overhead.
        ...
        CancellationToken ct)
    {
        ...
        var buffer = ArrayPool<byte>.Shared.Rent(effectiveChunkSize);
        var lengthBytes = new byte[4];
        try {
            int bytesRead;
            while ((bytesRead = await input.ReadAsync(buffer, 0, effectiveChunkSize, ct).ConfigureAwait(false)) > 0) {
    ...
    public static async Task DecryptAsync(Stream input, Stream output, byte algorithmId, byte formatVersion, Func<byte[], int, int, byte[]> decryptChunk, CancellationToken ct)
    {
        ...
        try {
            while (true) {
                ...
                AeadChunkCodec.EnsureCapacity(ref encryptedChunk, chunkLength);
                var totalRead = 0;
