...
**AES-GCM + RSA** hybrids, plus envelope / two-key flows via **`ITwoKeyEncryptionService`**.

Primary contracts: **`IEncryptionService`** (single key), **`ITwoKeyEncryptionService`** (per-operation DEK wrapped by a KEK), and **`EncryptionServiceBase`** (streaming, string,
and file helpers). Keys can be inline or resolved from **`Lyo.KeyStore`** by **`keyId`**.
