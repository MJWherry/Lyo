...
/// <summary>
/// Shared compact-AEAD chunk loops (see <see cref="AeadChunkCodec" />). Rents once per stream from <see cref="ArrayPool{T}" /> and
/// reuses those buffers for every chunk, so allocation is O(chunkSize) no matter how long the stream is. Buffers that held plaintext are zeroed before return to the pool. Used by
/// <see cref="EncryptionServiceBase" /> and the two-key envelope service.
/// </summary>
internal static class AeadStreamProcessor
{
    /// <summary>Largest encrypted chunk accepted (200 MB), to block DoS via huge length prefixes.</summary>
    public const int MaxEncryptedChunkSize = 200 * 1024 * 1024;

    /// <summary>Width of the per-chunk counter stored in the trailing bytes of each stream nonce.</summary>
    public const int CounterSize = 4;

    /// <summary>
    /// Encrypt: reads <paramref name="input" /> in <paramref name="effectiveChunkSize" /> slices and writes compact frames to <paramref name="output" />. Each chunk nonce is
    /// <paramref name="noncePrefix" /> (chosen once per stream and stored in the stream header) plus a 4-byte little-endian counter; the nonce is never written on the
    /// wire, so decrypt must rebuild the same sequence — reordered, replayed, or dropped chunks fail auth. The last chunk is marked final (truncation detection)
        ...
        CancellationToken ct)
    {
        ...
        try {
            // one-chunk lookahead so the last chunk can be flagged: `current` is emitted only after we know
            // whether another read returned data. Empty input still writes one zero-length final chunk so
            // dropping the entire payload is detectable.
    ...
    /// <summary>
    /// Decrypt: reads compact frames from <paramref name="input" />, rebuilds each nonce from <paramref name="noncePrefix" /> plus a local counter (the wire has
    /// no nonce) and authenticates <paramref name="aadNonFinal" /> / <paramref name="aadFinal" /> per chunk. Throws if the stream ends before a final-flagged chunk (truncation) or
    /// has bytes after it (extension).
    /// </summary>
