...
/// <summary>
/// AES Key Wrap (RFC 3394). Two-key stream DEK encoding when the KEK is a raw AES-sized key: wrap is deterministic, integrity-checked, and exactly
/// <c>dekLength + 8</c> bytes — no nonce or header, which fits the encrypted-DEK length cap. Implemented on AES-ECB so
/// every target framework works without extra packages.
/// </summary>
internal static class AesKeyWrap
{
    private const int BlockSize = 8;

    /// <summary>RFC 3394 default IV; seeing it after unwrap proves integrity.</summary>
    private const ulong DefaultIv = 0xA6A6A6A6A6A6A6A6;

    /// <summary>True when <paramref name="kekLength" /> is a legal AES key width (16, 24, or 32 bytes).</summary>
    public static bool IsValidKekLength(int kekLength) => kekLength is 16 or 24 or 32;

    /// <summary>Wraps <paramref name="keyData" /> (multiple of 8 bytes, at least 16) under <paramref name="kek" />; result is <c>keyData.Length + 8</c> bytes.</summary>
    public static byte[] Wrap(byte[] kek, byte[] keyData)
    {
        ...
        try {
            ...
            // each round starts with 8 bytes of plaintext key in the block buffer (after the first pass, r is ciphertext).
            SecurityUtilities.Clear(block);
        }
    }

    /// <summary>Unwraps RFC 3394 <paramref name="wrapped" /> under <paramref name="kek" />. Throws <see cref="DecryptionFailedException" /> if the integrity check fails.</summary>
    public static byte[] Unwrap(byte[] kek, byte[] wrapped)
    {
        ...
        try {
            ...
            // block buffer ends with 8 recovered plaintext-key bytes; r is returned and the caller must clear it.
            SecurityUtilities.Clear(block);
        }
    }
