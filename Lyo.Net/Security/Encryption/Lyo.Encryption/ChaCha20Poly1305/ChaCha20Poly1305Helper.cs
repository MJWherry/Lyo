...
public static class ChaCha20Poly1305Helper
{
    ...
    /// <summary>Encrypts into the caller-owned <paramref name="ciphertext" /> and <paramref name="tag" /> spans (sized to plaintext length and <see cref="TagSize" />).</summary>
    public static void Encrypt(ReadOnlySpan<byte> plaintext, byte[] key, ReadOnlySpan<byte> nonce, Span<byte> ciphertext, Span<byte> tag, byte[]? associatedData = null)
    {
        ...
        chacha.Init(true, new AeadParameters(new(key), 128, nonce.ToArray(), associatedData is { Length: > 0 } ? associatedData : null));
    ...
    /// <summary>Decrypts into the caller-owned <paramref name="plaintext" /> span (sized to the ciphertext length).</summary>
    public static void Decrypt(ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> tag, byte[] key, ReadOnlySpan<byte> nonce, Span<byte> plaintext, byte[]? associatedData = null)
    {
        ...
        try {
            ...
            try {
                var chacha = new Org.BouncyCastle.Crypto.Modes.ChaCha20Poly1305();
                chacha.Init(false, new AeadParameters(new(key), 128, nonce.ToArray(), associatedData is { Length: > 0 } ? associatedData : null));
