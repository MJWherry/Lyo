...
/// <summary>
/// In-process stand-in OpenID Connect IdP: discovery, JWKS, and a <c>/token</c> URL. Signs with Ed25519 so validation tests skip
/// RSA fixture setup. Used by the coordinator end-to-end test.
/// </summary>
internal sealed class FakeOidcIdentityProvider
{
    ...
    /// <summary>Builds a new <see cref="HttpMessageHandler" /> bound to this IdP. Fresh each call so the harness can attach one handler per typed HttpClient.</summary>
    ...
    internal sealed class Handler : HttpMessageHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct)
        {
            ...
            if (url == DiscoveryUrl) {
                ...
                return new(HttpStatusCode.OK) { Content = JsonContent.Create(doc) };
            }
            ...
            if (url == JwksUri) {
                ...
                return new(HttpStatusCode.OK) { Content = JsonContent.Create(jwks) };
            }
            ...
            if (url == TokenEndpoint) {
                ...
                foreach (var p in pairs) {
                    ...
                    if (eq > 0 && string.Equals(p.AsSpan(0, eq).ToString(), "code", StringComparison.Ordinal))
                        code = WebUtility.UrlDecode(p[(eq + 1)..]);
                }
