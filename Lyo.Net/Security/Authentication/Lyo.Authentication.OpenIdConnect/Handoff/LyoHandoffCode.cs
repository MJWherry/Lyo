...
/// fragment or a cross-origin cookie. The code id is the only thing that travels in the redirect URL; the actual tokens live in <see cref="IHandoffCodeStore" /> server-side until the
...
/// <param name="IssuedTo">
/// The exact origin (<c>scheme://host[:port]</c>) the API will accept on the consumer's <c>Origin</c> request header when the code is exchanged. Locks the code
/// to one frontend.
/// </param>
