...
{
  "LyoOidcBff": {
    "AllowedReturnOrigins": [ "http://localhost:5138", "https://localhost:7116", "https://app.example.com" ],
    "DefaultReturnUrl": "/",
...
- `AllowedReturnOrigins.` Exact origin (`scheme://host[:port]`) match for absolute `returnUrl` values and `Origin` checks on `/auth/handoff/exchange`. Same-origin relative `returnUrl` starting with `/` is always allowed. Absolute URLs outside the allowlist are rejected at `GET /auth/login/{provider}` (400) so a bad origin cannot complete Google login and then dump the browser on the API's `/`.
- `DefaultReturnUrl.` Used when `returnUrl` is missing. Relative `/` stays on the API origin, so local BFF setups should always send an absolute consumer handoff URL.
...
- **Browser consumers.** Use `Lyo.Authentication.Client` for handoff redemption, the session cookie, and `LyoAuthDelegatingHandler` for outbound refresh.
