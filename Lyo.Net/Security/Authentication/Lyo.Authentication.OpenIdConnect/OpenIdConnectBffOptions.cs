...
public sealed class OpenIdConnectBffOptions
{
    ...
    /// case-insensitively via <see cref="Uri.GetLeftPart" />(<see cref="UriPartial.Authority" />). Off-list absolute URLs are rejected at login (400). Relative paths starting
    /// with <c>/</c> are always allowed (they redirect to the API's own origin, which is only useful when API and frontend are co-located).
    /// </summary>
    public IList<string> AllowedReturnOrigins { get; set; } = [];

    /// <summary>
    /// Where to send the browser when no usable <c>returnUrl</c> is supplied. Default <c>/</c>. Absolute off-list <c>returnUrl</c> values are rejected rather than falling back
    /// here.
    /// </summary>
