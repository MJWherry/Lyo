...
/// survive. Bound from configuration via <see cref="Extensions.AddLyoOpenIdConnect(Microsoft.Extensions.DependencyInjection.IServiceCollection, Microsoft.Extensions.Configuration.IConfiguration, string)"/>.
/// </summary>
public sealed class OpenIdConnectBffOptions
{
    ...
    /// case-insensitively via <see cref="Uri.GetLeftPart"/>(<see cref="UriPartial.Authority"/>). Anything not on the list is downgraded to <see cref="DefaultReturnUrl"/>.
    /// Relative paths starting with <c>/</c> are always allowed (they redirect to the API's own origin, which is only useful when API and frontend are co-located).
    /// </summary>
    ...
    /// <summary>Lifetime of a <see cref="Handoff.LyoHandoffCode"/>. Must be long enough for the browser redirect + Gateway server-to-server exchange to complete, short enough to limit replay windows. Default <c>30s</c>.</summary>
