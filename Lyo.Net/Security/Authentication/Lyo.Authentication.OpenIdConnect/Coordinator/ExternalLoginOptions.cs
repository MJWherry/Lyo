...
public sealed class ExternalLoginOptions
{
    ...
    /// (e.g. <c>auth.tokens.read</c> + <c>auth.tokens.write</c> so they can mint their own PATs without an out-of-band admin grant). Has no effect on users that already exist
    /// when a fresh OIDC login arrives — existing user rows are not touched.
    /// </summary>
