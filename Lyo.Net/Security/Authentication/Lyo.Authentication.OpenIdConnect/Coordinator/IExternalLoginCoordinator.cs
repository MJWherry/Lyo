...
{
    /// <summary>Handles the OIDC callback for <paramref name="providerName"/>. Returns the freshly minted Lyo JWT and the post-login return target captured at <see cref="BuildLoginRedirectAsync"/> time.</summary>
    ...
    /// <param name="returnUrl">Pre-validated return target (relative path or allow-listed absolute URL). Sealed into the state cookie verbatim and echoed back to the caller after the callback succeeds.</param>
