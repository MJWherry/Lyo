...
/// <summary>Default <see cref="IExternalLoginCoordinator" />: runs the BFF callback per the plan's flow diagram.</summary>
public sealed class DefaultExternalLoginCoordinator : IExternalLoginCoordinator
{
    ...
    /// <summary>Builds a new coordinator.</summary>
        ...
        IUserScopeStore? scopes = null)
    {
        ...
        ArgumentHelpers.ThrowIfNull(tokens);
    ...
    public async Task<ExternalLoginResult> HandleCallbackAsync(string providerName, string code, string sealedState, string returnedState, CancellationToken ct = default)
    {
        try {
            ...
            await _audit.RecordAsync(_auditContext, _logger, AuthAuditEventKind.ExternalLoginRejected, provider: providerName, outcome: "failure", reason: ex.Reason, ct: ct)
    ...
    private static string NormalizeMode(string? mode) => string.Equals(mode, "api", StringComparison.OrdinalIgnoreCase) ? "api" : "browser";
    ...
    private async Task<ExternalLoginResult> HandleCallbackInnerAsync(string providerName, string code, string sealedState, string returnedState, CancellationToken ct)
    {
        ...
        var provider = _providers.Get(providerName);
        var pkceState = _state.Unseal(sealedState) ?? throw new ExternalLoginRejectedException("OidcStateInvalid", "missing or tampered state cookie");
        ...
        if (link is null) {
            ...
            await _audit.RecordAsync(_auditContext, _logger, AuthAuditEventKind.IdentityLinked, user.Id, subject, provider.Name, "success", ct: ct).ConfigureAwait(false);
        }
        else {
            user = await _users.GetByIdAsync(link.UserId, null, ct).ConfigureAwait(false) ??
        ...
        if (_scopes is not null)
            await UserScopeNames.SeedAsync(_scopes, user.Id, user.Scopes, null, ct).ConfigureAwait(false);

        var effective = await UserScopeNames.ResolveAsync(_scopes, user, ct).ConfigureAwait(false);
        await _tokens.RevokeRefreshTokensForUserAsync(user.Id, provider.Name, DateTime.UtcNow, "login_replaced", null, ct).ConfigureAwait(false);
        var issued = await _jwtIssuer.IssueAsync(user, effective, provider.Name, subject, true, ct).ConfigureAwait(false);
        await _audit.RecordAsync(
                _auditContext, _logger, AuthAuditEventKind.ExternalLoginSucceeded, user.Id, subject, provider.Name, "success", newlyProvisioned ? "jit_provisioned" : null, ct: ct)
            .ConfigureAwait(false);
    ...
    private async Task<LyoUser> ResolveOrProvisionAsync(
        IOpenIdConnectProvider provider,
        OidcClaimMappingResult mapping,
