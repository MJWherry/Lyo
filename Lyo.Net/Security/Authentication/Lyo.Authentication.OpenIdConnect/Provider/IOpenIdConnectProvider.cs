...
public interface IOpenIdConnectProvider
{
    ...
    /// <summary>OIDC discovery URL (e.g. <c>https://accounts.google.com/.well-known/openid-configuration</c>).</summary>
    string DiscoveryUrl { get; }

    /// <summary>Lyo API's registered OIDC client id at this provider.</summary>
    string ClientId { get; }

    /// <summary>Lyo API's client secret.</summary>
    string ClientSecret { get; }

    /// <summary>Redirect URI registered with the provider (must match Lyo's callback exactly).</summary>
    ...
    /// <summary>Maps validated id_token claims into a tuple the coordinator consumes: <c>(displayName, email, emailVerified, avatarUrl, locale, providerScopes)</c>.</summary>
...
/// <summary>Result of mapping provider id_token claims into the Lyo user shape.</summary>
...
/// <param name="EmailVerified">Provider <c>email_verified</c> claim.</param>
