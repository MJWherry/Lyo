...
public sealed class KeycloakOpenIdConnectProvider : IOpenIdConnectProvider
{
    ...
    public KeycloakOpenIdConnectProvider(IOptions<KeycloakOptions> options)
    {
        ...
        _name = string.IsNullOrWhiteSpace(_options.Name) ? $"keycloak:{_options.Realm}" : _options.Name!;
        _discoveryUrl = $"{_options.BaseUrl.TrimEnd('/')}/realms/{_options.Realm}/.well-known/openid-configuration";
    ...
    /// <inheritdoc/>
    public string DiscoveryUrl => _discoveryUrl;
    ...
    public OidcClaimMappingResult MapClaims(IReadOnlyDictionary<string, object?> claims)
        => KeycloakClaimMapper.Map(claims, _rolesToScopes);
