...
/// <summary>Converts Keycloak id_token claims to an <see cref="OidcClaimMappingResult" />, including <c>realm_access.roles</c> → Lyo scope mapping.</summary>
public static class KeycloakClaimMapper
{
    /// <summary>Pulls the usual Keycloak claims and translates realm roles into Lyo scopes with <paramref name="rolesToScopes" />.</summary>
    ...
    /// <summary>Takes realm-role names from <c>realm_access.roles</c> and remaps them through <paramref name="rolesToScopes" />.</summary>
    ...
    /// <summary>Reads <c>realm_access.roles</c> when it exists. Accepts either a <see cref="JsonElement" /> or a plain CLR collection.</summary>
