...
/// <summary>
/// Default <see cref="ILyoJwtValidator" />. Splits header/payload/signature, verifies the signature against the keystore key for the JWT <c>kid</c>, checks <c>iss</c>/
...
public sealed class Ed25519LyoJwtValidator : ILyoJwtValidator
{
    ...
    /// <summary>Builds a new validator.</summary>
    ...
    public async Task<ClaimsPrincipal?> ValidateAsync(string jwt, CancellationToken ct = default)
    {
        ...
        if (_authOptions.EnableDynamicScopeIntersection) {
            var current = new HashSet<string>(await UserScopeNames.ResolveAsync(_scopes, user, ct).ConfigureAwait(false), StringComparer.Ordinal);
