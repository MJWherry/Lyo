...
/// <summary>
/// Issues a Format-B refresh token of kind <c>internal</c>, scoped to <c>lyo:refresh</c>, and metadata-tagged with the access JWT <c>jti</c> and the originating
/// provider/subject (for scope re-resolution on rotation).
/// </summary>
public interface ILyoRefreshTokenIssuer
{
    ...
    /// <param name="parentJti"><c>jti</c> of the access JWT this refresh token is paired with.</param>
    ...
    /// Provider <c>sub</c> claim, when known. Used on rotation to re-resolve the <see cref="LinkedIdentity" /> and pick up scope
    /// grants/revocations.
    /// </param>
...
{
    /// <summary>Scope used to identify refresh tokens.</summary>
