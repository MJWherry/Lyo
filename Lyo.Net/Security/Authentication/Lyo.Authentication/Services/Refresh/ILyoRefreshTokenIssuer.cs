...
/// <summary>
/// Issues a Format-B refresh token of kind <c>internal</c>, scoped to <c>lyo:refresh</c>, and metadata-tagged with the access JWT's <c>jti</c> and the originating
...
public interface ILyoRefreshTokenIssuer
{
    ...
    /// Provider's <c>sub</c> claim, when one is known. Used on rotation to re-resolve the <see cref="LinkedIdentity" /> and pick up scope
    /// grants/revocations.
    /// </param>
