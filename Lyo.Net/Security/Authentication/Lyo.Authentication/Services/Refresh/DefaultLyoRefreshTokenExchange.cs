...
/// <summary>Default <see cref="ILyoRefreshTokenExchange" />. Validates → revokes (rotates) → issues a new JWT + refresh.</summary>
public sealed class DefaultLyoRefreshTokenExchange : ILyoRefreshTokenExchange
{
    ...
    /// <summary>Builds a new exchange.</summary>
    ...
    public async Task<IssuedLyoJwt?> ExchangeAsync(string presentedRefreshToken, CancellationToken ct = default)
    {
        ...
        var effectiveScopes = await UserScopeNames.ResolveAsync(_scopes, user, ct).ConfigureAwait(false);
