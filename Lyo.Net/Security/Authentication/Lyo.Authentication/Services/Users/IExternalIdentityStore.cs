...
/// <summary>Persistence boundary for <see cref="LinkedIdentity" />. In-memory for tests; <c>Lyo.Authentication.Postgres</c> in production.</summary>
/// <remarks>
...
public interface IExternalIdentityStore
{
    ...
    /// <summary>Creates or refreshes a link for <paramref name="userId" />, stamped with the resolved tenant.</summary>
    /// <remarks>
    /// If an active link already exists for (tenant, provider, subject) and it belongs to a different user, this throws. Otherwise the existing link's claims/email/scopes are
    /// refreshed and <c>UpdatedAt</c> and <c>LastUsedAt</c> are bumped. If no active link exists, a new one is created.
    /// </remarks>
