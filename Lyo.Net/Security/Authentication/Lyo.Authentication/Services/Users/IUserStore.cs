...
/// <remarks>Every method takes a <c>Guid? tenantId</c> that is resolved against the store's <c>TenancyOptions</c>: <c>SystemOnly</c> matches null-tenant rows, <c>SingleTenantDefault</c> falls back to the configured default, and <c>MultiTenantStrict</c> requires a non-empty value.</remarks>
