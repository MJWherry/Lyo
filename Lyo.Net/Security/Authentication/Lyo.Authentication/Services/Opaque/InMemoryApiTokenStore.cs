...
/// <remarks>The in-memory store does not enforce tenant filtering — <c>tenantId</c> is accepted for interface compatibility but ignored.</remarks>
public sealed class InMemoryApiTokenStore : IApiTokenStore
{
    ...
    /// <inheritdoc />
    public Task RevokeRefreshTokensForUserAsync(Guid userId, string? provider, DateTime revokedAt, string? reason, Guid? tenantId, CancellationToken ct = default)
    {
        foreach (var record in _records.Values) {
            if (!LyoRefreshTokenMatch.IsLiveRefreshForUser(record, userId, provider))
                continue;

            _records[record.Id] = record with { RevokedAt = revokedAt, RevokedReason = reason, UpdatedAt = revokedAt };
        }
    ...
    /// <inheritdoc />
    public Task DeleteAsync(string id, Guid? tenantId, CancellationToken ct = default)
    {
        ArgumentHelpers.ThrowIfNullOrWhiteSpace(id);
        if (!_records.TryRemove(id, out var _))
