...
public sealed class InMemoryApiTokenStore : IApiTokenStore
{
    ...
    /// <inheritdoc/>
    public Task InsertAsync(ApiTokenRecord record, CancellationToken ct = default)
    {
    ...
    /// <inheritdoc/>
    public Task<ApiTokenRecord?> GetByIdAsync(string id, CancellationToken ct = default)
    {
    ...
    /// <inheritdoc/>
    public Task TouchLastUsedAsync(string id, DateTime utcNow, CancellationToken ct = default)
    {
        ArgumentHelpers.ThrowIfNullOrWhiteSpace(id);
        _records.AddOrUpdate(
            id, _ => throw new InvalidOperationException($"Token id '{id}' not found."),
            (_, existing) => existing with { LastUsedAt = utcNow, UpdatedAt = utcNow });

        return Task.CompletedTask;
    }

    /// <inheritdoc/>
    public Task RevokeAsync(string id, DateTime revokedAt, string? reason, CancellationToken ct = default)
    {
        ArgumentHelpers.ThrowIfNullOrWhiteSpace(id);
        _records.AddOrUpdate(
            id, _ => throw new InvalidOperationException($"Token id '{id}' not found."),
    ...
    /// <inheritdoc/>
    public Task<IReadOnlyList<ApiTokenRecord>> ListForUserAsync(Guid userId, bool includeRevoked, CancellationToken ct = default)
    {
        var snapshot = _records.Values
            .Where(r => r.UserId == userId)
            .Where(r => includeRevoked || !r.RevokedAt.HasValue)
