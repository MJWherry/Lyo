...
{
    private readonly ConcurrentDictionary<string, Scope> _scopes = new(System.StringComparer.Ordinal);
    ...
    {
        var result = new HashSet<string>(System.StringComparer.Ordinal) { name };
    ...
    private static void ValidateName(string name)
    {
        foreach (var c in name) {
            ...
            throw new System.ArgumentException(
                $"Scope name '{name}' contains invalid character '{c}'. Allowed: lowercase ASCII letters, digits, '.', ':', '_', '-'.", nameof(name));
        }
    }
}
