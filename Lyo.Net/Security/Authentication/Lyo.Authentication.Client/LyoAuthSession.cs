...
/// <summary>
/// Mutable, server-side per-session token bundle. Lives in <see cref="LyoAuthSessionStore" /> keyed by <see cref="SessionId" />; the browser cookie carries only the
/// data-protected session id, never the tokens themselves.
/// </summary>
public sealed class LyoAuthSession
{
    ...
    /// <summary>Current Lyo JWT used for outbound API calls.</summary>
    public string AccessToken { get; private set; }

    /// <summary>Matching rotating refresh token, or <c>null</c> if the issuer did not mint one.</summary>
    ...
    /// <summary>
    /// Claims projected from the access JWT at session create / refresh. Used by <see cref="LyoAuthCookieAuthenticationHandler" /> to populate
    ...
    /// <summary>Builds a new session.</summary>
    ...
    /// <summary>Replaces access/refresh tokens after a successful refresh, recomputing claims and the updated-at timestamp.</summary>
