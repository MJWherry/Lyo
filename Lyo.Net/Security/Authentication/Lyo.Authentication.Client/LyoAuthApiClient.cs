...
public sealed class LyoAuthApiClient
{
    ...
    /// <summary>The configured consumer-side options. Exposed so callers (e.g. the handoff middleware) can read <see cref="LyoAuthClientOptions.AuthBaseUrl"/> without re-resolving the options.</summary>
    public LyoAuthClientOptions Options => _options;

    /// <summary>
    /// POSTs a handoff code to <c>/auth/handoff/exchange</c> with <paramref name="consumerOrigin"/> on the <c>Origin</c> header. Returns the issued tokens on success or <c>null</c> on any 4xx/5xx.
    /// </summary>
    ...
    /// The consumer's own absolute origin (<c>scheme://host[:port]</c>) — i.e. the public origin of the host running this handoff middleware. Must match the origin the
    /// API stamped onto the code when it was issued (the API derives that from the <c>returnUrl</c> passed to <c>/auth/login/{provider}</c>). Using the API's base URL here
    /// is wrong and produces <c>400 invalid_or_consumed_code</c>; pass the value of <c>$"{ctx.Request.Scheme}://{ctx.Request.Host.Value}"</c> from the inbound request.
    /// </param>
    ...
    public async Task<LyoTokenResponse?> ExchangeHandoffAsync(string handoffCode, string consumerOrigin, CancellationToken ct = default)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "/auth/handoff/exchange") {
            Content = JsonContent.Create(new { code = handoffCode }, options: JsonOptions)
        };
