...
public static class JwksEndpointBuilder
{
    ...
    /// <summary>Maps the JWKS endpoint at <paramref name="path"/> (default <c>/.well-known/jwks.json</c>). The endpoint is anonymous, returns JSON, and is safe to cache for short windows.</summary>
    public static IEndpointConventionBuilder MapLyoJwks(this IEndpointRouteBuilder endpoints, string path = DefaultPath)
    {
        ...
        return endpoints.MapGet(
                path,
                async (HttpContext ctx, JwkSetBuilder builder) => {
