...
public static class Extensions
{
    ...
    extension(IServiceCollection services)
    {
        ...
        /// registered — <c>AddLyoDiagnosticsWeb</c> wins via the same <c>TryAdd</c> when both packages are present), and replaces the registered <see cref="IAuthAuditContextAccessor" /> with
        /// <see cref="HttpAuthAuditContextAccessor" /> so audit events automatically carry the inbound caller's IP, User-Agent, and correlation id (the same id the structured logs and
        /// outbound HTTP headers use). Idempotent.
        /// </summary>
