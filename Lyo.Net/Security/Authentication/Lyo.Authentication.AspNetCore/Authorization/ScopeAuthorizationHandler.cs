...
/// <summary>
/// Handles <see cref="ScopeAuthorizationRequirement"/>. Walks every <c>scope</c> claim the principal holds (a) as one-per-claim entries (opaque-token shape) and (b) as a single
/// space-delimited entry (legacy JWT shape; the Lyo JWT validator already splits on issuance, but we still accept the legacy form for cross-issuer compatibility). When an
/// <see cref="IScopeRegistry"/> is registered, each held scope is expanded under <see cref="Scope.TransitiveImplies"/> so a holder of <c>admin</c> implicitly satisfies <c>people.read</c>
/// when that relationship has been declared.
/// </summary>
