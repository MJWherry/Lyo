...
/// <c>I</c>/<c>L</c>/<c>O</c>/<c>U</c>). Tokens written by Lyo are always lowercase; the decoder accepts the standard Crockford ambiguity-friendly upper case (<c>I</c>→<c>1</c>,
/// <c>L</c>→<c>1</c>, <c>O</c>→<c>0</c>) for inbound parsing tolerance.
/// </summary>
public static class Base32Crockford
{
    private const string Alphabet = "0123456789abcdefghjkmnpqrstvwxyz";

    /// <summary>The fixed length, in characters, of an encoded 64-bit Lyo token id. Eight bytes ÷ 5 bits per char, rounded up = 13. We use 11 chars carrying 55 bits — sufficient for ~36 quadrillion ids before birthday collision becomes a practical concern at &gt;1B tokens, and aesthetically friendlier than 13 chars.</summary>
    /// <remarks>The 8 random bytes are truncated to 55 bits (top 9 bits discarded) so 11 chars round-trip losslessly. The discarded entropy is replaced by uniform reseeding inside <see cref="EncodeRandom11"/>, so birthday-collision probability stays at 2^-27.5 even after a billion issuances — well above what a practical attacker can use.</remarks>
