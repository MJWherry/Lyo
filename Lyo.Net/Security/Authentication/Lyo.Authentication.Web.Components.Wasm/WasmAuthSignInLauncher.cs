...
/// <summary>
/// WASM-side <see cref="IAuthSignInLauncher" />. Sign-in 302s the browser straight to the API <c>/auth/login/{provider}</c> with the WASM origin's handoff path as the
/// <c>returnUrl</c> — the API redirects back here with <c>?lyo_handoff=...</c> on success, where <see cref="Pages.WasmAuthHandoffPage" /> redeems the code.
/// </summary>
public sealed class WasmAuthSignInLauncher : IAuthSignInLauncher
{
    ...
    /// <summary>Builds a new launcher.</summary>
    public WasmAuthSignInLauncher(NavigationManager navigation, WasmAuthSessionStore sessions, WasmAuthApiClient authApi, WasmAuthClientOptions options)
    {
