...
In the consuming WASM host's `Program.cs`: `appsettings.json` (served from the WASM client's `wwwroot/`): On the API side, the WASM origin **must** appear in
`LyoOidcBff.AllowedReturnOrigins`:
