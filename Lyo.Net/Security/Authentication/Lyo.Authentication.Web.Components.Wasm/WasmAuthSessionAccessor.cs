...
/// <summary>WASM <see cref="IAuthSessionAccessor" />. Reads <see cref="WasmAuthSessionStore" /> and rotates tokens via <see cref="WasmAuthApiClient" />.</summary>
public sealed class WasmAuthSessionAccessor : IAuthSessionAccessor
{
    ...
    public async Task<AuthSessionSnapshot?> GetCurrentAsync(CancellationToken ct = default)
    {
        ...
        return new(session.AccessToken, session.AccessTokenExpiresAt, !session.RefreshToken.IsNullOrWhitespace(), session.RefreshTokenExpiresAt, claims, scopes);
    }
    ...
    public async Task<bool> RefreshAsync(CancellationToken ct = default)
    {
        ...
        await _sessions.SetAsync(new(refreshed.AccessToken, refreshed.RefreshToken, now.AddSeconds(refreshed.ExpiresIn), refreshed.RefreshExpiresAtUtc(now)), ct).ConfigureAwait(false);
        return true;
    }
}