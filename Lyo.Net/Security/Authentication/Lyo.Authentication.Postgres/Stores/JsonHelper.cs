...
internal static class JsonHelper
{
    ...
    public static string SerializeStringList(IReadOnlyList<string>? values) =>
        values is null || values.Count == 0 ? "[]" : JsonSerializer.Serialize(values, Options);
    ...
    public static string? SerializeMetadata(IReadOnlyDictionary<string, object?>? values) =>
        values is null || values.Count == 0 ? null : JsonSerializer.Serialize(values, Options);
