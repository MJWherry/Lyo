...
/// <summary>
/// Host-agnostic HTTP wrapper for the <c>/auth/me</c> and <c>/auth/users/{id}</c> endpoints exposed by <c>Lyo.Authentication.OpenIdConnect</c>. The host adapter
/// attaches the bearer token (Server-side <c>LyoAuthDelegatingHandler</c> or the WASM-side equivalent).
/// </summary>
