...
/// <summary>
/// Host-agnostic HTTP wrapper for the <c>/tokens</c> management endpoints in <c>Lyo.Authentication.AspNetCore</c>. The host adapter is responsible for attaching the bearer token
/// (Server-side via <c>LyoAuthDelegatingHandler</c>, WASM-side via <see cref="Wasm.WasmAuthDelegatingHandler"/>) so that callers only need to know about the typed methods below.
/// </summary>
public interface IAuthTokenManagementClient
{
    ...
    /// <summary>
    /// Mints a new token. Returns <c>null</c> when the API responds non-2xx (missing scope, bad kind, no grantable scopes, etc.); the caller should treat that as failure and reload
    /// the token list to recover. The returned <see cref="AuthIssuedTokenResult.Plaintext"/> is shown to the user exactly once — never persist it client-side.
    /// </summary>
