...
/// <summary>
/// Host-agnostic HTTP wrapper for the <c>/tokens</c> management endpoints in <c>Lyo.Authentication.AspNetCore</c>. The host adapter attaches the bearer
/// token (Server-side via <c>LyoAuthDelegatingHandler</c>, WASM-side via <see cref="Wasm.WasmAuthDelegatingHandler" />) so callers only need the typed methods
/// below.
/// </summary>
public interface IAuthTokenManagementClient
{
    /// <summary>Returns the kinds of tokens this API understands, plus whether the current caller may mint each kind. Returns <c>null</c> on any non-2xx.</summary>
    Task<IReadOnlyList<AuthTokenKindDescriptor>?> ListKindsAsync(CancellationToken ct = default);

    /// <summary>Returns the caller's tokens. When <paramref name="includeRevoked" /> is <c>true</c>, includes revoked ones. Returns <c>null</c> on any non-2xx.</summary>
