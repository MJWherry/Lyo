...
/// <summary>
/// Mirror of the JSON returned by <c>GET /auth/me</c> and <c>GET /auth/users/{id}</c> in <c>Lyo.Authentication.OpenIdConnect</c>. Re-declared here so the components library does
/// not have to pull in the OIDC API project — the API ships the same shape.
/// </summary>
...
public sealed record AuthMeSnapshot(
    LyoUser User,
    string[] Scopes,
    LinkedIdentity[] LinkedIdentities);
