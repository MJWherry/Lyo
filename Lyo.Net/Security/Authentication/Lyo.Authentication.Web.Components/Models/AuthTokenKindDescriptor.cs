...
/// <summary>
/// Mirror of the JSON returned by <c>GET /tokens/kinds</c>. Tells the UI which token kinds the API understands, whether the current caller may issue each kind, and which scope they
/// would need to unlock the ones currently disallowed. The components library re-declares this shape so it can stay independent of <c>Lyo.Authentication.AspNetCore</c>.
/// </summary>
...
public sealed record AuthTokenKindDescriptor(
    ...
    [property: JsonPropertyName("requiredScope")] string RequiredScope);
