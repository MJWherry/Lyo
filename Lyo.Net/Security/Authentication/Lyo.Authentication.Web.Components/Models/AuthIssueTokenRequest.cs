...
/// <summary>Shape posted to <c>POST /tokens</c> from the token-management page. Mirror of <c>TokenManagementEndpointsMapper.CreateTokenRequest</c>.</summary>
/// <param name="DisplayName">User-facing label. Required.</param>
/// <param name="Kind">Kind to mint (one of <c>pat</c>, <c>svc</c>, <c>cli</c>, <c>webhook</c>). When <c>null</c> the API defaults to <c>pat</c>.</param>
/// <param name="Scopes">Requested scopes. Intersected with the caller's effective scopes; non-overlap returns <c>400 no_grantable_scopes</c> (except <c>webhook</c>).</param>
