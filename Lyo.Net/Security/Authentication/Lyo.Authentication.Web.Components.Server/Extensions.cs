...
/// <summary>Container helpers for the Blazor Server host adapter.</summary>
public static class Extensions
{
    /// <param name="services">Collection that receives the Server adapter registrations.</param>
    extension(IServiceCollection services)
    {
        ...
        /// <c>Lyo.Authentication.Client</c> BFF runtime. The caller must already have invoked <c>AddLyoAuthClient(...)</c> and <c>AddLyoAuthBlazorStateProvider()</c>, plus
        /// <c>AddLyoAuthWebComponents(...)</c> on the shared library.
        /// </summary>
        /// <remarks>
        /// Typed clients use <c>AddLyoAuthHandler()</c>, which by default chains <c>LyoCorrelationDelegatingHandler</c> as the outermost handler so every outbound call carries
