...
public sealed class PostgresApiTokenStoreTests
{
    ...
    public async Task DeleteAsync_RemovesRow()
    {
        var token = NewToken(NewId());
        await _fixture.TokenStore.InsertAsync(token, null, TestContext.Current.CancellationToken);
        await _fixture.TokenStore.DeleteAsync(token.Id, null, TestContext.Current.CancellationToken);
        Assert.Null(await _fixture.TokenStore.GetByIdAsync(token.Id, null, TestContext.Current.CancellationToken));
    }

    [Fact]
    public async Task RevokeRefreshTokensForUserAsync_RevokesLiveRefresh()
    {
        var owner = await CreateUserAsync();
        var refresh = NewToken(NewId(), owner.Id, ["lyo:refresh"], kind: "internal", metadata: new Dictionary<string, object?> { ["lyo_provider"] = "google" });
        var pat = NewToken(NewId(), owner.Id, ["people.read"]);
        await _fixture.TokenStore.InsertAsync(refresh, null, TestContext.Current.CancellationToken);
        await _fixture.TokenStore.InsertAsync(pat, null, TestContext.Current.CancellationToken);
        await _fixture.TokenStore.RevokeRefreshTokensForUserAsync(owner.Id, "google", DateTime.UtcNow, "login_replaced", null, TestContext.Current.CancellationToken);
        var loadedRefresh = await _fixture.TokenStore.GetByIdAsync(refresh.Id, null, TestContext.Current.CancellationToken);
        var loadedPat = await _fixture.TokenStore.GetByIdAsync(pat.Id, null, TestContext.Current.CancellationToken);
        Assert.NotNull(loadedRefresh?.RevokedAt);
        Assert.Null(loadedPat?.RevokedAt);
    }
    ...
    private static string NewId()
    {
        ...
        var n = Interlocked.Increment(ref _idSeq);
        var bytes = new byte[11];
        TestData.Fill(bytes, TestData.Seed + n);
    ...
    private static ApiTokenRecord NewToken(
        ...
        => new(
            id, TestData.Create(32, TestData.Seed + id.GetHashCode()), kind ?? ApiTokenKind.Pat, ApiTokenRing.Live, userId, "test token", scopes ?? [], metadata,
            DateTime.UtcNow, null, null, null, null, null, rotatedFromId);
}