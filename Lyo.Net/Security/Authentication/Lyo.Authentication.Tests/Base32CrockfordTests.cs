...
public class Base32CrockfordTests
{
    ...
    public void EncodeRandom11_ProducesExactly11Chars()
    {
        ...
        foreach (var c in encoded) {
            Assert.True(
                (c >= '0' && c <= '9') || (c is >= 'a' and <= 'z' && c is not 'i' and not 'l' and not 'o' and not 'u'), $"Unexpected char '{c}' in encoded id '{encoded}'.");
        }
    }
    ...
    {
        Assert.Throws<ArgumentException>(() => Base32Crockford.EncodeRandom11(new byte[7]));
        Assert.Throws<ArgumentException>(() => Base32Crockford.EncodeRandom11(new byte[9]));
    }
    ...
    public void IsValidId_AcceptsExactlyEncodedLength()
    {
        ...
        Assert.False(Base32Crockford.IsValidId(new('1', 12)));
    }
}