...
Hosts that scan untrusted files call `IContentThreatScanner` (and optional reputation) during ingest, then `SaveFileAsync` into a keyed `IFileStorageService`. FileStorage does not scan on save.
