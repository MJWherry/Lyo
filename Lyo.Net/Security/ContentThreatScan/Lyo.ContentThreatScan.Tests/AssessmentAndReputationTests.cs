...
public sealed class AssessmentAndReputationTests
{
    ...
    public async Task MalwareBazaar_http_stub_flags_known_entries()
    {
        ...
        ReputationPipelineOptions pipelineOptions = new() {
        ...
        var verdict = await pipelineImplementation.InspectAsync(
            new(new(digestBuffer), new ReadOnlyMemory<byte>(plaintextSample)), new("dropper.json"), TestContext.Current.CancellationToken);
    ...
    private static HttpClient BazaarClientFactory(string cannedJsonPayload)
        => new(
            new ScriptedHandler((request, cancellationToken) => {
                ...
                return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(cannedJsonPayload, Encoding.UTF8, "application/json") });
            }));

    private sealed class ScriptedHandler(Func<HttpRequestMessage, CancellationToken, Task<HttpResponseMessage>> fn) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) => fn(request, cancellationToken);
    }
}