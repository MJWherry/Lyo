...
/// <summary>
/// Light content sampling plus MD5 for change detection (directory snapshots and similar). Same algorithm as the old <c>Lyo.FileSystemWatcher.Utilities.Fingerprint</c> /
...
public static class SparseFileFingerprinter
{
    ...
    /// <summary>100 MiB (<see cref="FileSizeUnitInfo.Megabyte" /> × 100) — extra sample points on large files.</summary>
    public static readonly long DefaultLargeFileThreshold = FileSizeUnitInfo.Megabyte.ConvertToBytes(100);

    /// <summary>1 GiB (<see cref="FileSizeUnitInfo.Gigabyte" />) — sparse path using mtime plus a smaller content sample.</summary>
    public static readonly long DefaultVeryLargeThreshold = FileSizeUnitInfo.Gigabyte.ConvertToBytes(1);

    /// <summary>MD5 of (size + samples [+ mod time when very large]). Returns <see langword="null" /> if <paramref name="path" /> is missing.</summary>
    ...
    /// <summary>MD5 of size plus last-write time (no content I/O).</summary>
