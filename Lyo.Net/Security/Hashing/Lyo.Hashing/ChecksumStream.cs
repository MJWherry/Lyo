...
/// <summary>Stream wrapper that updates a non-cryptographic <see cref="ChecksumAlgorithm" /> as bytes pass through (same idea as <see cref="HashingStream" />).</summary>
public sealed class ChecksumStream : Stream
{
    ...
    /// <summary>Wraps <paramref name="baseStream" /> and folds <paramref name="algorithm" /> over every byte read or written.</summary>
    ...
    /// <summary>Checksum of all bytes seen so far as big-endian (4 bytes for 32-bit checksums, 8 for CRC-64). Safe to call more than once.</summary>
    public byte[] GetChecksum() => Checksummer.ToBigEndianBytes(_calculator.GetCurrentValue(), _calculator.HashSizeInBytes);

    /// <summary>Checksum of all bytes seen so far as a raw number (32-bit results sit in the low bits).</summary>
    public ulong GetChecksumValue() => _calculator.GetCurrentValue();

    /// <summary>Hex of <see cref="GetChecksum" /> using the chosen A–F letter case.</summary>
