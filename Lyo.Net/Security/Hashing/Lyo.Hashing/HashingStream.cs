...
/// <summary>Stream wrapper that hashes bytes as they flow through.</summary>
public sealed class HashingStream : Stream
{
    ...
    /// <summary>Hash after every byte has passed through.</summary>
    public byte[] GetHash()
    {
        if (_hashFinalized)
    ...
    /// <summary>Uppercase hex of <see cref="GetHash" /> (legacy default for this type).</summary>
    public string GetHashString() => HexEncoding.ToHexString(GetHash());

    /// <summary>Hex of <see cref="GetHash" /> with the chosen A–F letter case.</summary>
        ...
        if (!_disposed && disposing) {
            if (!_hashFinalized) {
                try {
                    ...
                    // Swallow errors while finalizing on dispose
                }
            }
