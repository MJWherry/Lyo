...
/// <summary>
/// Static helpers for non-cryptographic checksums (CRC-32, CRC-32C, CRC-64/ECMA-182, Adler-32). They catch accidental corruption only and are <strong>not</strong> a security
/// boundary — see the package README. On modern .NET, CRC-32 and CRC-64 buffer paths call <c>System.IO.Hashing</c>; remaining algorithms use internal table-driven
/// implementations that match across targets.
/// </summary>
public static class Checksummer
{
    /// <summary>Runs <paramref name="algorithm" /> over <paramref name="data" /> and returns big-endian bytes (4 for 32-bit checksums, 8 for CRC-64).</summary>
    ...
    /// <summary>Runs <paramref name="algorithm" /> over <paramref name="stream" /> from the current position to EOF. Does not dispose <paramref name="stream" />.</summary>
    ...
    /// <summary>Runs <paramref name="algorithm" /> over <paramref name="data" /> and returns the raw number (32-bit results sit in the low bits).</summary>
    ...
    /// <summary>Runs <paramref name="algorithm" /> over <paramref name="stream" /> and returns the raw number. Does not dispose <paramref name="stream" />.</summary>
