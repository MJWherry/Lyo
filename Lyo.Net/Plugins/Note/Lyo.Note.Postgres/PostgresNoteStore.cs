...
/// <summary>Postgres-backed INoteStore.</summary>
public sealed class PostgresNoteStore : EntityRefPostgresStoreBase, INoteStore, IHealth
{
    ...
    public PostgresNoteStore(
        ...
        PostgresNoteOptions noteOptions,
        IEnumerable<IEntityRefActionInterceptor>? interceptors = null)
        : base(entityRefOptions, noteOptions?.Tenancy ?? throw new ArgumentNullException(nameof(noteOptions)), interceptors)
    {
    ...
    public Task<HealthResult> CheckHealthAsync(CancellationToken ct = default)
        => PostgresHealth.CheckAsync(_contextFactory, PostgresNoteOptions.Schema, ct);
