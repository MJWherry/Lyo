...
/// <summary>
/// Regression checks for the job-system review fixes: Started CAS guard, queued-run cancellation, shutdown requeue, dispatch suppression, idempotency-key and schedule-slot
/// handling on create/rerun/child-creation, and the batch latest-runs query.
/// </summary>
...
public class JobRunLifecycleFixTests
{
    ...
    public async Task RequeueJobRun_WhenNotRunning_ReturnsError()
    {
        ...
        // A pending user cancellation cannot be forgotten by a shutdown hand-back.
        Assert.Null(result);
        Assert.NotNull(error);
    ...
    public async Task CreateJobRun_WithSuppressDispatch_SucceedsWhenMqDisconnected()
    {
        ...
        // The caller owns dispatch, so a disconnected MQ cannot block persistence.
        Assert.True(result.IsSuccess);
        Assert.Equal(JobState.Queued, result.Data!.State);
    }
    ...
    [Fact]
    public async Task CreateJobRun_WithDuplicateScheduleSlot_ReturnsExistingRunWithoutThrowing()
    {
        var scheduleId = await SeedScheduleAsync(_fixture.JobDefinitionId);
        var slot = DateTime.UtcNow.AddMinutes(-1);
        var first = await _fixture.JobService.CreateJobRun(
            new(_fixture.JobDefinitionId, "test-user", false, scheduleId: scheduleId) { ScheduledSlotUtc = slot }, TestContext.Current.CancellationToken);
        Assert.True(first.IsSuccess, first.Error?.Detail ?? "first create failed");

        var second = await _fixture.JobService.CreateJobRun(
            new(_fixture.JobDefinitionId, "test-user", false, scheduleId: scheduleId) { ScheduledSlotUtc = slot }, TestContext.Current.CancellationToken);
        Assert.True(second.IsSuccess, second.Error?.Detail ?? "duplicate slot must be idempotent");
        Assert.Equal(first.Data!.Id, second.Data!.Id);
    }

    [Fact]
    public async Task CreateJobRun_ConcurrentDuplicateScheduleSlots_AllResolveToOneRun()
    {
        var scheduleId = await SeedScheduleAsync(_fixture.JobDefinitionId);
        var slot = DateTime.UtcNow.AddMinutes(-2);
        var tasks = Enumerable.Range(0, 4)
            .Select(
                _ => _fixture.JobService.CreateJobRun(
                    new(_fixture.JobDefinitionId, "test-user", false, scheduleId: scheduleId) { ScheduledSlotUtc = slot }, TestContext.Current.CancellationToken))
            .ToList();
    ...
    private async Task<Guid> SeedScheduleAsync(Guid definitionId)
    {
