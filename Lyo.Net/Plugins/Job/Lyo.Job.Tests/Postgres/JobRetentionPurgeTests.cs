...
/// <summary>
/// Regression tests for retention-purge FK handling: purging a run referenced by a workflow run step, or a parent run whose children survive the batch, cannot violate
...
public class JobRetentionPurgeTests
{
    ...
    {
        var definitionId = await CreateDefinitionAsync(1);
    ...
    public async Task Purge_ParentWithSurvivingChildren_DetachesChildrenAndDeletesParent()
    {
        ...
        // Child finished recently. it survives the purge and cannot block deleting the parent.
        Guid childId;
        await using (var db = await CreateDbContextAsync()) {
