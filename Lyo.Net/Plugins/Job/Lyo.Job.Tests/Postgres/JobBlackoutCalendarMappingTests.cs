...
public class JobBlackoutCalendarMappingTests
{
        ...
        var sharedCalendar = new JobBlackoutCalendarReq {
            ...
            CreateBlackoutWindows = [
                new() {
                    ...
                    StartDateUtc = new DateTime(2026, 12, 25, 0, 0, 0, DateTimeKind.Utc),
    ...
    [Fact]
    public void JobBlackoutWindowReq_Holiday_RoundTripsSlugAndObservedFlag()
    {
        var req = new JobBlackoutWindowReq {
            Name = "Christmas",
            HolidaySlug = "christmas-day",
            IncludeObservedDate = true,
            StartTime = TimeOnly.Parse("00:00"),
            EndTime = TimeOnly.Parse("23:59"),
            Policy = JobBlackoutPolicy.Skip,
            Enabled = true
        ...
        var entity = Mapper.Map<JobBlackoutWindow>(req);
        Assert.Equal("christmas-day", entity.HolidaySlug);
        Assert.True(entity.IncludeObservedDate);
        Assert.Equal(nameof(DayFlags.None), entity.DayFlags);
        var res = Mapper.Map<JobBlackoutWindowRes>(entity);
        Assert.Equal("christmas-day", res.HolidaySlug);
        Assert.True(res.IncludeObservedDate);
    }

    [Fact]
    public void JobBlackoutWindowReq_CalendarDays_RoundTripsMonthsAndDays()
    {
        var req = new JobBlackoutWindowReq {
            Name = "Month end",
            MonthFlags = MonthFlags.EveryMonth,
        ...
        var entity = Mapper.Map<JobBlackoutWindow>(req);
        Assert.Equal(nameof(MonthFlags.EveryMonth), entity.MonthFlags);
        Assert.Equal([31], entity.DaysOfMonth);
        var res = Mapper.Map<JobBlackoutWindowRes>(entity);
        Assert.Equal(MonthFlags.EveryMonth, res.MonthFlags);
        Assert.Equal([31], res.DaysOfMonth);
    }
}