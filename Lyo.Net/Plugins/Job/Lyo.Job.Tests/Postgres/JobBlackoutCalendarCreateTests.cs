...
public class JobBlackoutCalendarCreateTests(JobPostgresFixture fixture) : IClassFixture<JobPostgresFixture>
{
    ...
    [Fact]
    public async Task CreateDefinition_WithHolidayAndCalendarDayWindows_PersistsSlugAndDays()
    {
        await using var scope = fixture.ServiceProvider.CreateAsyncScope();
        var create = scope.ServiceProvider.GetRequiredService<ICreateService<JobContext>>();
        var factory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<JobContext>>();
        ...
        var created = await create.CreateAsync<JobDefinitionReq, JobDefinition, JobDefinitionRes>(
            new() {
                Name = $"BlackoutHoliday-{definitionId:N}",
                Type = "Test",
                WorkerType = "cs",
                Enabled = true,
                CreateBlackoutCalendar =
        ...
        var windows = schedule.JobBlackoutCalendar!.JobBlackoutWindows.OrderBy(w => w.Name).ToList();
        Assert.Equal(2, windows.Count);
        Assert.Equal("christmas-day", windows[0].HolidaySlug);
        Assert.Equal(nameof(DayFlags.None), windows[0].DayFlags);
        Assert.Null(windows[0].DaysOfMonth);
        Assert.Equal("Inventory freeze", windows[1].Name);
        Assert.Equal(nameof(MonthFlags.Dec), windows[1].MonthFlags);
        Assert.Equal([31], windows[1].DaysOfMonth);
        Assert.Null(windows[1].HolidaySlug);
    }
}