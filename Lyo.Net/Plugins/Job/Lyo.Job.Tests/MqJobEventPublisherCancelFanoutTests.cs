...
/// <summary>
/// Regression checks for the cancellation fanout fix: each worker instance must declare its own exclusive, auto-delete queue bound to the cancel routing key, so every
...
public class MqJobEventPublisherCancelFanoutTests
{
    ...
    private static MqJobEventPublisher CreatePublisher(IMqService mq) => new(mq, NullLogger<MqJobEventPublisher>.Instance, new JobMqOptions());
