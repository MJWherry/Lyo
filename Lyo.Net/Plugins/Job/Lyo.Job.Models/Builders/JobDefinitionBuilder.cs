...
public class JobDefinitionBuilder(JobDefinitionReq? request = null)
{
    ...
    /// <summary>Attaches an existing blackout calendar to each schedule on this definition (unless a schedule overrides it).</summary>
    ...
    /// <summary>Creates an inline blackout calendar on this definition and attaches it to each schedule (unless a schedule overrides it).</summary>
    ...
    /// <summary>Appends a do-not-run window to the definition-level inline blackout calendar.</summary>
    ...
    public JobDefinitionBuilder AddJobParameter(string key, LyoTypeInfo type, object? value = null, string? description = null, bool required = true)
    {
        _request.CreateParameters.Add(
            new() {
                Description = description,
                Key = key,
                Type = type.FullName,
    ...
    public JobDefinitionBuilder AddEncryptedJobParameter(string key, LyoTypeInfo type, byte[]? value = null, string? description = null)
    {
        _request.CreateParameters.Add(
            new() {
                Description = description,
    ...
    public JobDefinitionBuilder AddPaginationAmount(int pageAmount, bool required = false)
        => AddJobParameter(Constants.Data.JobRunParameterKey.PaginationAmount, LyoTypeInfo.Int, pageAmount, required: required);

    /// <summary>
    /// Sets automatic retries: <paramref name="maxRetryCount" /> attempts with a <paramref name="backoffSeconds" /> base delay that grows per
    /// <paramref name="backoffType" />.
    /// </summary>
    ...
    /// <summary>Sets the heartbeat timeout, in minutes, after which a Running job is treated as dead.</summary>
    ...
    /// <summary>Caps concurrent active runs (Queued + Running). 0 = unlimited.</summary>
    ...
    /// <summary>
    /// Turns on the circuit breaker: auto-disable after <paramref name="threshold" /> consecutive failures, auto-reset after
    /// <paramref name="resetMinutes" /> minutes.
    /// </summary>
    ...
    /// <summary>Sets message priority (0-9) for runs of this definition. Higher values are consumed first.</summary>
    ...
    /// <summary>Sets how many days finished runs stay before the maintenance service purges them. 0 = host default.</summary>
    ...
    /// <summary>Caps how many runs may be created per hour. 0 = unlimited.</summary>
    ...
    /// <summary>Sets SLA expectations for runs of this definition.</summary>
    ...
    /// <summary>Turns on failure alerting with an optional consecutive-failure threshold and webhook URL.</summary>
