...
public partial class JobContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<JobBlackoutWindow>(entity => {
            ...
            entity.Property(e => e.Enabled).HasColumnName("enabled");
            entity.Property(e => e.HolidaySlug).HasMaxLength(64).HasColumnName("holiday_slug");
            entity.Property(e => e.IncludeObservedDate).HasDefaultValue(false).HasColumnName("include_observed_date");
            entity.Property(e => e.MonthFlags).HasMaxLength(108).HasColumnName("month_flags");
            entity.Property(e => e.DaysOfMonth).HasColumnType("integer[]").HasColumnName("days_of_month");
            entity.Property(e => e.CreatedTimestamp).HasColumnType("timestamp with time zone").HasColumnName("created_timestamp");
            entity.Property(e => e.UpdatedTimestamp).HasColumnType("timestamp with time zone").HasColumnName("updated_timestamp");
        ...
        modelBuilder.Entity<JobParameter>(entity => {
            ...
            entity.Property(e => e.Type).HasMaxLength(1024).HasColumnName("type");
            ...
            entity.Property(e => e.DefaultKind).HasMaxLength(10).HasDefaultValue(nameof(LyoParameterDefaultKind.Literal)).HasColumnName("default_kind");
            entity.Property(e => e.DefaultTemplate).HasColumnType("text").HasColumnName("default_template");
        ...
        modelBuilder.Entity<JobRun>(entity => {
            ...
            // Serves the scheduler's latest-run lookups (newest run per definition, optionally filtered by result). Without the trailing
            // descending timestamp, Postgres had to sort the whole per-definition partition on every refresh.
            entity.HasIndex(e => new { e.JobDefinitionId, e.CreatedTimestamp }).IsDescending(false, true).HasDatabaseName("ix_job_run_definition_created_desc");

            entity.HasIndex(e => new { e.JobDefinitionId, e.Result, e.CreatedTimestamp })
                .IsDescending(false, false, true)
                .HasDatabaseName("ix_job_run_definition_result_created_desc");

            // Dead-job detection scans Running/Cancelling runs by heartbeat age on every maintenance tick.
            entity.HasIndex(e => new { e.State, e.LastHeartbeatUtc }).HasDatabaseName("ix_job_run_state_last_heartbeat");

            // Concurrency and rate-limit counts filter on both columns. Two single-column indexes forced a bitmap merge.
            entity.HasIndex(e => new { e.JobDefinitionId, e.State }).HasDatabaseName("ix_job_run_definition_state");

            // Retention purge orders by finished_timestamp inside a definition.
            entity.HasIndex(e => e.FinishedTimestamp).HasDatabaseName("ix_job_run_finished_timestamp");

            // Optimistic concurrency via Postgres' built-in xmin system column. A shadow property mapped to an existing system column, so
            // the migration is metadata-only and no physical column is added. Guards the read-modify-write paths (notably maintenance
            // dead-job detection) against overwriting a worker's concurrent finish.
    ...
    private static Models.Enums.JobRunResult? ToJobRunResult(string? v) => v == null ? null : TypeConversion.EnumOrNull<Models.Enums.JobRunResult>(v);

    private static JobState ToJobState(string v) => TypeConversion.EnumOrDefault<JobState>(v);

    private static JobWorkflowRunState ToJobWorkflowRunState(string v) => TypeConversion.EnumOrDefault<JobWorkflowRunState>(v);

    private static JobWorkflowStepState ToJobWorkflowStepState(string v) => TypeConversion.EnumOrDefault<JobWorkflowStepState>(v);
