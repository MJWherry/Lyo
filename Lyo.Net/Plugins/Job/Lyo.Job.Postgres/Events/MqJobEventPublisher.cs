...
/// <para>
/// Register via <c>services.AddMqJobEventPublisher()</c> after registering your <see cref="IMqService" /> implementation (for example <c>services.AddRabbitMq(...)</c>).
/// </para>
...
public sealed class MqJobEventPublisher : IJobEventPublisher
{
    ...
    public MqJobEventPublisher(IMqService mqService, ILogger<MqJobEventPublisher> logger, JobMqOptions mqOptions, IServiceScopeFactory? scopeFactory = null)
    {
    ...
    public async Task SetupAsync(CancellationToken ct = default)
    {
        ...
        await EnsureExchangeCreatedAsync(ct).ConfigureAwait(false);

        // Completion queue: the scheduler subscribes here to detect finished runs.
        await EnsureQueueCreatedAsync(Constants.Mq.QueueJobRunFinish, null, ct).ConfigureAwait(false);

        // Definition-update queue: the scheduler subscribes here to refresh its cache.
    ...
    {
        // Cancellations are broadcast through the exchange onto a per-instance exclusive queue. A shared per-worker-type queue would be a
        ...
        // Typed subscribe accepts both enveloped and legacy raw-Guid messages, and acks unparseable messages instead of redelivering them forever.
    ...
    private async Task<IReadOnlyList<string>> ResolveWorkerTypesAsync(CancellationToken ct)
    {
        ...
        try {
            ...
            _logger.LogWarning(ex, "Job definition table is not available yet; using JobMqOptions:WorkerTypes only.");
        }

        return types.ToList();
    }

    private async Task EnsureExchangeCreatedAsync(CancellationToken ct)
    {
        if (!await _mqService.CreateExchange(Constants.Mq.JobEventExchange, Constants.Mq.JobEventExchangeType, true, false, null, ct).ConfigureAwait(false))
