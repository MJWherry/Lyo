...
public sealed class JobLyoMapper : ILyoMapper
{
    ...
    private static DateTime ToUtcDateTime(DateTime value) => JobTimestamps.ToUtc(value);
    ...
    internal static JobParameter ReqToNew(JobParameterReq r)
        => new() {
            ...
            Key = r.Key,
            Description = r.Description,
            Type = LyoTypeInfo.NormalizeFullName(r.Type),
            Value = r.Value,
            ...
            Options = r.Options,
            DefaultKind = r.DefaultKind.ToString(),
    ...
    internal static JobBlackoutWindow ReqToNew(JobBlackoutWindowReq r)
        => new() {
            ...
            Enabled = r.Enabled,
            HolidaySlug = r.HolidaySlug,
            IncludeObservedDate = r.IncludeObservedDate,
            MonthFlags = r.MonthFlags?.ToString(),
            DaysOfMonth = r.DaysOfMonth is { Count: > 0 } ? r.DaysOfMonth.ToList() : null,
            CreatedTimestamp = UtcNow()
        };
    ...
    internal static JobRunResultEntity ReqToNew(JobRunResultReq r) => new() { Key = r.Key, Type = LyoTypeInfo.NormalizeFullName(r.Type), Value = r.Value };
    ...
    internal static JobRunLog ReqToNew(JobRunLogReq r)
        => new() {
            ...
            Timestamp = ToUtcDateTime(r.Timestamp)
        };
    ...
    internal static JobDefinitionRes ToRes(JobDefinition e)
        => new(
            ...
            TypeConversion.EnumOrDefault<JobRetryBackoffType>(e.RetryBackoffType), e.Priority, e.RetentionDays, e.MaxRunsPerHour, e.ExpectedDurationMinutes, e.MustStartByMinutes, e.AlertOnFailure,
    ...
    internal static JobParameterRes ToRes(JobParameter e)
        => new(
            e.Id, e.JobDefinitionId, e.Key, e.Description, LyoTypeInfo.NormalizeFullName(e.Type), LyoParameterMapper.MaskValue(e.Value, e.EncryptedValue),
            LyoParameterMapper.MaskEncryptedValue(e.EncryptedValue), true, e.Required, e.ValidationRegex, e.MinLength, e.MaxLength, e.AllowedValues, e.Options, e.Order,
            TypeConversion.EnumOrDefault<LyoParameterDefaultKind>(e.DefaultKind), e.DefaultTemplate);

    internal static JobScheduleRes ToRes(JobSchedule e)
        => new(
            e.Id, e.JobDefinitionId, TypeConversion.EnumOrDefault<MonthFlags>(e.MonthFlags), TypeConversion.EnumOrDefault<DayFlags>(e.DayFlags), TypeConversion.EnumOrDefault<ScheduleType>(e.Type),
            ...
            e.JobScheduleParameters.Select(ToRes).ToList(), e.CronExpression, TypeConversion.EnumOrDefault<JobMisfirePolicy>(e.MisfirePolicy), e.StartDateUtc, e.EndDateUtc, e.TimeZoneId,
    ...
    internal static JobScheduleParameterRes ToRes(JobScheduleParameter e)
        => new(e.Id, e.JobScheduleId, e.Key, LyoTypeInfo.NormalizeFullName(e.Type), e.Value, e.Description, null, e.Enabled);

    internal static JobTriggerRes ToRes(JobTrigger e)
        => new(
            e.Id, e.TriggersJobDefinitionId, e.TriggerJobResultKey, TypeConversion.EnumOrDefault<ComparisonOperatorEnum>(e.TriggerComparator), e.TriggerJobResultValue, e.Description, e.Enabled,
            null, e.JobTriggerParameters.Select(ToRes).ToList(), null);

    internal static JobTriggerParameterRes ToRes(JobTriggerParameter e)
        => new(e.Id, e.JobTriggerId, e.Key, LyoTypeInfo.NormalizeFullName(e.Type), e.Value, e.Description, null, e.Enabled);
    ...
    internal static JobBlackoutWindowRes ToRes(JobBlackoutWindow e)
        => new(
            e.Id, e.JobBlackoutCalendarId, e.Name, TypeConversion.EnumOrDefault<DayFlags>(e.DayFlags), ParseTime(e.StartTime), ParseTime(e.EndTime),
            TypeConversion.EnumOrDefault<JobBlackoutPolicy>(e.Policy), e.Enabled, e.StartDateUtc, e.EndDateUtc, e.HolidaySlug, e.IncludeObservedDate,
            string.IsNullOrWhiteSpace(e.MonthFlags) ? null : TypeConversion.EnumOrNull<MonthFlags>(e.MonthFlags), e.DaysOfMonth);
    ...
    internal static JobWorkflowStepRes ToRes(JobWorkflowStep e)
        => new(
            e.Id, e.JobWorkflowId, e.JobDefinitionId, e.StepName, e.StepOrder, e.DependsOnStepIds, TypeConversion.EnumOrDefault<JobWorkflowFailurePolicy>(e.FailurePolicy), e.ParametersJson,
            e.Enabled);
    ...
    internal static JobRunParameterRes ToRes(JobRunParameter e)
        => new(
            e.Id, e.JobRunId, e.Key, LyoTypeInfo.NormalizeFullName(e.Type), LyoParameterMapper.MaskValue(e.Value, e.EncryptedValue), e.Description,
            LyoParameterMapper.MaskEncryptedValue(e.EncryptedValue), false);

    internal static JobRunResultRes ToRes(JobRunResultEntity e) => new(e.Id, e.JobRunId, e.Key, LyoTypeInfo.NormalizeFullName(e.Type), e.Value);

    internal static JobRunLogRes ToRes(JobRunLog e) => new(e.Id, e.JobRunId, TypeConversion.EnumOrDefault<JobLogLevel>(e.Level), e.Message, e.Context, e.StackTrace, e.Timestamp);
    ...
    internal static JobWorkerInstanceRes ToRes(JobWorkerInstance e)
        => new() {
            ...
            State = TypeConversion.EnumOrDefault<JobWorkerInstanceState>(e.State),
    ...
    private static void Apply(JobParameterReq r, JobParameter e)
    {
        ...
        e.Key = r.Key;
        e.Description = r.Description;
        e.Type = LyoTypeInfo.NormalizeFullName(r.Type);
        e.Value = r.Value;
        ...
        e.Options = r.Options;
        e.DefaultKind = r.DefaultKind.ToString();
        e.DefaultTemplate = r.DefaultTemplate;
        e.Order = r.Order;
    }
    ...
    private static void Apply(JobBlackoutWindowReq r, JobBlackoutWindow e)
    {
        ...
        e.Enabled = r.Enabled;
        e.HolidaySlug = r.HolidaySlug;
        e.IncludeObservedDate = r.IncludeObservedDate;
        e.MonthFlags = r.MonthFlags?.ToString();
        e.DaysOfMonth = r.DaysOfMonth is { Count: > 0 } ? r.DaysOfMonth.ToList() : null;
    }
    ...
    private static void Apply(JobRunLogReq r, JobRunLog e)
    {
        ...
        e.Timestamp = ToUtcDateTime(r.Timestamp);
    }
