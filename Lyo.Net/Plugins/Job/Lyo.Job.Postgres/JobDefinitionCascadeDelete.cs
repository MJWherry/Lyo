...
/// <summary>
/// Deletes every row that references a <see cref="JobDefinition" /> so the definition itself can be removed without violating PostgreSQL foreign keys (<c>ClientSetNull</c> /
/// restrict semantics).
/// </summary>
public static class JobDefinitionCascadeDelete
{
    public static void RemoveDependents(JobContext db, Guid definitionId)
    {
        ...
        var runs = JobRunDependents.IncludeDependents(db.JobRuns.Where(r => r.JobDefinitionId == definitionId)).ToList();

        var runIds = runs.Select(r => r.Id).ToHashSet();

        // Workflow run-steps reference runs and workflow steps, so delete them before either.
