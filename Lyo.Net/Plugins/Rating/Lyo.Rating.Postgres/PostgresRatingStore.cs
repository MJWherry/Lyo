...
/// <summary>Postgres-backed IRatingStore.</summary>
public sealed class PostgresRatingStore : EntityRefPostgresStoreBase, IRatingStore, IHealth
{
    ...
    public PostgresRatingStore(
        ...
        PostgresRatingOptions ratingOptions,
        IEnumerable<IEntityRefActionInterceptor>? interceptors = null)
        : base(entityRefOptions, ratingOptions?.Tenancy ?? throw new ArgumentNullException(nameof(ratingOptions)), interceptors)
    {
    ...
    public Task<HealthResult> CheckHealthAsync(CancellationToken ct = default)
        => PostgresHealth.CheckAsync(_contextFactory, PostgresRatingOptions.Schema, ct);
    ...
    public async Task SaveAsync(RatingRecord rating, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var forId = EntityRefPersistedGuid.PersistedEntityId(rating.SubjectRef);
        var fromId = EntityRefPersistedGuid.PersistedEntityId(rating.ActorRef);
        await using var context = await _contextFactory.CreateDbContextAsync(ct).ConfigureAwait(false);
        var existing = await context.Ratings.WhereActive()
            .WhereTenant(tenant)
            .FirstOrDefaultAsync(
                r => r.SubjectEntityType == rating.SubjectEntityType && r.SubjectEntityId == forId && r.ActorEntityType == rating.ActorEntityType && r.ActorEntityId == fromId &&
        ...
        if (existing != null) {
            ...
            var entity = new RatingEntity {
                ...
                SubjectEntityType = rating.SubjectEntityType,
                SubjectEntityId = forId,
                ActorEntityType = rating.ActorEntityType,
    ...
    public async Task<IReadOnlyList<RatingRecord>> GetForEntityAsync(EntityRef forEntity, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var entities = await context.Ratings.WhereActive()
            .WhereTenant(tenant)
            .Where(r => r.SubjectEntityType == forEntity.EntityType && r.SubjectEntityId == forId)
            .ToListAsync(ct)
            .ConfigureAwait(false);
        ...
        CancellationToken ct = default)
    {
        ...
        var entity = await context.Ratings.WhereActive()
            .WhereTenant(tenant)
            .FirstOrDefaultAsync(
                r => r.SubjectEntityType == forEntity.EntityType && r.SubjectEntityId == forId && r.ActorEntityType == fromEntity.EntityType && r.ActorEntityId == fromId &&
    ...
    public async Task AddReactionAsync(EntityRef ratingRef, EntityRef fromEntity, RatingReactionType reactionType, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        if (existing != null) {
            ...
            var reaction = new RatingReactionEntity {
                Id = Guid.NewGuid(),
                SubjectEntityType = ratingRef.EntityType,
                SubjectEntityId = ratingId.ToString(),
    ...
    public async Task<IReadOnlyList<RatingRecord>> GetFromEntityAsync(EntityRef fromEntity, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var entities = await context.Ratings.WhereActive()
            .WhereTenant(tenant)
            .Where(r => r.ActorEntityType == fromEntity.EntityType && r.ActorEntityId == fromId)
            .ToListAsync(ct)
            .ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<RatingRecord>> GetForEntityTypeAsync(string forEntityType, Guid? forEntityId = null, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var tenant = ResolveTenant(tenantId);
        await using var context = await _contextFactory.CreateDbContextAsync(ct).ConfigureAwait(false);
        var query = context.Ratings.WhereActive().WhereTenant(tenant).Where(r => r.SubjectEntityType == forEntityType);
        if (forEntityId.HasValue)
            query = query.Where(r => r.SubjectEntityId == forEntityId.Value.ToString());
    ...
    public async Task DeleteAsync(Guid id, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var reactions = await context.RatingReactions.Where(r => r.SubjectEntityType == "Rating" && r.SubjectEntityId == id.ToString()).ToListAsync(ct).ConfigureAwait(false);
    ...
    public async Task DeleteForEntityFromEntityAsync(EntityRef forEntity, EntityRef fromEntity, string? subject = null, Guid? tenantId = null, CancellationToken ct = default)
    {
        ...
        var entities = await context.Ratings.WhereActive()
            .WhereTenant(tenant)
            .Where(r => r.SubjectEntityType == forEntity.EntityType && r.SubjectEntityId == forId && r.ActorEntityType == fromEntity.EntityType && r.ActorEntityId == fromId &&
                r.Subject == subject)
            .ToListAsync(ct)
            .ConfigureAwait(false);
