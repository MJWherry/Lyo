...
/// <summary>DI registration for PostgreSQL reporting (service layer only. Endpoints live in Lyo.Reporting.Api).</summary>
public static class Extensions
{
    extension(IServiceCollection services)
    {
        ...
        public IServiceCollection AddReportingDbContextFactory(PostgresReportingOptions options)
        {
            ...
            services.AddPostgresDbContextFactory<ReportingContext, PostgresReportingOptions>(options);
        ...
        /// <see cref="ReportGenerationThrottle" />, <see cref="ReportGenerationLiveTracker" />, and <see cref="ReportRetentionService" />. Requires AddLyoQueryServices, cache,
        /// and IIOTempService for generate staging. For HTML/PDF, call <c>AddReportingWebRenderer()</c> from Lyo.Reporting.Web on the host. Persist outputs with
        /// <see cref="AddReportingGenerationHooks" />. Map HTTP endpoints via <c>Lyo.Reporting.Api</c> <c>BuildReportingGroup</c>.
        /// </summary>
        public IServiceCollection AddPostgresReportingManagement(PostgresReportingOptions options)
        {
            ...
            services.TryAddScoped<IReportDataSourceResolver, ReportDataSourceResolver>();
            services.TryAddScoped<ReportParameterOptionsExecutor>();
            services.AddScoped<ReportRetentionService>();
            services.TryAddSingleton<ReportGenerationThrottle>();
            services.TryAddSingleton<ReportGenerationLiveTracker>();
        ...
        /// <see cref="PostgresReportingOptions.MaintenanceInterval" />. Call after <see cref="AddPostgresReportingManagement(PostgresReportingOptions)" />. Not needed on hosts
        /// that already schedule <see cref="ReportRetentionService.CleanupAsync(CancellationToken)" /> on their own.
        /// </summary>
        ...
        /// <summary>
        /// Registers host <see cref="ReportGenerationHooks" /> used by <see cref="ReportService" /> (for example save staged output via FileStorage). Replaces any previously
        /// registered hooks instance.
        /// </summary>
        ...
        /// <summary>Registers a generation profile (defaults for format, filename, and path) keyed by <see cref="ReportingGenerationProfile.Key" />.</summary>
        /// <remarks>
        ...
        /// <summary>Registers a generation profile using a key.</summary>
        ...
        /// <summary>Registers an <see cref="IReportDataProvider" /> (usually on the API host).</summary>
        ...
        /// <summary>Registers a concrete <see cref="IReportDataProvider" /> instance.</summary>
...
/// <summary>Fluent builder used to construct a <see cref="ReportingGenerationProfile" />.</summary>
