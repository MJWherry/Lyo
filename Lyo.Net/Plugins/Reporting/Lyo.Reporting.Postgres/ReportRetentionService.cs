...
/// generations Failed after <see cref="PostgresReportingOptions.StuckGenerationTimeout" /> (for example the host crashed mid-generation). Hosts schedule
/// <see cref="CleanupAsync(CancellationToken)" /> themselves (for example via Lyo.Scheduler or <c>AddReportingMaintenanceWorker</c>). Output blobs live in host storage, so
    ...
    ReportGenerationLiveTracker? liveTracker = null)
{
    ...
    /// <summary>
    /// Slack added to <see cref="PostgresReportingOptions.GenerationTimeout" /> when deriving the recovery cutoff. A generation cancelled at its timeout still needs a moment to
    /// unwind and persist Failed, and recovery must not race that write.
    /// </summary>
    private static readonly TimeSpan RecoveryGrace = TimeSpan.FromMinutes(5);

    /// <summary>Safety valve: most rows skipped by cleanup-hook failures in a single pass before the pass stops.</summary>
    ...
    /// <summary>Runs one maintenance pass (stuck-run recovery, then retention). Returns how many generations were deleted. 0 when retention is not configured.</summary>
    ...
    public async Task<int> CleanupAsync(ReportGenerationHooks? hooks, CancellationToken ct = default)
    {
        ...
        // explanatory error) for at least one retention interval instead of disappearing in the same pass.
    ...
    {
        if (options.GenerationRetention is not { } retention)
            return 0;
    ...
    /// <summary>
    /// Marks Pending/Running generations older than <see cref="PostgresReportingOptions.StuckGenerationTimeout" /> as Failed so crashed hosts do not strand rows forever. Returns
    /// how many generations were recovered. 0 when recovery is not configured.
    /// </summary>
    /// <remarks>
    /// Age alone does not mean stuck. Generations executing in this process are skipped via <see cref="ReportGenerationLiveTracker" />, and when
    /// <see cref="PostgresReportingOptions.GenerationTimeout" /> is longer than the stuck timeout the cutoff stretches to the timeout plus grace. Otherwise recovery would fail
    /// runs that are still legitimately working.
    /// </remarks>
    public async Task<int> RecoverStuckGenerationsAsync(CancellationToken ct = default)
    {
        if (options.StuckGenerationTimeout is not { } stuckTimeout)
            return 0;

        var minimumAge = stuckTimeout;
        if (options.GenerationTimeout is { } generationTimeout) {
            if (generationTimeout + RecoveryGrace > minimumAge)
                minimumAge = generationTimeout + RecoveryGrace;
        }
        else if (liveTracker is null) {
            logger.LogWarning(
                "Stuck-run recovery is enabled with no {Timeout} and no live tracker registered, so a generation running longer than {StuckTimeout} will be marked Failed while " +
                "it may still be working. Set {Timeout} or register {Tracker}.", nameof(PostgresReportingOptions.GenerationTimeout), stuckTimeout,
                nameof(PostgresReportingOptions.GenerationTimeout), nameof(ReportGenerationLiveTracker));
        }

        var cutoff = DateTime.UtcNow - minimumAge;
        var liveIds = liveTracker?.LiveIds.ToArray() ?? [];
        ...
        while (!ct.IsCancellationRequested) {
            var batch = await db.ReportGenerations
                .Where(g => ((g.Status == nameof(ReportGenerationStatus.Running) && (g.StartedTimestamp ?? g.CreatedTimestamp) < cutoff) ||
                        (g.Status == nameof(ReportGenerationStatus.Pending) && g.CreatedTimestamp < cutoff)) && !liveIds.Contains(g.Id))
                .OrderBy(g => g.CreatedTimestamp)
                .Take(BatchSize)
                .ToListAsync(ct)
                .ConfigureAwait(false);
    ...
    /// <summary>Invokes the host cleanup hook for one generation (used by retention and by definition-delete cleanup).</summary>
