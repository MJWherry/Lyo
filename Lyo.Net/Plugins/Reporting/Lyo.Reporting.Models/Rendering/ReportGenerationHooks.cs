...
/// <summary>Optional callbacks around report generation. Consumers use hooks to persist or post-process the staged output.</summary>
public sealed class ReportGenerationHooks
{
    ...
    /// <summary>
    /// Invoked before a generation row is removed (retention cleanup, generation delete, or definition delete) so hosts can delete the persisted output (for example a
    /// FileStorage blob). Throwing skips deletion of that row.
    /// </summary>
    public Func<ReportCleanupContext, CancellationToken, ValueTask>? OnCleanupAsync { get; init; }
}

/// <summary>Context for <see cref="ReportGenerationHooks.OnCleanupAsync" />: a generation row about to be deleted.</summary>
public sealed class ReportCleanupContext
{
    public required Guid GenerationId { get; init; }

    /// <summary>Persisted output id (consumer-defined, for example FileStorage). Null when the generation produced no saved output.</summary>
