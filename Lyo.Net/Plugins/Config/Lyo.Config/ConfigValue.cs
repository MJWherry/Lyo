...
/// <summary>Model for a typed configuration value persisted as JSON.</summary>
public sealed class ConfigValue
{
    /// <summary>
    /// Holds the CLR type name for JSON, same convention as <see cref="ConfigDefinitionRecord.SubjectEntityType" /> / <see cref="ConfigDefinitionRecord.ForValueType" />:.
    /// <see cref="Type.FullName" /> (via <see cref="GetTypeName(System.Type)" />).
    /// </summary>
    public string TypeName { get; set; } = string.Empty;

    /// <summary>Value of the serialized JSON payload.</summary>
    public string Json { get; set; } = "null";

    /// <summary>Makes a typed config value from the supplied value.</summary>
    public static ConfigValue From<T>(T value, JsonSerializerOptions? options = null) => From(typeof(T), value, options);

    /// <summary>Produces a typed config value from the supplied value and explicit type.</summary>
    ...
    /// <summary>Reads the value as the supplied type.</summary>
    ...
    /// <summary>Reads the value as the supplied runtime type.</summary>
    ...
    /// <summary>Looks up the serialized CLR type name to a runtime type.</summary>
    ...
    /// <summary>Looks up a type name stored as <see cref="Type.FullName" /> or assembly-qualified name (for example legacy rows).</summary>
    ...
    /// <summary>True when the supplied type name resolves to the same CLR type.</summary>
    ...
    /// <summary>
    /// Gives the CLR type name for storage and for <see cref="ConfigDefinitionRecord.ForValueType" />, matching <see cref="ConfigDefinitionRecord.SubjectEntityType" />:
    /// <see cref="Type.FullName" />, then an assembly-qualified name, then <see cref="Type.Name" />.
    /// </summary>
