...
public sealed record PortfolioUploadedFile(string FileName, string ContentType, byte[] Content)
{
    public long Size => Content.LongLength;
}

public sealed record PortfolioTransformOptions(
    bool Reverse,
...
public sealed record PortfolioTransformResult(string OutputFileName, string OutputContentType, byte[] OutputBytes, IReadOnlyList<KeyValuePair<string, string>> Details);

public sealed class PortfolioFileTransformer
{
    ...
    public PortfolioTransformResult ProbeFile(PortfolioUploadedFile file)
    {
        var details = new List<KeyValuePair<string, string>> {
    ...
    public static async Task<PortfolioUploadedFile> ReadBrowserFileAsync(IBrowserFile browserFile, CancellationToken ct = default)
    {
    ...
    private static StepResult Encrypt(byte[] bytes, string fileName, PortfolioTransformOptions options)
    {
    ...
    private static StepResult Decrypt(byte[] bytes, string fileName, PortfolioTransformOptions options)
    {
        var algorithm = options.DataEncryptionAlgorithm;
        var service = CreateEncryptionService(algorithm, CreatePlaceholderKeyStore(), options.DataAesGcmKeySize, options.DataAesSivKeySize);
    ...
    private static async Task<StepResult> TwoKeyEncryptAsync(byte[] bytes, string fileName, PortfolioTransformOptions options, CancellationToken ct)
    {
    ...
    private static async Task<StepResult> TwoKeyDecryptAsync(byte[] bytes, string fileName, PortfolioTransformOptions options, CancellationToken ct)
    {
        var dataAlgorithm = options.DataEncryptionAlgorithm;
        var keyAlgorithm = options.KeyEncryptionAlgorithm;
        var service = CreateTwoKeyEncryptionService(
    ...
    private static StepResult Decompress(byte[] bytes, string fileName, CompressionAlgorithm algorithm)
    {
        var service = new CompressionService(options: new() { DefaultAlgorithm = algorithm });
    ...
    private static void AppendTwoKeySizeDetails(
        ...
        EncryptionAlgorithm dataAlgorithm,
        EncryptionAlgorithm keyAlgorithm,
        PortfolioTransformOptions options)
    {
    ...
    private static byte[] DeriveSymmetricKey(string secret, EncryptionAlgorithm algorithm, AesGcmKeySizeBits aesGcmOrCcmKeySize, AesSivKeySizeBits aesSivKeySize)
    {
        ...
        return algorithm switch {
            ...
            var _ => throw new NotSupportedException($"{algorithm} is not supported.")
        };
    }
    ...
    private static string FormatSingleKeyEncryptionSummary(PortfolioTransformOptions options)
    {
    ...
    private static string FormatTwoKeyEncryptionSummary(PortfolioTransformOptions options)
    {
    ...
    private sealed record StepResult(string FileName, byte[] Bytes, IReadOnlyList<KeyValuePair<string, string>> Details);
}