...
namespace Lyo.Portfolio.Api.FileStorageWorkbench;
...
public static class SetupFileStorageWorkbenchEndpoints
{
    private static IFileStorageService GetFileStorage(IServiceProvider services)
        => services.GetRequiredKeyedService<IFileStorageService>(PortfolioRoutes.FileStorageWorkbench.ServiceKey);

    private static IMultipartUploadService GetMultipartUploadService(IServiceProvider services)
        => services.GetRequiredKeyedService<IMultipartUploadService>(PortfolioRoutes.FileStorageWorkbench.ServiceKey);

    private static IStagedFileUploadService GetStagedFileUploadService(IServiceProvider services)
        => services.GetRequiredKeyedService<IStagedFileUploadService>(PortfolioRoutes.FileStorageWorkbench.ServiceKey);

    private static IKeyStore GetKeyStore(IServiceProvider services) => services.GetRequiredKeyedService<IKeyStore>(PortfolioRoutes.FileStorageWorkbench.ServiceKey);
    ...
    /// <summary>Shared by <c>Workbench/FileStorage/files/save-stream</c> and <see cref="PortfolioRoutes.DirectFileUpload.FilePath" />.</summary>
    ...
    extension(WebApplication app)
    {
        ...
        {
            var group = app.MapGroup(PortfolioRoutes.FileStorageWorkbench.Route).WithTags("FileStorageWorkbench");
            group.MapGet(
                ...
                "files/{fileId:guid}/access-links", async (
                    ...
                    return Results.Ok(
                        new DownloadAccessLinkResponse(
                            result.LinkId, result.Token, $"{PortfolioRoutes.FileStorageWorkbench.Route}/files/access/{result.Token}/download",
                            $"{PortfolioRoutes.FileStorageWorkbench.Route}/files/access/{result.Token}/presigned-read", result.CreatedUtc, result.ExpiresAtUtc));
                });
        ...
        {
            app.MapPost(
                    PortfolioRoutes.DirectFileUpload.FilePath, async (
                        IFormFile file, [FromQuery] string? originalFileName, [FromQuery] bool compress, [FromQuery] bool encrypt, [FromQuery] string? keyId,
                        [FromQuery] string? pathPrefix, [FromQuery] int? chunkSize, [FromQuery] string? contentType, [FromQuery] string? tenantId, IServiceProvider services,
                        ICacheService cache, CancellationToken ct) => {
                        var fileStorage = GetFileStorage(services);
