...
/// <summary>Fluent builder for <see cref="LyoProblemDetails" /> with optional tracing and structured <c>errors</c> rows.</summary>
public sealed class LyoProblemDetailsBuilder
{
    /// <summary>Fallback root <see cref="LyoProblemDetails.Detail" /> when the problem is driven only by <see cref="LyoProblemDetails.Errors" />.</summary>
    ...
    public static LyoProblemDetailsBuilder FromException(Exception ex, string? errorCode = null)
    {
        ...
        if (fieldErrors is { Count: > 0 }) {
            ...
            // Build() fills Detail from DefaultValidationDetailSummary; the exception Message repeats the field errors.
            return b;
        }

        // Stack traces name internal types, file paths, and line numbers. Put those in the host log for this exception, not in the response body.
        for (var e = ex; e != null; e = e.InnerException)
            b.AddApiError(code, e.Message);
    ...
    /// <summary>Sets distributed trace and span ids (from <see cref="Activity" /> or headers, for example).</summary>
    ...
    /// <summary>Sets RFC 7807 <c>instance</c> (usually the request path or URI).</summary>
    ...
    /// <summary>Field-validation helper that uses <see cref="Constants.ApiErrorCodes.InvalidField" />.</summary>
...
/// <summary>Helpers for common error cases.</summary>
