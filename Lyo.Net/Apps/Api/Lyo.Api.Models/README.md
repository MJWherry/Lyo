...
HTTP contract models shared by Lyo minimal APIs and their clients. Distinct from [`Lyo.Query.Models`](../../../Data/Query/Lyo.Query.Models/README.md) (projection DTOs + filter trees).

## Features

- **`ExportFormatExtensions`** (`Enums/`). Maps `ExportFormat` onto the shared `FileTypeInfo` catalog: `FileType`, and `DefaultFileName` (`export` plus the format's default extension). Grids and endpoints read extension and MIME type from here instead of repeating per-format switches.
...
`Lyo.Api` endpoints emit these concrete result records (see [`Common/Response/Result.cs`](Common/Response/Result.cs) and [
...
In-process modules sometimes return [`Lyo.Result`](../../../Core/Result/Lyo.Result/README.md). On the wire the contract remains ProblemDetails.
