...
/// <summary>RFC 9457 problem details plus Lyo-specific <see cref="Errors" /> and tracing. Serialized with the default JSON contract (no custom converter).</summary>
...
{
    /// <summary>
    /// Root <see cref="Detail" /> plus structured <see cref="Errors" /> descriptions when present. Prefer this on exception/UI surfaces so callers see validation entries, not
    /// only the summary.
    /// </summary>
    public string GetFullMessage()
    {
        ...
        // FromCode and single-error cases often copy Detail onto Errors[0].
    ...
    /// <summary>Maps stable <see cref="Constants.ApiErrorCodes" /> values onto HTTP status codes for problem responses.</summary>
    ...
    /// <summary>Default <see cref="Constants.ApiErrorCodes" /> value for an HTTP status, used when an exception has no explicit error code.</summary>
    ...
    /// <summary>HTTP status title for <see cref="MapErrorCodeToHttpStatus" /> (used when wrapping export, for example).</summary>
