...
/// (images other than SVG, audio, video, archives, Open XML Office documents, and PDF).
/// </summary>
public static class LyoApiCompressionDefaults
{
    /// <summary>ASP.NET Core wildcard matching any response <c>Content-Type</c> not listed in <see cref="ExcludedMimeTypes" />.</summary>
    public static IReadOnlyList<string> MimeTypes { get; } = ["*/*"];

    /// <summary>MIME types skipped by response compression. Never includes <c>application/octet-stream</c>, so unknown or bin downloads still compress.</summary>
    ...
    private static IReadOnlyList<string> BuildExcludedMimeTypes()
    {
        ...
        foreach (var type in FileTypeInfo.All) {
            var skip = type.Category is FileTypeCategory.Compressed or FileTypeCategory.Audio or FileTypeCategory.Video or FileTypeCategory.PackageManager
