...
public static partial class ServiceCollectionExtensions
{
    ...
    extension(IServiceCollection services)
    {
        ...
        /// <see cref="RelationalEventId.PendingModelChangesWarning" /> so soft or cross-schema navigations do not block <c>MigrateAsync</c> against an unchanged migration snapshot.
        /// </summary>
        public IServiceCollection AddDbContextFactoryWithLyoNavigations<TContext>(Action<DbContextOptionsBuilder> optionsAction)
            where TContext : DbContext
        {
            ArgumentHelpers.ThrowIfNull(optionsAction);
        ...
        /// <summary>Registers <see cref="IDbContextFactory{TContext}" /> with typed settings and <see cref="LyoComposingModelCustomizer" />.</summary>
