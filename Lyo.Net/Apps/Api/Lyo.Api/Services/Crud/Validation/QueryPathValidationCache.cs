...
/// <summary>
/// Per-request cache so the same dotted path is not re-checked for sort, where, select, and include. Complements process-wide <see cref="SharedEntityMetadataCache" /> and
...
public sealed class QueryPathValidationCache
{
    ...
    /// <summary>CLR property path checks (same rules as where/sort).</summary>
        ...
        where TDbModel : class
    {
        ...
        try {
            ...
            // Filter path accepted but projection normalization differs (for example edge cases). Omit select warm.
        }

        return ok;
    }

    /// <summary>Select path normalization (projection). Reuses session entries warmed from successful filter checks when paths match.</summary>
    ...
    /// <summary>EF navigation include path, using the same rules as <see cref="EntityLoaderService.CollectIncludePathErrors" /> per path.</summary>
