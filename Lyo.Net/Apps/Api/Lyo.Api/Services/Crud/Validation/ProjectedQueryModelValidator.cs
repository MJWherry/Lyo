...
{
    public static Result<ProjectedQueryValidatorInput<TContext, TDbModel>> Validate<TContext, TDbModel>(ProjectedQueryValidatorInput<TContext, TDbModel> input)
        ...
        where TDbModel : class
    {
        ...
        foreach (var s in sortBy) {
            ...
            if (string.IsNullOrWhiteSpace(name)) {
                errors.Add(new($"Sort entry is missing a property name for type '{typeof(TDbModel).Name}'.", Constants.ApiErrorCodes.InvalidSortByField));
                continue;
            }

            if (!pathCache.TryValidateFilterPropertyPath<TDbModel>(filter, name, out var message))
                errors.Add(new(FormatSortMessage<TDbModel>(name, message), Constants.ApiErrorCodes.InvalidSortByField));
        }
        ...
        where TDbModel : class
    {
        ...
        void AddFieldErrorIfInvalid(string field)
        {
            if (string.IsNullOrWhiteSpace(field)) {
                errors.Add(new($"Where clause condition is missing a field name for type '{typeof(TDbModel).Name}'.", Constants.ApiErrorCodes.InvalidWhereField));
                return;
            }

            if (!pathCache.TryValidateFilterPropertyPath<TDbModel>(filter, field, out var message))
                errors.Add(new(FormatWhereMessage<TDbModel>(field, message), Constants.ApiErrorCodes.InvalidWhereField));
        }
    }
    ...
    private static class Holder<TContext, TDbModel>
        where TContext : DbContext where TDbModel : class
    {
        internal static readonly IValidator<ProjectedQueryValidatorInput<TContext, TDbModel>> Instance = ValidatorBuilder<ProjectedQueryValidatorInput<TContext, TDbModel>>.Create()
            .Custom(Collect)
            .Build();
        ...
        private static IReadOnlyList<Error> Collect(ProjectedQueryValidatorInput<TContext, TDbModel> input)
        {
            ...
            if (input.Include.Count > 0) {
                if (input.Db is null)
                    errors.Add(new("Database context is required to validate include paths.", Constants.ApiErrorCodes.InvalidRequest));
                else {
                    foreach (var include in input.Include.Where(i => !string.IsNullOrWhiteSpace(i))) {
                        if (!pathCache.TryValidateInclude<TContext, TDbModel>(input.Loader, input.Db, include, out var includeMsg))
                            errors.Add(new(includeMsg!, Constants.ApiErrorCodes.InvalidInclude));
                    }
                }
            }
...
/// <summary>
/// Input to <see cref="ProjectedQueryModelValidator.Validate{TContext,TDbModel}" />. <see cref="Db" /> is required when <see cref="Include" /> is non-empty. QueryProject
/// passes includes derived from projection, not raw client includes.
/// </summary>
    ...
    where TContext : DbContext where TDbModel : class
{
    ...
    /// <summary>Per-request path validation cache (filter paths, select normalization, and includes).</summary>
