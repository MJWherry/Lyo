    ...
    where TContext : DbContext
{
        ...
        where TDbModel : class
    {
        ...
        await QueryCacheInvalidation.InvalidateQueryCachesForEntityKeysAsync(cache, cacheOptions, typeof(TDbModel), [typeConversion.GetPrimaryKeyValues(entity, context)], ct, Logger)
        ...
        where TDbModel : class
    {
        try {
            ...
            // Dictionary that stores old data and request, keyed by entity reference.
            ...
            foreach (var request in requests) {
                ...
                // Update every matching entity.
                foreach (var entity in entities) {
                    var oldData = MapOrCast<TDbModel, TResult>(Mapper, entity);
                    var entityKeys = typeConversion.GetPrimaryKeyValues(entity, context);

                    // Store old data in the dictionary BEFORE mapping.
            ...
            var preSaveStates = CapturePreSaveStates(context, oldDataMap.Keys);
            ...
            foreach (var entity in context.ChangeTracker.Entries<TDbModel>()) {
                ...
                if (oldDataMap.TryGetValue(entity.Entity, out var oldInfo)) {
                    ...
                    var s = ClassifyUpdateResult(preSaveStates.GetValueOrDefault(entity.Entity));
            ...
            if (updated > 0) {
                ...
                await QueryCacheInvalidation.InvalidateQueryCachesForEntityKeysAsync(cache, cacheOptions, typeof(TDbModel), keySets, ct, Logger).ConfigureAwait(false);
            }
        ...
        where TDbModel : class
    {
        ...
        try {
            ...
            if (entityMap.Count > 0) {
                var preSaveStates = CapturePreSaveStates(context, entityMap.Values.SelectMany(list => list.Select(item => item.Entity)));
                await context.SaveChangesAsync(ct);

                // Run after hooks and create results from pre-save EF state (SaveChanges sets Modified to Unchanged).
                foreach (var (mapIndex, entityList) in entityMap) {
                    try {
                        foreach (var (entity, oldData, entityKeys) in entityList) {
                            ...
                            var resultState = ClassifyUpdateResult(preSaveStates.GetValueOrDefault(entity));
        ...
        where TDbModel : class
    {
        try {
            ...
            // For an individual retry, just update the first entity.
            var entity = entities[0];
        ...
        where TDbModel : class
    {
        ...
        // Use Keys when they were provided.
        ...
        // Use Query when it was provided.
        ...
        where TDbModel : class
    {
        ...
        try {
            ...
            var preSaveState = context.Entry(entity).State;
            var written = await context.SaveChangesAsync(ct);
            after?.Invoke(ctx);
            var newData = MapOrCast<TDbModel, TResult>(Mapper, entity);
            return ResultFactory.UpdateSuccess(keys, oldData, newData, ClassifyUpdateResult(preSaveState, written));
        }
        catch (Exception ex) {
    ...
    /// <summary>
    /// Snapshots each entity's EF state after mapping and before SaveChanges. After a successful write EF sets Modified to Unchanged, so post-save State cannot classify Updated vs
    /// NoChange (and would skip query-cache invalidation).
    /// </summary>
    private static Dictionary<TDbModel, EntityState> CapturePreSaveStates<TDbModel>(TContext context, IEnumerable<TDbModel> entities)
        where TDbModel : class
    {
        context.ChangeTracker.DetectChanges();
        var states = new Dictionary<TDbModel, EntityState>();
        foreach (var entity in entities)
            states[entity] = context.Entry(entity).State;

        return states;
    }

    /// <summary>
    /// Classifies an Update from pre-save EF state (and SaveChanges row count as a fallback). Matching Patch: only skip invalidation when nothing was written.
    /// </summary>
    private static UpdateResultEnum ClassifyUpdateResult(EntityState preSaveState, int savedChanges = 0)
        => preSaveState switch {
            EntityState.Modified => UpdateResultEnum.Updated,
            EntityState.Unchanged when savedChanges > 0 => UpdateResultEnum.Updated,
