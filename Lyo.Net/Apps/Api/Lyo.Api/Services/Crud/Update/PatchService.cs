    ...
    where TContext : DbContext
{
        ...
        where TDbModel : class
    {
        ...
        try {
            ...
            // For a single entity patch, just patch the first one.
        ...
        where TDbModel : class
    {
        try {
            ...
            foreach (var request in requests) {
                ...
                // Patch every matching entity when AllowMultiple is true.
    ...
    {
        await using var context = await ContextFactory.CreateDbContextAsync(ct).ConfigureAwait(false);
        var entityMap = new Dictionary<int, List<(TDbModel Entity, PatchResult<TResult> Patch)>>();
        ...
        try {
            ...
            foreach (var request in requests) {
                try {
                    ...
                    var patchedEntities = new List<(TDbModel Entity, PatchResult<TResult> Patch)>();
                    foreach (var entity in entities) {
                        var patchResult = PatchEntityProperties<TDbModel, TResult>(entity, request);
                        patchedEntities.Add((entity, patchResult));
                    }
            ...
            if (entityMap.Count > 0) {
                ...
                foreach (var (mapIndex, entities) in entityMap) {
                    try {
                        var req = requests[mapIndex];
                        foreach (var (entity, patch) in entities) {
                            var ctx = new PatchContext<TDbModel, TContext>(req, entity, context, serviceProvider);
                            after?.Invoke(ctx);
                            if (patch.Result == PatchResultEnum.Updated) {
                                var mapped = MapOrCast<TDbModel, TResult>(Mapper, entity);
                                successes.Add(ResultFactory.PatchSuccess(patch.OldData!, mapped, patch.UpdatedProperties));
                            }
                            else
                                successes.Add(patch);
                        }
        ...
        where TDbModel : class
    {
        try {
            ...
            // For an individual retry, just patch the first entity.
            var entity = entities[0];
