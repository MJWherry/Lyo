...
/// <summary>Singleton limits for bulk CRUD operations: most items per request, parallelism, and overall timeout.</summary>
[DebuggerDisplay("{ToString(),nq}")]
public sealed class BulkOperationOptions
{
    /// <summary>Most concurrent tasks when <see cref="UseParallelProcessing" /> is enabled.</summary>
    ...
    /// <summary>Upper bound on time allowed for one bulk operation.</summary>
    public TimeSpan Timeout { get; init; } = TimeSpan.FromMinutes(5);

    /// <summary>Most items accepted in one bulk request body.</summary>
