...
/// <summary>EF navigation loading, include path checks, and referenced-type discovery for query caching and includes.</summary>
public interface IEntityLoaderService
{
    ...
    /// <summary>Returns distinct EF entity CLR types reachable from <paramref name="includes" /> (used for cache tags and invalidation).</summary>
    List<Type> GetReferencedTypes<TContext, TDbModel>(TContext context, IEnumerable<string> includes)
        where TContext : DbContext where TDbModel : class;

    /// <summary>Checks that all include paths are valid navigation paths. Throws <see cref="ArgumentException" /> for any invalid path.</summary>
