...
/// <summary>Root <see cref="QueryReq" />: arbitrary From/Joins (EF Join/GroupJoin on ON columns) plus sparse Select. Not navigation-based. That is <c>/QueryProject</c>.</summary>
    ...
    where TContext : DbContext
{
    ...
    private const int MaxPlanCacheEntries = 1024;

    // Shape keys come straight off the request body, so an unbounded dictionary here is a leak any caller can drive by varying its select list.
    private static readonly BoundedCache<string, RootQueryShapePlan> PlanCache = new(MaxPlanCacheEntries, StringComparer.Ordinal);
    ...
    public async Task<ProjectedQueryRes<object?>> QueryAsync(QueryReq request, RootQueryEntityRegistry registry, CancellationToken ct = default)
    {
        ...
        try {
            ...
            var cacheKey = QueryCacheKeyBuilder.AppendProjectedShapeSuffix(
                QueryCacheKeyBuilder.BuildRootQuery(request, typeof(TContext).FullName ?? typeof(TContext).Name), request.Options.ZipSiblingCollectionSelections);
            ...
            async Task<(ProjectedQueryRes<object?>? projected, string[]? tags)> BuildEntryAsync(CancellationToken token)
            {
                ...
                return (result, BuildTags(registry, plan, fromEntry, result));
            }
        ...
        CancellationToken ct)
    {
        ...
        fromSet = ApplyOuterWhereAndSort(fromSet, fromEntry.ClrType, request, PrimaryKeyNames(fromEntry, plan));
        ...
        // Fetch one item past the page so hasMore is an observation rather than the `items.Count >= amount` guess, which reports more on every exactly-full last page.
        var probeAmount = amount == int.MaxValue ? amount : amount + 1;
        var joinStart = Stopwatch.GetTimestamp();
        var projectedRows = await RootQueryJoinExecutor.ExecuteAsync(fromSet, fromEntry.ClrType, start, probeAmount, scopedJoins, plan, ct).ConfigureAwait(false);
        ...
        var probeFoundMore = items.Count > amount;
        if (probeFoundMore)
        ...
        var hasMore = request.Options.TotalCountMode == QueryTotalCountMode.Exact && total.HasValue ? start + items.Count < total.Value : probeFoundMore;
        return ResultFactory.ProjectedQuerySuccess<object?>(request, items, start, items.Count, total, hasMore, entityTypes: plan.EntityTypeNames);
    }
    ...
    private static List<object?> CollapseFanOutToFromItems(List<object?[]> rows, RootQueryShapePlan plan)
    {
        ...
        // Parent join alias for nesting: the join whose ON left is that alias.
        ...
        IReadOnlyDictionary<string, List<RootQueryJoinPlan>> childJoinsByParent)
    {
        ...
        // Deduplicate identical bags (the same contact repeated if somehow duplicated).
    ...
    /// <summary>Root query never writes, so entities are read untracked rather than depending on how the host configured the context's tracking behavior.</summary>
    private static IQueryable GetDbSetQueryable(DbContext context, Type clrType)
    {
        ...
        var noTracking = typeof(EntityFrameworkQueryableExtensions).GetMethods(BindingFlags.Public | BindingFlags.Static)
            .First(m => m.Name == nameof(EntityFrameworkQueryableExtensions.AsNoTracking) && m.GetParameters().Length == 1)
            .MakeGenericMethod(clrType);

        return (IQueryable)noTracking.Invoke(null, [set])!;
    }
    ...
    private IQueryable ApplyOuterWhereAndSort(IQueryable source, Type clrType, QueryReq request, IReadOnlyList<string> primaryKeyNames)
    {
        ...
        return (IQueryable)method.Invoke(this, [source, request, primaryKeyNames])!;
    }

    private IQueryable<TEntity> ApplyOuterWhereAndSortGeneric<TEntity>(IQueryable source, QueryReq request, IReadOnlyList<string> primaryKeyNames)
        where TEntity : class
    {
        var q = (IQueryable<TEntity>)source;
        ...
        foreach (var sort in request.SortBy.OrderBy(s => s.Priority ?? 0))
            q = filterService.SortByProperty(q, StripAliasPrefix(sort.PropertyName, request.From.Alias), sort.Direction ?? SortDirection.Desc);

        // Paging an unordered query is undefined, and paging a non-unique sort column duplicates rows across pages. The primary key closes both cases: it is the whole sort when
        // the caller supplied none, and the final tiebreaker when they did.
        foreach (var keyName in primaryKeyNames)
            q = filterService.SortByProperty(q, keyName, SortDirection.Asc);

        return q;
    }

    /// <summary>Primary key property names of the From entity, in key order. Falls back to the single key the shape plan resolved for fan-out collapse.</summary>
    private static IReadOnlyList<string> PrimaryKeyNames(RootQueryEntityEntry fromEntry, RootQueryShapePlan plan)
    {
        var names = fromEntry.PrimaryKey?.Properties.Select(p => p.PropertyInfo?.Name).Where(n => !string.IsNullOrEmpty(n)).Select(n => n!).ToArray();
        return names is { Length: > 0 } ? names : [plan.FromPrimaryKey.Name];
    }
    ...
    private static RootQueryShapePlan BuildPlan(QueryReq request, RootQueryEntityRegistry registry, RootQueryEntityEntry fromEntry)
    {
        ...
        foreach (var join in request.Joins) {
            ...
            var onPlans = join.On.Select(o => {
                    ...
                    // Normalize so Right is the join being added when that is possible.
    ...
    /// <summary>
    /// Tags a cached root-query page. Beyond the type tags for every entity in the shape, this emits an instance tag per returned From row when the projection carries that
    /// row's primary key. Granular invalidation after a single-row patch fires instance tags only, so a page carrying nothing but type tags survived the write and kept serving
    /// the pre-patch row.
    /// </summary>
    private static string[] BuildTags(RootQueryEntityRegistry registry, RootQueryShapePlan plan, RootQueryEntityEntry fromEntry, ProjectedQueryRes<object?>? result)
    {
        ...
        var primaryKeyName = plan.FromPrimaryKey.Name;
        foreach (var item in items) {
            if (item is not IDictionary<string, object?> row)
                continue;

            if (TryGetCaseInsensitive(row, primaryKeyName, out var keyValue) && keyValue is not null)
                tags.Add(QueryCacheTagBuilder.EntityInstanceTag(fromEntry.ClrType, [keyValue]));
        }

        return tags.ToArray();
    }

    private static bool TryGetCaseInsensitive(IDictionary<string, object?> row, string name, out object? value)
    {
