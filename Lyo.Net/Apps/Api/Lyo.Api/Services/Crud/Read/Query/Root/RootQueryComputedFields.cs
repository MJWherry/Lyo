...
internal static partial class RootQueryComputedFields
{
    ...
    /// <summary>Normalizes Mustache braces and appends missing <c>alias.property</c> Select paths named by templates.</summary>
        ...
        IFormatterService? formatter)
    {
        ...
        foreach (var item in items) {
            ...
            if (item is IDictionary nested)
                ...
                // A single From-side Select collapses to a scalar. Promote so computed keys can attach.
    ...
    private static void ApplyOne(Dictionary<string, object?> root, Dictionary<string, object?> flat, ComputedField field, RootQueryShapePlan plan, IFormatterService formatter)
    {
        ...
        if (!TryGetDeepestJoinAlias(placeholders, plan, out var joinAlias, out var resultName)) {
            // From-only (or constant placeholders that resolve as scalars): a root scalar.
        ...
        // Join-scoped: only on deepest join bags. Never the root array, and never "{alias}.{name}".
    ...
    {
        // Root-level join bags (the target is a root join).
        if (string.Equals(
        ...
        // Nested: find the root join that owns the chain, then zip into nested bags matching targetAlias.
        ...
        for (var bi = 0; bi < bags.Count; bi++) {
            ...
            foreach (var child in childJoins) {
                ...
                if (string.Equals(child.Alias, targetAlias, StringComparison.OrdinalIgnoreCase)) {
                    ...
                    // Re-zip with global index tracking is awkward. Flattening the path for nested depth>1 is rare for v1.
    ...
    private static void CollectProperty(List<object?> bags, string currentAlias, string targetAlias, string propertyName, RootQueryShapePlan plan, List<object?> column)
    {
        ...
        foreach (var bag in bags) {
            if (bag is not IDictionary d) {
                // Still need to fan-out nulls if the target is deeper. Skip for an empty bag.
                continue;
            }
