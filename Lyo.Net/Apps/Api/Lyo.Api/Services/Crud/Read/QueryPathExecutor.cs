...
/// <summary>Result of building a query path (key-constrained, subquery, or the standard path).</summary>
...
/// <summary>Builds the base queryable for a query (key-constrained, subquery, or the standard filter path).</summary>
...
/// <summary>Builds the base queryable for a query. Shared with QueryService.</summary>
    ...
    ILogger<QueryPathExecutor> logger) : IQueryPathExecutor
{
    ...
    {
        try {
            var sqlFirstQueryable = context.Set<TDbModel>().AsNoTracking();
        ...
        var baseQueryable = context.Set<TDbModel>().AsNoTracking();
        ...
        // The nested clause runs in memory, so phase 1 loads every row the outer filter matches. Probe one past the ceiling. A filter broad enough to blow past it would
        // otherwise pull an entire table into the process and take the host down with it.
        var rowCeiling = queryOptions.MaxSubQueryMaterializedRows;
        var phase1Results = await filteredQueryable.Take(rowCeiling + 1).ToListAsync(ct).ConfigureAwait(false);
        if (phase1Results.Count > rowCeiling) {
            throw new InvalidQueryException(
                $"This filter needs in-memory evaluation and matches more than {rowCeiling} rows before the nested clause is applied. Narrow the outer filter.");
        }
