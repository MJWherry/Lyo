...
/// <summary>Filter conditions taken from a WhereClause for MatchedOnly mode, with the group operator (And/Or) that combines them.</summary>
...
/// <summary>Resolves projection specs, builds projection expressions, and projects entities onto selected fields.</summary>
public interface IProjectionService
{
    /// <summary>Resolves and normalizes requested field paths for one entity type.</summary>
    ...
    /// <summary>Normalizes a field path (for example "contactAddresses.address.street") for a root type.</summary>
    string NormalizeFieldPath(Type rootType, string path);

    /// <summary>Returns include paths derived from projected field specs (for loading navigation properties needed by Select).</summary>
    ...
    /// <summary>Non-empty when any projected path is empty, wildcard-only, uses <c>count</c> incorrectly, or names an unknown segment.</summary>
    ...
    /// <summary>Same as the generic overload, using an explicit root CLR type (for example for non-generic callers).</summary>
    ...
    /// <summary>Projects entities onto selected fields in memory.</summary>
    ...
    /// <summary>Converts raw SQL projection results (tuple or anonymous) to dictionary format.</summary>
    ...
    /// <summary>Takes filter conditions from a WhereClause for projection-level filtering (MatchedOnly mode).</summary>
    ProjectedFilterConditions GetProjectedFilterConditions<TDbModel>(WhereClause? queryNode)
        where TDbModel : class;

    /// <summary>Applies MatchedOnly filtering to collection includes on entities (mutates them in place).</summary>
...
/// <summary>Resolved field spec used for projection.</summary>
public sealed record ProjectedFieldSpec(string RequestedPath, string NormalizedPath, string[] NormalizedParts);

/// <summary>Filter condition used for projection-level filtering.</summary>
