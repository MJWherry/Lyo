...
/// <summary>Finds SQL-level projected shapes that fan out relational rows (collection navigations), so callers can prefer split queries.</summary>
internal static class SqlProjectionJoinShape
{
    /// <summary>True when the projection likely causes one-to-many joins or more than one result row per root (reader fan-out).</summary>
    ...
    /// <summary>True when the path walks a collection navigation (same row multiplication as a join to a child table).</summary>
