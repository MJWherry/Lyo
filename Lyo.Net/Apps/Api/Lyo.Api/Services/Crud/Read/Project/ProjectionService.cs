...
{
    /// <summary>Bare dotted path used as a template without braces.</summary>
    ...
    {
        if (specs.Count == 1) {
            // Normalize collection results (for example List<string>) to List<object?> so they match in-memory projection.
    ...
    public void MergeSiblingCollectionProjectionRows(IReadOnlyList<object?> items, Type entityType, IReadOnlyList<ProjectedFieldSpec> specs, bool zipSiblingCollectionSelections)
    {
        ...
        // Shallower merge keys first (for example ContactAddresses before ContactAddresses.Address) so parallel columns
        ...
        where TDbModel : class
    {
        ...
        foreach (var condition in queryConditions) {
            try {
                ...
                // A dropped condition silently widens MatchedOnly. The collection keeps items the caller filtered out, and the response looks like the filter was ignored.
                logger?.LogWarning(
                    ex, "MatchedOnly filtering skipped condition on '{Field}' for {Entity}: the path does not resolve for projection", condition.Field, typeof(TDbModel).Name);
            }
        }
    ...
    /// <summary>Groups sibling collection fields into one slot so EF emits a single join plus Select.</summary>
    ...
    /// <summary>One <c>collection.Select(x =&gt; new {{ ... }})</c> for every sibling leaf (single join).</summary>
    ...
    /// <summary>Walks <paramref name="rootExpr" /> along <paramref name="prefixParts" /> and stops at the collection property (no Select).</summary>
    ...
    {
        // Single-slot projection: the body is one expression (for example a merged collection Select). No outer anonymous wrapper.
    ...
    private static Dictionary<string, object?> PromoteToDictionary(object item, IReadOnlyList<ProjectedFieldSpec> specs)
    {
        ...
        // Single-field projection: wrap the scalar value with the spec's requested path as the key.
    ...
    private string FormatComputedValue(string template, Dictionary<string, object?> row)
    {
        ...
        // The entire template is a bare dotted path (no braces).
        ...
        // The formatter bag resolves flat dotted keys (`Docket.Number`) and C# expression tokens on the same pass.
    ...
    /// <summary>Letters and digits only, lowercased. Aligns tokens like {lastname} with projected keys LastName.</summary>
    ...
    private static class ComparisonEvaluator
    {
        ...
        private static bool EvaluateScalar(object? actualValue, ComparisonOperatorEnum comparator, object? expectedValue)
        {
            ...
            return comparator switch {
                ...
                ComparisonOperatorEnum.GreaterThan => CompareOrdered(actualValue, expectedValue, static c => c > 0),
                ComparisonOperatorEnum.GreaterThanOrEqual => CompareOrdered(actualValue, expectedValue, static c => c >= 0),
                ComparisonOperatorEnum.LessThan => CompareOrdered(actualValue, expectedValue, static c => c < 0),
                ComparisonOperatorEnum.LessThanOrEqual => CompareOrdered(actualValue, expectedValue, static c => c <= 0),
                ComparisonOperatorEnum.Regex => MatchesRegex(actualText, expectedText),
                ComparisonOperatorEnum.NotRegex => !MatchesRegex(actualText, expectedText),
                // An unrecognized comparator cannot be shown to match. Returning true kept every collection item under MatchedOnly, which reads as "the filter did nothing".
                var _ => false
            };
        }

        /// <summary>Orders two values the way the database would where possible: compare in the actual value's own type, and fall back to text only when conversion fails.</summary>
        private static bool CompareOrdered(object? actualValue, object? expectedValue, Func<int, bool> accept)
        {
            ...
            if (actualValue is IComparable comparable) {
                try {
                    var converted = actualValue.GetType().IsInstanceOfType(expectedValue)
                        ? expectedValue
                        : Convert.ChangeType(expectedValue.ToString(), actualValue.GetType(), CultureInfo.InvariantCulture);

                    if (converted is not null)
                        return accept(comparable.CompareTo(converted));
                }
                catch (Exception ex) when (ex is InvalidCastException or FormatException or OverflowException or ArgumentException) {
                    // Not convertible to the actual value's type. Fall through to text ordering.
                }
            }

            var actualText = actualValue.ToString();
            var expectedText = expectedValue.ToString();
            return actualText != null && expectedText != null && accept(string.Compare(actualText, expectedText, StringComparison.OrdinalIgnoreCase));
        }

        private static bool MatchesRegex(string? actualText, string? pattern)
        {
            if (actualText is null || string.IsNullOrEmpty(pattern))
                return false;

            try {
                return Regex.IsMatch(actualText, pattern!, RegexOptions.None, LyoParameterValidator.RegexMatchTimeout);
            }
