    ...
    where TContext : DbContext
{
        ...
        where TDbModel : class
    {
        ...
        var includeList = includes?.AsReadOnlyList() ?? Array.Empty<string>();
        ...
        var includeProblem = TryIncludePathProblem<TContext, TDbModel>(context, includeList);
        ...
        if (result.IsSuccess)
            await QueryCacheInvalidation.InvalidateQueryCachesForEntityKeysAsync(cache, cacheOptions, typeof(TDbModel), [primaryKeyForCache], ct, Logger).ConfigureAwait(false);
        ...
        where TDbModel : class
    {
        ...
        // For a single delete, just delete the first entity.
        ...
        where TDbModel : class
    {
        try {
            ...
            foreach (var request in requests) {
                ...
                // Delete every matching entity when AllowMultiple is true.
            ...
            foreach (var _ in results.Where(r => r.IsSuccess))
                // After hooks run on successfully deleted entities. The entity is detached, so pass null.
            ...
            if (deleted > 0)
                await QueryCacheInvalidation.InvalidateQueryCachesForEntityKeysAsync(cache, cacheOptions, typeof(TDbModel), deletedKeySets, ct, Logger).ConfigureAwait(false);
        ...
        where TDbModel : class
    {
        try {
            ...
            // For an individual retry, just delete the first entity.
            var entity = entities[0];
            await loaderService.LoadIncludes(context, entity, includeList, ct);
            var keys = request.Keys?.FirstOrDefault() ?? [];
        ...
        where TContextModel : DbContext where TDbModel : class
    {
        ...
        var mat = includes.AsReadOnlyList();
        if (mat.Count == 0)
            return null;
        ...
        return LyoProblemDetailsBuilder.CreateWithActivity()
            ...
            .AddErrors(errs.Select(e => new ApiError(e.Code, e.Message)))
            .Build();
    }
