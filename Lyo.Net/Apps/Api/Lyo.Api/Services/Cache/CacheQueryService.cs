...
/// The admin query result is never written back into the cache.
/// </summary>
public sealed class CacheQueryService(ICacheService cache, IWhereClauseService filterService, IProjectionService projectionService, QueryOptions queryOptions)
{
    ...
    /// <summary>Known tag-index prefixes written into <see cref="ICacheService.Items" /> (<c>Lyo.Cache</c> local plus Fusion).</summary>
    internal static readonly string[] TagNamePrefixes = ["__fc:t:", "__tag:"];

    public ProjectedQueryRes<object?> QueryProjected(ProjectionQueryReq queryRequest, CancellationToken ct = default)
    {
    ...
    public async Task<CacheMutationRes> ClearAsync(CancellationToken ct = default)
    {
        ct.ThrowIfCancellationRequested();
    ...
    /// <summary>
    /// Same contract as <c>IDeleteService.DeleteAsync</c>: keys or where-clause. <see cref="CacheItemTypeEnum.Key" /> and <see cref="CacheItemTypeEnum.Tag" /> share this path.
    /// </summary>
    public async Task<DeleteResult<object?>> DeleteAsync(DeleteRequest request, CancellationToken ct = default)
    {
    ...
    /// <summary>Same contract as <c>IDeleteService.DeleteBulkAsync</c>. Each matched key or tag becomes one <see cref="DeleteResult{T}" /> row.</summary>
    public async Task<DeleteBulkResult<object?>> DeleteBulkAsync(IReadOnlyList<DeleteRequest>? requests, CancellationToken ct = default)
    {
    ...
    /// <summary>Strips Fusion or local tag-index prefixes so grid-selected <c>Name</c> values work with <see cref="ICacheService.InvalidateCacheItemByTag" />.</summary>
