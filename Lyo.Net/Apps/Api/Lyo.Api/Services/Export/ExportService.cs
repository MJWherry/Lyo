    ...
    where TContext : DbContext
{
        ...
        where TDbEntity : class
    {
        ...
        ArgumentHelpers.ThrowIfNull(request.Query);
        ...
        try {
            ...
            // Keyed export should page exactly to the key set. Oversized Amount trips include page-size checks.
    ...
    /// <summary>Export failed after an inner query error. The root summary becomes a transport-specific message while preserving <see cref="LyoProblemDetails.Errors" />.</summary>
    private static LyoProblemDetails AsExportFailure(LyoProblemDetails queryError)
    {
        if (queryError.Errors.Count <= 0) {
            return LyoProblemDetailsBuilder.CreateWithActivity()
                ...
                .WithMessage("Invalid export request.")
                .AddApiError(Constants.ApiErrorCodes.InvalidQuery, queryError.Detail)
                .Build();
        }
    ...
    /// <summary>Analyzes export columns to determine which are simple property lookups versus SmartFormat templates. Returns null when no columns are specified.</summary>
    ...
    private Dictionary<string, PropertyInfo>? ResolveColumns<T>(Dictionary<string, string>? columns)
    {
        ...
        foreach (var kv in columns) {
            var prop = responseType.FindProperty(kv.Value);
