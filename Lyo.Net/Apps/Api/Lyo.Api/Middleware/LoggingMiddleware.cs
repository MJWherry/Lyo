...
/// <see cref="ValidationException" />) and logs them at Warn. Unhandled exceptions are logged at Error. Writes <see cref="LyoProblemDetails" /> as <c>application/problem+json</c>.
...
{
    private static readonly string ProblemJsonContentType = FileTypeInfo.ProblemJson.MimeType;
    ...
    /// <summary>Invokes the next middleware and wraps logging plus exception handling.</summary>
    ...
    /// <summary>
    /// Lyo JWTs use <c>lyo:user</c> / <c>sub</c> (not <see cref="ClaimTypes.NameIdentifier" />). <c>email</c> and <c>name</c> are the JWT claim names, not the SOAP URIs.
    /// </summary>
