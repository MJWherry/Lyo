...
/// <summary>Cache payload round-trip for <see cref="ProjectedQueryRes{T}" /> has to deserialize polymorphic <c>QueryRequest</c>.</summary>
public sealed class ProjectedQueryResCacheSerializeTests
{
    [Fact]
    public void RoundTrip_RootQueryReq_ViaProjectedQueryRes()
    {
        var req = QueryReqBuilder.New().From("p", "Person").AddSelects("p.FirstName").SetPagination(0, 10).Build();
        ...
        Assert.IsType<QueryReq>(restored.QueryRequest);
        Assert.Equal("Person", ((QueryReq)restored.QueryRequest).From.EntityType);
    }
}