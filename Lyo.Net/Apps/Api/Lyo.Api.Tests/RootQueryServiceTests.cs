...
/// <summary>SQLite-backed execution checks for root <c>/Query</c> (joins, left-miss, fan-out collapse, From-side paging).</summary>
public sealed class RootQueryServiceTests : IAsyncDisposable
{
    ...
    {
        var req = BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 50).SetTotalCountMode(QueryTotalCountMode.Exact).Build();
    ...
    public async Task InnerJoin_ExcludesPeopleWithoutMatchingJoinRows()
    {
        ...
        var req = BasePersonContactAddressQuery(JoinType.Inner).SetPagination(0, 50).Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        ...
        var req = QueryReqBuilder.New()
            ...
            .Join(
                "a", nameof(AddressEntity), JoinType.Left, on => {
                    on.Add(new() { From = "c.AddressId", To = "a.Id" });
                }, "a")
    ...
    {
        var req = BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 50).AddComputedField("oo", "{{p.firstname}}{{c.startdate}}").Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
    ...
    {
        var req = QueryReqBuilder.New().From("p", nameof(PersonEntity)).AddSelects("p.FirstName").AddComputedField("label", "Hi {p.FirstName}").SetPagination(0, 50).Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        var lonely = FindByFirstName(res.Items!, "Lonely");
    ...
    {
        var req = BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 50).AddComputedField("oo", "{p.FirstName}{a.StreetName}").Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        var multi = FindByFirstName(res.Items!, "Multi");
        Assert.Null(GetValue(multi, "oo"));
        ...
        foreach (var c in contacts) {
            ...
            Assert.True(oo.Contains("Oak", StringComparison.OrdinalIgnoreCase) || oo.Contains("Pine", StringComparison.OrdinalIgnoreCase), $"Expected street in oo, got '{oo}'");
        }
    }
    ...
    {
        var req = BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 50).Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        Assert.Equal(3, res.Items!.Count);
        var multi = FindByFirstName(res.Items, "Multi");
        var contacts = GetJoinList(multi, "c");
        Assert.Equal(2, contacts.Count);
        var streets = contacts.Select(c => GetNestedBag(AsDict(c), "a"))
            .Where(a => a != null)
    ...
    {
        var req = BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 2).SetTotalCountMode(QueryTotalCountMode.Exact).Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        Assert.Equal(2, res.Items!.Count);
        Assert.Equal(3, res.Total);
    ...
    {
        var page0 = await _service.QueryAsync(BasePersonContactAddressQuery(JoinType.Left).SetPagination(0, 1).AddSort("p.FirstName", SortDirection.Asc).Build(), _registry, Ct);
        var page1 = await _service.QueryAsync(BasePersonContactAddressQuery(JoinType.Left).SetPagination(1, 1).AddSort("p.FirstName", SortDirection.Asc).Build(), _registry, Ct);
    ...
    {
        var req = QueryReqBuilder.New().From("p", nameof(PersonEntity)).AddSelects("p.FirstName").SetPagination(0, 10).Build();
        var res = await _service.QueryAsync(req, _registry, Ct);
        Assert.True(res.IsSuccess, res.Error?.Detail);
        Assert.Equal(3, res.Items!.Count);
    ...
    public async Task ShortEntityTypeNames_Resolve()
    {
        var req = QueryReqBuilder.New()
            .From("p", "Person")
            .Join("c", "ContactAddress", JoinType.Left, on => on.Add(new() { From = "p.Id", To = "c.PersonId" }), "c")
            .Join("a", "Address", JoinType.Left, on => on.Add(new() { From = "c.AddressId", To = "a.Id" }), "a")
            .AddSelects("p.FirstName", "c.StartDate", "a.StreetName")
            .SetPagination(0, 50)
            .Build();
    ...
    public async Task JoinQueryScope_Where_FiltersJoinSource()
    {
        ...
        var req = QueryReqBuilder.New()
            .From("p", nameof(PersonEntity))
            .Join(
                "c", nameof(ContactAddressEntity), JoinType.Left, on => on.Add(new() { From = "p.Id", To = "c.PersonId" }), "c", scope => {
