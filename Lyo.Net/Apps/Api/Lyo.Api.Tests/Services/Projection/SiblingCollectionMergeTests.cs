...
{
    /// <summary>Nested scope: collection to navigation to leaves zip when only one Select path exists but computed adds a parallel column.</summary>
    [Fact]
    public void MergeSiblingCollectionProjectionRows_Zips_OneSelectPlusParallelComputed_NestedPrefix()
    {
        ...
        var list = Assert.IsType<IList<Dictionary<string, object?>>>(mergedObj, false);
    ...
    /// <summary><c>contactaddresses.*</c> plus computed parallel <c>contactaddresses.address.n</c> becomes nested <c>Address.n</c> per row.</summary>
    [Fact]
    ...
    /// <summary>
    /// Mixed depths under the same collection (id on row + address.streetname) become one <c>ContactAddresses</c> array, not parallel <c>contactaddresses.id</c> +
    ...
    /// <summary>
    /// With <c>contactaddresses.*</c> plus explicit nested leaves, do not add a separate merge at <c>contactaddresses.address</c> (empty <c>ContactAddresses.Address</c> at
    ...
    public void MergeSiblingCollectionProjectionRows_ZipsParallelCollectionColumns()
    {
        ...
        var list = Assert.IsType<IList<Dictionary<string, object?>>>(merged, false);
        Assert.Equal(2, list.Count);
    ...
    public void MergeSiblingCollectionProjectionRows_DoesNotMerge_WhenOnlyOneSiblingField()
    {
        ...
        Assert.Empty(pathErrors);
        // Single-path ProjectEntities returns the raw value (not a dictionary row); use an explicit row to exercise merge behavior.
    ...
    public void StripAutoDerivedDependencyLeavesFromMergedCollections_OmitsTemplateOnlyColumnsFromZippedCharges()
    {
        ...
        var outRow = Assert.IsType<Dictionary<string, object?>>(afterComputed[0]);
        var list = Assert.IsType<IList<Dictionary<string, object?>>>(outRow["DocketCharges"], false);
        Assert.Equal(2, list.Count);
