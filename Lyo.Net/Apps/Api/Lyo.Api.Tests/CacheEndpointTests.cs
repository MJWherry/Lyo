...
public sealed class CacheEndpointTests
{
    ...
    public void QueryProjected_FiltersSortsAndPages_WithoutCaching()
    {
        ...
        // WhereClause compiles through ICacheService.GetOrSet; which is not the admin QueryProject payload.
    ...
    [Fact]
    public async Task MapCacheEndpoints_DefaultsToRequiringAuthorization()
    {
    ...
    private static async Task<WebApplication> StartAppAsync(EndpointAuth? auth = null)
    {
        var builder = WebApplication.CreateBuilder();
        builder.WebHost.UseTestServer();
        ...
        app.MapCacheEndpoints("Cache", auth: auth ?? EndpointAuth.Anonymous());
        await app.StartAsync();
        return app;
    }
}
