...
Optional export add-on for [Lyo.Api](../Lyo.Api/README.md). Registers the Export CRUD endpoint together with `IExportService<TContext>`.
...
Pass `ExportApiFeature.Instance` on endpoint builders to enable export. JSON export is already built into `Lyo.Api` through `AddBuiltInExportFormatHandlers()` (invoked from `AddLyoCrudServices`). CSV and XLSX each need their format addon package.
