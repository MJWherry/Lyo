...
public interface IWebRendererService
{
    ...
    /// <summary>
    /// Renders <paramref name="componentType" /> the same way as the generic overload. The type must implement <see cref="IComponent" />.
    /// </summary>
    Task<string> RenderToHtmlAsync(Type componentType, Dictionary<string, object>? parameterDictionary = null, CancellationToken ct = default);

    /// <summary>
    /// Resolves <paramref name="typeFullName" /> with <c>LyoTypeInfo.TryResolveClrType</c> and renders it. The type must implement <see cref="IComponent" />.
    /// </summary>
    Task<string> RenderToHtmlAsync(string typeFullName, Dictionary<string, object>? parameterDictionary = null, CancellationToken ct = default);

    string RenderToHtml<T>(Dictionary<string, object>? parameterDictionary = null)
        where T : IComponent;

    string RenderToHtml(Type componentType, Dictionary<string, object>? parameterDictionary = null);

    string RenderToHtml(string typeFullName, Dictionary<string, object>? parameterDictionary = null);

    Task<byte[]> RenderToHtmlBytesAsync<T>(Dictionary<string, object>? parameterDictionary = null, CancellationToken ct = default)
        where T : IComponent;

    Task<byte[]> RenderToHtmlBytesAsync(Type componentType, Dictionary<string, object>? parameterDictionary = null, CancellationToken ct = default);

    Task<byte[]> RenderToHtmlBytesAsync(string typeFullName, Dictionary<string, object>? parameterDictionary = null, CancellationToken ct = default);

    byte[] RenderToHtmlBytes<T>(Dictionary<string, object>? parameterDictionary = null)
        where T : IComponent;

    byte[] RenderToHtmlBytes(Type componentType, Dictionary<string, object>? parameterDictionary = null);

    byte[] RenderToHtmlBytes(string typeFullName, Dictionary<string, object>? parameterDictionary = null);
