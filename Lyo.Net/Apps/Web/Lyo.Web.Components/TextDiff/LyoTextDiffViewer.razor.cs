...
public partial class LyoTextDiffViewer : ComponentBase, IAsyncDisposable
{
    ...
    /// <summary>Bundles everything the live editor pane template draws for one side, so side-by-side and above/below share a single markup block.</summary>
    private LivePane PaneFor(bool isLeft, string positionClass)
        => isLeft
            ? new(
                positionClass, "Left", 0, _leftSearchText, _leftSearchCurrent, _leftSearchTotal, EventCallback.Factory.Create<string>(this, OnLeftSearchTextChangedAsync),
                EventCallback.Factory.Create<KeyboardEventArgs>(this, e => OnEditorSearchKeyDown(e, true)),
                EventCallback.Factory.Create<MouseEventArgs>(this, PreviousLeftSearch), EventCallback.Factory.Create<MouseEventArgs>(this, NextLeftSearch), LeftLineCount,
                _leftLineTypes, _leftPaneLines, _leftEditorText)
            : new(
                positionClass, "Right", 1, _rightSearchText, _rightSearchCurrent, _rightSearchTotal, EventCallback.Factory.Create<string>(this, OnRightSearchTextChangedAsync),
                EventCallback.Factory.Create<KeyboardEventArgs>(this, e => OnEditorSearchKeyDown(e, false)),
                EventCallback.Factory.Create<MouseEventArgs>(this, PreviousRightSearch), EventCallback.Factory.Create<MouseEventArgs>(this, NextRightSearch), RightLineCount,
                _rightLineTypes, _rightPaneLines, _rightEditorText);
    ...
    private sealed record LivePane(
        ...
        int SearchCurrent,
        int SearchTotal,
        EventCallback<string> SearchTextChanged,
        EventCallback<KeyboardEventArgs> SearchKeyDown,
        EventCallback<MouseEventArgs> PreviousSearch,
        EventCallback<MouseEventArgs> NextSearch,
        int LineCount,
        List<DiffLineType> LineTypes,
