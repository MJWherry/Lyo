    ...
    private static object[] ParseKeyInput(string input)
    {
        ...
        return trimmed.Split(',')
            .Select(part => {
                ...
                if (TypeConversion.TryConvertTo<long>(normalizedPart, out var longValue))
                    return longValue;

                return TypeConversion.TryConvertTo<decimal>(normalizedPart, out var decimalValue) ? decimalValue : (object)normalizedPart;
            })
            .ToArray();
    }
