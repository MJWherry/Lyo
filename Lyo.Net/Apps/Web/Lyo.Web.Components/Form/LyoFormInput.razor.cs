...
public partial class LyoFormInput<TModel, TValue>
{
    ...
    /// <summary>
    /// Visible line count for multiline string fields. If set, the input is treated as multiline even if <see cref="AllowMultiline" /> is false.
    ...
    /// <summary>
    /// Column width hint when hosted inside a <c>LyoFormGrid</c>. Default is <see cref="LyoFieldSize.Auto" />, which derives the width from the property type
    ...
    private RenderFragment RenderInput()
        => builder => {
            ...
            // If Items list is provided, draw as selection
    ...
    private void RenderFlagsEnumInput(RenderTreeBuilder builder, string label, bool isReadOnly)
    {
        if (isReadOnly) {
            ...
            // Activator - the button that displays current selection (MudBlazor 9: must explicitly wire OnClick)
            builder.AddAttribute(
