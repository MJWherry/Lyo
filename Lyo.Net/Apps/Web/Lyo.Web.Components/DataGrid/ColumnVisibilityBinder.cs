...
/// <summary>Follows which columns are hidden for the projected grid. Persisted in ClientStore.</summary>
public sealed class ColumnVisibilityBinder
{
    ...
    /// <param name="onChanged">Callback invoked when the set of hidden columns changes (for example to persist grid state).</param>
    ...
    /// <summary>
    /// Applies <see cref="LyoProjectedColumn.HiddenByDefault" /> when no grid state was loaded from the store (first visit). Call once after column registration (for example first
    /// draw).
    /// </summary>
