...
/// <summary>Constructs Dev-menu JSON dialog chips from the last grid query.</summary>
internal static class DataGridDevChips
{
    ...
    private static Color ForStatus(int code)
        => HttpStatusCodeInfo.FromCode(code).Category switch {
            HttpStatusCodeCategory.Success => Color.Success,
            HttpStatusCodeCategory.ClientError => Color.Warning,
            HttpStatusCodeCategory.ServerError => Color.Error,
            var _ => Color.Default
        };
}
