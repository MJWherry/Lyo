...
/// <summary>Formats projected timestamps for grid cells (UTC ISO to a given IANA zone, typically the browser).</summary>
public static class LyoDateTimeDisplay
{
    /// <summary>Local general date/time (<c>g</c>) in UTC when no zone is supplied. Prefer <see cref="LyoTimestamp" /> in the UI.</summary>
    public static string FormatProjected(object? _, object? value) => Format(value);

    /// <summary>Newest timestamp in a projected collection, formatted like <see cref="FormatProjected" />.</summary>
    public static string FormatLatestProjected(object? _, object? value) => Format(Latest(value));

    /// <summary>Formats <paramref name="value" /> using <see cref="TimeZoneInfo.Utc" />.</summary>
    public static string Format(object? value) => Format(value, TimeZoneInfo.Utc);

    /// <summary>Formats a UTC instant in <paramref name="timeZone" /> with culture general date/time (<c>g</c>).</summary>
    ...
    /// <summary>
    /// Picks the display string for <see cref="LyoTimestampKind" /> from a UTC instant. Relative kinds use <paramref name="relativeWindow" /> as a plus/minus bound
    /// (default 24 hours).
    /// </summary>
    ...
    /// <summary>Compact relative text when the absolute delta is inside <paramref name="window" />; otherwise returns false.</summary>
    ...
    /// standard/daylight name). Never uses the IANA id. UTC stays <c>UTC</c>; unknown names fall back to a UTC offset (<c>UTC-4</c>).
    /// </summary>
    ...
    /// <summary>Parses a projected value to UTC. Unspecified kind is read as UTC (API payloads).</summary>
    ...
    /// <summary>Most recent timestamp in a scalar, JSON array, or collection of projected values.</summary>
    ...
    /// <summary>Parses a projected JSON or CLR value to UTC <see cref="DateTime" />.</summary>
