...
public partial class LyoParameterOptionsSelect : ComponentBase, IAsyncDisposable
{
    ...
    /// <summary>POST route that resolves Sproc (and optionally Query) Options into picker rows. Defaults to <c>Reporting/ResolveParameterOptions</c>.</summary>
    [Parameter]
    public string ResolveOptionsRoute { get; set; } = "Reporting/ResolveParameterOptions";

    /// <summary>Stored type name. When the catalog editor kind is <see cref="LyoTypeEditorKind.Formatter" />, custom values are kept (templates are free text).</summary>
    [Parameter]
    ...
    /// <summary>Wire shape for multi <see cref="Value" /> JSON array elements (default is strings for option keys).</summary>
    [Parameter]
    ...
    {
        IEnumerable<string> keys = _items.Select(i => i.Key);
        if (!string.IsNullOrWhiteSpace(value)) {
            keys = _items
                .Where(i => i.Key.Contains(value, StringComparison.OrdinalIgnoreCase) || i.Label.Contains(value, StringComparison.OrdinalIgnoreCase))
                .Select(i => i.Key);
            if (keys.All(k => !string.Equals(k, value, StringComparison.Ordinal)))
                keys = keys.Prepend(value);
        }
            ...
            if (ParameterOptionsJson.TryDeserialize(OptionsJson, out var options) && options is not null) {
                switch (options.Kind) {
                    ...
                    case ParameterOptionsKind.Sproc:
                        await LoadResolvedOptionsAsync(options, ct);
                        break;
                    default:
    ...
    private async Task LoadResolvedOptionsAsync(ParameterOptions options, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(ResolveOptionsRoute)) {
            _helper = "Sproc options require a resolve endpoint.";
            return;
        }

        var siblings = SiblingValues ?? new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase);
        var req = new ParameterOptionsResolveReq { OptionsJson = ParameterOptionsJson.Serialize(options), SiblingValues = siblings.ToDictionary(kv => kv.Key, kv => kv.Value, StringComparer.OrdinalIgnoreCase) };
        var res = await ApiClient.PostAsAsync<ParameterOptionsResolveReq, ParameterOptionsResolveRes>(ResolveOptionsRoute, req, ct: ct);
        if (res is null) {
            _helper = "Options resolve failed.";
            return;
        }
        ...
        _items.AddRange(res.Items.Where(i => !string.IsNullOrEmpty(i.Key)));
        if (_items.Count == 0)
            _helper = "No options returned.";
    }

    private bool IsFormatterType => LyoTypeInfo.FromName(TypeName).EditorKind == LyoTypeEditorKind.Formatter;
    ...
    private void PruneStaleSelection()
    {
        ...
        if (IsFormatterType && !AllowMultiple)
            return;
