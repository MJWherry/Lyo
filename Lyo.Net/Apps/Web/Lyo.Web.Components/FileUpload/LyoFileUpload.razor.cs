...
public partial class LyoFileUpload : IDisposable
{
    ...
    /// <summary>
    /// Optional CSS <c>max-width</c> for file status chips (for example <c>min(100%, 12rem)</c> or <c>100%</c>). Keeps chips inside narrow columns; combine with
    /// <see cref="ChipFileNameMaxLength" />.
    /// </summary>
    [Parameter]
    public string? ChipMaxWidthCss { get; set; }

    private string? ChipContainerStyle => string.IsNullOrWhiteSpace(ChipMaxWidthCss) ? null : $"max-width:{ChipMaxWidthCss.Trim()};overflow:hidden;";

    /// <summary>
    /// <see cref="ClientFileDisplayMode.Chips" /> displays per-file chips (subject to <see cref="ShowFiles" />). <see cref="ClientFileDisplayMode.ExternalList" /> hides all chip
    ...
    /// <summary>
    /// If set, the component streams browser files to temp disk instead of buffering in memory. The <see cref="OnClientFilePathReady" /> /
    ...
    /// <summary>Shortens long file names while keeping the extension visible (for example <c>my_long_name...jpg</c>).</summary>
