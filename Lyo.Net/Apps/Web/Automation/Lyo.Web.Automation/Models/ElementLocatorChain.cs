...
/// <summary>
/// Ordered locator path from the current browsing context (page or iframe) to a target element: each segment is resolved relative to the previous one (Selenium uses nested
/// <c>FindElement</c>; Playwright uses chained <c>ILocator</c>).
/// </summary>
[DebuggerDisplay("{ToString(),nq}")]
public sealed class ElementLocatorChain
{
    /// <summary>Locator segments from outermost (inside the current document / frame) to innermost.</summary>
    ...
    /// <summary>Code convenience constructor: <c>new ElementLocatorChain(a, b, c)</c>.</summary>
    ...
    /// <summary>Wraps one segment (same as <see cref="ElementLocator.Then" /> for the first step).</summary>
    ...
    /// <summary>Appends a segment (fluent form: <c>loc.Then(...).Then(...)</c>).</summary>
