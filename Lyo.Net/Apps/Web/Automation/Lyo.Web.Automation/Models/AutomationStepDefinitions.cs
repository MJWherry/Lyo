...
[JsonDerivedType(typeof(RandomizeViewportAutomationStep), "randomizeViewport")]
...
/// ). This is the explicit resize step; for a size drawn from a pool use <see cref="RandomizeViewportAutomationStep" />.
/// </summary>
[DebuggerDisplay("{ToString(),nq}")]
...
/// <summary>
/// Picks a width×height from <see cref="Sizes" /> (or <see cref="DesktopDisplaySize.Common" />) with optional pixel jitter, then applies it like
/// <see cref="SetViewportSizeAutomationStep" />. Use after navigate (optionally after <see cref="DelayAutomationStep" />) to mimic a user resizing the window.
/// </summary>
[DebuggerDisplay("{ToString(),nq}")]
public sealed record RandomizeViewportAutomationStep(
    IReadOnlyList<DesktopDisplaySize>? Sizes = null,
    int JitterPixels = 0,
    string? Name = null)
    : AutomationStepDefinition(Name)
{
    ...
    {
        var n = Sizes?.Count ?? 0;
        return FormatStepDebugLine(n == 0 ? $"randomizeViewport jitter={JitterPixels}" : $"randomizeViewport jitter={JitterPixels} sizes={n}");
    }
}
...
/// <summary>Converts every element in a list ref to attribute or text; results stored as a string list variable.</summary>
[DebuggerDisplay("{ToString(),nq}")]
...
/// <summary>Converts each item in a source string-list variable through a template and stores the result list.</summary>
[DebuggerDisplay("{ToString(),nq}")]
...
/// <summary>Sends all files from a local directory through the configured runtime file storage abstraction.</summary>
[DebuggerDisplay("{ToString(),nq}")]
