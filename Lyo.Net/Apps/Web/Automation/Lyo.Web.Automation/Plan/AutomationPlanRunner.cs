...
public sealed class AutomationPlanRunner : IAutomationPlanRunner
{
    ...
    /// <summary>Runs the plan and yields final variables plus per-step <see cref="AutomationPlanExecutionContext" />.</summary>
    ...
    /// <summary>Re-applies run/step correlation on loggers when child work may not inherit ambient scopes (for example thread pool).</summary>
        ...
        CancellationToken ct)
    {
        ...
        switch (step) {
            ...
            case SetViewportSizeAutomationStep sv:
                ...
                return;
            case RandomizeViewportAutomationStep rv:
                ArgumentHelpers.ThrowIf(rv.JitterPixels < 0, "Jitter must be non-negative.", nameof(rv.JitterPixels));
                var picked = DesktopDisplaySize.Pick(rv.Sizes, rv.JitterPixels);
                await browser.SetViewportSizeAsync(picked.Width, picked.Height, ct).ConfigureAwait(false);
                return;
