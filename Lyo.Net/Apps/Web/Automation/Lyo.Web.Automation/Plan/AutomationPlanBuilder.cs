...
public sealed class AutomationPlanBuilder
{
    ...
    /// <summary>Constructs the plan with an immutable step list and a time-ordered <see cref="AutomationStepDefinition.StepId" /> for any step that did not set one.</summary>
    ...
    /// <summary>Sets viewport/window size for the active tab (Playwright: layout viewport; Selenium: window size). Same as <see cref="ResizeWindow" />.</summary>
    ...
    /// <summary>Explicit mid-plan resize; equivalent to <see cref="SetViewportSize" />.</summary>
    public AutomationPlanBuilder ResizeWindow(int width, int height, string? stepName = null) => SetViewportSize(width, height, stepName);

    /// <summary>
    /// Picks a desktop size from <paramref name="sizes" /> (or <see cref="DesktopDisplaySize.Common" />) with optional ±<paramref name="jitterPixels" />, then resizes.
    /// Pair with <see cref="Delay" /> if the site should see a pause before the resize.
    /// </summary>
    public AutomationPlanBuilder RandomizeViewport(IReadOnlyList<DesktopDisplaySize>? sizes = null, int jitterPixels = 0, string? stepName = null)
    {
        ArgumentHelpers.ThrowIf(jitterPixels < 0, "Jitter must be non-negative.", nameof(jitterPixels));
        if (sizes != null) {
            foreach (var size in sizes) {
                ArgumentHelpers.ThrowIf(size.Width <= 0, "Width must be positive.", nameof(sizes));
                ArgumentHelpers.ThrowIf(size.Height <= 0, "Height must be positive.", nameof(sizes));
            }
        }

        _steps.Add(new RandomizeViewportAutomationStep(sizes, jitterPixels, stepName));
        return this;
    }
    ...
    /// <summary>Polls for a descendant of an existing element ref (see <see cref="FindDescendantAutomationStep" />). Locator <c>Value</c> may contain <c>{{var}}</c>
    ...
    /// <summary>Converts every element in a list ref to text or attribute values; stored as a string list variable.</summary>
    ...
    /// <summary>Constructs a string from a template using <c>{{var}}</c> placeholders, then stores it.</summary>
    ...
    /// <summary>Converts every item in a source string-list variable through a template and stores the resulting list.</summary>
    ...
    /// <param name="urlListFromCompletedStepIndex">If set, use the list as it was after this zero-based step; when null, use the variable’s final value.</param>
    ...
    /// <summary>Sends all files from a directory through the configured runtime file storage service.</summary>
