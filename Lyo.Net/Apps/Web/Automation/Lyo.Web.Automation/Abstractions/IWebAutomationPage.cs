...
{
    /// <summary>
    /// Polls until the first matching element is visible, then yields it. Pass a single <see cref="ElementLocator" /> (implicitly a one-segment chain) or a path from
    ...
    /// <summary>Polls until at least one match is visible, then yields every match for the final segment (nested under the resolved parent path when multiple segments).</summary>
    Task<IReadOnlyList<IWebAutomationElement>> PollForElementsAsync(ElementLocatorChain chain, CancellationToken ct = default);

    /// <summary>
    /// Resolves the chain once (one bounded Selenium/Playwright wait per segment, no outer <c>PollingMaxAttempts</c> retries). Yields <see langword="null" /> when nothing
    ...
    /// <summary>
    /// Resolves the chain once and yields every match for the final segment (no outer polling retries). Returns <see langword="null" /> when the chain cannot be resolved within
    ...
    /// <summary>Yields the full HTML source of the current document.</summary>
    ...
    /// <summary>
    /// Sets width and height for the active browsing surface. Playwright converts this to the page layout viewport (<c>SetViewportSizeAsync</c>). Selenium converts it to the OS
    /// browser window size (<c>Manage().Window.Size</c>), which is not identical to the CSS layout viewport. Prefer Playwright when precise viewport semantics matter.
    /// </summary>
