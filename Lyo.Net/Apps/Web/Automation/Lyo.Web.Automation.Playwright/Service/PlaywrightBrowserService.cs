...
public sealed class PlaywrightBrowserService : IPlaywrightBrowserService
{
    ...
    {
        ThrowIfDisposed();
        return CreateSessionCore(sessionOptions?.Clone() ?? _serviceOptions.Clone());
    }

    /// <inheritdoc />
    public IPlaywrightBrowserSession CreateSession(Action<PlaywrightBrowserOptions> configure)
    {
        ArgumentHelpers.ThrowIfNull(configure);
        ThrowIfDisposed();
        var effective = _serviceOptions.Clone();
        configure(effective);
        return CreateSessionCore(effective);
    }

    private IPlaywrightBrowserSession CreateSessionCore(PlaywrightBrowserOptions effective)
    {
