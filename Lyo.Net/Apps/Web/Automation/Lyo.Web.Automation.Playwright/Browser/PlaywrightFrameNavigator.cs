...
public sealed class PlaywrightFrameNavigator
{
    ...
    public async Task SwitchToFrameAsync(ElementLocator locator, CancellationToken ct = default)
    {
        ...
        await RunFrameOpAsync(
                "switch_frame", async () => {
                    ...
                    await page.Locator(sel).WaitForAsync(new() { State = WaitForSelectorState.Attached, Timeout = _browser.Options.LocatorDefaultTimeoutMs }).ConfigureAwait(false);
    ...
    public void SwitchToFrame(ElementLocator locator) => SwitchToFrameAsync(locator, CancellationToken.None).GetAwaiter().GetResult();
    ...
    /// <summary>Yields to the top-level document.</summary>
    public void SwitchToDefaultContent() => RunFrameOp("default_content", () => _iframeSelectors.Clear());

    /// <summary>Enters a frame and yields a scope that calls <see cref="SwitchToParentFrame" /> on dispose.</summary>
    ...
    public PlaywrightFrameScope EnterFrame(ElementLocator locator) => EnterFrameAsync(locator, CancellationToken.None).GetAwaiter().GetResult();
    ...
    internal ILocator ResolveLocator(ElementLocator elementLocator)
    {
        ...
        var cur = _browser.GetRequiredPage().FrameLocator(_iframeSelectors[0]);
    ...
    private void RunFrameOp(string operation, Action action)
    {
        ...
        try {
            ...
            _browser.Metrics.IncrementCounter(
                _browser.ResolveMetric(nameof(Wm.Metrics.FrameOperation)), tags: PlaywrightMetricTags.ForOperation(_browser, operation, new[] { ("result", "success") }));

            log.LogDebug("Frame operation {Operation} completed", operation);
        }
        catch (Exception ex) {
            _browser.Metrics.IncrementCounter(
                _browser.ResolveMetric(nameof(Wm.Metrics.FrameOperation)), tags: PlaywrightMetricTags.ForOperation(_browser, operation, new[] { ("result", "failure") }));
...
public sealed class PlaywrightFrameScope : IDisposable
{
    ...
    internal PlaywrightFrameScope(PlaywrightFrameNavigator navigator) => _navigator = navigator;
