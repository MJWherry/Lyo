...
/// <summary>Converts <see cref="ElementLocator" /> to a CSS selector for an <c>iframe</c> element (for <see cref="Microsoft.Playwright.IPage.FrameLocator" />).</summary>
public static class PlaywrightFrameSelectors
{
    /// <summary>Constructs a selector string for the iframe element that hosts the frame.</summary>
    public static string ToIframeSelector(ElementLocator locator)
    {
        ...
        return locator.Kind switch {
            ...
            ElementLocatorKind.CssSelector => locator.Value.StartsWith("iframe", StringComparison.OrdinalIgnoreCase) ? locator.Value : $"iframe {locator.Value}",
            ...
            ElementLocatorKind.TagName => locator.Value.Equals("iframe", StringComparison.OrdinalIgnoreCase) ? "iframe" : $"iframe {locator.Value}",
            ElementLocatorKind.XPath => throw new NotSupportedException("XPath frame selection is not supported; use CssSelector or Id."),
            ElementLocatorKind.LinkText or ElementLocatorKind.PartialLinkText =>
                throw new NotSupportedException("Link text frame selection is not supported for iframe targeting."),
    ...
    private static string CssEscapeClass(string value) => value.Replace("\\", "\\\\").Replace("\"", "\\\"");
}