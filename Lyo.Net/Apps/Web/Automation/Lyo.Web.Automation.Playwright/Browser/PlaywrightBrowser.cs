...
public sealed class PlaywrightBrowser : IWebAutomationBrowser, IDisposable, IAsyncDisposable
{
    ...
    /// <summary>Correlation id when created from <see cref="IPlaywrightBrowserService.CreateSession(PlaywrightSessionOptions?)" />; otherwise <see cref="Guid.Empty" />.</summary>
    public Guid SessionId => ExecutionContext?.SessionId ?? Guid.Empty;

    /// <summary>Session-scoped paths when created from <see cref="IPlaywrightBrowserService.CreateSession(PlaywrightSessionOptions?)" />.</summary>
    ...
    /// <summary>Dialog utility methods.</summary>
        ...
        IMetrics? metrics = null)
    {
        ...
        Logger = ExecutionContext.BuildBrowserLogger(baseLogger);
    ...
    async Task IWebAutomationNavigator.ReloadAsync(CancellationToken ct)
    {
        ...
        await Page!.ReloadAsync(new() { WaitUntil = Options.NavigationWaitUntil }).ConfigureAwait(false);
    ...
    async Task IWebAutomationPage.SetViewportSizeAsync(int width, int height, CancellationToken ct)
    {
        ...
        if (Options.ScreenWidth < width)
            Options.ScreenWidth = width;
        if (Options.ScreenHeight < height)
    ...
    async Task IWebAutomationNavigator.NavigateAsync(string url, Func<string, bool> onRequest, CancellationToken ct)
    {
        ...
        Page!.Request += Handle;
        try {
            await Page.GotoAsync(url, new() { WaitUntil = Options.NavigationWaitUntil }).ConfigureAwait(false);
            var deadline = DateTime.UtcNow + TimeSpan.FromMilliseconds(Options.NavigationTimeoutMs);
            while (!Volatile.Read(ref done)) {
                ct.ThrowIfCancellationRequested();
                if (DateTime.UtcNow >= deadline)
                    throw new PlaywrightException("onRequest did not match any request URL within the configured navigation timeout.");
    ...
    public async Task StartBrowserAsync(CancellationToken ct = default)
    {
        ...
        var launchOptions = new BrowserTypeLaunchOptions {
            ...
            Channel = Options.BrowserKind == PlaywrightBrowserKind.Chromium ? Options.Channel : null,
            Args = Options.ResolveLaunchArguments(),
            SlowMo = Options.SlowMoMilliseconds,
            DownloadsPath = ExecutionContext?.DownloadDirectory
        };

        _browser = await browserType.LaunchAsync(launchOptions).ConfigureAwait(false);
        var contextOptions = new BrowserNewContextOptions {
            ViewportSize = new() { Width = Options.ViewportWidth, Height = Options.ViewportHeight },
            ScreenSize = new() { Width = Options.ScreenWidth, Height = Options.ScreenHeight },
            IgnoreHTTPSErrors = Options.IgnoreHttpsErrors,
            AcceptDownloads = true
        };

        var userAgent = Options.ResolveContextUserAgent();
        if (userAgent != null)
        ...
        Context.SetDefaultTimeout(Options.LocatorDefaultTimeoutMs);
    ...
    /// <summary>One locator wait; no outer polling retries. Yields <see langword="null" /> if not found.</summary>
    ...
    /// <summary>Resolves a chained path once (first segment respects the current frame stack via <see cref="PlaywrightFrameNavigator" />). Yields <see langword="null" /> if not
    /// found.</summary>
    ...
    /// <summary>Resolves the chain once and yields every match for the final segment. Returns <see langword="null" /> if not found.</summary>
    ...
    /// <summary>Waits until at least one match is visible, then yields every matching element for the final segment; each attempt runs <see cref="GetElementsChainCoreAsync"
    /// />.</summary>
