...
/// <summary>JavaScript dialog utility methods (<c>alert</c>, <c>confirm</c>, <c>prompt</c>) via Playwright <see cref="IPage.Dialog" />.</summary>
public sealed class PlaywrightDialogs
{
    ...
    /// <summary>Adds a handler that auto-accepts every dialog until disposed.</summary>
    ...
    /// <summary>Adds a one-shot handler that accepts the next dialog, then unsubscribes.</summary>
    ...
    /// <summary>Waits for the next dialog and returns its message (blocks until a dialog is displayed).</summary>
