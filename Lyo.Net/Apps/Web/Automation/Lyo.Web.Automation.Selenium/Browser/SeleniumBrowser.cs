...
public class SeleniumBrowser : IDisposable, IWebAutomationBrowser
{
    ...
    /// <summary>JavaScript alert / confirm / prompt utility methods.</summary>
    ...
    /// <param name="urlOverride">If set, used for <c>url</c> instead of the current document URL.</param>
    ...
    /// <summary>Throws if the browser is not started; yields the active WebDriver.</summary>
    ...
    /// <param name="by">Locator (for example By.Id, By.Name, By.XPath).</param>
    /// <param name="ct">Cancellation token.</param>
    /// <yields>The element if found, or null if cancelled.</yields>
    ...
    /// <see cref="SeleniumBrowserOptions.PollingMaxAttempts" />. Yields <see langword="null" /> if not found.
    /// </summary>
    ...
    /// <summary>One nested wait per <see cref="SeleniumBrowserOptions.SeleniumMaxWaitSeconds" />; no outer polling retries. Yields <see langword="null" /> if not found.</summary>
    ...
    /// <summary>Resolves a locator chain once (respects frame context if switched). Yields <see langword="null" /> if not found.</summary>
    ...
    /// <summary>Single-attempt chain resolution (one bounded wait per segment). Yields <see langword="null" /> if any segment is not found.</summary>
    ...
    /// <summary>Waits until at least one match exists, then yields every matching element for the final segment; each attempt runs <see cref="GetElementsChainCoreAsync"
    /// />.</summary>
    ...
    /// <summary>Single-attempt multi-match resolution (one bounded wait per segment). Yields <see langword="null" /> if the chain cannot be resolved.</summary>
