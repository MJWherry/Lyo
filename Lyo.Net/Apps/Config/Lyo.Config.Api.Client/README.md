...
`AddConfigApiClientFromConfiguration` (defined in [`ConfigApiClient.cs`](./ConfigApiClient.cs)):

1. Binds `ConfigApiClientOptions` from the section named by `configSectionName` (default **`"ConfigApi"`** via `ConfigApiClientOptions.SectionName`); the section is optional.
2. Registers `IOptions<ConfigApiClientOptions>` (only when not already present).
3. Calls `services.AddHttpClient<IConfigApiClient, ConfigApiClient>(...)` and returns the `IHttpClientBuilder` so callers can chain `.AddHttpMessageHandler<…>()` /
   `.AddPolicyHandler(…)` or replace the primary handler.
4. Wires the typed client:
...
### `EnsureStatusCode` behavior in detail
...
- **`HTTP 2xx`** → `ConfigResolveConditionalResult { Outcome = Ok, ETag, Resolved }`. If the server returns `204 No Content` or the client used `headOnly = true`, `Resolved` is
  `null`.
- **`HTTP 304 Not Modified`** → `ConfigResolveConditionalResult { Outcome = NotModified, ETag = <server etag or ifNoneMatch fallback>, Resolved = null }`. **Always returned —
  even when `EnsureStatusCode == true`**, because `304` is the expected polling response.
- **`HTTP 4xx/5xx`**:
    ...
    - When `EnsureStatusCode == false`: the method returns `ConfigResolveConditionalResult { Outcome = Failed, ETag, Resolved = null, Failure = (StatusCode, ReasonPhrase) }`
      so callers can branch on the status code without using exceptions for control flow. Use this mode when, for example, you want to treat `409 Conflict` (validation
      failure on the server) as a recoverable signal rather than a crash.

### Slug validation

`appKind` and `appId` are checked with `AppConfigEntity.TryCreate(...)` **before** any HTTP request. Invalid slugs throw `ArgumentException` with the same message
the API would return as `400 Bad Request`. There is no separate per-method validation toggle.
...
- [`Lyo.Config`](../../../Features/Config/Lyo.Config/README.md)
- [`Lyo.Config.Api`](../Lyo.Config.Api/README.md)
- [`Lyo.Config.Api.Hosting`](../Lyo.Config.Api.Hosting/README.md)
- [`Lyo.Config.Api.Models`](../Lyo.Config.Api.Models/README.md)
