...
HTTP client typed for [`Lyo.Config.Api`](../Lyo.Config.Api/README.md). Conditional app-config reads with `?version` / `If-None-Match` polling, an optional `X-Api-Key` header, and DI extensions. `IConfigApiClient` covers the resolve route (`/api/config/{appKind}/{appId}`). `ConfigApiStore` implements `IConfigStore` over `/api/config/manage/...` so any language can hit the same routes; C# workbenches (TestGateway) register `AddConfigApiStore()` and send plaintext plus `IsEncrypted`. The API encrypts at rest.

References [`Lyo.Config.Api.Models`](../Lyo.Config.Api.Models/README.md) for `ConfigResolveOutcome` / `ConfigResolveConditionalResult`, and `Lyo.Config` for `AppConfigEntity` slug validation and `ResolvedConfigRecord`. For a polling host that publishes config via `IOptionsMonitor<T>`, see [`Lyo.Config.Api.Hosting`](../Lyo.Config.Api.Hosting/README.md).
...
- Registers `IOptions<ConfigApiClientOptions>` and `ConfigApiClientOptions` (only when not already present).
- Calls `services.AddHttpClient<IConfigApiClient, ConfigApiClient>(...)` and returns the `IHttpClientBuilder` so callers can chain `.AddPolicyHandler(…)` / `.AddHttpMessageHandler<…>()` or replace the primary handler.
...
- Appends `deflate` / `gzip` / `br` to `Accept-Encoding` based on `AcceptEncodings`.
...
- `AddConfigApiStore()` registers scoped `IConfigStore` → `ConfigApiStore` over the host `IApiClient`. Clients send plaintext plus `IsEncrypted`; the API encrypts.
...
Section: `ConfigApi` by default. Subclasses `Lyo.Api.Client.ApiClientOptions`.
...
Because the interface inherits `Lyo.Api.Client.IApiClient`, callers get the generic CRUD / Query helpers exposed by `ApiClient` on the same instance. This client only adds the
resolve method. Operator writes use `ConfigApiStore` (`AddConfigApiStore()`), which implements `IConfigStore` over `/api/config/manage`.
...
`AppConfigEntity.TryCreate(...)` checks `appKind` and `appId` **before** any HTTP request. Invalid slugs throw `ArgumentException` with the same message the API would return as `400 Bad Request`. There is no separate per-method validation toggle.

## `ConfigPolling.PollUntilChangedAsync`

Long-running pollers that simply want the next changed `ResolvedConfigRecord` can use this helper:
...
A server `Ok` with no body is treated as a contract bug by `OperationHelpers.ThrowIfNull`.
