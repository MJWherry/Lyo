...
Typed HTTP client for [`Lyo.Config.Api`](../Lyo.Config.Api/README.md). Conditional app-config reads with `If-None-Match` / `?version` polling, an optional `X-Api-Key` header, and one DI extension. The client exposes only the resolve route (`/api/config/{appKind}/{appId}`). The management API ( `/manage/...`) is for operator tools, not service callers.
