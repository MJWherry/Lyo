...
Thin Web host that calls `AddConfigApi` and `MapConfigApiEndpoints` from [`Lyo.Config.Api`](../Lyo.Config.Api/README.md). Embed those extensions in another API (for example Gateway) instead of running this host when composing surfaces.
