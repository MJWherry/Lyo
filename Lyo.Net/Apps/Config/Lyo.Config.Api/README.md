...
`Lyo.Config.Api` can run standalone (see [`Lyo.Config.Api.Host`](../Lyo.Config.Api.Host/README.md)) or be embedded into another host that already owns the WebApplication pipeline. Registration is two extensions plus one middleware, all in `Lyo.Config.Api`:
