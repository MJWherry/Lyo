...
Connects `IConfigApiClient` ([`Lyo.Config.Api.Client`](../Lyo.Config.Api.Client/README.md)) to `Microsoft.Extensions.Options` and `Microsoft.Extensions.DependencyInjection`. A `BackgroundService` keeps a shared `ResolvedConfigRecord` ledger (`304` polling + ETags). Then one definition-key JSON blob binds each `IOptionsMonitor<TOptions>`.
...
From your worker/API host, reference the project `Lyo.Config.Api.Hosting` (`Microsoft.Extensions.Hosting` is assumed).
...
The entire background poller is gated by `ConfigApiPollingOptions.Enabled` (defaults to `true`):
...
> In production, the polling service is the **only** writer to `ConfigApiResolvedLedger`. With `Enabled == false`, every options monitor is effectively driven by
> `missingDefinitionKeyBehavior`. Use that mode for test hosts or for services that consume config exclusively via REST.

## `ConfigApiResolvedLedger`

The shared in-process snapshot that ties the background poller to all `IOptionsMonitor<T>` instances is [`ConfigApiResolvedLedger`](./ConfigApiResolvedLedger.cs). It is
...
`GetReloadToken` is subscribed via `ChangeToken.OnChange` by `ConfigApiOptionsMonitor<T>`, so every successful ledger swap triggers re-materialization of *all* registered options
types in lock-step.

## What happens when a definition key is missing

`ConfigApiResolvedLedger.Current.TryGetValue(definitionKey, …)` then `configValue.GetValue<T>(ConfigJsonSerializerOptions.Default)` is the materialization path. Failure modes
are controlled by the `ConfigApiMissingDefinitionKeyBehavior` selected at registration:
...
Pick `UseDefaultInstance` for features that may have no bindings yet; keep `Throw` for required configuration so misconfiguration fails fast at startup.
...
1. The hosted service probes `IConfigApiClient.ResolveForAppAsync` with `AppKind`/`AppId` and `If-None-Match` from `ConfigApiResolvedLedger.CurrentEtag`.
