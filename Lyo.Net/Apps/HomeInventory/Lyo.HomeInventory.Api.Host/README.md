# Lyo.HomeInventory.Api.Host

Thin host: `AddHomeInventoryApiFromConfiguration` and `BuildHomeInventoryGroup`. Prefer embedding those calls in another host when composing APIs. Binds http://localhost:5102 (see Properties/launchSettings.json).

## Features

- OpenAPI and Scalar in Development only.
- Local cache + query services as required by HomeInventory CRUD.
...
```bash
dotnet run --project Lyo.Net/Apps/HomeInventory/Lyo.HomeInventory.Api.Host
```
...
- `Lyo.HomeInventory.Postgres` (transitive, lyo)
