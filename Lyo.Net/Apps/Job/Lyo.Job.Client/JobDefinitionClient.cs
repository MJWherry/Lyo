...
/// <summary>Query calls for job definitions.</summary>
public sealed class JobDefinitionClient(IApiClient client, string? routePrefix = null)
{
    public Task<QueryRes<JobDefinitionRes>> QueryAsync(QueryConcreteReq request, CancellationToken ct = default)
        => client.PostAsAsync<QueryConcreteReq, QueryRes<JobDefinitionRes>>(ApiRouteBuilder.Build(routePrefix, JobRoutes.DefinitionsQuery), request, ct: ct);

    /// <summary>Distinct non-empty <see cref="JobDefinitionRes.WorkerType" /> values from the Job API.</summary>
