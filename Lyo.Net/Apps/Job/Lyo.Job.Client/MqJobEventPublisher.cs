...
/// <para>Register with <c>services.AddMqJobEventPublisher()</c> after adding <see cref="IMqService" /> (for example RabbitMQ).</para>
/// </summary>
public sealed class MqJobEventPublisher : IJobEventPublisher
{
    ...
    public MqJobEventPublisher(IMqService mqService, ILogger<MqJobEventPublisher> logger, JobMqOptions mqOptions, IJobClient? jobClient = null)
    {
    ...
    {
        // Cancels fan out through the exchange to a per-instance exclusive queue. A shared per-worker-type queue would be a
        // competing-consumer queue: with scaled-out workers only one instance would get each cancel, and if it is not the instance
        // running the job the cancel is silently lost.
    ...
    private async Task SetupWorkerQueuesAsync(IReadOnlyList<string> workerTypes, CancellationToken ct)
    {
        ...
        // Cancel queues are not created here. Each worker instance declares its own exclusive queue in
