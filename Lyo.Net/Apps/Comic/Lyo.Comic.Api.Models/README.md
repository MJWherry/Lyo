...
Request and response DTOs shared between [`Lyo.Comic.Api`](../Lyo.Comic.Api/README.md) and [`Lyo.Comic.Api.Client`](../Lyo.Comic.Api.Client/README.md). Targets **`netstandard2.0`**
and **`net10.0`** so the same contracts can be referenced from any host (server, Blazor, MAUI, console).
...
`FromEntityType` / `FromEntityId`** so the API can record who made the assertion — the server **never trusts** an authenticated identity here (see the *Limitations* section of the
Comic.Api README).
