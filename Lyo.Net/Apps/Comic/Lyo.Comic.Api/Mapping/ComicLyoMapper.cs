...
/// <summary>Hand-rolled ILyoMapper for the Comic API. All mappings use explicit property assignments — no reflection or third-party mapping library.</summary>
public sealed class ComicLyoMapper : ILyoMapper
{
    public TResult Map<TResult>(object source)
        => source switch {
            ...
            var _ => throw new InvalidOperationException($"No mapping configured from {source.GetType().Name} to {typeof(TResult).Name}.")
        };

    public void Map<TSource, TDest>(TSource source, TDest destination)
    {
        switch (source, destination) {
            case (ComicSeriesReq req, SeriesEntity e):
                Apply(req, e);
                break;
            case (ComicVolumeReq req, VolumeEntity e):
                Apply(req, e);
                break;
            case (ComicChapterReq req, ChapterEntity e):
                Apply(req, e);
                break;
            case (ComicPageReq req, PageEntity e):
                Apply(req, e);
                break;
            case (ComicCharacterReq req, CharacterEntity e):
                Apply(req, e);
                break;
            default:
                throw new InvalidOperationException($"No mapping configured from {typeof(TSource).Name} to {typeof(TDest).Name}.");
        }
    }
    ...
    private static SeriesEntity ReqToNew(ComicSeriesReq r)
    {
        ...
        return new() {
            ...
            AlternateTitles = r.AlternateTitles.Select(a => new AlternateTitleEntity {
                    Id = Guid.NewGuid(),
                    SeriesId = id,
                    Title = a.Title,
                    Language = a.Language
    ...
    private static VolumeEntity ReqToNew(ComicVolumeReq r)
        => new() {
            Id = Guid.NewGuid(),
            SeriesId = r.SeriesId,
    ...
    private static ChapterEntity ReqToNew(ComicChapterReq r)
        => new() {
            Id = Guid.NewGuid(),
            SeriesId = r.SeriesId,
            VolumeId = r.VolumeId,
    ...
    private static CharacterEntity ReqToNew(ComicCharacterReq r)
        => new() {
            Id = Guid.NewGuid(),
            SeriesId = r.SeriesId,
            Name = r.Name,
    ...
    internal static ComicSeriesRes ToRes(SeriesEntity e)
        => new() {
            Id = e.Id,
            Title = e.Title,
            Slug = e.Slug,
            ...
            AlternateTitles = e.AlternateTitles.Select(a => new ComicAlternateTitleRes { Id = a.Id, Title = a.Title, Language = a.Language }).ToList()
        };

    internal static ComicVolumeRes ToRes(VolumeEntity e)
        => new() {
            Id = e.Id,
            SeriesId = e.SeriesId,
    ...
    internal static ComicChapterRes ToRes(ChapterEntity e)
        => new() {
            Id = e.Id,
            SeriesId = e.SeriesId,
            VolumeId = e.VolumeId,
    ...
    internal static ComicPageRes ToRes(PageEntity e)
        => new() {
            Id = e.Id,
    ...
    internal static ComicCharacterRes ToRes(CharacterEntity e)
        => new() {
            Id = e.Id,
            SeriesId = e.SeriesId,
            Name = e.Name,
