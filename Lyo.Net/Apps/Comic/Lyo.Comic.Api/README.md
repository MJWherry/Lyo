...
Default prefix is **`/api/comic`**. CRUD and `POST /Query` are generated by **`BuildComicApiEndpoints`** via **`ApiEndpointBuilder`** (plain `GET /{id}` is suppressed for series,
...
| POST   | **`/api/comic/series/Query`**                                         | `BuildComicApiEndpoints` (`Query`)  | Lyo.Api query. `POST /QueryProject` is also registered.                                                                                                                                                                |
...
| POST / DELETE       | **`/api/comic/volumes[, /Update, /Upsert, /Query, /{id}]`**          | `BuildComicApiEndpoints` (`All & ~Get`) | CRUD + Query (no special update hook).                                       |
...
| POST / DELETE       | **`/api/comic/chapters[, /Update, /Upsert, /Query, /{id}]`**          | `BuildComicApiEndpoints` (`All & ~Get`) | CRUD + Query (no special update hook).   |
