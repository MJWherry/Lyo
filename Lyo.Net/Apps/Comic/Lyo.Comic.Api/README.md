...
- **Comic API requires authentication.** `BuildComicApiEndpoints` uses **`.RequireAuthorization()`**, and `/api/comic` plus `/files` are mapped with the same requirement. Anonymous endpoints are **`GET /health`**, JWKS, and **`/auth/*`** (Google OIDC BFF). Configure **`GoogleAuth`**, **`LyoJwt`**, and **`PostgresUser`** on the host.
- **`IsFavorited`** is filled when the request carries a Lyo JWT (`lyo:user` / `sub`). Anonymous callers still see `null`.
- **`/files/upload` has `DisableAntiforgery()`**. Combine with auth (already required) before exposing it externally.
