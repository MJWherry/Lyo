...
Default prefix is **`/api/comic`**. CRUD and `POST /QueryConcrete` are generated by **`BuildComicApiEndpoints`** via **`ApiEndpointBuilder`** (plain `GET /{id}` is suppressed for series,
...
Aggregates **tags, ratings, comments, favorites** (and related counts) for comic aggregates without N+1 queries: uses **`IQueryService<TContext>`** with **`QueryConcreteReqBuilder`** so
