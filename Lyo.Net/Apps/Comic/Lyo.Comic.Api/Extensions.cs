...
public static class Extensions
{
    ...
    /// <summary>
    /// Registers all Comic API services: the five postgres stores, enrichment service,
    /// local cache, Lyo.Api query/CRUD services, the custom mapper, and the comic file
    /// storage stack (LocalKeyStore → TwoKeyEncryption → PostgresFileMetadataStore → LocalFileStorageService).
    /// </summary>
    ...
    private static IServiceCollection AddComicFileStorage(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddKeyedScoped<IFileMetadataStore>(FileStorageKey, (provider, _) => {
        ...
        services.AddFileStorageServiceKeyed(
            FileStorageKey,
            opts => opts.RootDirectoryPath = configuration["ComicFileStorage:RootDirectoryPath"] ?? "./comic-files",
            provider => provider.GetRequiredKeyedService<IFileMetadataStore>(FileStorageKey),
            FileStorageKey);
    ...
    /// <summary>
    /// Registers standard CRUD + Query endpoints via ApiEndpointBuilder for each Comic entity.
    /// Series/Volume/Chapter skip the plain GET (enriched GET is provided by MapComicApi).
    /// Pages include the plain GET since no async enrichment is needed.
    /// </summary>
    public static WebApplication BuildComicApiEndpoints(this WebApplication app, string prefix = "/api/comic")
    {
        ...
        app.CreateBuilder<ComicDbContext, SeriesEntity, ComicSeriesReq, ComicSeriesRes>(
                $"{prefix}/series", "ComicSeries")
            .AllowAnonymous()
            .WithCrud(ApiFeatureFlag.All & ~ApiFeatureFlag.Get,
                new CrudConfiguration<ComicDbContext, SeriesEntity, ComicSeriesReq> {
                    BeforeUpdate = ctx => {
                        ...
                        foreach (var a in ctx.Request.Data.AlternateTitles)
                            ctx.DbContext.AlternateTitles.Add(new AlternateTitleEntity {
                                Id = Guid.NewGuid(), SeriesId = ctx.Entity.Id, Title = a.Title, Language = a.Language
                            });
                    },
                    BeforeUpsert = ctx => {
                        ctx.DbContext.Entry(ctx.Entity).Collection(e => e.AlternateTitles).Load();
        ...
        app.CreateBuilder<ComicDbContext, VolumeEntity, ComicVolumeReq, ComicVolumeRes>(
                $"{prefix}/volumes", "ComicVolume")
            .AllowAnonymous()
            .WithCrud(ApiFeatureFlag.All & ~ApiFeatureFlag.Get, new())
            .Build();

        // Chapters: all operations except plain GET
        app.CreateBuilder<ComicDbContext, ChapterEntity, ComicChapterReq, ComicChapterRes>(
                $"{prefix}/chapters", "ComicChapter")
            .AllowAnonymous()
            .WithCrud(ApiFeatureFlag.All & ~ApiFeatureFlag.Get, new())
            .Build();

        // Pages: all operations including plain GET (ComicPageRes has no async enrichment)
        app.CreateBuilder<ComicDbContext, PageEntity, ComicPageReq, ComicPageRes>(
                $"{prefix}/pages", "ComicPage")
            .AllowAnonymous()
            .WithCrud(ApiFeatureFlag.All, new())
            .Build();

        // Characters: all CRUD + Query; volume appearances are managed via the join table
        app.CreateBuilder<ComicDbContext, CharacterEntity, ComicCharacterReq, ComicCharacterRes>(
                $"{prefix}/characters", "ComicCharacter")
            .AllowAnonymous()
            .WithCrud(ApiFeatureFlag.All, new())
            .Build();
