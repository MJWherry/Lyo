...
- Binds `ComicApiClientOptions` from the section named by `configSectionName` (defaults to **`"ComicApi"`**, see `ComicApiClientOptions.SectionName`). The section is optional — if
  it does not exist the defaults from `ApiClientOptions` are used.
