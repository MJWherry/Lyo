...
public class PdfServiceTests : IDisposable, IAsyncDisposable
{
    ...
    /// <summary>One-page PdfPig-openable PDF whose <see cref="IPdfDocumentText" /> is used only for synthetic word/layout tests.</summary>
    ...
    public void ExtractKeyValuePairs_VerticalLayout_SameLineValue_ReadsTextAfterKey()
    {
        ...
        // Same baseline Y: "Pages:" then value tokens to the right (typical of forms).
    ...
    public void InferKeyValuePairsFromFormatting_UsesFontNameBoldVsRegular()
    {
        ...
        // Stacked label (bold) then value (regular), then a same-line bold label plus regular value.
    ...
    public void ExtractKeyValuePairs_VerticalLayout_ReadsValueBelowKey()
    {
        ...
        // Higher top sits higher on the page; key row then value row below (smaller Y centroid).
    ...
    public void InferTableHeadersFromFormatting_SecondRowMustMatchInferenceStyleToMerge()
    {
        ...
        // Same X banding as TwoLineUnderlinedHeader so horizontal gap clustering yields three columns (tight
