...
public sealed class LyoPdfAnnotationResult
{
    ...
    /// <summary>For <see cref="PdfAnnotationExtractionType.KeyValue" />, whether values sit to the right of keys or on the lines below.</summary>
    public PdfKeyValueLayout KeyValueLayout { get; set; } = PdfKeyValueLayout.Horizontal;

    /// <summary>Signals used when inferring key/value or table headers without chips (bold, underline, optional punctuation-terminated labels).</summary>
    ...
    /// characters as label terminators, in that order (e.g. <c>":;=?|"</c>).
    /// </summary>
    public string KeyValueInferDelimiters { get; set; } = ":;";

    /// <summary>
    /// For <see cref="PdfAnnotationExtractionType.Table" />, optional header label of the row-key column. When set, only that column is treated as
    /// <see cref="ColumnHeader.IsKey" /> (continuation lines missing this cell merge into the previous row). Overrides a <c>*</c> prefix on chips.
    /// </summary>
    public string? TableKeyColumnLabel { get; set; }

    /// <summary>
    /// For <see cref="PdfAnnotationExtractionType.BoundingBoxText" />, how many vertical columns to split the region into (≥ 1). Values &gt; 1 use gutter/band
    /// detection from the PDF layout.
    /// </summary>
    public int ColumnCount { get; set; } = 1;

    /// <summary>Filled when <see cref="ColumnCount" /> is greater than 1: one string per column, left to right.</summary>
