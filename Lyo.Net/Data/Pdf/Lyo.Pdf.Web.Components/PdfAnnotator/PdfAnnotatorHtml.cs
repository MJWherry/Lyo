...
/// <summary>Builds HTML for the PDF bounding-box annotator. Use <see cref="GetForBlazor" /> when embedding in Blazor (postMessage).</summary>
public static class PdfAnnotatorHtml
{
    /// <summary>HTML meant for an iframe; annotations go to the parent window via postMessage.</summary>
    /// <param name="pdfBase64">Base64-encoded PDF content.</param>
    /// <returns>Full HTML document string.</returns>
    /// <remarks>PDF.js is loaded from pinned CDN URLs embedded in this HTML (<c>import()</c> inside the iframe module).</remarks>
    ...
    /// <summary>
    /// Read-only PDF viewer with region overlays (same coordinate model as the annotator). <paramref name="annotationsJsonArray" /> is a JSON array of
