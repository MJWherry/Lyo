...
public sealed class PdfTextExtractor : ITextExtractor
{
    ...
    public Task<IReadOnlyList<PdfWord>> GetWordsAsync(int? page = null, CancellationToken ct = default) => Task.Run(() => GetWordsInternal(_owner, page), ct);
    ...
    public IReadOnlyList<PdfWord> GetWordsBetween(string? startText = null, string? endText = null, int? page = null) => GetWordsBetweenInternal(_owner, startText, endText, page);
    ...
    public IReadOnlyList<PdfTextLine> GetLinesInBoundingBox(PdfBoundingBox region, double? yTolerance = null) => GetLinesInBoundingBoxInternal(_owner, region, yTolerance);

    public Task<PdfColumnarText> GetColumnarTextInBoundingBoxAsync(PdfBoundingBox region, int columnCount, double? yTolerance = null, CancellationToken ct = default)
    ...
    public IReadOnlyList<KvColumnResult> ExtractKeyValuePairs(
        ...
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.ExtractKeyValueDuration, Constants.Metrics.ExtractKeyValueSuccess, Constants.Metrics.ExtractKeyValueFailure, () => {
                ArgumentHelpers.ThrowIfNull(words);
    ...
    public IReadOnlyList<IReadOnlyDictionary<string, string?>> ExtractTable(
        ...
        PdfInferFormattingFlags? inferFormattingForHeaderRows = null)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.ExtractTableDuration, Constants.Metrics.ExtractTableSuccess, Constants.Metrics.ExtractTableFailure, () => {
                ArgumentHelpers.ThrowIfNull(words);
    ...
    public async Task<DataTable.Models.DataTable> ExtractDataTableAsync(ColumnHeader[] headers, int? page = null, double yTolerance = 5.0, CancellationToken ct = default)
    ...
    public async Task<IReadOnlyList<KvColumnResult>?> ExtractKeyValuePairsAsync(
        ...
        CancellationToken ct = default)
        => await Task.Run(
                () => ExtractKeyValuePairs(startSection, knownKeys, sectionsInOrder, defaultEndSection, startPage, endPage, yTolerance, keyValueLayout, keyValueColumnCount), ct)
    ...
    public async Task<IReadOnlyList<IReadOnlyDictionary<string, string?>>?> ExtractTableAsync(
        ...
        PdfInferFormattingFlags? inferFormattingForHeaderRows = null,
        CancellationToken ct = default)
        => await Task.Run(() => ExtractTable(startSection, headers, sectionsInOrder, defaultEndSection, startPage, endPage, yTolerance, inferFormattingForHeaderRows), ct)
            .ConfigureAwait(false);
        ...
        PdfInferFormattingFlags? inferFormattingForHeaderRows = null)
    {
        ...
        var section = GetSection(startSection, sectionsInOrder, defaultEndSection, startPage, endPage, yTolerance);
        return section is null ? null : ExtractTable(section.Words, headers, ResolveExtractYTolerance(yTolerance), inferFormattingForHeaderRows);
    }
    ...
    public async Task<DataTable.Models.DataTable?> ExtractDataTableAsync(
        ...
        double? yTolerance = null,
        CancellationToken ct = default)
        => await Task.Run(() => ExtractDataTable(startSection, headers, sectionsInOrder, defaultEndSection, startPage, endPage, yTolerance), ct).ConfigureAwait(false);
    ...
    private IReadOnlyList<PdfWord> GetWordsInternal(PdfReader pdfRead, int? page)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.WordsDuration, Constants.Metrics.WordsSuccess, Constants.Metrics.WordsFailure, () => pdfRead.WithPdf(pdfDoc => {
                var document = pdfDoc;
                if (page.HasValue) {
    ...
    private IReadOnlyList<PdfTextLine> GetLinesInternal(PdfReader pdfRead, int? page, double? yTolerance)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.LinesDuration, Constants.Metrics.LinesSuccess, Constants.Metrics.LinesFailure, () => pdfRead.WithPdf(pdfDoc => {
                var document = pdfDoc;
    ...
    private IReadOnlyList<PdfWord> GetWordsBetweenInternal(PdfReader pdfRead, string? startText, string? endText, int? page)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.WordsBetweenDuration, Constants.Metrics.WordsBetweenSuccess, Constants.Metrics.WordsBetweenFailure, () => {
    ...
    private IReadOnlyList<PdfTextLine> GetLinesBetweenInternal(PdfReader pdfRead, string? startText, string? endText, int? page, double? yTolerance)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.LinesBetweenDuration, Constants.Metrics.LinesBetweenSuccess, Constants.Metrics.LinesBetweenFailure, () => {
                var tolerance = yTolerance ?? _options.DefaultYTolerance;
                var lines = pdfRead.WithPdf(d => GetDocumentLines(d, tolerance, page));
    ...
    private IReadOnlyList<PdfTextLine> GetLinesInBoundingBoxInternal(PdfReader pdfRead, PdfBoundingBox region, double? yTolerance)
        => PdfOperationMetrics.Execute(
            _metrics, Constants.Metrics.LinesDuration, Constants.Metrics.LinesSuccess, Constants.Metrics.LinesFailure, () => pdfRead.WithPdf(pdfDoc => {
                ...
                // Page-content words: keep only those whose overlap meets the threshold
                ...
                // Annotation text: Intersects only (form-field rects are large; the caller box may sit entirely inside)
    ...
    /// <summary>When drawn text and an AcroForm widget share the same text, keep the tighter box and drop the duplicate.</summary>
    ...
    private ColumnHeader[] InferTableHeadersFromFormattingInternal(List<PdfWord> words, double? yTolerance, PdfInferFormattingFlags inferFlags, char[] keyValueDelimiters)
    {
        ...
        if (useFontEmphasis) {
            ...
            for (var li = 0; li < scan; li++) {
                ...
                // Drop lines that lack the requested emphasis (e.g. plain body rows when inferring underline).
            ...
            if (scored.Count > 0) {
                ...
                // On a score tie, take the topmost line (smallest index; lines are sorted by descending Y).
        ...
        // Lookahead: keep consecutive lines that still look like the styled header (spacing + formatting).
    ...
    /// <summary>Infers one column header per horizontal band by clustering words on X gaps (not every word is a column).</summary>
    private ColumnHeader[] InferColumnHeadersFromEmphasizedBlock(IReadOnlyList<PdfTextLine> blockLines)
    {
        ...
        // Single header row: column edges come from horizontal spacing only.
        if (blockLines.Count == 1)
            return BuildColumnHeadersFromGapClusters(anchorCols);

        // Multi-line header: if the anchor line did not split, try the top physical line.
    ...
    /// <summary>Reads annotation text from Content, /Contents, or /V. Supports FT=/Tx (text) and FT=/Btn (checkbox).</summary>
    private static string? GetAnnotationText(Annotation ann)
    {
        ...
        // /Contents — FreeText, Text, and similar annotation kinds
        ...
        // /Btn checkbox: V = /1 or /Yes when checked, /Off when not. Rendered as [x] Label or [] Label
        ...
        // /Tx text input: V carries the string value
    ...
    /// <summary>Checkbox label: TU (tooltip) first; otherwise T with internal names like c1_1[0] stripped.</summary>
    private static string? GetCheckboxLabel(DictionaryToken dict)
    {
        ...
        // Drop leading internal field names such as "c1_1[0]" or "f1_08[0]"
        var stripped = Regex.Replace(t.Trim(), @"^[\w\-]+\s*\[\d+\]\s*", "", RegexOptions.IgnoreCase);
        return stripped.IsNullOrWhitespace() ? null : stripped.Trim();
    }

    /// <summary>Reads a string from a dictionary key (StringToken, HexToken, or NameToken).</summary>
    ...
    /// <summary>Parses /DA (Default Appearance) for font, size, and color. Shape: /Font size Tf [gray g | R G B rg].</summary>
    ...
    /// <summary>Median X of the widest inter-word gap per line (when the gap clears a minimum), used to find a two-column gutter.</summary>
    ...
    /// <summary>Clusters words into visual rows. Words whose vertical mid-points differ by at most yTolerance share a line.</summary>
    ...
    /// <summary>Combines consecutive lines whose Y gap is ≤ threshold into one logical row.</summary>
    ...
    /// <summary>
    /// Pulls key/value pairs from words using known keys. <paramref name="keyValueColumnCount" /> &gt; 1 splits the region into that many vertical bands (e.g. duplicate keys
    /// side by side).
    /// </summary>
    ...
    /// <summary>Partitions words into <paramref name="columnCount" /> equal-width vertical bands using the region's min/max X.</summary>
    ...
    /// <summary>Pulls key/value pairs from one column of words.</summary>
        ...
        PdfKeyValueLayout keyValueLayout)
    {
        ...
        // Keep strict line boundaries for key/value extraction (do not merge lines).
        var lines = rawLines.OrderByDescending(l => l.Y).ToList();
        var result = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase);

        // Active value columns: Key, ValueColumnLeft (value start X), LineY
    ...
    /// <summary>
    /// Prefers stacked label-over-value fields, but many PDFs also put label+value on one line (e.g. "Pages: 13 pages"). Same-line values are taken first; otherwise values come
    /// from later lines in the key's horizontal band.
    /// </summary>
    ...
    /// <summary>Words on the key's line, to the right of the key span, excluding other keys.</summary>
    ...
    /// <summary>Builds a table from words using column headers. Data rows are expected below the header row.</summary>
        ...
        PdfInferFormattingFlags? inferFormattingForHeaderRows = null)
    {
        ...
        // Locate the first matching header row in reading order.
        ...
        // Assign each header an X position
        ...
        // Build formatted header cells from the PDF header words
        ...
        // When the header spans two visual lines, both are in headerWords — data starts after the second line.
        ...
        // Fold multi-line rows using explicit key columns.
    ...
    private static bool TryFindHeaderStartX(IReadOnlyList<PdfWord> headerWords, string headerLabel, out double startX)
    {
        ...
        // Fall back to the first token match when the full phrase is not contiguous.
    ...
    private static Dictionary<string, string> BuildKnownKeyAliases(IEnumerable<string> knownKeys)
    {
        ...
        foreach (var key in keys) {
            ...
            // PDFs often glue ':' onto the label word ("Pages:") while callers type "Pages".
    ...
    /// <summary>Folds a trailing "Dt." / "Dt" token into the previous header when PDF word splits turn "Offense Dt." into two columns.</summary>
    ...
    /// <summary>Holds optional format through PDF table extraction until it is applied on the DataTable map.</summary>
