...
{
    /// <summary>ISO/IEC 15417 quiet zone: at least 10 times the narrowest bar width. Here that is one module.</summary>
    ...
    /// <summary>Writes a standard bottom-up BMP (positive <c>biHeight</c>) so common decoders and viewers can open it. Also applies the minimum quiet zone.</summary>
    private static byte[] RenderBmp(bool[] modules, BarcodeOptions options)
    {
        ...
        // BITMAPFILEHEADER (14-byte BMP file header)
        ...
        // Bottom-up DIB. Positive height means the first scanline in the file is the image's bottom row.
