...
MudBlazor page that puts <BarcodeWorkbench /> from [`Lyo.Barcode.Web.Components`](../Lyo.Barcode.Web.Components/README.md) in a MudContainer for the Lyo gateway test harness.

## Components

| Component | Purpose |
| ------------------------ | ------------------------------------------------------------------------------------------------- |
| <BarcodeTestWorkbench /> | Sets the title to "Barcode test workbench" and renders <BarcodeWorkbench />. No extra parameters. |

## Services the host must register

Matches Lyo.Barcode.Web.Components: register an IBarcodeService implementation such as AddNativeBarcodeService and the usual Lyo.Web.Components host wiring.
