...
- A **logo upload** (PNG/SVG only) queues an **`Overlay`** stage. The pad behind the logo uses the QR **light** color; when the *Logo border* checkbox is on, the stroke uses the QR
  **dark** color so the border stays visible.
