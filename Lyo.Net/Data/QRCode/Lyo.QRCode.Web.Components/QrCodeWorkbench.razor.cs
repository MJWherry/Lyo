...
/// <summary>
/// Blazor workbench for QR generation: three columns (output and styling, typed payloads, result). After the raw QR is built, the workbench chains
/// <see cref="IImageDecorationService" /> primitives (overlay, frame, caption, padding) via <see cref="IImageDecorationService.Pipeline(byte[])" /> to apply the workbench badge
/// preset. MudBlazor picker colors become opaque <c>#RRGGBB</c> for ImageSharp before they enter the pipeline.
/// </summary>
public partial class QrCodeWorkbench : IAsyncDisposable
{
    /// <summary>Badge-style presets that chain <see cref="IImageDecorationPipeline" /> stages after raw QR generation (PNG only). Replaces the removed <c>QrFrameStyle</c> enum.</summary>
    public enum QrBadgePreset
    {
        /// <summary>No decoration; the raw QR bytes come back unchanged.</summary>
        None = 0,

        /// <summary>Header band (optional notch) plus outer card plus a thin frame matching the header chrome.</summary>
        BadgeWithHeader = 1,

        /// <summary>Outer rounded card plus a thin frame in a neutral slate stroke.</summary>
        SimpleRoundedPanel = 2,

        /// <summary>Stroke around the QR only; a supplied caption becomes a footer band underneath.</summary>
        BorderOnly = 3
    }
    ...
    /// <summary>Object URL for SVG full-tab preview (navigating to a data: SVG is often blocked).</summary>
    ...
    /// <summary>
    /// When non-empty, the Format control lists only these <see cref="QRCodeFormat" /> values (e.g. PNG and SVG for hosts that register only the built-in encoder). When
    /// null or empty, every enum value is listed.
    /// </summary>
    [Parameter]
    public IReadOnlyList<QRCodeFormat>? AllowedFormats { get; set; }

    /// <summary>Formats listed in the workbench Format dropdown.</summary>
    ...
    /// <summary>PNG frame modes that can draw optional caption text (header or footer band).</summary>
    private bool FrameShowsCaptionField => _format == QRCodeFormat.Png && SelectedFrameStyle != QrBadgePreset.None;

    /// <summary>Data-URL fallback for the SVG thumbnail when a blob URL is missing.</summary>
    ...
    /// <summary>Raster or SVG thumbnail source (SVG prefers a blob URL so &lt;img&gt; scales like PNG).</summary>
    ...
    /// <summary>Href for full-size preview (new tab). SVG prefers blob: because navigating to data:image/svg+xml is often blocked.</summary>
    ...
    /// <summary>Rough size hint assuming ~37 modules per side (typical short URL).</summary>
    ...
    /// <summary>6-digit <c>#RRGGBB</c> for ImageSharp (do not truncate <see cref="MudColor" /> strings — length-9 forms are not always <c>#RRGGBBAA</c>).</summary>
    ...
    /// <summary>Maps the workbench preset onto a chain of <see cref="IImageDecorationPipeline" /> stages applied after raw QR generation (PNG only).</summary>
    ...
    /// <summary>Builds the QR payload string from the current kind and fields.</summary>
    ...
    /// <summary>Dashboard-friendly buckets for <see cref="QRCodeOptions.Size" /> (pixels per module). Total image side is about module count × this value.</summary>
