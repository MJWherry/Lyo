...
QRCoder implementation of `IQRCodeService` from [`Lyo.QRCode`](../Lyo.QRCode/README.md). Use this when you need JPEG / Bitmap output on Windows or want QRCoder's renderers. Use `BuiltInQRCodeService` when you want the in-library ISO encoder with no extra NuGet.
...
- JPEG / Bitmap outputs go through `System.Drawing` and work on Windows. PNG/SVG paths run cross-platform.
