...
/// <summary>
/// <see cref="IQRCodeService" /> backed by the QRCoder NuGet package. PNG/SVG run on every platform; JPEG/BMP still need Windows because QRCoder routes them through
/// <c>System.Drawing</c>. Decoration (logo overlay, frame, caption, padding) is out of scope here; chain <c>Lyo.Images.IImageDecorationService</c> on the returned bytes for
/// that.
/// </summary>
public class QRCoderQRCodeService : IQRCodeService
{
    ...
    /// <summary>Builds a <see cref="QRCoderQRCodeService" />.</summary>
    /// <param name="options">QR code service options.</param>
    ...
    /// <summary>Starting QR code format.</summary>
    public QRCodeFormat DefaultFormat => _options.DefaultFormat;

    /// <summary>Builds a QR code from a <see cref="QRCodeBuilder" />.</summary>
    ...
    /// <summary>Builds a QR code from text or payload data.</summary>
    ...
    /// <summary>Builds many QR codes in one batch.</summary>
    ...
    /// <summary>Heavy rasterization (large payloads or large pixels-per-module) runs on the thread pool; lighter work stays inline to skip scheduling cost.</summary>
    private static bool ShouldOffloadQrRasterization(string data, QRCodeOptions options) => data.Length > 4096 || options.Size > 512;

    /// <summary>Produces QR image bytes through QRCoder.</summary>
