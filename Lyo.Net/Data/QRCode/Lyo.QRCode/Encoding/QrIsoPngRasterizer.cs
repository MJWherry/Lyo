...
/// <summary>1-bit indexed PNG rasterizer for <see cref="QRIsoMatrix" /> (PNG layout from MIT-licensed QRCoder).</summary>
internal static class QrIsoPngRasterizer
{
    public static byte[] ToPng(QRIsoMatrix qr, int pixelsPerModule, byte[] darkRgba, byte[] lightRgba, bool drawQuietZones)
    {
        ...
        var size = (matrix.Count - (drawQuietZones ? 0 : 8)) * pixelsPerModule;
        png.WriteHeader(size, size, 1, PngBuilder.ColorType.Indexed);
        png.WritePalette(darkRgba, lightRgba);
        var scanLines = DrawScanlines(matrix, pixelsPerModule, drawQuietZones);
    ...
    private static ArraySegment<byte> DrawScanlines(List<BitArray> moduleMatrix, int pixelsPerModule, bool drawQuietZones)
    {
        var matrixSize = moduleMatrix.Count - (drawQuietZones ? 0 : 8);
        var quietZoneOffset = drawQuietZones ? 0 : 4;
        var bytesPerScanline = (matrixSize * pixelsPerModule + 7) / 8 + 1;
        ...
        return new(scanlines, 0, scanLinesLength);
    }
    ...
    private sealed class PngBuilder : IDisposable
    {
        ...
        public byte[] GetBytes()
        {
            ...
            // Walk chunks and fill in CRC32 checksums.
            ...
            while (chunkOffset < bytes.Length) {
                ...
                // CRC covers type plus data.
                var crc = Crc32(bytes, chunkOffset + 4, dataLength + 4);

                // Store CRC at the chunk tail.
        ...
        /// <summary>Writes the IHDR chunk. Must be first in the file.</summary>
        ...
        /// <summary>Writes the PLTE chunk, plus tRNS when any color has alpha. Must precede IDAT.</summary>
        ...
        /// <summary>Writes the IDAT chunk holding the image bytes.</summary>
        public void WriteScanlines(ArraySegment<byte> scanlines)
        {
            ...
            // zlib header.
            _stream.WriteByte(0x78); // 8 = Deflate, 7 = max window
            _stream.WriteByte(0x9C); // check bits

            // Compressed payload.
            idatStream.WriteTo(_stream);
            // Adler-32 of the uncompressed scanlines.
            var adler = Adler32(scanlines.Array!, 0, scanlines.Count);
            WriteIntBigEndian(adler);
            WriteChunkEnd();
        }

        /// <summary>Writes the IEND chunk. Must be last in the file.</summary>
        ...
        {
            // Leave 4 bytes for CRC32; GetBytes fills them later.
        ...
        {
            using var deflateStream = new DeflateStream(output, CompressionLevel.Fastest, true);
            deflateStream.Write(bytes.Array!, 0, bytes.Count);
        }

        // RFC 1950 Adler-32. Not optimized.
        ...
        // REC-PNG-20031110 CRC-32. Not optimized.
