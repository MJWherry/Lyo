...
internal sealed partial class QrIsoEncoder
{
    ...
    /// <summary>
    /// Determines the appropriate QR code version based on the data segment and error correction level. Validates that the data fits within the requested version, or finds the
    /// minimum version if not specified.
    /// </summary>
    ...
    /// <summary>Calculates the Micro QR code data which then can be used in one of the rendering classes to generate a graphical representation.</summary>
    /// <param name="plainText">The payload which shall be encoded in the QR code</param>
    /// <param name="eccLevel">The level of error correction data</param>
    ...
    /// <summary>
    /// Validates the specified error correction level. Returns the provided level if it is valid, or the level M if the provided level is Default. Throws an exception if an
    /// invalid level is provided.
    /// </summary>
    ...
    /// <summary>
    /// Generates a QR code data structure using the provided BitArray, error correction level, and version. The BitArray provided is assumed to already include the count,
    /// encoding mode, and/or ECI mode information.
    /// </summary>
    /// <param name="bitArray">
    /// The BitArray containing the binary-encoded data to be included in the QR code. It should already contain the count, encoding mode, and/or ECI mode
    /// information.
    /// </param>
    ...
    /// <summary>
    /// Generates a BitArray containing the format string for a QR code based on the error correction level and mask pattern version. The format string includes the error
    /// correction level, mask pattern version, and error correction coding.
    /// </summary>
    ...
    /// <summary>
    /// Encodes the version information of a QR code into a BitArray using error correction coding similar to format information encoding. This method is used for QR codes
    /// version 7 and above.
    /// </summary>
    ...
    /// <summary>
    /// Calculates the Error Correction Codewords (ECC) for a segment of data using the provided ECC information. This method applies polynomial division, using the message
    /// polynomial and a generator polynomial, to compute the remainder which forms the ECC codewords.
    /// </summary>
    ...
    /// <summary>
    /// Converts all polynomial item coefficients from their alpha exponent notation to decimal representation in place. This conversion facilitates operations that require
    /// polynomial coefficients in their integer forms.
    /// </summary>
    ...
    /// <summary>Determines the most efficient encoding mode for the given plain text based on its character content and a flag indicating whether to force UTF-8 encoding.</summary>
    ...
    /// <summary>Checks if a character falls within a specified range.</summary>
    private static bool IsInRange(char c, char min, char max) => (uint)(c - min) <= (uint)(max - min);

    /// <summary>Converts a segment of a BitArray representing QR code data into a polynomial, padding the final byte if necessary (for Micro QR variants like M1 or M3).</summary>
    ...
    /// <summary>Calculates the generator polynomial used for creating error correction codewords.</summary>
    ...
    /// <summary>Converts a segment of a BitArray into its decimal (integer) equivalent.</summary>
    ...
    /// <summary>Converts a decimal number to binary and stores the result in a BitArray starting from a specific index.</summary>
    ...
    /// <summary>Determines the number of bits used to indicate the count of characters in a segment, depending on the QR code version and the encoding mode.</summary>
    ...
    /// <summary>Calculates the data length based on the encoding mode, text content, and whether UTF-8 is forced.</summary>
    ...
    /// <summary>
    /// Converts an array of bytes into a BitArray, considering the proper bit order within each byte. Unlike the constructor of BitArray, this function preserves the MSB-to-LSB
    ...
    /// <summary>
    /// Converts an array of bytes into a BitArray at a specified offset, considering the proper bit order within each byte. Unlike the constructor of BitArray, this function
    /// preserves the MSB-to-LSB order within each byte.
    /// </summary>
    /// <param name="byteArray">The byte array to convert into a BitArray.</param>
    ...
    /// <summary>Performs a bitwise XOR operation between two polynomials, commonly used in QR code error correction coding.</summary>
    ...
    /// <summary>
    /// Multiplies a generator polynomial by a leading term polynomial, reducing the result by a specified lower exponent, used in constructing QR code error correction
    /// codewords.
    /// </summary>
    ...
    /// <summary>Multiplies two polynomials, treating coefficients as exponents of a primitive element (alpha), which is common in error correction algorithms such as Reed-Solomon.</summary>
