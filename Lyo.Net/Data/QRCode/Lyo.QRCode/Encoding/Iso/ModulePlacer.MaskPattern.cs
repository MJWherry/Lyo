...
internal sealed partial class QrIsoEncoder
{
    ...
    {
        /// <summary>
        /// Provides static methods and properties to handle mask patterns used in QR code generation. Mask patterns are applied to QR codes to break up patterns in the data matrix
        /// that might confuse scanners.
        /// </summary>
        private static class MaskPattern
        {
            /// <summary>A dictionary mapping each mask pattern index to its corresponding function that calculates whether a given pixel should be masked.</summary>
            ...
            /// <summary>Mask pattern 1: (x + y) % 2 == 0 Applies a checkerboard mask on the QR code.</summary>
            public static bool Pattern1(int x, int y) => (x + y) % 2 == 0;

            /// <summary>Mask pattern 2: y % 2 == 0 Applies a horizontal striping mask on the QR code.</summary>
            public static bool Pattern2(int x, int y) => y % 2 == 0;

            /// <summary>Mask pattern 3: x % 3 == 0 Applies a vertical striping mask on the QR code.</summary>
            public static bool Pattern3(int x, int y) => x % 3 == 0;

            /// <summary>Mask pattern 4: (x + y) % 3 == 0 Applies a diagonal striping mask on the QR code.</summary>
            public static bool Pattern4(int x, int y) => (x + y) % 3 == 0;

            /// <summary>Mask pattern 5: ((y / 2) + (x / 3)) % 2 == 0 Applies a complex pattern mask on the QR code, mixing horizontal and vertical rules.</summary>
            public static bool Pattern5(int x, int y) => (int)(Math.Floor(y / 2d) + Math.Floor(x / 3d)) % 2 == 0;

            /// <summary>Mask pattern 6: ((x * y) % 2 + (x * y) % 3) == 0 Applies a mask based on the product of x and y coordinates modulo 2 and 3.</summary>
            public static bool Pattern6(int x, int y) => x * y % 2 + x * y % 3 == 0;

            /// <summary>Mask pattern 7: (((x * y) % 2 + (x * y) % 3) % 2) == 0 Applies a mask based on a more complex function involving the product of x and y coordinates.</summary>
            public static bool Pattern7(int x, int y) => (x * y % 2 + x * y % 3) % 2 == 0;

            /// <summary>Mask pattern 8: (((x + y) % 2) + ((x * y) % 3) % 2) == 0 Combines rules of checkers and complex multiplicative masks.</summary>
            public static bool Pattern8(int x, int y) => ((x + y) % 2 + x * y % 3) % 2 == 0;

            /// <summary>
            /// Calculates a penalty score for a Micro QR code to evaluate the effectiveness of a mask pattern. A lower score indicates a QR code that is easier for decoders to read
            /// accurately.
            /// </summary>
            ...
            /// <summary>
            /// Calculates a penalty score for a QR code to evaluate the effectiveness of a mask pattern. A lower score indicates a QR code that is easier for decoders to read
            /// accurately. The score is the sum of four penalty rules applied to the QR code.
            /// </summary>
            /// <param name="qrCode">The QR code data structure to be evaluated.</param>
            /// <returns>The total penalty score of the QR code.</returns>
