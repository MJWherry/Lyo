...
internal sealed partial class QrIsoEncoder
{
    ...
    {
        /// <summary>
        /// Places the version information on the QR code matrix for versions 7 and higher. Version information is encoded into two small rectangular areas near the bottom left and
        /// top right corners outside the timing patterns.
        /// </summary>
        ...
        /// <summary>Places the format information on the QR code, encoding the error correction level and mask pattern used.</summary>
        /// <param name="qrCode">The QR code data structure to modify.</param>
        ...
        /// <summary>Applies the most effective mask pattern to the QR code based on minimizing the penalty score, which evaluates how well the pattern will work for QR scanners.</summary>
        ...
        /// <summary>Places data bits into the QR code's module matrix following a specific pattern that navigates around blocked modules.</summary>
        ...
        /// <summary>Reserves separator areas around the positioning patterns of a QR code to ensure that these crucial areas remain unmodified during data placement.</summary>
        ...
        public static void ReserveSeperatorAreas(int version, int size, BlockedModules blockedModules)
        {
            ...
            blockedModules.Add(new(7, 0, 1, 8)); // Vertical block near the top left finder pattern
            blockedModules.Add(new(0, 7, 7, 1)); // Horizontal block near the top left finder pattern
            if (version > 0) // Non-micro QR codes have 3 finder patterns
            {
                // Block areas around the bottom-left finder pattern
                blockedModules.Add(new(0, size - 8, 8, 1)); // Horizontal block near the bottom left finder pattern
                blockedModules.Add(new(7, size - 7, 1, 7)); // Vertical block near the bottom left finder pattern
                // Block areas around the top-right finder pattern
                blockedModules.Add(new(size - 8, 0, 1, 8)); // Vertical block near the top right finder pattern
                blockedModules.Add(new(size - 7, 7, 7, 1)); // Horizontal block near the top right finder pattern
            }
        }

        /// <summary>Reserves areas for version information on QR codes that are version 7 or higher. Also reserves space for format information.</summary>
        /// <param name="size">The size of the QR code matrix.</param>
        ...
        public static void ReserveVersionAreas(int size, int version, BlockedModules blockedModules)
        {
            ...
            blockedModules.Add(new(8, 0, 1, 6)); // Near the top timing pattern
            blockedModules.Add(new(8, 7, 1, 1)); // Small square near the top left finder pattern
            blockedModules.Add(new(0, 8, 6, 1)); // Near the left timing pattern
            blockedModules.Add(new(7, 8, 2, 1)); // Extension of the above block
            blockedModules.Add(new(size - 8, 8, 8, 1)); // Near the right timing pattern
            blockedModules.Add(new(8, size - 7, 1, 7)); // Near the bottom timing pattern

            // If the version is 7 or higher, additional blocks for version information are added.
            if (version >= 7) {
                blockedModules.Add(new(size - 11, 0, 3, 6)); // Top right version information block
                blockedModules.Add(new(0, size - 11, 6, 3)); // Bottom left version information block
            }
        }

        /// <summary>Places a dark module on the QR code matrix as per QR code specification, which requires a dark module at a specific position for all QR codes.</summary>
        ...
        /// <summary>Places alignment patterns on the QR code matrix. Alignment patterns help ensure the scanner can correctly interpret the QR code at various scales and orientations.</summary>
        ...
        public static void PlaceTimingPatterns(QrIsoMatrix qrCode, BlockedModules blockedModules)
        {
            ...
            if (qrCode.Version > 0) {
                ...
                blockedModules.Add(new(6, 8, 1, size - 16)); // Horizontal timing pattern area
                blockedModules.Add(new(8, 6, size - 16, 1)); // Vertical timing pattern area
            }
            else // Micro QR codes
            {
                ...
                // Add the areas occupied by the timing patterns to the list of blocked modules.
                blockedModules.Add(new(0, 8, 1, size - 8)); // Horizontal timing pattern area
                blockedModules.Add(new(8, 0, size - 8, 1)); // Vertical timing pattern area
            }
        }
    }
}