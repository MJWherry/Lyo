...
/// <summary>
/// <c>sms:</c> or <c>smsto:</c> URI. Starts as <c>sms:</c> (RFC-style). <c>smsto:</c> can crash Google Messages on Android when paired with a long <c>body</c> query.
/// </summary>
...
public sealed class SmsPayload : IQrPayload
{
    ...
    /// <summary>Builds an SMS QR payload.</summary>
    ...
    public string ToQrString()
    {
        ...
        // smsto:/sms: recipients are digits with an optional leading +. Do not percent-encode +; many clients expect a literal +.
        ...
        if (s.Length > MaxSmsQrUriLength) {
            throw new InvalidFormatException(
                ...
                nameof(Body), Body, $"body under ~{MaxSmsQrUriLength - 200} characters for typical phone numbers");
        }
    ...
    /// <inheritdoc />
    public override string ToString() => $"{(UseSmstoScheme ? "smsto" : "sms")}:{PhoneNumber}, bodyLen={Body?.Length ?? 0}";
}