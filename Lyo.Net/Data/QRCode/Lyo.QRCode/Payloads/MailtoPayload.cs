...
/// <summary><c>mailto:</c> URI, including optional headers.</summary>
...
public sealed class MailtoPayload : IQrPayload
{
    ...
    /// <summary>Builds a mailto payload.</summary>
    ...
    public string ToQrString()
    {
        ...
        // Per RFC 6068, addr-spec stays in plain form before '?'. Percent-encoding '@' yields mailto URIs that Uri.TryCreate rejects on .NET.
        ...
        var s = sb.ToString();
        if (!Uri.TryCreate(s, UriKind.Absolute, out var mailtoUri) || !string.Equals(mailtoUri.Scheme, Uri.UriSchemeMailto, StringComparison.OrdinalIgnoreCase))
    ...
    /// <inheritdoc />
    public override string ToString() => $"MailtoPayload to={To}, subjectLen={Subject?.Length ?? 0}, bodyLen={Body?.Length ?? 0}, cc={Cc != null}, bcc={Bcc != null}";
}