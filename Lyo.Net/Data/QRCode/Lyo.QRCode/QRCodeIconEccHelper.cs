...
/// <summary>
/// Picks a high enough error-correction level when a center logo covers part of the symbol. Larger logos need stronger ECC than the default; otherwise the composite looks
/// valid but may not scan.
/// </summary>
internal static class QRCodeIconEccHelper
{
    /// <summary>Level used to encode the QR matrix. Never lower than <paramref name="requested" />.</summary>
    ...
    /// <summary>Minimum ECC so typical payloads stay scannable when a logo covers <paramref name="iconPercent" />% of the image side.</summary>
    ...
    private static QRCodeErrorCorrectionLevel Max(QRCodeErrorCorrectionLevel a, QRCodeErrorCorrectionLevel b) => Order(a) >= Order(b) ? a : b;

    private static int Order(QRCodeErrorCorrectionLevel e)
        => e switch {
