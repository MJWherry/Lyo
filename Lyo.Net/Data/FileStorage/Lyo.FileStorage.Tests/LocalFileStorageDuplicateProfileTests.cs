...
public sealed class LocalFileStorageDuplicateProfileTests
{
    ...
    public async Task ReturnExisting_PlainThenCompress_ThrowsConflict()
    {
        ...
        var ex = await Assert.ThrowsAsync<ConflictException>(() =>
            scope.Storage.SaveFileAsync(payload, "compressed.txt", compress: true, ct: TestContext.Current.CancellationToken));
    ...
    public async Task ReturnExisting_CompressThenPlain_ThrowsConflict()
    {
        ...
        await scope.Storage.SaveFileAsync(payload, "compressed.txt", compress: true, ct: TestContext.Current.CancellationToken);
        await Assert.ThrowsAsync<ConflictException>(() =>
            scope.Storage.SaveFileAsync(payload, "plain.txt", ct: TestContext.Current.CancellationToken));
    }
    ...
    public async Task ReturnExisting_EncryptedDifferentKeyId_ThrowsConflict()
    {
        ...
        await Assert.ThrowsAsync<ConflictException>(() =>
            scope.Storage.SaveFileAsync(payload, "second.txt", encrypt: true, keyId: "key-b", ct: TestContext.Current.CancellationToken));
    }
    ...
    public async Task ReturnExisting_AfterDelete_AllowsReuploadWithDifferentProfile()
    {
        ...
        var compressed = await scope.Storage.SaveFileAsync(payload, "compressed.txt", compress: true, ct: TestContext.Current.CancellationToken);
