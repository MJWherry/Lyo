...
Blazor Server / Interactive UI for Lyo.FileStorage. Workbench grids and dialogs for file metadata, expected storage keys, download access links, and two-key encryption keys.

Two integration modes, controlled by FileStorageWorkbenchOptions:
...
| Component | Role |
| ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FileStorageWorkbench (FileStorageWorkbench.razor / .razor.cs) | Top-level workbench with Files / Browser / Keys tabs and registration sanity checks. |
| FileStorageWorkbenchHeader | Title bar and tab selector. |
...
| FileStoreKeysTab (.razor, .razor.cs) | Keystore listing with the current version marked, file count per key, and DEK migrate / rotate controls. |
...
| FileStorageWorkbenchServiceResolver | Looks up IFileStorageService and IKeyStore against the configured key names so the rest of the UI is decoupled from keyed-DI mechanics. |
