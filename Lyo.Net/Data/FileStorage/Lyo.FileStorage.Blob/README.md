...
`Lyo.FileStorage.Blob.Tests` provides isolated unit coverage for `BlobFileStorageOptions` (CPK resolution, section names, base defaults), **`BlobStagedFileUploadService`** (offline SAS PUT generation), and the shared `CloudObjectKeyBuilder`. Path-prefix traversal coverage lives in `Lyo.FileStorage.Tests` against the shared `Lyo.Exceptions.FileHelpers` helper. Live container I/O would need Azurite.
