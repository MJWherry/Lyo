...
public sealed class S3FileStorageService : FileStorageServiceBase, IFileStorageDiagnosticsService, IAsyncDisposable
{
            ...
            auditHandlers, contentPolicy)
    {
        ...
        // Swap generic metric names for the S3-specific set
    ...
    protected override async Task<HealthResult> CheckHealthLightweightAsync(CancellationToken ct)
    {
        ...
        try {
            // GetBucketLocation needs s3:GetBucketLocation, which object-only IAM often omits. GetObject on a missing key uses s3:GetObject
            // (same grant as streamed downloads); 404 means the bucket answered.
    ...
    public override async Task<FileStoreResult> CopyFileAsync(Guid sourceFileId, CopyFileRequest? request = null, CancellationToken ct = default)
    {
        ...
        try {
            ...
            // Destination object exists but metadata did not persist — delete the orphan.
            if (copyCompleted) {
                try {
    ...
    {
        // Try the suffix-based GetObjectKey first (one HEAD). Fall back to FindObjectKeyAsync only when that misses, so the usual path is one
        // round-trip while still accepting legacy or alternate suffixes.
    ...
    protected override async Task<Stream?> ReadFromStorageAsync(Guid fileId, string? pathPrefix, CancellationToken ct)
    {
        ...
        // Dispose of the wrapper also releases the response HTTP handle.
    ...
    {
        // Header stays small (a few KiB even at max key-id/version + wrapped DEK); a range GET is enough — skip downloading the whole ciphertext.
    ...
    protected override async Task UpdateFileHeaderAsync(Guid fileId, string? pathPrefix, string targetKeyId, string targetKeyVersion, byte[] newEncryptedDek, CancellationToken ct)
    {
        ...
        // Write the object to a temp file, rewrite the header, then stream the new payload back to S3 without holding the whole blob in RAM.
        var spoolPath = TempSpool.CreatePath("fs-s3-hdr", fileId);
        try {
            var getRequest = new GetObjectRequest { BucketName = _options.BucketName, Key = objectKey };
            long objectLength;
            await using (var spool = new FileStream(
                ...
                if (newHeaderBytes.Length == oldHeaderSize) {
                    // Header length unchanged — overwrite in place.
                    spool.Position = 0;
                    ...
                    // Header length changed — write a sibling temp file, then replace.
                    var staged = TempSpool.CreatePath("fs-s3-hdr-staged", fileId);
    ...
    {
        // Locate and remove any object for this id (suffix may differ)
        var objectKey = await FindObjectKeyAsync(fileId, pathPrefix, ct).ConfigureAwait(false);
        if (objectKey != null) {
            try {
                var deleteRequest = new DeleteObjectRequest { BucketName = _options.BucketName, Key = objectKey };
    ...
    /// <summary>
    /// Completes a multipart staging object with S3 server-side <c>CopyObject</c> instead of pulling bytes through the client. Hash still
    /// download the staging object once to a local temp file; the upload that <see cref="FileStorageServiceBase.SaveFromStreamAsync" /> would have done is an
    /// in-bucket copy. Only the no-compress/no-encrypt finalize path; use the streaming pipeline when transforms are required.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        var tempPath = TempSpool.CreatePath("fs-s3-mpufin", targetFileId);
        long observedLength;
        byte[] computedHash;
        try {
            ...
            try {
                ...
                // Metadata write failed — drop the destination so the caller's staging delete is the only leftover.
                try {
        ...
        CancellationToken ct)
    {
        ...
        try {
            ...
            // GetPreSignedURL itself is sync; the method stays async so callers can pass a cancellation token.
    ...
    {
        // SourceFileName is already on metadata; skip HEAD on the hinted key. Many IAM policies allow GetObject but deny HeadObject,
        // and S3 returns 403 (not 404) when ListBucket is missing — that used to abort key resolution.
    ...
    /// prefix is missing, or the row is absent. Lets <see cref="FindObjectKeyAsync" /> skip the N+1 HEAD probes.
    /// </summary>
    ...
    private IAmazonS3 CreateS3Client()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_options.ServiceUrl)) {
            ...
            config.ForcePathStyle = true; // Needed for S3-compatible endpoints
        }
