...
- **SFTP physical IO.** Streams via commit-on-close upload and leased read.
- **Path jail.** POSIX paths under `Sftp.RootRemoteDirectory`.
- **DI.** `AddSftpFileStorageService` and `AddSftpFileStorageServiceFromConfiguration`, same shape as the Blob extensions.
