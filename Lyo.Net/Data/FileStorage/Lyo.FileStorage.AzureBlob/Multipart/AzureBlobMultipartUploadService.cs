...
public sealed class AzureBlobMultipartUploadService : IMultipartUploadService
{
    ...
    private readonly ILogger<AzureBlobMultipartUploadService> _logger;
    ...
    public AzureBlobMultipartUploadService(
        ...
        IMetrics? metrics = null)
    {
        ...
        _logger = (loggerFactory ?? NullLoggerFactory.Instance).CreateLogger<AzureBlobMultipartUploadService>();
    }
    ...
    public async Task<MultipartBeginResult> BeginAsync(MultipartBeginRequest request, CancellationToken ct = default)
    {
        ...
        // Azure blocks cap at 4000 MiB; PartSizeBytes is an int (~2 GiB), so it already sits below the platform max.
    ...
    public async Task<FileStoreResult> CompleteAsync(CompleteMultipartUploadRequest request, CancellationToken ct = default)
    {
        ...
        var state = JsonSerializer.Deserialize<AzureBlobProviderState>(session.ProviderStateJson);
    ...
    public async Task AbortAsync(Guid sessionId, CancellationToken ct = default)
    {
        ...
        try {
            if (s.ProviderKind == MultipartUploadProviderKind.AzureBlob) {
                var state = JsonSerializer.Deserialize<AzureBlobProviderState>(s.ProviderStateJson);
    ...
    private sealed class AzureBlobProviderState
    {
