...
> **Keyed / multi-tenant DI** — `Lyo.FileStorage.AzureBlob` does not currently ship the keyed builder pattern that `Lyo.FileStorage.S3` exposes via `AddS3FileStorageServiceKeyed`. Use *
...
**`AzureBlobFileStorageService`** follows the core base type: concurrent calls on one instance should be OK; **`IFileMetadataStore`**, keystores, and **`ILogger`** must be consistent with your DI lifetimes.
