...
/// <summary>FTP-backed <see cref="IFileStorageService" /> (physical IO only; presigned/multipart unsupported).</summary>
public sealed class FtpFileStorageService : FileStorageServiceBase
{
    private readonly IFtpClient _ftp;
    private readonly string _root;

    /// <summary>Creates an FTP file storage service.</summary>
            ...
            auditHandlers, contentPolicy, malwareScanner)
    {
        ...
        _ftp = ftpClient;
        _root = ftpClient.RootRemoteDirectory;
        Logger.LogInformation("Initialized FTP file storage under {Root}", _root);
    }
    ...
    protected override async Task<HealthResult> CheckHealthLightweightAsync(CancellationToken ct)
    {
        ...
        try {
            await _ftp.HealthPingAsync(ct).ConfigureAwait(false);
            sw.Stop();
    ...
    protected override async Task<Stream?> ReadFromStorageAsync(Guid fileId, string? pathPrefix, CancellationToken ct)
    {
        ...
        return path is null ? null : await _ftp.OpenReadAsync(path, ct).ConfigureAwait(false);
    }
    ...
    {
        var path = await FindFilePathAsync(fileId, pathPrefix, ct).ConfigureAwait(false);
        if (path is null || !await _ftp.FileExistsAsync(path, ct).ConfigureAwait(false))
            throw new FileNotFoundException($"File {fileId} not found on FTP; cannot rotate header.", fileId.ToString());

        var bytes = await _ftp.DownloadBytesAsync(path, ct).ConfigureAwait(false);
        if (bytes.Length < 13)
        ...
        dest.Position = 0;
        await _ftp.UploadAsync(path, dest, ct).ConfigureAwait(false);
        Logger.LogDebug("Updated FTP file header for {FileId}", fileId);
    }
    ...
    protected override async Task CleanupPartialFileAsync(Guid fileId, string? pathPrefix, CancellationToken ct)
    {
        try {
            var path = await FindFilePathAsync(fileId, pathPrefix, ct).ConfigureAwait(false);
            if (path is not null && await _ftp.FileExistsAsync(path, ct).ConfigureAwait(false))
                await _ftp.DeleteFileAsync(path, ct).ConfigureAwait(false);
        }
        catch (Exception ex) {
            Logger.LogWarning(ex, "Failed to cleanup partial FTP file {FileId}", fileId);
        }
    }
    ...
    public override async Task<FileStoreResult> CopyFileAsync(Guid sourceFileId, CopyFileRequest? request = null, CancellationToken ct = default)
    {
        ...
        if (src is null)
            throw new FileNotFoundException($"Source FTP file missing for id {sourceFileId}");
        ...
        await _ftp.CopyFileAsync(src, dest, ct).ConfigureAwait(false);
    ...
    public override async Task<FileStoreResult> MoveFileAsync(Guid fileId, MoveFileRequest request, CancellationToken ct = default)
    {
        ...
        await _ftp.RenameAsync(src, dest, ct).ConfigureAwait(false);
    ...
    private async Task<string?> FindFilePathAsync(Guid fileId, string? pathPrefix, CancellationToken ct)
    {
        ...
        if (await _ftp.FileExistsAsync(basePath, ct).ConfigureAwait(false))
            return basePath;
        ...
        foreach (var c in candidates) {
            if (await _ftp.FileExistsAsync(c, ct).ConfigureAwait(false))
                return c;
        }
