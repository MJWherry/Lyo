...
- **FTP physical IO.** Streams via commit-on-close upload and leased read.
- **Path jail.** POSIX paths under `Ftp.RootRemoteDirectory`.
- **DI.** `AddFtpFileStorageService` and `AddFtpFileStorageServiceFromConfiguration`, same shape as the Blob extensions.
