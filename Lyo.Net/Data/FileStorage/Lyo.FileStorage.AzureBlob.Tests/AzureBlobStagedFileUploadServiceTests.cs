...
/// <summary>Staged upload SAS PUT generation via <see cref="AzureBlobStagedFileUploadService" /> (client-side signing; no live Azure call for Begin).</summary>
public sealed class AzureBlobStagedFileUploadServiceTests : IDisposable
{
    ...
    public AzureBlobStagedFileUploadServiceTests()
    {
    ...
    private AzureBlobStagedFileUploadService CreateStaged() => new(_storage, _options, _store, _container);
    ...
    {
        var cpkOptions = new AzureBlobFileStorageOptions {
        ...
        using var cpkStorage = new AzureBlobFileStorageService(cpkOptions, new LocalFileMetadataStore(_metadataRoot), containerClient: cpkContainer);
        var staged = new AzureBlobStagedFileUploadService(cpkStorage, cpkOptions, _store, cpkContainer);
