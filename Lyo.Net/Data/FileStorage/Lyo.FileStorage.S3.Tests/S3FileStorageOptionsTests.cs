...
/// <summary>Pure-data coverage for the <see cref="S3FileStorageOptions" /> surface (binding section and base defaults).</summary>
public sealed class S3FileStorageOptionsTests
{
    [Fact]
    public void SectionName_IsStable()
        =>
            // Public binding contract used by appsettings.json.
            Assert.Equal("S3FileStorageOptions", S3FileStorageOptions.SectionName);

    [Fact]
    public void EnableMetrics_DefaultsToBaseTrue()
    {
        // p5-options-defaults invariant: base defaults stay true for the S3 backend.
    ...
    {
        // Confirms BuildRequiredPutHeaders returns null when no SSE is configured (parity with the default flow).
        var opts = new S3FileStorageOptions { BucketName = "bucket" };
