...
/// <summary>
/// Encode/decode streaming stages shared by every storage backend: single-pass compress-then-encrypt uploads and pipelined decrypt/decompress reads. Persisted
/// ciphertext sizes and headers are read through <see cref="IFileStoragePhysicalIO" />.
/// </summary>
internal sealed class FileStorageStreamingPipelines
{
    ...
    /// <summary>Builds pipelines with polymorphic blob I/O and optional crypto services.</summary>
    ...
    /// <summary>Disposes <paramref name="stream" /> asynchronously when that API exists; otherwise disposes synchronously.</summary>
    ...
    /// <summary>
    /// Writes decrypted and/or decompressed plaintext into <paramref name="plainWriter" /> from <paramref name="metadata" /> compression and encryption flags, using
    /// <see cref="Pipe" /> backpressure so entire files are not buffered in memory.
    /// </summary>
    ...
    /// <summary>Runs the single-pass pipeline that compresses plaintext, pipes the compressed stream, and encrypts concurrently so saves stay memory-efficient.</summary>
        ...
        CancellationToken ct)
    {
        ...
        var encryptionTask = RunEncryptFromPipeReaderAsync(_twoKeyEncryptionService!, pipe, encryptedHashStream, keyId, chunkSize, ct);
    ...
    /// <summary>
    /// Reading stage of the compress-then-encrypt pipeline. Always completes <see cref="Pipe.Reader" /> — on failure with the exception — so the compressing stage's
    /// backpressure-blocked flush is released and the fault travels instead of deadlocking <c>Task.WhenAll</c>.
    /// </summary>
    private static async Task RunEncryptFromPipeReaderAsync(
    ...
    {
        try {
            using (var pipeReaderStream = pipe.Reader.AsStream(true))
                await encryptionService.EncryptToStreamAsync(pipeReaderStream, encryptedHashStream, keyId, null, chunkSize, ct).ConfigureAwait(false);
    ...
    /// <summary>
    /// Reading stage of the decrypt-then-decompress pipeline. Always completes <see cref="Pipe.Reader" /> — on failure with the exception — so the decrypting stage's
    /// backpressure-blocked flush is released and the fault travels instead of deadlocking <c>Task.WhenAll</c>.
    /// </summary>
    private async Task DecompressCompressedPipeToPlainAsync(Pipe pipeCompressed, PipeWriter plainWriter, CompressionAlgorithm algorithm, int? chunkSize, CancellationToken ct)
    {
        try {
            ...
            await pipeCompressed.Reader.CompleteAsync().ConfigureAwait(false);
            await plainWriter.CompleteAsync().ConfigureAwait(false);
        }
        catch (Exception ex) {
            await pipeCompressed.Reader.CompleteAsync(ex).ConfigureAwait(false);
            await plainWriter.CompleteAsync(ex).ConfigureAwait(false);
            throw;
        }
    }

    /// <summary>Hashes, sizes, crypto metadata, and filenames captured by <see cref="SaveWithCompressEncryptPipelineAsync" />.</summary>
