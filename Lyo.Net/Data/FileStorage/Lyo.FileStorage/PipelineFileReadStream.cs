...
/// <summary>Wraps a <see cref="PipeReader" /> stream backed by an in-flight async pipeline. Cancels and awaits that pipeline on dispose.</summary>
internal sealed class PipelineFileReadStream : Stream
{
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing && !_disposed) {
            ...
            // Best-effort wait so the pipeline sees cancellation and releases resources before this Stream returns.
            // Bounded so sync-over-async callers never hang forever. Prefer DisposeAsync when you can wait cleanly.
            try {
                _pipelineTask.Wait(TimeSpan.FromSeconds(5));
            }
            catch (Exception ex) when (IsExpectedPipelineTeardownException(ex)) {
                // Expected when cancelled or the reader closed early.
            }
    ...
    /// <summary>True when the exception is a cancellation-shaped failure that teardown should swallow.</summary>
