...
/// <summary>In-memory multipart session store for tests and single-node work without PostgreSQL.</summary>
public sealed class InMemoryMultipartUploadSessionStore : IMultipartUploadSessionStore
{
    ...
    {
        var updated = _sessions.AddOrUpdate(
            sessionId, _ => throw new NotFoundException($"Cannot set status for missing multipart session {sessionId}."), (_, existing) => existing with { Status = status });
