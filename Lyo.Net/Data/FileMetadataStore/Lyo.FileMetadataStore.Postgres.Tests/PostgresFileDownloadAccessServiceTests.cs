...
public class PostgresFileDownloadAccessServiceTests
{
    ...
    private static FileStoreResult CreateMetadata(Guid id, byte[] hash)
        => new(
            ...
            HashAlgorithm.Sha256, "text/plain", null, FileAvailability.Available);
    ...
    public async Task ValidateAndConsume_NotBeforeInFuture_Denies()
    {
        ...
        var created = await service.CreateLinkAsync(new(fileId, NotBeforeUtc: now.AddMinutes(5)), TestContext.Current.CancellationToken);
    ...
    public async Task ValidateAndConsume_MaxDownloads_EnforcedUnderConcurrency()
    {
        ...
        var tasks = Enumerable.Range(0, 20)
            .Select(_ => service.ValidateAndConsumeDownloadAsync(created.Token, nowUtc: DateTime.UtcNow, ct: TestContext.Current.CancellationToken));
