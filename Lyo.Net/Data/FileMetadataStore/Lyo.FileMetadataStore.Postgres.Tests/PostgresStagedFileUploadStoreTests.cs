...
public sealed class PostgresStagedFileUploadStoreTests
{
    private readonly FileMetadataPostgresFixture _fixture;

    public PostgresStagedFileUploadStoreTests(FileMetadataPostgresFixture fixture) => _fixture = fixture;

    private PostgresStagedFileUploadStore CreateStore()
    {
        Assert.NotNull(_fixture.ServiceProvider);
        var factory = _fixture.ServiceProvider.GetRequiredService<IDbContextFactory<FileMetadataStoreDbContext>>();
        return new(factory);
    }
    ...
    private static StagedFileUploadRecord SampleRecord(Guid? stageId = null)
    {
        ...
        return new(
            ...
            "application/octet-stream", 1024, null, null, HashAlgorithm.Sha256, MultipartUploadProviderKind.AwsS3, "{}", null, null);
    }
