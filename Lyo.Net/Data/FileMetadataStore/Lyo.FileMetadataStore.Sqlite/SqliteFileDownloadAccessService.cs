...
public sealed class SqliteFileDownloadAccessService : IFileDownloadAccessService
{
    ...
    private readonly ILogger<SqliteFileDownloadAccessService> _logger;

    public SqliteFileDownloadAccessService(SqliteFileMetadataStoreDbContext dbContext, ILockService lockService, ILoggerFactory? loggerFactory = null)
    {
        ArgumentHelpers.ThrowIfNull(dbContext);
        ArgumentHelpers.ThrowIfNull(lockService);
        _dbContext = dbContext;
        _lockService = lockService;
        _logger = (loggerFactory ?? NullLoggerFactory.Instance).CreateLogger<SqliteFileDownloadAccessService>();
    }
    ...
    public async Task<CreateFileDownloadAccessLinkResult> CreateLinkAsync(CreateFileDownloadAccessLinkRequest request, CancellationToken ct = default)
    {
        ...
        var fileExists = await _dbContext.FileMetadata.AsNoTracking().AnyAsync(i => i.Id == request.FileId.ToString() && i.DeletedAt == null, ct).ConfigureAwait(false);
        if (!fileExists)
            throw new FileNotFoundException($"Cannot create access link because file metadata does not exist for fileId '{request.FileId}'.");

        var (token, tokenHash) = FileDownloadAccessToken.Create();
    ...
    {
        var tokenHash = FileDownloadAccessToken.TryHash(token);
        if (tokenHash == null)
            return await FailAsync(FileDownloadAccessConsumeFailureReason.InvalidToken, null, actorId, ipAddress, "token_invalid", ct).ConfigureAwait(false);

        var lockKey = FileDownloadAccessToken.LockKey(tokenHash);
        ...
        var fileStillActive = await _dbContext.FileMetadata.AsNoTracking().AnyAsync(m => m.Id == link.FileId.ToString() && m.DeletedAt == null, ct).ConfigureAwait(false);
        if (!fileStillActive)
        ...
        var updateCount = await _dbContext.Database.ExecuteSqlInterpolatedAsync(
                $"""
                 UPDATE file_download_access_links
