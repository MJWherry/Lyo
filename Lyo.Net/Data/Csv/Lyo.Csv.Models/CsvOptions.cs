...
/// <summary>
/// Service-level options for CSV dialect, encoding, and DataTable value pooling. CSV starts with value pooling off (unique-heavy grids); enable via <see cref="Pooling" />
...
public sealed class CsvOptions
{
    ...
    /// <summary>Field delimiter (single character as string). Starts as <c>,</c>.</summary>
    public string Delimiter { get; set; } = ",";

    /// <summary>If true, the first row is treated as column headers for typed mapping and DataTable imports (unless overridden).</summary>
    public bool HasHeaderRecord { get; set; } = true;

    /// <summary>If true, blank lines are skipped while reading.</summary>
    public bool IgnoreBlankLines { get; set; } = true;

    /// <summary>If true, field values are trimmed of leading/trailing whitespace after parse.</summary>
    public bool TrimFields { get; set; } = true;

    /// <summary>If true, rows whose field count differs from the first data/header row throw <see cref="CsvBadDataException" />.</summary>
    public bool DetectColumnCountChanges { get; set; } = true;

    /// <summary>Culture used for typed value conversion.</summary>
    public CultureInfo Culture { get; set; } = CultureInfo.InvariantCulture;

    /// <summary>Text encoding for file/stream read and write. Starts as UTF-8.</summary>
    public Encoding Encoding { get; set; } = new UTF8Encoding(false);

    /// <summary>Quote character wrapping fields that contain delimiter, quote, escape, or newlines. Starts as <c>"</c>.</summary>
    ...
    /// <summary>Comment line marker when <see cref="AllowComments" /> is enabled. Starts as <c>#</c>.</summary>
