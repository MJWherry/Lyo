...
Interfaces and value types for the Lyo CSV stack. [`Lyo.Csv`](../Lyo.Csv/README.md) implements this contract so consumers can depend on ICsvService, ICsvReader, and ICsvWriter without the implementation package.

## Interfaces

- ICsvService. Combined reader and writer with validate, compare, split, and combine methods. Exposes Writer, Reader, SetEncoding(Encoding), and SetOptions(CsvOptions).
...
- ICsvValueConverter. Cell text to CLR conversion contract.
- CsvBadDataException. Malformed CSV or conversion failures.
...
- CsvSchema + CsvColumn. Expected columns for ValidateAsync.
- ColumnMapping. Explicit column-name to property mapping for ParseFileWithMappingAsync / ParseStreamWithMappingAsync.
- CsvParseError. Error metadata passed to CsvParseOptions.OnError.
- CsvParseResult / CsvExportResult. Result envelopes used by the higher-level helpers.
- CsvStatistics. Row and column counts from GetStatisticsAsync.
