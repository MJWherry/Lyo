...
/// <summary>
/// Incremental CSV field parser over a <see cref="TextReader" />. Reads from a rented char buffer; <see cref="ReadRow" /> / <see cref="ReadRowAsync" /> return a reused list
/// that is valid only until the next read.
/// </summary>
...
#endif
{
    ...
    /// <summary>Joined-field snapshot for error text (not assembled on the hot path).</summary>
    public string? RawRecord => _row.Count == 0 ? null : string.Join(_options.Delimiter, _row);

    /// <summary>1-based physical row index; advances after each successful data row.</summary>
    ...
    /// <summary>Advances to the next data row. Returns the reused field list, or null at EOF. Copy the list to keep it past the next read.</summary>
...
#if !NETSTANDARD2_0
    /// <summary>Asynchronously advances to the next data row. Returns the reused field list, or null at EOF.</summary>
    ...
    private bool ReadRowSync()
    {
        ...
        while (true) {
            ...
            if (atLineStart && !inQuotes && _options.AllowComments) {
                if (ch is ' ' or '\t') {
                    // skip leading whitespace before a comment marker
                }
    ...
    {
        // Parse from the current buffer first; issue one ReadAsync only when the buffer is empty.
        var inQuotes = false;
        var fieldStarted = false;
        var any = false;
        var atLineStart = true;
