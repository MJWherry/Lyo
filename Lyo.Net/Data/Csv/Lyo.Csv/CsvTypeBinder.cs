...
/// <summary>Per-type property maps used for typed CSV bind and emit.</summary>
internal static class CsvTypeBinder
{
    ...
    /// <summary>Installs a converter for <paramref name="type" />, replacing any built-in mapping.</summary>
    ...
    /// <summary>Emits one typed row cell-by-cell without allocating an intermediate string array.</summary>
    ...
    /// <summary>Called from compiled setters to coerce cell text onto <paramref name="targetType" />.</summary>
    ...
    /// <summary>Fallback formatter used by compiled writers for types without a dedicated converter.</summary>
    ...
    private static object? ConvertTo(Type targetType, string? text, CultureInfo culture)
    {
        ...
        // Guid/enum parse is culture-invariant via the shared converter; numerics below honor the caller culture.
        if (underlying == typeof(Guid) || underlying.IsEnum)
            return TypeConversion.TryConvertTo(text, underlying, out var converted) ? converted : null;
