...
[BenchmarkDescription("Serializes and parses a NestedRecord whose Address reference member itself nests a Geo coordinate. The CSV writer flattens these nested objects into leaf columns, so this captures the round-trip cost and the data structure that a flat row count alone hides.")]
[BenchmarkParameter("RowCount", Unit = "rows", Description = "Number of NestedRecord rows round-tripped (1,000 or 10,000).")]
[BenchmarkDataShape(typeof(NestedRecord), Notes = "Record with a nested Address object that itself nests a Geo coordinate (nesting depth 2); the CSV writer flattens these into leaf columns.")]
        ...
        for (var i = 0; i < count; i++)
            rows.Add(new NestedRecord {
                ...
                Address = new NestedAddress {
                    ...
                    Geo = new NestedGeo { Latitude = 51.5 + i % 10 * 0.01, Longitude = -0.12 - i % 10 * 0.01 },
                },
            });
