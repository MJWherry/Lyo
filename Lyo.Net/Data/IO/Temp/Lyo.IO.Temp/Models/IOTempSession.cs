...
/// <summary>Stock <see cref="IIOTempSession" /> implementation.</summary>
...
public sealed class IOTempSession : IIOTempSession
{
    ...
    /// <summary>UTF-8 with no BOM so byte counts match <see cref="Encoding.GetByteCount(string)" /> and stream round-trips.</summary>
    ...
    /// <summary>Opens a session under <see cref="IOTempSessionOptions.RootDirectory" /> and creates the session folder on disk.</summary>
    /// <param name="options">Layout, caps, and naming; defaults when null.</param>
    /// <param name="logger">Optional logger.</param>
    /// <param name="metrics">Used when <see cref="IOTempSessionOptions.EnableMetrics" /> is true.</param>
    /// <param name="onDispose">Optional callback with <see cref="SessionDirectory" /> after dispose is attempted (for example service unregistration).</param>
    /// <param name="storageProvider">Storage backend; default is the filesystem under the session's logical root.</param>
        ...
        IFileSystem? storageProvider = null)
    {
        ...
        _storage = storageProvider ?? new LocalFileSystem(_options.RootDirectory);
    ...
    /// <inheritdoc />
    public event Action<string>? FileAppended;
    ...
    /// <inheritdoc />
    public event Action<string>? Overflow;

    /// <inheritdoc />
    public event Action<string>? Cleared;

    /// <inheritdoc />
    public event Action<string>? Disposed;
...
#region Sub-sessions
    ...
    /// <summary>
    /// Test helper: session under <c>{TempPath}/lyo-io-temp-tests/{subdirectoryName ?? new Guid}</c>. The root is created so tests need not call
    /// <see cref="Directory.CreateDirectory(string)" /> first. Passing the test class or method as <paramref name="subdirectoryName" /> makes leftover folders from a failed CI
    /// run easy to find.
    /// </summary>
    /// <param name="subdirectoryName">Stable parent folder name; a new GUID is used when null or whitespace.</param>
    /// <param name="logger">Optional logger.</param>
    /// <param name="metrics">Optional metrics sink.</param>
    /// <param name="storageProvider">Optional storage; filesystem is the default.</param>
...
#region Discovery
    ...
    {
        ThrowIfDisposed();
        // Provider enumeration lists every descendant; apply a glob here if the caller passed one.
    ...
    {
        foreach (var entry in _storage.ListDirectory(dir)) {
            if (!entry.IsDirectory)
                yield return entry.Path;
            else {
                foreach (var sub in EnumerateAllFiles(entry.Path))
                    yield return sub;
            }
        }
    }
    ...
    private IEnumerable<string> EnumerateAllDirectories(string dir)
    {
        foreach (var entry in _storage.ListDirectory(dir)) {
            ...
            foreach (var sub in EnumerateAllDirectories(entry.Path))
                yield return sub;
        }
    }
    ...
    {
        // Simple * wildcards, matching Directory.EnumerateFiles pattern rules.
        if (pattern == "*")
            return true;
        ...
        // Turn a glob into a simple regex-style match.
...
#region Delete operations
    ...
    public bool DeleteFile(string path)
    {
        ...
        try {
            ...
            var size = _files.Contains(safePath) ? _storage.GetLength(safePath) : 0L;
    ...
    public bool DeleteDirectory(string path)
    {
        ...
        try {
            ...
            var freedBytes = filesUnder.Sum(f => _storage.FileExists(f) ? _storage.GetLength(f) : 0L);
            ...
            _metrics.IncrementCounter(Constants.Metrics.DeleteDirectorySuccess);
            DirectoryDeleted?.Invoke(safePath);
            return true;
        }
        catch (Exception ex) {
...
#region Move operations
    ...
    public string MoveFrom(string sourcePath)
    {
        ...
        try {
            string dest;
            if (IOTempExternalIo.FileExists(_storage, sourcePath))
                dest = MoveFileFrom(sourcePath);
            else if (IOTempExternalIo.DirectoryExists(_storage, sourcePath))
    ...
    {
        var size = IOTempExternalIo.GetLength(_storage, sourcePath);
        ...
        if (IOTempExternalIo.IsOnStorage(_storage, sourcePath))
            _storage.Move(sourcePath, dest);
        else {
            CopyHostFile(sourcePath, dest);
...
#region Write (overwrite) operations
    ...
    public string WriteFile(string path, string text)
    {
        ...
        try {
            ...
            var oldSize = _storage.GetLength(safePath);
...
#region Session mutation
    ...
    public void Clear()
    {
        ...
        foreach (var dir in deletedDirs)
            DirectoryDeleted?.Invoke(dir);

        Cleared?.Invoke(SessionDirectory);
    }
    ...
    private async Task<string> CopyFileFromAsync(string sourcePath, CancellationToken ct)
    {
        ...
        if (IOTempExternalIo.IsOnStorage(_storage, sourcePath))
            ...
            await CopyHostFileAsync(sourcePath, dest, ct).ConfigureAwait(false);
        _files.Add(dest);
        Interlocked.Add(ref _totalBytesUsed, size);
        FileCreated?.Invoke(dest);
        return dest;
    ...
    private async Task CopyDirectoryRecursiveAsync(string src, string dst, CancellationToken ct)
    {
        ...
        if (!IOTempExternalIo.IsOnStorage(_storage, src)) {
            CopyDirectoryRecursiveFromHost(src, dst);
            return;
        }

        foreach (var entry in _storage.ListDirectory(src)) {
            ct.ThrowIfCancellationRequested();
            if (entry.IsDirectory) {
                var destSub = PathHelpers.Combine(_storage.PathStyle, dst, PathHelpers.GetFileName(_storage.PathStyle, entry.Path));
                _directories.Add(destSub);
                DirectoryCreated?.Invoke(destSub);
                await CopyDirectoryRecursiveAsync(entry.Path, destSub, ct).ConfigureAwait(false);
            }
            else {
                ValidateFileSize(entry.Length);
                var destFile = PathHelpers.Combine(_storage.PathStyle, dst, PathHelpers.GetFileName(_storage.PathStyle, entry.Path));
                await _storage.CopyFileAsync(entry.Path, destFile, ct).ConfigureAwait(false);
                _files.Add(destFile);
    ...
    private void CopyDirectoryRecursive(string src, string dst)
    {
        ...
        foreach (var entry in _storage.ListDirectory(src)) {
            if (entry.IsDirectory) {
                ...
                _directories.Add(destSub);
                DirectoryCreated?.Invoke(destSub);
                CopyDirectoryRecursive(entry.Path, destSub);
            }
            else {
                ValidateFileSize(entry.Length);
                var destFile = PathHelpers.Combine(_storage.PathStyle, dst, PathHelpers.GetFileName(_storage.PathStyle, entry.Path));
                _storage.CopyFile(entry.Path, destFile);
                _files.Add(destFile);
                Interlocked.Add(ref _totalBytesUsed, entry.Length);
                FileCreated?.Invoke(destFile);
            }
    ...
    private void CopyDirectoryRecursiveFromHost(string src, string dst)
    {
        _storage.CreateDirectory(dst);
        foreach (var file in Directory.GetFiles(src)) {
            var destFile = PathHelpers.Combine(_storage.PathStyle, dst, Path.GetFileName(file));
            var length = new FileInfo(file).Length;
            ValidateFileSize(length);
            CopyHostFile(file, destFile);
            _files.Add(destFile);
            Interlocked.Add(ref _totalBytesUsed, length);
            FileCreated?.Invoke(destFile);
        }

        foreach (var dir in Directory.GetDirectories(src)) {
            var destSub = PathHelpers.Combine(_storage.PathStyle, dst, Path.GetFileName(dir));
            _directories.Add(destSub);
            DirectoryCreated?.Invoke(destSub);
            CopyDirectoryRecursiveFromHost(dir, destSub);
        }
    }

    private void CopyHostFile(string sourcePath, string dest)
    {
        using var src = File.OpenRead(sourcePath);
        using var dst = _storage.OpenCreate(dest);
        src.CopyTo(dst);
    }

    private async Task CopyHostFileAsync(string sourcePath, string dest, CancellationToken ct)
    {
        using var src = File.OpenRead(sourcePath);
        using var dst = await _storage.OpenCreateAsync(dest, ct).ConfigureAwait(false);
...
#region Disposal
    ...
    /// <summary>
    /// Removes <see cref="SessionDirectory" /> with retries on short-lived I/O errors, records metrics, and runs the construction-time <c>onDispose</c> callback. Delete
    /// failures are logged; the session is still treated as disposed.
    /// </summary>
    public void Dispose()
    {
        if (_disposed)
        ...
        try {
            ...
            Disposed?.Invoke(SessionDirectory);
        }
    }

    /// <summary>Same as <see cref="Dispose" />; returns a completed value task.</summary>
...
#region Helpers
    ...
    private void FreeFileCountFor(int requiredSlots)
    {
        ...
        var candidates = _options.OverflowStrategy == TempOverflowStrategy.DeleteOldest
            ? _files.Where(_storage.FileExists).OrderBy(_storage.GetCreationTimeUtc).ToList()
            : _files.Where(_storage.FileExists).OrderByDescending(_storage.GetLength).ToList();
        ...
        foreach (var file in candidates) {
            ...
            var size = _storage.FileExists(file) ? _storage.GetLength(file) : 0L;
            try {
                _storage.DeleteFile(file);
                _files.Remove(file);
                Interlocked.Add(ref _totalBytesUsed, -size);
                freed++;
                Overflow?.Invoke(file);
                FileDeleted?.Invoke(file);
    ...
    private void FreeTotalSpaceFor(long requiredBytes)
    {
        ...
        var candidates = _options.OverflowStrategy == TempOverflowStrategy.DeleteOldest
            ? _storage.ListDirectory(SessionDirectory).Where(e => !e.IsDirectory).OrderBy(e => e.CreatedAtUtc).ToList()
            : _storage.ListDirectory(SessionDirectory).Where(e => !e.IsDirectory).OrderByDescending(e => e.Length).ToList();
        ...
        foreach (var entry in candidates) {
            ...
            try {
                ...
                Interlocked.Add(ref _totalBytesUsed, -entry.Length);
                freed += entry.Length;
                Overflow?.Invoke(entry.Path);
                FileDeleted?.Invoke(entry.Path);
                _logger.LogInformation(
                    "Deleted {FilePath} ({Size:N0} bytes) to make room for overflow in session {SessionDirectory}", entry.Path, entry.Length, SessionDirectory);
            }
            catch (Exception ex) {
                _logger.LogWarning(ex, "Failed to delete {FilePath} during overflow cleanup in session {SessionDirectory}", entry.Path, SessionDirectory);
            }
        }
