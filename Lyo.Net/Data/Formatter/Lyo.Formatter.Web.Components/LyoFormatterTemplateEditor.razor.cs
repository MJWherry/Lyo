...
/// <summary>Debounced template text box with an in-place colored overlay of <c>{placeholders}</c>. Pair with <see cref="LyoFormatterPreview" /> through <see cref="LyoFormatterLiveSession" />. Typing <c>{</c> opens a caret-anchored dropdown of context keys and nested object properties.</summary>
public partial class LyoFormatterTemplateEditor : IAsyncDisposable
{
    ...
    /// <summary>Shared session. Takes precedence over a cascaded session when both are set.</summary>
    [Parameter]
    public LyoFormatterLiveSession? Session { get; set; }

    /// <summary>Optional cascaded session so a parent can wrap distant editor/preview pairs in one place.</summary>
    [CascadingParameter]
    public LyoFormatterLiveSession? CascadedSession { get; set; }

    /// <summary>Field caption. Empty hides the label.</summary>
    [Parameter]
    public string Label { get; set; } = "Template";

    /// <summary>Hint shown when the template is empty.</summary>
    [Parameter]
    public string Placeholder { get; set; } = "{Name} — {Count}";

    /// <summary>Visible rows for the textarea. Starts as 6.</summary>
    [Parameter]
    public int Lines { get; set; } = 6;

    /// <summary>Milliseconds to wait after typing before the paired preview rebuilds. Written to <see cref="LyoFormatterLiveSession.DebounceInterval" />.</summary>
    [Parameter]
    public int DebounceInterval { get; set; } = LyoFormatterLiveSession.DefaultDebounceMs;

    /// <summary>When true, the textarea cannot be edited.</summary>
    [Parameter]
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_jsModule != null) {
            try {
                ...
                // element may already have left the DOM
            }
    ...
    /// <summary>Keyboard handler for the context-key dropdown. Invoked from the editor JS module so Arrow/Enter/Tab can preventDefault.</summary>
    [JSInvokable]
    ...
    /// <summary>Template changed in the textarea. Caret is captured in JS before Blazor re-renders so overlay updates cannot move the insertion point.</summary>
    [JSInvokable]
    ...
    /// <summary>Repositions the floating dropdown when the editor or window scrolls or resizes.</summary>
    [JSInvokable]
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (_pendingCaret >= 0) {
            ...
            try {
                ...
                // caret restore failed; next render will retry
            }
        }
    ...
    private async Task PositionSuggestAsync()
    {
        ...
        try {
            ...
            // suggestion list not mounted yet
        }
    }
    ...
    private async Task ScrollActiveSuggestionAsync()
    {
        ...
        try {
            ...
            // suggestion list may already be gone
        }
    }
    ...
    private async Task EnsureJsAsync()
    {
        ...
        try {
            ...
            // textarea ref not ready yet (prerender); try again on the next render
        }
    }
