...
| Component | Purpose |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`LyoFormatterTemplateEditor`** | Template textarea with an in-place colored overlay of `{placeholders}`. Typing `{` opens a dropdown of session context keys; object/dictionary values also list nested properties (`User.First`). Typing updates the overlay immediately; preview rebuilds after `DebounceInterval` (default 300 ms). Chip row under the field is hoverable so keys link to preview values. `Session` parameter wins over a cascaded session. |
...
- **Autocomplete** — `LyoFormatterContextCatalog` walks `Session.Context` (dictionary keys or public properties, nested up to 3 levels). Typing `{` lists keys and nested object properties (`User`, `User.First`). Arrow/Enter/Tab/Escape move and insert.
