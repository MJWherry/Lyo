...
public class FtpClientLeaseTests
{
    [Fact]
    [Trait("Category", "Integration")]
    public async Task CancelWhileWaitingForLease_ThrowsAndDoesNotLeakPermit()
    {
        var host = await FtpTestHost.TryStartAsync(TestContext.Current.CancellationToken, maxPooledClients: 1);
        if (host is null) {
    ...
    {
        var host = await FtpTestHost.TryStartAsync(TestContext.Current.CancellationToken, maxPooledClients: 2);
        if (host is null) {
            Assert.Skip("Docker/Testcontainers unavailable");
            return;
        }
