...
**File storage reads:** register this factory if any stored files may have `CompressionAlgorithm` = LZ4; [
`ICompressionResolver`](../Lyo.Compression/README.md#icompressionresolver-per-algorithm-dispatch) dispatches by metadata on `GetFileAsync` / `GetFileStreamAsync`.
