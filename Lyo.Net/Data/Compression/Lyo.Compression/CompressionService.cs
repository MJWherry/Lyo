...
/// <summary>
/// Default <see cref="ICompressionService" /> backed by registered <see cref="ICompressorFactory" /> instances, with optional
...
public sealed class CompressionService : ICompressionService, ICompressionResolver
{
        ...
        ICompressionAlgorithmSelector? algorithmSelector = null)
    {
        ...
        // Last-writer-wins if two factories advertise the same algorithm; addons can override built-in implementations.
    ...
    {
        // FileStream already buffers internally; BufferedStream adds another layer
        // that helps small reads and cuts system calls.
    ...
    {
        // Some compressors check whether the output stream is readable, so use ReadWrite access
        // even though we only write. BufferedStream adds another buffering layer
        // that cuts system calls and improves throughput.
    ...
    /// <summary>
    /// Runs an atomic file operation by writing to a temporary file first, then renaming it onto the target. If the operation fails, no
    /// partial file is left at the target path.
    /// </summary>
    private static void AtomicFileOperation(string targetFilePath, Action<string> writeOperation)
    {
        // GUID-based temp name to avoid clashes with existing files.
    ...
    /// <summary>Runs an atomic file operation asynchronously by writing to a temporary file first, then renaming it onto the target.</summary>
    ...
    /// <summary>
    /// Replaces <paramref name="targetFilePath" /> with <paramref name="tempFilePath" /> in one rename where the platform supports it. On net10+ this is
    ...
    private static void ValidateFilePath(string? filePath, string paramName)
    {
        ...
        // Canonicalize the path to block directory traversal (e.g., ../../../etc/passwd).
        // Resolves relative paths and normalizes directory separators.
    ...
    /// <summary>Encoding by name, falling back to UTF-8 when the name is invalid or missing.</summary>
    ...
    private CompressionInfo CompressWithCompressor(byte[] bytes, ICompressor compressor, bool binaryCompressMatchesStreamFormat, out byte[] compressed)
    {
        ...
        try {
            if (binaryCompressMatchesStreamFormat) {
                // Fast path: the binary API emits the same wire format the stream Decompress path reads; for block codecs (LZ4/Zstd) it is a direct
                ...
                // consistent wire format per algorithm. Presize the output near worst case to skip MemoryStream doubling-growth copies.
    ...
    private DecompressionInfo DecompressWithCompressor(byte[] compressedBytes, ICompressor compressor, out byte[] decompressed)
    {
        ...
        try {
            ...
            // checking size. Presizing the output at the compressed length (output is almost always at least that big) skips most doubling-growth copies.
    ...
    private void CompressWithCompressor(Stream inputStream, Stream outputStream, ICompressor compressor)
    {
        ...
        try {
            // Rewind if the stream supports seeking.
    ...
    private void DecompressWithCompressor(Stream inputStream, Stream outputStream, ICompressor compressor)
    {
        ...
        try {
            ...
            // Stop decompression bombs mid-flight (works for non-seekable outputs too, unlike an after-the-fact position check).
    ...
    public FileDecompressionInfo DecompressFile(string inputFilePath, string? outputFilePath = null)
    {
        ...
        try {
            ...
            // The streaming bomb guard (MaxLengthStream) already applied MaxInputSize during decompression.
    ...
    public async Task<FileCompressionInfo> CompressFileAsync(string inputFilePath, string? outputFilePath = null, CancellationToken ct = default)
    {
        ...
        var originalSize = originalFileInfo.Length;
        try {
            // Pick chunk size from file size.
    ...
    public bool IsLikelyCompressed(byte[]? data) => TryDetectAlgorithm(data, out _);
    ...
    public bool TryDetectAlgorithm(byte[]? data, out CompressionAlgorithm? algorithm)
    {
        ...
        // Each algorithm owns its magic prefixes, and only loaded assemblies register, so detection covers exactly the addons the host pulled in.
        // Raw Deflate and Brotli declare no prefixes and are therefore never detected — they have no header to match.
        algorithm = CompressionAlgorithm.Detectable.FirstOrDefault(a => a.MatchesMagic(data));
    ...
    public async Task CompressStringToStreamAsync(string text, Stream outputStream, Encoding? encoding = null, CancellationToken ct = default)
    {
        ...
        try {
            var bytes = encoding.GetBytes(text);
            // Pick chunk size from string data size.
    ...
    public async Task<string> DecompressStringFromStreamAsync(Stream inputStream, Encoding? encoding = null, CancellationToken ct = default)
    {
        ...
        try {
            ...
            // Pick chunk size from stream size when available.
            ...
            // Since we control MemoryStream creation, use GetBuffer() to skip an extra copy,
            // but honor Length so we do not read past the actual data.
            var length = (int)outputStream.Length;

            // MemoryStream.GetBuffer() returns the underlying buffer, which may be larger than Length.
            // That skips the copy ToArray() would create.
            // Use GetString with offset and count to avoid extra allocations (.NET Standard 2.0 compatible).
            if (outputStream is MemoryStream memStream) {
                var buffer = memStream.GetBuffer();
                // GetString(byte[], int, int) is available on .NET Standard 2.0.
                // Still avoids the ToArray() copy while staying compatible.
            ...
            // Fallback for non-MemoryStream (should not happen; defensive).
