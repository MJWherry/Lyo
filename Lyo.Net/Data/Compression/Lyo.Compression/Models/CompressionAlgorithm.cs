...
/// <summary>
/// Extensible identifier for a stream compression algorithm. Use the built-in static fields (<see cref="GZip" />, <see cref="Deflate" />, <see cref="Brotli" />, <see cref="ZLib" />)
/// for algorithms shipped in the base <c>Lyo.Compression</c> package; addon packages (e.g. <c>Lyo.Compression.LZ4</c>) define their own subclass and a <c>static readonly Instance</c>
/// singleton.
/// </summary>
/// <remarks>
/// <para>
/// Replaces the closed <c>enum CompressionAlgorithm</c>. The base record carries the algorithm <see cref="Name" /> and default file <see cref="Extension" />; addon packages expose
/// their own <c>CompressionAlgorithm</c>-derived record so consumers can pattern-match by type (e.g. <c>algorithm is Lz4CompressionAlgorithm</c>) and so addons can self-register
...
public abstract record CompressionAlgorithm
{
    ...
    public static CompressionAlgorithm? TryFromExtension(string? extension)
        => !string.IsNullOrEmpty(extension) && _byExt.TryGetValue(extension!, out var algo) ? algo : null;
    ...
    public static CompressionAlgorithm? TryFromName(string? name)
        => !string.IsNullOrEmpty(name) && _byName.TryGetValue(name!, out var algo) ? algo : null;
...
public sealed record BuiltInCompressionAlgorithm : CompressionAlgorithm
{
    ...
    /// <param name="extension">Default file extension including the leading dot.</param>
    public BuiltInCompressionAlgorithm(string name, string extension) : base(name, extension) { }
}
