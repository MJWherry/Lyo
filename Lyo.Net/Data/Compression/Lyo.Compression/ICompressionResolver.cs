...
/// <see cref="CompressionServiceOptions.Default" /> when built without explicit options.
/// </summary>
public interface ICompressionResolver
{
    /// <summary>Cached <see cref="ICompressor" /> for <paramref name="algorithm" /> at <paramref name="level" /> (null = configured default level).</summary>
    ICompressor GetCompressor(CompressionAlgorithm algorithm, CompressionLevel? level = null);

    /// <summary>Compresses <paramref name="bytes" /> with <paramref name="algorithm" /> at an optional per-call <paramref name="level" />.</summary>
    CompressionInfo Compress(byte[] bytes, CompressionAlgorithm algorithm, out byte[] compressed, CompressionLevel? level = null);

    /// <summary>Decompresses <paramref name="compressedBytes" /> using <paramref name="algorithm" />.</summary>
    DecompressionInfo Decompress(byte[] compressedBytes, CompressionAlgorithm algorithm, out byte[] decompressed);

    /// <summary>Compresses streams with <paramref name="algorithm" /> at an optional per-call <paramref name="level" />.</summary>
    void Compress(Stream inputStream, Stream outputStream, CompressionAlgorithm algorithm, CompressionLevel? level = null);

    /// <summary>Decompresses streams using <paramref name="algorithm" />.</summary>
    void Decompress(Stream inputStream, Stream outputStream, CompressionAlgorithm algorithm);

    /// <summary>Asynchronously compresses streams with <paramref name="algorithm" /> at an optional per-call <paramref name="level" />.</summary>
    Task CompressAsync(
        Stream inputStream,
        Stream outputStream,
    ...
    /// <summary>Asynchronously decompresses streams using <paramref name="algorithm" />.</summary>
