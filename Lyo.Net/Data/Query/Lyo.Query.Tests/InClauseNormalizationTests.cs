...
/// <summary>
/// Regression coverage for In/NotIn values passed as a collection. C# overload resolution picks the <c>params T[]</c> overload for <c>In(field, someList)</c> (with T inferred
...
public class InClauseNormalizationTests : WhereClauseServiceTests
{
    ...
    {
        // Payload shape from clients built before the builder fix: value is [["Alice","Bob"]].
        var svc = CreateService();
