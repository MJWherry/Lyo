...
/// <summary>Process-wide in-memory caches for entity metadata, shared by BaseWhereClauseService and ProjectionService.</summary>
public static class SharedEntityMetadataCache
{
    ...
    /// <summary>Resolves a property by name. Used by query and projection services.</summary>
    public static PropertyInfo? ResolveProperty(Type type, string name)
        => PropertyCache.GetOrAdd($"{MetadataCacheKeys.PropertyPrefix}{type.FullName}:{name}", _ => ResolvePropertyCore(type, name));

    /// <summary>Normalizes a dotted field path to canonical property names on <paramref name="rootType" />. Used by projection and query services.</summary>
    /// <param name="rootType">Entity type at the root of the path.</param>
    /// <param name="path">Dotted path (may include <c>*</c> or collection <c>count</c> segments as supported by projection).</param>
    /// <returns>Normalized dotted path using actual CLR property names.</returns>
    /// <exception cref="ArgumentException">Raised when a segment is invalid or not found.</exception>
    public static string NormalizeFieldPath(Type rootType, string path)
        => NormalizedPathCache.GetOrAdd($"{MetadataCacheKeys.NormalizedPathPrefix}{rootType.FullName}:{path}", _ => NormalizeFieldPathCore(rootType, path));

    /// <summary>Tries to normalize a field path without throwing. Failures are not cached (only successful normalizations use <see cref="NormalizedPathCache" />).</summary>
    /// <param name="rootType">Entity type at the root of the path.</param>
    /// <param name="path">Dotted path to normalize.</param>
    ...
    internal static PropertyPathMetadata GetOrAddPropertyPath<T>(string propertyName, Func<PropertyPathMetadata> factory)
        => PropertyPathCache.GetOrAdd($"{MetadataCacheKeys.PropertyPathPrefix}{typeof(T).FullName}:{propertyName}", _ => factory());
    ...
    /// <summary>Memoizes sort-key lambda plus key CLR type per entity type and property path (OrderBy/ThenBy reuse the same tree).</summary>
    ...
    /// <summary>Cached AsQueryable method for projection count expressions. Keyed by element type.</summary>
    public static MethodInfo GetProjectionAsQueryableMethod(Type elementType)
        => ReflectedMethodCache.GetOrAdd(
            $"{MetadataCacheKeys.ReflectedMethodPrefix}ProjectionAsQueryable:{elementType.FullName}",
    ...
    /// <summary>Cached Queryable.Count method for projection count expressions. Keyed by element type.</summary>
    public static MethodInfo GetProjectionQueryableCountMethod(Type elementType)
        => ReflectedMethodCache.GetOrAdd(
            $"{MetadataCacheKeys.ReflectedMethodPrefix}ProjectionQueryableCount:{elementType.FullName}",
            _ => typeof(Queryable).GetMethods(BindingFlags.Public | BindingFlags.Static)
    ...
    /// <summary>Used by query and projection services. Excludes string and byte[].</summary>
