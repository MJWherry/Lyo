...
/// <summary>Times the projection pipeline: field resolution, SQL projection expression build, and in-memory entity projection.</summary>
[BenchmarkDescription(
    "Projection pipeline over BenchPerson: resolve field specs, build the SQL projection expression, and project entities in memory. Covers a flat selection (Name, Age, IsActive) and a nested-path selection that reaches into the Address object and the Contacts collection (Name, Address.City, Contacts.Kind).")]
...
public class ProjectionBenchmarks
{
    ...
    // No Baseline on purpose: the four methods time unrelated operations (metadata resolve, expression
    // build, full-set projection), so a shared "vs baseline" ratio is meaningless (~200,000x noise
    // comparing a sub-microsecond resolve against a 100k-row projection). Each method is judged on its own SLA instead.
    [Benchmark]
    ...
    public int ResolveProjectedFields() => _projection.ResolveProjectedFields<BenchPerson>(_fields).Specs.Count;
    ...
    public bool TryBuildSqlProjectionExpression() => _projection.TryBuildSqlProjectionExpression<BenchPerson>(_specs).Projection is not null;
    ...
    public int ProjectEntities() => _projection.ProjectEntities(_items, _specs, QueryIncludeFilterMode.Full, new([])).Count;
    ...
    public int ProjectEntities_Nested() => _projection.ProjectEntities(_items, _nestedSpecs, QueryIncludeFilterMode.Full, new([])).Count;
}