...
/// <summary>Times object-to-DTO mapping through <see cref="ILyoMapper" /> (Mapster) for single objects and collections with nested children.</summary>
[BenchmarkDescription(
    "Object-to-DTO mapping via ILyoMapper (Mapster): a single entity and a 100-entity list, each carrying a nested child collection. ChildCount scales the nested collection so mapping cost reflects graph depth, not just row count.")]
...
[BenchmarkDataShape(
    typeof(MapEntity), Notes = "Entity with scalar fields plus a Children collection of MapChild (nested object), illustrating the nested mapping the row count alone hides.")]
...
public class MappingBenchmarks
{
    ...
    private MapEntity CreateEntity(int seed)
        => new() {
            Id = Guid.NewGuid(),
            Name = $"Entity {seed}",
            Age = 20 + seed % 50,
            Children = Enumerable.Range(0, ChildCount).Select(c => new MapChild { Id = Guid.NewGuid(), Label = $"child-{c}", Value = c }).ToList()
        };
