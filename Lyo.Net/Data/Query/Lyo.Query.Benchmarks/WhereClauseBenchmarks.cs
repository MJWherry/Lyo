...
/// <summary>In-memory measures for the where-clause engine: expression build, queryable filtering, and single-entity matching.</summary>
...
public class WhereClauseBenchmarks
{
    ...
    public void Setup()
    {
        ...
        // Walks the nested Address object and the Contacts collection (dotted member plus .Count path).
        _nestedPathClause = WhereClauseBuilder.And().Contains("Address.City", "ville").GreaterThan("Contacts.Count", 1).Build();
    }

    // No Baseline on purpose: the methods time unrelated operations (expression compile, full
    ...
    // judged on its own SLA instead (same idea as ProjectionBenchmarks).
    [Benchmark]
