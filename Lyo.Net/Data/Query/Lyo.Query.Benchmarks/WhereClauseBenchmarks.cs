...
[BenchmarkDescription("Where-clause engine over an in-memory BenchPerson list: a simple single-predicate clause (Age >= 30), a nested boolean clause (IsActive AND (Age > 50 OR Name contains ...)), and a nested-path clause that reaches into the Address object and the Contacts collection (Address.City contains AND Contacts.Count > 1). Covers expression build, IQueryable filtering, and single-entity matching.")]
...
public class WhereClauseBenchmarks
{
    ...
    public void Setup()
    {
        ...
        _simpleClause = WhereClauseBuilder.And()
            .GreaterThanOrEqual(nameof(BenchPerson.Age), 30)
            .Build();

        _nestedClause = WhereClauseBuilder.And()
            .Equals(nameof(BenchPerson.IsActive), true)
            .AddGroupOr(or => or
                .GreaterThan(nameof(BenchPerson.Age), 50)
                .Contains(nameof(BenchPerson.Name), "Person 1"))
            .Build();

        // Reaches into the nested Address object and the Contacts collection (dotted member + .Count path).
        _nestedPathClause = WhereClauseBuilder.And()
            .Contains("Address.City", "ville")
            .GreaterThan("Contacts.Count", 1)
            .Build();
    }
    ...
    public Expression<Func<BenchPerson, bool>>? BuildExpression_Nested()
        => ((BaseWhereClauseService)_service).BuildExpressionFromWhereClause<BenchPerson>(_nestedClause);
    ...
    public Expression<Func<BenchPerson, bool>>? BuildExpression_NestedPath()
        => ((BaseWhereClauseService)_service).BuildExpressionFromWhereClause<BenchPerson>(_nestedPathClause);
    ...
    public int ApplyWhereClause_Simple()
        => _service.ApplyWhereClause(_queryable, _simpleClause).Count();
    ...
    public int ApplyWhereClause_Nested()
        => _service.ApplyWhereClause(_queryable, _nestedClause).Count();
    ...
    public int ApplyWhereClause_NestedPath()
        => _service.ApplyWhereClause(_queryable, _nestedPathClause).Count();
    ...
    public bool MatchesWhereClause_Nested()
        => _service.MatchesWhereClause(_single, _nestedClause);
}
