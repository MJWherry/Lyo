...
/// <summary>
/// End-to-end root <c>/Query</c> (From/Joins) measures against PostgreSQL (Testcontainers). Isolated so Docker-less CI can filter this class out with BenchmarkDotNet
/// filters.
/// </summary>
...
[BenchmarkSla(MaxMeanMs = 40, Standard = "Root join queries against local PostgreSQL should complete within tens of milliseconds for modest pages.")]
public class RootQueryBenchmarks
{
    ...
    public void Setup()
    {
        ...
        for (var i = 0; i < Math.Max(Amount * 2, 100); i++) {
            ...
            // Fan-out: the first definitions each receive 2 runs.
            if (i < Amount) {
    ...
    {
        var req = QueryReqBuilder.New().From("d", nameof(JobDefinition)).AddSelects("d.Name").SetPagination(0, Amount).Build();
