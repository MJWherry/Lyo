...
/// <summary>
/// Self-contained entity used for in-memory where-clause/sort/projection benchmarks (no EF dependencies). Carries a
/// nested <see cref="Address" /> object and a <see cref="Contacts" /> collection so the engine's nested-path handling
/// (e.g. <c>Address.City</c>, <c>Contacts.Count</c>) is exercised, not just flat scalar fields.
/// </summary>
...
internal static class QueryBenchmarkSupport
{
    internal static BaseWhereClauseService CreateWhereClauseService()
    {
        ...
        return new BaseWhereClauseService(cache, cacheOptions, valueConversion);
    }
    ...
    internal static List<BenchPerson> GeneratePeople(int count)
    {
        ...
        for (var i = 0; i < count; i++)
            people.Add(new BenchPerson {
                ...
                Address = new BenchAddress {
                    City = Cities[i % Cities.Length],
                    Country = "Lyoland",
                    PostalCode = 10000 + i % 9000,
                },
                Contacts = [
                    new BenchContact { Kind = "email", Value = $"person{i}@example.com" },
                    new BenchContact { Kind = "phone", Value = $"+1-555-{i % 10000:D4}" },
                ],
            });
