...
public partial class QueryRunPanel : IAsyncDisposable
{
    ...
    /// <summary>Bumped when restoring persisted state so host rows rehydrate without fighting in-progress edits.</summary>
    [Parameter]
    ...
    private string GetRequestUri()
    {
        ...
        if (Run.RunMode == QueryWorkbenchRunMode.RootQuery) {
            // Root /Query lives on the dynamic CRUD base — entity type is only in From.EntityType.
    ...
    private async Task RunQuery()
    {
        ...
        try {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Post, uri) { Content = new StringContent(json, Encoding.UTF8, FileTypeInfo.Json.MimeType) };
    ...
    private static string GetSizeLabel(int bytes) => FileSizeUnitInfo.FormatBestFit(bytes);
    ...
    private IEnumerable<string> RoutesForSelectedHost()
    {
        ...
        // Root /Query: dropdown lists dynamic CRUD bases (entity type lives in From.EntityType).
    ...
    /// <summary>Drops the last path segment (entity type) from a configured entity route.</summary>
