...
{
    /// <summary><c>/QueryConcrete</c> body (full entities). Omitted in older saved state — derived from <see cref="QueryRequest" /> on load.</summary>
    public QueryConcreteReq? EntityQuery { get; set; }

    public ProjectionQueryReq QueryRequest { get; set; } = new() { Start = 0, Amount = 20 };

    /// <summary>Root <c>/Query</c> body (From/Joins). Omitted in older saved state.</summary>
