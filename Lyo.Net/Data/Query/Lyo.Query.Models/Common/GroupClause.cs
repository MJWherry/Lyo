...
/// <summary>Boolean combination of child clauses via <see cref="Operator" /> (AND/OR).</summary>
[DebuggerDisplay("{ToString(),nq}")]
public class GroupClause : WhereClause, IEquatable<GroupClause>
{
    /// <summary>Whether children combine with AND or OR.</summary>
    public GroupOperatorEnum Operator { get; set; }

    /// <summary>Child nodes evaluated per <see cref="Operator" />.</summary>
    public List<WhereClause> Children { get; set; }

    /// <summary>Builds a group with <see cref="GroupOperatorEnum.And" /> and an empty child list.</summary>
    public GroupClause()
    {
    ...
    /// <summary>Builds a group with the given operator and children.</summary>
    ...
    /// <summary>Builds a group with the given operator, optional description, and child nodes.</summary>
    ...
    /// <summary>Builds a group from a sequence of child clauses.</summary>
    ...
    public override string Print(int indent = 0)
    {
        ...
        // Operator indent is computed per child from that child's first line.
        var lines = new List<string> { pad + "(" };
        for (var i = 0; i < Children.Count; i++) {
            var childText = Children[i].Print(indent + 1);
            var childLines = childText.Split('\n');
            if (i == 0) {
                // First child: append its lines as-is (already start at the correct inner indent).
                foreach (var line in childLines)
                    lines.Add(line);
            }
            else {
                // Leading-space indent of the child's first line.
                ...
                // Put the operator at the same column as the child's first-line content.
                lines.Add(operatorPad + op + (trimmedFirst.Length > 0 ? " " + trimmedFirst : string.Empty));

                // Remaining lines: keep relative indent vs the child's first-line indent.
