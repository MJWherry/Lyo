...
/// <summary>
/// Fluent builder for <see cref="WhereClause" /> trees: combines <see cref="ConditionClause" /> leaves with <see cref="GroupOperatorEnum" />, nested groups,
/// sub-clauses for two-phase filtering, and typed expression paths via <see cref="WhereClauseBuilderFor{T}" />.
/// </summary>
...
public class WhereClauseBuilder
{
    ...
    /// <summary>Assigns the optional <see cref="WhereClause.Description" /> on the built group (when applicable).</summary>
    ...
    /// type, wrapping the collection in a one-element array. Unwrap it so the condition value is the flat collection.
    /// </summary>
    ...
    /// <summary>Adds an arbitrary pre-built clause node as a sibling under this builder's top-level group.</summary>
    ...
    /// <summary>Merges an inner AND group into the current group (left-associative when operators differ).</summary>
    public WhereClauseBuilder AddAnd(Action<WhereClauseBuilder> configure)
    {
        ...
        if (_groupOperator == GroupOperatorEnum.And) {
            // Same operator as parent: append (wrap a single child into GroupClause if needed).
        ...
        // Different operator: build left-associative grouping.
    ...
    /// <summary>Merges an inner OR group into the current group (left-associative when operators differ).</summary>
    ...
    // Explicit grouping API so callers can add grouped nodes.
    /// <summary>Adds an explicit nested <see cref="GroupClause" /> built by <paramref name="configure" />.</summary>
    ...
    /// <summary>Shortcut for <see cref="AddGroup(Lyo.Query.Models.Enums.GroupOperatorEnum, System.Action{WhereClauseBuilder})" /> with <see cref="GroupOperatorEnum.And" />.</summary>
    public WhereClauseBuilder AddGroupAnd(Action<WhereClauseBuilder> configure) => AddGroup(GroupOperatorEnum.And, configure);

    /// <summary>Shortcut for <see cref="AddGroup(Lyo.Query.Models.Enums.GroupOperatorEnum, System.Action{WhereClauseBuilder})" /> with <see cref="GroupOperatorEnum.Or" />.</summary>
    ...
    // Grouped WhereClause using this builder's operator (does not add to children).
    /// <summary>Nested group clause without appending it to the current builder’s child list (advanced composition).</summary>
    ...
    /// <summary>Builds one <see cref="GroupClause" /> over <paramref name="nodes" /> with operator <paramref name="op" />.</summary>
    public static WhereClause CombineAs(GroupOperatorEnum op, params WhereClause[] nodes) => new GroupClause(op, nodes);

    /// <summary>Starts a builder with AND as the top-level combiner.</summary>
    public static WhereClauseBuilder And() => new(GroupOperatorEnum.And);

    /// <summary>Starts a builder with OR as the top-level combiner.</summary>
    public static WhereClauseBuilder Or() => new(GroupOperatorEnum.Or);

    /// <summary>Typed helper that resolves property paths from lambda expressions.</summary>
    ...
    /// <exception cref="InvalidOperationException">Raised when no child clauses were added.</exception>
    public WhereClause Build()
    {
        OperationHelpers.ThrowIf(_children.Count == 0, "Cannot build query with no conditions");

        // Always wrap children in a GroupClause using this builder's operator.
        // Do not flatten a single child — keep grouping explicit.
    ...
    /// <summary>Builds a ConditionClause with an optional SubQuery for two-phase execution (root in DB, subquery in memory).</summary>
    ...
    /// <summary>Builds an AND group via a nested configure callback (same as <c>WhereClauseBuilder.And(); configure; Build()</c>).</summary>
    ...
    /// <summary>Builds an OR group via a nested configure callback.</summary>
    ...
    /// </summary>
    /// <returns>An AND group of remaining non-empty conditions, a single condition, or null when nothing remains.</returns>
    ...
    /// <summary>Typed view of <see cref="WhereClauseBuilder" /> that resolves field paths from expressions (honors <see cref="QueryPropertyNameAttribute" />).</summary>
    public class WhereClauseBuilderFor<T>(WhereClauseBuilder parent)
    {
        private static string PathFrom<TProp>(Expression<Func<T, TProp>> selector) => QueryBuilderExpressionPathHelper.GetPropertyPath(selector);
