...
/// <summary>Binds sibling parameter values into a root <see cref="QueryReq" /> template (<c>{{ParamKey}}</c> placeholders) and reads key/label pairs from projected query rows.</summary>
public static class ParameterOptionsBinder
{
    ...
    /// <summary>Distinct placeholder keys (without braces) from <paramref name="query" /> where values.</summary>
    ...
    /// <summary>
    /// Clones <paramref name="template" />, substitutes <c>{{Key}}</c> placeholders from <paramref name="siblingValues" />, and returns false when any placeholder has no
    ...
    /// <summary>Distinct placeholder keys from <paramref name="sprocParameters" /> values.</summary>
    public static IReadOnlyList<string> GetSprocInputParameterKeys(IReadOnlyDictionary<string, string>? sprocParameters)
    {
        var keys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (sprocParameters is null)
            return [];

        foreach (var value in sprocParameters.Values)
            CollectFromValue(value, keys);

        return keys.OrderBy(k => k, StringComparer.OrdinalIgnoreCase).ToList();
    }

    /// <summary>
    /// Substitutes <c>{{Key}}</c> placeholders in sproc argument values. Returns false when any placeholder has no non-whitespace sibling value.
    /// </summary>
    public static bool TryBindSprocParameters(
    ...
    {
        ArgumentHelpers.ThrowIfNull(siblingValues);
        bound = new(StringComparer.OrdinalIgnoreCase);
        ...
        var map = ToIgnoreCaseMap(siblingValues);
        var required = GetSprocInputParameterKeys(template);
        var missing = required.Where(k => !map.TryGetValue(k, out var v) || string.IsNullOrWhiteSpace(v)).ToList();
        if (missing.Count > 0) {
            missingKeys = missing;
            return false;
        ...
        foreach (var kvp in template)
            bound[kvp.Key] = ReplacePlaceholders(kvp.Value, map);
    ...
    /// <summary>
    /// Reads picker key/label from a projected row. Prefers columns named <c>Key</c>/<c>Value</c>; otherwise uses property names of the first two
    ...
    /// <summary>Reads picker key/label using explicit column names, then the Key/Value convention.</summary>
    public static bool TryReadKeyValue(object? row, string? keyField, string? labelField, out string key, out string label)
    {
        key = "";
        label = "";
        if (row is null)
        ...
        var keyName = string.IsNullOrWhiteSpace(keyField) ? "Key" : keyField.Trim();
        var labelName = string.IsNullOrWhiteSpace(labelField) ? "Value" : labelField.Trim();
        if (TryGetRowProperty(row, keyName, out var keyObj)) {
            key = FormatCell(keyObj);
            label = TryGetRowProperty(row, labelName, out var labelObj) ? FormatCell(labelObj) : key;
            return !string.IsNullOrEmpty(key);
        }

        return TryReadKeyValue(row, (IReadOnlyList<string>?)null, out key, out label);
    }

    /// <summary>Static options from JSON-array <c>AllowedValues</c> (key = label for each canonical token).</summary>
