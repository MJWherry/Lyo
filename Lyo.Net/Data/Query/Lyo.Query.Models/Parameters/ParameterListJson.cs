...
/// <summary>
/// Serialize / parse definition list payloads stored in string columns (<c>AllowedValues</c>, multi <c>Value</c>).
/// Payload is a JSON array of typed primitives; parse always yields canonical invariant strings for UI and validation.
/// </summary>
public static class ParameterListJson
{
    /// <summary>
    /// Deserializes a JSON array into canonical string forms. Null/whitespace or invalid/non-array JSON yields an empty list.
    /// Accepts string, number, and boolean elements; null elements are skipped. No pipe-separated fallback.
    /// </summary>
    ...
    /// <summary>
    /// Serializes chip/string values to a JSON array. Returns null when there are no non-empty values after kind filtering.
    /// <paramref name="kind" /> selects JSON string, number, or bool wire shape.
    /// </summary>
