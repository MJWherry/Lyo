...
internal sealed class XlsxReader : IXlsxReader
{
    ...
    private DataTable.Models.DataTable ConvertWorksheetToDataTable(IXLWorksheet ws, bool useHeaderRow, bool includeFormatting, bool useFooterRow = false)
    {
        ...
        // Top-left worksheet coordinate of each merged range maps to its span. Covered cells stay empty.
    ...
    /// <summary>Keeps only non-default formatting. Theme colors and workbook defaults are dropped so unstyled sheets stay out of the format map.</summary>
    private static DataTableCellFormat? ExtractMeaningfulFormat(IXLCell cell)
    {
        ...
        // Drop default bottom/center-ish vertical; keep Top explicitly.
        ...
        // Keep border color only when an edge is set (ClosedXML still reports black otherwise).
        var borderColor = hasBorder ? NormalizeColor(TryGetColorHex(style.Border.TopBorderColor), true) : null;
        // Omit default font size/name and workbook default black/white — ClosedXML reports them on every cell.
    ...
    private static string? TryGetColorHex(XLColor? color)
    {
        ...
        // Indexed automatic/empty fills are not real styles.
    ...
    private static void SetPropertyValue(PropertyInfo property, object target, object? raw)
    {
        ...
        try {
            ...
            if (targetType.IsEnum) {
                property.SetValue(target, TypeConversion.ConvertTo(raw.ToString(), targetType));
                return;
            }
