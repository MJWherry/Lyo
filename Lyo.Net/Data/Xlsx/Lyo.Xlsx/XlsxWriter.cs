...
internal sealed class XlsxWriter : IXlsxWriter
{
    ...
    static XlsxWriter()
    {
        // Still needed by the ClosedXML import/DataTable read path when loading workbooks on non-Windows hosts.
    ...
    // Throw synchronously so an already-cancelled token surfaces OperationCanceledException (not a derived TaskCanceledException).
