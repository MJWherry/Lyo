...
/// <summary>Value kind on an <see cref="XlsxCell" />; selects XLSX cell type and style.</summary>
...
/// <summary>
/// One worksheet cell held so it can be written to the OpenXML stream and measured for approximate column width without re-inspecting the source value.
/// </summary>
internal readonly struct XlsxCell
{
    ...
    /// <summary>Inline-string text and width-measurement text (also the display form of numbers, dates, and booleans).</summary>
    public string DisplayText { get; }

    /// <summary>Numeric payload: the number itself, OADate serial for dates, or 1/0 for booleans.</summary>
    public double Numeric { get; }

    /// <summary>Columns this cell spans (1 = no spanning). The cell is the top-left anchor of the merged range.</summary>
    public int ColSpan { get; }

    /// <summary>Rows this cell spans (1 = no spanning). The cell is the top-left anchor of the merged range.</summary>
    public int RowSpan { get; }

    /// <summary>Optional per-cell format from a DataTable sparse format map.</summary>
    ...
    /// <summary>Copy of this cell marked as the anchor of a merged range spanning the given columns and rows.</summary>
    public XlsxCell WithSpan(int colSpan, int rowSpan) => new(Kind, DisplayText, Numeric, colSpan, rowSpan, Format);

    /// <summary>Copy with the given format attached.</summary>
    public XlsxCell WithFormat(DataTableCellFormat? format) => new(Kind, DisplayText, Numeric, ColSpan, RowSpan, format);
}

/// <summary>
/// Streams one or more worksheets into an XLSX package via <see cref="OpenXmlWriter" />, keeping memory bounded regardless of row count. Column widths come from a
/// bounded sample of the leading rows instead of ClosedXML's graphics-engine auto-fit. When cells carry <see cref="DataTableCellFormat" />, a style cache maps unique
/// formats to stylesheet xf indices (capped to avoid pathological stylesheets).
/// </summary>
internal sealed class OpenXmlStreamWriter : IDisposable
{
    ...
    /// <summary>Leading rows (plus the header) sampled to size columns before streaming begins.</summary>
    private const int WidthSampleRows = 200;

    /// <summary>Cap on an approximated column width, in character units.</summary>
    ...
    /// <summary>Writes a worksheet: bold header then <paramref name="rows" />, with column widths approximated from the leading rows.</summary>
    ...
    /// <summary>
    /// Writes a worksheet. When <paramref name="headerFormats" /> or cell <see cref="XlsxCell.Format" /> values are present, those styles go into the stylesheet. Header-map
    /// formats beat the default bold-header style; a missing header format keeps bold header.
    /// </summary>
    ...
    /// <summary>
    /// Writes a worksheet with optional header/footer format maps and an optional trailing <paramref name="footer" /> row (bold like the header). Per-cell and
    /// <paramref name="footerFormats" /> beat the bold default.
    /// </summary>
    public void WriteSheet(
        string sheetName,
        IReadOnlyList<string> headers,
        ...
        CancellationToken ct = default)
    {
        ...
        // Sample leading rows to size columns; <cols> must be written before <sheetData>, so widths come from this bounded buffer.
        ...
        // pendingRows[col] > 0 means a prior rowspan covers this column; mergeRefs collects A1-style merge ranges.
        ...
        using (var writer = OpenXmlWriter.Create(worksheetPart)) {
            ...
            // mergeCells is legal after sheetData in the worksheet schema, so streaming stays intact.
    ...
    private void WriteDataRow(OpenXmlWriter writer, XlsxCell[] row, string[] columnLetters, uint rowIndex, int[] pendingRows, List<string> mergeRefs)
    {
        ...
        while (c < columnLetters.Length) {
            if (pendingRows[c] > 0) {
                // Covered by a rowspan from an earlier row: emit nothing for this coordinate.
                pendingRows[c]--;
                c++;
                continue;
            }
    ...
    /// <summary>
    /// Writes a trailing footer row. Starts as bold-header style like <see cref="WriteHeaderRow" />; per-cell or <paramref name="footerFormats" /> win. Preserves merges like
    /// data rows.
    /// </summary>
