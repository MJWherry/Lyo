...
/// <summary>
/// Exports data to XLSX (Excel) format via a streaming OpenXML writer: typed rows, selected properties, multi-sheet workbooks, Lyo data tables, and async APIs on modern
/// targets.
/// </summary>
public interface IXlsxWriter
{
    ...
    /// <summary>
    /// Writes a row/column dictionary map to <paramref name="xlsxFilePath" />; <paramref name="useHeaderRow" /> controls whether the first row is treated as headers;
    /// <paramref name="useFooterRow" /> writes the last ordered row as a trailing footer.
    /// </summary>
