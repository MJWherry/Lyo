...
public class MetricsTests : IDisposable
{
    ...
    public async Task Metrics_Error_RecordsMetrics()
    {
        ...
        // Force an error by disposing, then using the watcher
        watcher.Dispose();

        // Error metrics must be recorded when errors occur
        // Thresholds may need tuning against the real error scenarios
        await Task.Delay(100, TestContext.Current.CancellationToken);
    }
}

// Minimal IMetrics stand-in used by these tests
