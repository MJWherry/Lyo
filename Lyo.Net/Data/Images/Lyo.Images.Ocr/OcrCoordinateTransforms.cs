...
{
    /// <summary>
    /// Maps a Tesseract-style rectangle (origin top-left, Y increasing downward) into <see cref="BoundingBox2D" /> with Y-up (origin bottom-left), matching
    /// <see cref="Lyo.Common.Metadata.Records.BoundingBox2D" /> height semantics.
    /// </summary>
    ...
    /// <summary>Maps a Y-up pixel box to PDF points using uniform scale (caller should rasterize with matching aspect ratio).</summary>
    ...
    /// <param name="rasterWidthPx">Rendered image width in pixels.</param>
    /// <param name="rasterHeightPx">Rendered image height in pixels.</param>
    public static BoundingBox2D MapPixelBoxToPdfPoints(BoundingBox2D boxPixelsYUp, double pageWidthPts, double pageHeightPts, int rasterWidthPx, int rasterHeightPx)
    {
        ...
        return new(boxPixelsYUp.Left * sx, boxPixelsYUp.Right * sx, boxPixelsYUp.Top * sy, boxPixelsYUp.Bottom * sy);
    }
}