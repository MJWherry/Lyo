...
public class ImageServiceTests
{
    ...
    /// <summary>Builds an image with several distinct colors (stripes or a grid).</summary>
    ...
    public async Task RotateAsync_WithValidImage_ReturnsRotatedImage()
    {
        ...
        outputStream.Position = 0;
        using var resultImage = await Image.LoadAsync(outputStream, TestContext.Current.CancellationToken);
        // SkiaImageService rotates inside the same canvas (no expansion), so size stays 100x200
        Assert.Equal(100, resultImage.Width);
    ...
    public async Task GetPaletteAsync_ReturnsColorsOrderedByUsage()
    {
        ...
        // 5-bit bucket for G=128 expands via Expand5BitChannelTo8(16) to 0x84, not 0x80 (see SkiaImageService.ExtractPalette)
    ...
    public async Task CompressAsync_WithValidImage_ReturnsCompressedImage()
    {
        ...
        Assert.True(result.Data);
        Assert.True(outputStream.Length > 0);
        // Compressed image is usually smaller (not guaranteed)
