...
SkiaSharp implementation of `IImageService` from [`Lyo.Images`](../Lyo.Images/README.md): resize, crop, rotate, watermark, format conversion, thumbnails, compression, metadata (optional MetadataExtractor EXIF in the Skia pipeline), palette extraction, and batch processing.
...
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Platforms | Strong on Linux and mobile. Native Skia assets. | Pure managed. Broader format coverage. |
| EXIF | Extended EXIF via MetadataExtractor where wired. | EXIF via ImageSharp metadata APIs. |
...
- Native rendering on Linux and mobile.
- Text rendering for watermarks is reliable.
- Raster formats include JPEG, PNG, WebP, GIF, BMP, ICO, depending on the Skia build.
