...
public partial class SpriteSheetWorkbench : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try {
            ...
            // Circuit already gone; disposal errors are expected.
        }
        catch (JSException) {
            // Canvas may already be gone from the DOM when disposal runs;
            // the JS module keeps per-canvas state in a WeakMap that GC will reclaim.
        }
    }
    ...
    {
        // Compare by reference only: a duplicate upload (same bytes) is dropped from the uploader without a matching
        // list entry, and content matching would delete an older sheet that happens to share the same file bytes.
    ...
    /// <returns><see langword="true" /> when a PNG carried <c>LyoGrid</c> metadata.</returns>
