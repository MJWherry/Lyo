...
/// <summary>
/// Always-on tests that exercise JSON load plus <see cref="ConfigurationBinder" /> (<c>Get&lt;T&gt;()</c>) and DI registration. Integration OCR tests skip unless
...
public sealed class TesseractOcrConfigurationTests(TesseractOcrTestFixture fixture)
{
    ...
    public void Get_Materializes_OcrEngineOptions_From_Appsettings()
    {
        ...
        Assert.NotNull(tess);
        // TessdataDirectory may be empty in committed appsettings; binding still runs (a breakpoint-friendly path).
