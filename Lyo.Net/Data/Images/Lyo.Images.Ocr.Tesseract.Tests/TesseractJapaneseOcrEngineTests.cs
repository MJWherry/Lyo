...
/// <summary>Japanese OCR needs <c>jpn.traineddata</c> alongside <c>eng.traineddata</c> in tessdata and a Japanese-capable font for raster synthesis.</summary>
    ...
    private static byte[]? TryCreatePngWithJapaneseText(string text)
    {
        foreach (var family in JapaneseFontCandidates) {
            try {
                ...
                // Family missing or glyph not covered — try the next candidate.
            }
        }
