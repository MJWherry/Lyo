...
/// <summary>
/// Loads <c>appsettings.json</c> (plus optional Development and env), registers <see cref="IConfiguration" /> in DI. Materializes options via <see cref="ConfigurationBinder" /> (
/// <c>Get&lt;T&gt;()</c>).
/// </summary>
public sealed class TesseractOcrTestFixture : IDisposable
{
    /// <summary>Text passed to xUnit <c>SkipUnless</c> when native OCR integration is turned off.</summary>
    ...
    public bool RunNativeIntegration
        => (Configuration.GetSection("OcrTesseractTests").Get<OcrTesseractTestsOptions>()?.RunIntegration ?? false) || string.Equals(
            Environment.GetEnvironmentVariable(RunIntegrationEnvVar), "1", StringComparison.Ordinal);
    ...
    public static string TessdataResolutionHint
        => "No folder containing eng.traineddata was found. Install tesseract-ocr-eng (or set OcrEngine:Tesseract:TessdataDirectory / OcrTesseractTests:TessdataDirectory in appsettings), " +
            "or download eng.traineddata into tessdata/. Enable integration via OcrTesseractTests:RunIntegration or " + RunIntegrationEnvVar + "=1.";

    /// <summary>Used when Japanese OCR tests are skipped because <c>jpn.traineddata</c> is missing from the resolved tessdata folder.</summary>
    ...
    public TesseractOcrTestFixture()
    {
        ...
        _services = new ServiceCollection().AddSingleton(Configuration).BuildServiceProvider();
    }

    public void Dispose() => _services.Dispose();

    /// <summary>OCR engine options bound from <c>OcrEngine</c> through the configuration binder.</summary>
    public OcrEngineOptions GetOcrEngineOptions() => Configuration.GetSection(OcrEngineOptions.SectionName).Get<OcrEngineOptions>() ?? new();

    /// <summary>Tesseract subsection bound from <c>OcrEngine:Tesseract</c> through the configuration binder.</summary>
    ...
    /// <summary>True when <paramref name="tessdataDirectory" /> contains <c>{languageCode}.traineddata</c> (e.g. <c>jpn</c>, <c>fra</c>).</summary>
    ...
    /// <summary>Folder containing <c>eng.traineddata</c>: explicit test override, then <c>OcrEngine:Tesseract:TessdataDirectory</c>, then discovery.</summary>
    ...
    /// <summary>Bound from the <c>OcrTesseractTests</c> JSON section.</summary>
