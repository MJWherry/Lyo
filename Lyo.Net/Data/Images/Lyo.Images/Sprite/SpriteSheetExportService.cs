...
public sealed class SpriteSheetExportService(IImageService imageService) : ISpriteSheetExportService
{
    ...
    /// <summary>Default FPS for flat spritesheets, or when animation timing is missing.</summary>
    ...
    /// <summary>Reads grid size embedded by <see cref="ExportAnimatedImageToSpriteSheetPngAsync" /> (PNG tEXt chunk).</summary>
    ...
    /// <summary>Grows <paramref name="sampled" /> to <paramref name="targetCount" /> by duplicating entries when the grid has more cells than samples.</summary>
    ...
    {
        ArgumentHelpers.ThrowIfNull(sampled);
        var n = sampled.Count;
    ...
    /// <summary>
    /// Preview/export FPS from one animation loop: <c>frame_count / loop_duration</c>, clamped 1–60. A single frame or missing timing returns
    ...
    /// <summary>
    /// Frames to place on the sheet for one loop: min(source frames, max(1, round(loop seconds × target fps))). Static or missing timing uses every source frame.
    /// </summary>
    ...
    /// <summary>
    /// Chooses row and column counts so rows × cols ≥ <paramref name="frameCount" />. Uneven totals leave extra cells at the end of the grid; export fills them using
