...
/// <summary>Abstract base for <see cref="IImageService" /> implementations (e.g. ImageSharp, SkiaSharp) sharing validation, metrics wiring, and shared helpers.</summary>
public abstract class ImageServiceBase : IImageService
{
    /// <summary>Logger used by this service.</summary>
    protected ILogger Logger { get; }

    /// <summary>Metrics sink used by this service.</summary>
    protected IMetrics Metrics { get; }

    /// <summary>Configured image-service options.</summary>
    protected ImageServiceOptions Options { get; }

    /// <summary>Metric-name map. Derived classes may mutate this dictionary to override names.</summary>
    protected Dictionary<string, string> MetricNames { get; }

    /// <summary>Decoration primitives (overlay / frame / caption / outer padding). Delegated to <see cref="ImageDecorationService" /> so backends do not re-implement them.</summary>
    protected ImageDecorationService Decoration { get; }

    /// <summary>Builds a new <see cref="ImageServiceBase" />.</summary>
    ...
    /// <inheritdoc />
    /// <remarks>Override in a derived type that can load fonts (see <c>Lyo.Images.Skia</c>). The base implementation returns a failed result rather than drawing.</remarks>
    public virtual async Task<Result<bool>> WatermarkAsync(
        Stream inputStream,
        Stream outputStream,
        ...
        CancellationToken ct = default)
    {
        ...
        // Watermark needs font loading — derived types (Skia) implement drawing.
        Logger.LogWarning("WatermarkAsync on ImageServiceBase does not draw; use Lyo.Images.Skia or another derived service.");
    ...
    public virtual async Task<Result<ImagePalette>> GetPaletteAsync(Stream imageStream, int colorCount, CancellationToken ct = default)
    {
        ...
        try {
            ...
            // Shrink large images first so palette work stays cheap.
    ...
    /// <summary>Stream length when supported; null for non-seekable or unsupported streams.</summary>
    ...
    /// <summary>Builds the metric-name dictionary. Override in derived classes to customize names.</summary>
    ...
    /// <summary>Runs a single image request.</summary>
    protected virtual async Task<ImageOperationResult> ProcessRequestAsync(ImageProcessRequest request, CancellationToken ct)
    {
        ...
        // Map Result<bool> onto ImageOperationResult.
        if (result.IsSuccess)
    ...
    /// <summary>Maps <see cref="ResizeMode" /> onto ImageSharp ResizeMode.</summary>
    ...
    protected static ImageFormat DetectFormat(IImageFormat? decodedFormat)
    {
        ...
        return formatName switch {
            ...
            "ico" => ImageFormat.Ico,
            var _ => ImageFormat.Jpeg // Unknown decoded name.
        };
    }

    /// <summary>Infers image format from <see cref="ImageInfo" />.</summary>
