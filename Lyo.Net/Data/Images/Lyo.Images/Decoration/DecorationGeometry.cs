...
/// <summary>Shared geometry (rounded-rect / top-rounded-rect paths, hex color parsing) used by decoration primitives.</summary>
internal static class DecorationGeometry
{
    /// <summary>Bézier handle factor for a quarter-circle via cubic curves (stable with ImageSharp.Drawing 2.x; avoids degenerate ArcTo output).</summary>
    private const float QuarterCircleBezierK = 0.5522847498f;

    /// <summary>Closed rounded-rect path using cubic Bézier quarter-circle approximations.</summary>
    ...
    /// <summary>Rounded top corners only; square bottom edge (caption-band header).</summary>
    ...
    /// <summary>Rounded bottom corners only; square top edge (caption-band footer).</summary>
    ...
    /// <summary>Tries to parse a hex color, retrying without the alpha channel when MudBlazor-style <c>#RRGGBBAA</c> values are given.</summary>
    ...
    /// <summary>Parses <paramref name="hex" />, falling back to <paramref name="fallback" /> when null or invalid.</summary>
