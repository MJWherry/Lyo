...
/// <summary>Composites an overlay onto a raster background, or splices an embedded base64 PNG into an SVG document.</summary>
internal static class OverlayDrawer
{
    /// <summary>Mutates <paramref name="background" /> in place: optional resize, optional pad fill, overlay paint, optional stroke.</summary>
    public static async Task ApplyToRasterAsync(Image<Rgba32> background, byte[] overlayImageBytes, OverlayOptions options, CancellationToken ct)
    {
        ...
        background.Mutate(ctx => {
            if (!string.IsNullOrWhiteSpace(options.PadColorHex) && DecorationGeometry.TryParseColor(options.PadColorHex, out var pad)) {
                // Clamp pad to the canvas so tiny backgrounds do not get negative rectangles.
    ...
    /// <summary>Embeds a resized PNG of the overlay in the SVG just before <c>&lt;/svg&gt;</c>; returns the edited SVG (or the original on failure).</summary>
