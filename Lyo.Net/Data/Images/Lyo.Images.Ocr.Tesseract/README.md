...
2. Run your process from that output folder, or put those DLLs on the loader search path.
...
- Start the app and call `IOcrEngine.ReadAsync` once. If tessdata is wrong you typically get engine creation errors referencing missing `*.traineddata`.

## Automated tests

- Install library plus language data as above (`tesseract-ocr-eng` or `tesseract-ocr` on Debian/Ubuntu).
