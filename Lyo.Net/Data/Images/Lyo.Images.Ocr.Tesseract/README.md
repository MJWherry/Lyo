...
Tesseract `IOcrEngine` for `Lyo.Images.Ocr`. An internal lock serializes calls because native Tesseract instances are not safely concurrent.
...
### Bind Tesseract from configuration
...
## 1. Native Tesseract runtime

**Windows**

1. Restore/build the app so the Tesseract NuGet copies `x64\tesseract50.dll` (and Leptonica) into the output folder.
2. Run the process from that output folder, or put those DLLs on the loader search path.
...
3. Install English traineddata (pick one):
    ...
    - Or the `tesseract-ocr` meta package (CLI + common languages): `sudo apt-get install -y tesseract-ocr`
...
**Why `/usr/local/lib` often fails.** On Linux the loader looks next to your built app, under `x64/` inside the configuration output folder, for example
...
## 2. Language data (`traineddata`), custom layout
...
- Otherwise ship or download `eng.traineddata` into a folder you control (other languages mean more `*.traineddata` files).
- Manual download of the English fast model: [tessdata_fast eng.traineddata](https://github.com/tesseract-ocr/tessdata_fast/raw/main/eng.traineddata)
...
- Start the app and call `IOcrEngine.ReadAsync` once. If tessdata is wrong you typically get engine creation errors that mention missing `*.traineddata`.

## Automated tests

- Install the library plus language data as above (`tesseract-ocr-eng` or `tesseract-ocr` on Debian/Ubuntu).
- Enable OCR integration either way:
- Preferred: set `OcrTesseractTests:RunIntegration` to `true` in `appsettings.Development.json`, or edit `appsettings.json` locally (do not commit `true` if CI must stay off).
...
If `OcrEngineOptions` was already registered (e.g. via `AddOcrEngineOptionsFromConfiguration`), call `AddTesseractOcrEngine` without the `configureShared` delegate so shared options are not registered twice.
