...
/// <summary>Tesseract <see cref="IOcrEngine" /> (not thread-safe internally; serializes calls).</summary>
public sealed class TesseractOcrEngine : IOcrEngine, IDisposable
{
    /// <summary>Cap on word-iterator steps so pathological segmentation cannot loop forever in native code.</summary>
    ...
    {
        ArgumentHelpers.ThrowIfNull(sharedOptions);
        ArgumentHelpers.ThrowIfNull(tesseractOptions);
