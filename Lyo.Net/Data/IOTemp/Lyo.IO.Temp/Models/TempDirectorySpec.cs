...
public record TempDirectorySpec
{
    ...
    /// <summary>
    /// Optional per-file size selector. Receives the 0-based file index and returns the desired size in bytes.
    /// When set, overrides <see cref="FileSizeBytes"/> for every file in this directory level.
    /// </summary>
    ...
    public static TempDirectorySpec Flat(int fileCount, long fileSizeBytes)
        => new() { FileCount = fileCount, FileSizeBytes = fileSizeBytes };

    /// <summary>Creates a flat spec using a <see cref="FileSizeUnitInfo"/> unit and an amount.</summary>
    public static TempDirectorySpec Flat(int fileCount, FileSizeUnitInfo unit, double amount)
        => Flat(fileCount, unit.ConvertToBytes(amount));

    /// <summary>
    /// Creates a flat spec where the file count and each file's size are chosen uniformly at random from the given ranges.
    /// The <see cref="FileSizeSelector"/> is set so every call returns an independent random size within [<paramref name="minSize"/>, <paramref name="maxSize"/>].
    /// </summary>
    public static TempDirectorySpec Random(int minFiles, int maxFiles, long minSize, long maxSize)
    {
        ...
        return new() {
            FileCount = fileCount,
            FileSizeBytes = minSize,
            FileSizeSelector = _ => minSize + (sizeRange == 0 ? 0 : (long)(GetRandom().NextDouble() * sizeRange))
        };
    }

    /// <summary>
    /// Creates a flat spec with randomised file count and sizes using <see cref="FileSizeUnitInfo"/> units.
    /// </summary>
    ...
    private static System.Random GetRandom() => _rng ??= new System.Random();
#endif
}
