...
/// <summary>Assert helpers for <see cref="IIOTempSession" />.</summary>
public static class IOTempSessionExtensions
{
    extension(IIOTempSession session)
    {
        /// <summary>Throws <see cref="InvalidOperationException" /> if any path in <see cref="IIOTempSession.Files" /> is missing on disk (for example deleted outside the session).</summary>
        ...
        /// <summary>
        /// Throws <see cref="InvalidOperationException" /> if session byte usage is farther from <paramref name="expectedBytes" /> than <paramref name="toleranceBytes" />.
        /// </summary>
